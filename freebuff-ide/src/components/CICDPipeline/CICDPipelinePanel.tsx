...
import {
  Play, Square, CheckCircle, XCircle, Clock, Loader2, RotateCcw,
  ChevronDown, ChevronRight, ArrowRight, GitBranch, Package,
  Shield, Rocket, Bell, Eye, Copy, Check, Terminal
} from 'lucide-react'
          ...
          {stageProgress.map((stage, i) => (
            <React.Fragment key={stage.id}>
              ...
              {i < stageProgress.length - 1 && (
                <ArrowRight size={10} className="text-ide-text-secondary flex-shrink-0" />
              )}
            </React.Fragment>
          ))}
        </div>

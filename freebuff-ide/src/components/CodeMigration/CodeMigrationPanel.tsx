...
import {
  ArrowRight, RefreshCw, Code, CheckCircle, AlertTriangle,
  ChevronDown, ChevronRight, Zap, FileText, Copy, Check,
  GitBranch, Layers, Terminal, Play, Clock
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Code Migration</span>
        </div>
      ...
      <div className="px-3 py-2 border-b border-ide-border bg-gradient-to-r from-orange-500/5 to-blue-500/5">
        <div className="flex items-center gap-3 text-xs">
          ...
          <ArrowRight size={12} className="text-ide-text-secondary" />
            ...
            {rules.map(rule => (
              <div key={rule.id}>
                <div onClick={() => setExpandedRule(expandedRule === rule.id ? null : rule.id)} className="flex items-center gap-2 px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
                  {expandedRule === rule.id ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                  {rule.status === 'done' ? <CheckCircle size={12} className="text-green-400" /> :
                   rule.status === 'processing' ? <RefreshCw size={12} className="text-blue-400 animate-spin" /> :
                   <Clock size={12} className="text-ide-text-secondary" />}
                ...
                {expandedRule === rule.id && (
                  <div className="px-6 pb-2 space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <ArrowRight size={12} className="text-ide-text-secondary flex-shrink-0" />

...
import {
  BarChart3, Activity, AlertTriangle, CheckCircle, TrendingUp,
  Code, FileText, Layers, GitBranch, Clock, Target, Zap, Eye
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="flex border-b border-ide-border">
        {[
          ...
          { key: 'complexity' as const, label: 'Complexity', icon: Activity },
          { key: 'quality' as const, label: 'Quality', icon: CheckCircle },
        ].map(tab => (
          <button
            key={tab.key}
        ...
        {selectedMetric === 'overview' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: 'Total Files', value: projectMetrics.totalFiles, icon: FileText, color: 'text-blue-400' },
                { label: 'Total Lines', value: projectMetrics.totalLines.toLocaleString(), icon: Code, color: 'text-green-400' },
                { label: 'Code Lines', value: projectMetrics.totalCodeLines.toLocaleString(), icon: Code, color: 'text-cyan-400' },
                { label: 'Comments', value: projectMetrics.totalComments.toLocaleString(), icon: Eye, color: 'text-yellow-400' },
                { label: 'Functions', value: projectMetrics.totalFunctions, icon: Zap, color: 'text-purple-400' },
                { label: 'Classes', value: projectMetrics.totalClasses, icon: Layers, color: 'text-pink-400' },
              ].map(stat => (
              ...
              <div key={file.name} className="bg-ide-bg-secondary/30 border border-ide-border/50 rounded p-2">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1">
                    {file.maintainability >= 70 ? <CheckCircle size={10} className="text-green-400" /> : <AlertTriangle size={10} className="text-yellow-400" />}
                    <span className="text-xs">{file.maintainability}%</span>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    <span className="text-ide-text-secondary">Code:</span>

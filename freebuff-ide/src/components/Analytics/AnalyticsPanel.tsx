...
import {
  BarChart3, TrendingUp, Users, Clock, Code, GitCommit,
  FileText, Zap, Activity, Calendar, ChevronDown
} from 'lucide-react'
...
const TOP_CONTRIBUTORS = [
  ...
  { name: 'AI Agent (Code)', commits: 89, lines: 6200, avatar: '🤖' },
...
export default function AnalyticsPanel({ onClose }: { onClose: () => void }) {
  ...
  const stats: MetricData[] = useMemo(() => [
    { label: 'Total Commits', value: MOCK_DAILY.reduce((s, d) => s + d.commits, 0), change: 12.5, icon: GitCommit, color: 'text-green-400' },
    { label: 'Lines Added', value: MOCK_DAILY.reduce((s, d) => s + d.linesAdded, 0), change: 8.3, icon: Code, color: 'text-blue-400' },
    { label: 'Files Changed', value: MOCK_DAILY.reduce((s, d) => s + d.filesChanged, 0), change: -2.1, icon: FileText, color: 'text-yellow-400' },
    { label: 'Active Hours', value: 86, change: 5.7, icon: Clock, color: 'text-purple-400' },
  ], [])
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {activeTab === 'overview' && (
          <>
            ...
            <div>
              <div className="text-xs text-ide-text-secondary mb-2">Commit Activity</div>

...
import {
  FileText, Copy, Check, Download, Code, Globe, Lock, Eye,
  ChevronDown, ChevronRight, Plus, Trash2, Edit3, ExternalLink,
            ...
            {filtered.map((ep, i) => (
              <div key={i} onClick={() => setSelectedEndpoint(selectedEndpoint === ep ? null : ep)} className="border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
                <div className="flex items-center gap-2 px-3 py-2">
                  {selectedEndpoint === ep ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                  ...
                  {ep.security.length > 0 && <Lock size={10} className="text-yellow-400" />}
                </div>
                {selectedEndpoint === ep && (
        ...
        {activeTab === 'docs' && (
          <div className="p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={downloadSpec} className="text-xs text-sky-400 flex items-center gap-1"><Download size={10} /> Export JSON</button>
            </div>

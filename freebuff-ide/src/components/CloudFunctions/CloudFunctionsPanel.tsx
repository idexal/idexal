...
import {
  Zap, Play, Pause, Trash2, Clock, CheckCircle, XCircle, RefreshCw,
  ChevronDown, ChevronRight, Copy, Check, ExternalLink, Terminal,
  Code, Settings, Activity, Globe, ArrowUpDown
} from 'lucide-react'
            ...
            <div key={fn.id}>
              <div onClick={() => setExpandedFn(expandedFn === fn.id ? null : fn.id)} className="px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
                <div className="flex items-center gap-2 mb-1">
                  {expandedFn === fn.id ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                ...
                {expandedFn === fn.id && (
                  <div className="mt-2 ml-5 space-y-2 bg-ide-bg-secondary/20 rounded p-2">
                    ...
                    <div className="flex gap-1">
                      <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary hover:text-ide-text flex items-center gap-0.5"><Terminal size={8} /> Logs</button>
                      <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary hover:text-ide-text flex items-center gap-0.5"><Code size={8} /> Edit</button>

...
import {
  Package, Search, Download, Trash2, RefreshCw, ArrowUp,
  ChevronDown, ChevronRight, ExternalLink, Loader2, CheckCircle,
  AlertCircle, Filter, Plus, Minus
} from 'lucide-react'
          ...
          <div className="border-b border-ide-border">
            ...
            {searchResults.map(pkg => (
              <div key={pkg.name} className="px-3 py-2 border-b border-ide-border/50 flex items-center justify-between hover:bg-ide-bg-secondary/30">
                ...
                >
                  {installingPkg === pkg.name ? <Loader2 size={10} className="animate-spin" /> : <Download size={10} />}
                  Install
                </button>
              </div>
            ))}
          ...
          <div key={pkg.name} className="border-b border-ide-border/50">
            ...
            >
              <div className="flex items-center gap-1.5 mr-2">
                {expandedPkg === pkg.name ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1 ml-2">
                {pkg.isOutdated && (
                  <button
                    ...
                    title="Update"
                  >
                    {installingPkg === pkg.name ? <Loader2 size={12} className="animate-spin" /> : <ArrowUp size={12} />}
                  </button>
                )}
                <button
            ...
            <div className="max-h-32 overflow-y-auto px-3 py-1 space-y-1 border-t border-ide-border/50">
              {taskLog.map((t, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  {t.success ? <CheckCircle size={10} className="text-green-400" /> : <AlertCircle size={10} className="text-red-400" />}

...
import {
  Brain, Play, Pause, StopCircle, RefreshCw, Search, Plus,
  ChevronDown, ChevronRight, Clock, CheckCircle, XCircle,
  AlertTriangle, Zap, Terminal, Code, GitBranch, Shield,
  Activity, Copy, Check, Settings, Trash2, Edit3, ArrowRight
} from 'lucide-react'
...
const AGENT_TYPES: Record<AgentType, { icon: typeof Code; color: string; label: string }> = {
  code: { icon: Code, color: 'text-blue-400', label: 'Code Generator' },
  review: { icon: Shield, color: 'text-green-400', label: 'Code Reviewer' },
  debug: { icon: AlertTriangle, color: 'text-red-400', label: 'Debugger' },
  test: { icon: CheckCircle, color: 'text-yellow-400', label: 'Test Writer' },
  deploy: { icon: Zap, color: 'text-purple-400', label: 'Deployer' },
  security: { icon: Shield, color: 'text-orange-400', label: 'Security Scanner' },
  refactor: { icon: RefreshCw, color: 'text-cyan-400', label: 'Refactorer' },
  docs: { icon: Edit3, color: 'text-pink-400', label: 'Doc Writer' },
}

const MOCK_AGENTS: Agent[] = [
  {
    id: 'a1', name: 'Code Review Agent', type: 'review', status: 'completed', description: 'Reviews code for best practices, security issues, and performance',
...
const STATUS_CONFIG: Record<AgentStatus, { color: string; bg: string; icon: typeof CheckCircle }> = {
  idle: { color: 'text-gray-400', bg: 'bg-gray-400/10', icon: Clock },
  running: { color: 'text-blue-400', bg: 'bg-blue-400/10', icon: Activity },
  ...
  paused: { color: 'text-yellow-400', bg: 'bg-yellow-400/10', icon: Pause },
}

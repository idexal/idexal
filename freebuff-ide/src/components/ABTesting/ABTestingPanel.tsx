...
import {
  FlaskConical, Play, Pause, Trash2, Plus, TrendingUp, TrendingDown,
  ChevronDown, ChevronRight, Users, BarChart3, CheckCircle, Clock,
  Target, AlertTriangle, Eye, Copy, Check
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        ...
        <div className="flex items-center gap-1">
          <button className="p-1 bg-emerald-600/20 text-emerald-400 rounded hover:bg-emerald-600/30"><Plus size={14} /></button>
            ...
            <div key={exp.id}>
              <div onClick={() => setExpandedId(expandedId === exp.id ? null : exp.id)} className="px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
                <div className="flex items-center gap-2 mb-1">
                  {expandedId === exp.id ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                ...
                {exp.winner && (
                  <div className="mt-1 ml-5 text-[10px] text-emerald-400 flex items-center gap-0.5">
                    <CheckCircle size={8} /> Winner: {exp.variants.find(v => v.id === exp.winner)?.name}
                  </div>
                )}
              </div>
              {expandedId === exp.id && (

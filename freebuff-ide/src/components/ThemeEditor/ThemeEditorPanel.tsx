...
import {
  Palette, Plus, Save, Trash2, Copy, Check, Download, Upload,
  RotateCcw, Eye, Sliders, Sun, Moon, Monitor, ChevronDown
} from 'lucide-react'
...
const DEFAULT_THEME_COLORS: ThemeColor[] = [
  ...
  { name: 'Activity Bar', value: '#11111b', category: 'UI' },
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredColors.map(color => (
          <div key={color.name} className="flex items-center gap-2 px-3 py-1.5 border-b border-ide-border/30 hover:bg-ide-bg-secondary/30">
            ...
            >
              {copiedColor === (editingColors[color.name] || color.value) ? <Check size={10} className="text-green-400" /> : <Copy size={10} />}
            </button>
          </div>
        ))}
      </div>

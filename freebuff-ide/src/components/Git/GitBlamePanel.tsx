...
import {
  GitCommit, User, Calendar, Copy, Check, Search, ChevronDown,
  ChevronRight, Hash, FileText, Eye, Clock, RefreshCw, ExternalLink
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          <GitCommit size={16} className="text-yellow-400" />
              ...
              >
                ...
                <div className="w-16 flex items-center px-1 py-0.5">
                  ...
                  >
                    ...
                    {copiedHash === line.commitHash ? <Check size={8} className="text-green-400" /> : null}
                  </button>
                </div>
              ...
              {isExpanded && (
                <div className="px-4 py-2 bg-ide-bg-secondary/20 border-b border-ide-border/30 ml-8">
                  <div className="text-[10px] space-y-1">
                    <div className="flex items-center gap-2">
                      <GitCommit size={10} className="text-yellow-400" />
                      <span className="font-semibold">{line.message}</span>
                    </div>
                    <div className="flex items-center gap-4 text-ide-text-secondary">
                      <span className="flex items-center gap-1"><User size={8} /> {line.author} &lt;{line.authorEmail}&gt;</span>
                      <span className="flex items-center gap-1"><Calendar size={8} /> {line.date.toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] flex items-center gap-0.5">
                        <ExternalLink size={8} /> View Commit
                      </button>
                      <button

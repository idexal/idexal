...
import {
  GitBranch, Search, ZoomIn, ZoomOut, RotateCcw, Maximize2,
  AlertTriangle, ArrowRight, Package, Layers, Eye, EyeOff
} from 'lucide-react'
  ...
  return (
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="px-3 py-2 border-b border-ide-border flex items-center gap-2">
        ...
        <button onClick={() => setZoom(z => Math.min(z + 0.2, 3))} className="p-1 bg-ide-bg-secondary rounded text-ide-text-secondary"><ZoomIn size={14} /></button>
        <button onClick={() => setZoom(z => Math.max(z - 0.2, 0.3))} className="p-1 bg-ide-bg-secondary rounded text-ide-text-secondary"><ZoomOut size={14} /></button>
        <button onClick={() => setZoom(1)} className="p-1 bg-ide-bg-secondary rounded text-ide-text-secondary"><RotateCcw size={14} /></button>
      </div>

...
import {
  ...
  AlertCircle, Lightbulb, Bug, Flame, Filter
} from 'lucide-react'
...
const TAG_CONFIG: Record<TodoTag, { icon: React.ReactNode; color: string; bgColor: string; priority: TodoItem['priority'] }> = {
  TODO: { icon: <Lightbulb className="w-3 h-3" />, color: 'text-blue-400', bgColor: 'bg-blue-400/10', priority: 'low' },
  FIXME: { icon: <AlertTriangle className="w-3 h-3" />, color: 'text-yellow-400', bgColor: 'bg-yellow-400/10', priority: 'medium' },
  HACK: { icon: <Flame className="w-3 h-3" />, color: 'text-orange-400', bgColor: 'bg-orange-400/10', priority: 'high' },
  BUG: { icon: <Bug className="w-3 h-3" />, color: 'text-red-400', bgColor: 'bg-red-400/10', priority: 'critical' },
  NOTE: { icon: <Search className="w-3 h-3" />, color: 'text-gray-400', bgColor: 'bg-gray-400/10', priority: 'low' },
  XXX: { icon: <AlertCircle className="w-3 h-3" />, color: 'text-purple-400', bgColor: 'bg-purple-400/10', priority: 'high' },
}
          ...
          <div className="py-1">
            {Object.entries(groupedByFile).map(([file, items]) => (
              <div key={file}>
                ...
                >
                  {expandedFiles.has(file) ? <ChevronDown className="w-3 h-3 text-ide-text-muted" /> : <ChevronRight className="w-3 h-3 text-ide-text-muted" />}

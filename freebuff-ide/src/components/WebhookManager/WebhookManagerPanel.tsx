...
import {
  Webhook, Plus, Trash2, CheckCircle, XCircle, Clock, RefreshCw,
  ChevronDown, ChevronRight, Copy, Check, ExternalLink, Send,
  AlertTriangle, Eye, ToggleLeft, ToggleRight, Zap
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        ...
        <div className="flex items-center gap-1">
          <button className="p-1 bg-orange-600/20 text-orange-400 rounded hover:bg-orange-600/30"><Plus size={14} /></button>
          ...
          <div key={wh.id}>
            <div onClick={() => setExpandedId(expandedId === wh.id ? null : wh.id)} className="px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
              <div className="flex items-center gap-2 mb-1">
                {expandedId === wh.id ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                <button onClick={e => { e.stopPropagation(); toggleEndpoint(wh.id) }}>
                  {wh.enabled ? <ToggleRight size={18} className="text-green-400" /> : <ToggleLeft size={18} className="text-ide-text-secondary" />}
                </button>
                ...
                <div className="mt-2 ml-8 space-y-2 bg-ide-bg-secondary/20 rounded p-2">
                  ...
                  <div className="text-[10px]">
                    ...
                    <button onClick={e => { e.stopPropagation(); navigator.clipboard?.writeText(wh.secret); setCopied(wh.id); setTimeout(() => setCopied(null), 1500) }} className="ml-1">
                      {copied === wh.id ? <Check size={8} className="text-green-400 inline" /> : <Copy size={8} className="inline" />}
                    </button>
                  </div>
                  <div className="flex gap-1">
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary flex items-center gap-0.5"><Send size={8} /> Test</button>
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary flex items-center gap-0.5"><Eye size={8} /> View</button>
          ...
          return (
            <div key={d.id} className="px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20">
              <div className="flex items-center gap-2 mb-0.5">
                {d.status < 300 ? <CheckCircle size={10} className="text-green-400" /> : <XCircle size={10} className="text-red-400" />}

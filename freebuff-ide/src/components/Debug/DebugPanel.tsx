...
import { Play, Pause, Square, ArrowDownToLine, ArrowRight, ArrowUpFromLine, RefreshCw, Bug, Variable, ListTree, Target, Plus, X, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function DebugPanel({ onClose }: DebugPanelProps) {
  ...
  const views: { id: DebugView; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'callstack', label: 'Call Stack', icon: <ListTree className="w-4 h-4" /> },
    { id: 'breakpoints', label: 'Breakpoints', icon: <Target className="w-4 h-4" /> },
    { id: 'watch', label: 'Watch', icon: <Bug className="w-4 h-4" /> },
  ]
    ...
    <div className="h-full flex flex-col bg-ide-surface">
      ...
      <div className="h-10 px-4 flex items-center justify-between border-b border-ide-border">
        <div className="flex items-center gap-2">
          <Bug className="w-4 h-4 text-ide-warning" />
          ...
          <div className="p-2">
            {breakpoints.map((bp) => (
              <div key={bp.id} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-ide-border/30 group">
                ...
                >
                  <X className="w-3 h-3 text-ide-text-muted" />
                </button>
              </div>
            ))}
          </div>
        ...
        {view === 'watch' && (
          <div className="p-2">
            ...
            <div className="flex items-center gap-2 px-2 py-1 mt-1">
              <Plus className="w-3 h-3 text-ide-text-muted" />
              <input
                type="text"
                value={newWatch}

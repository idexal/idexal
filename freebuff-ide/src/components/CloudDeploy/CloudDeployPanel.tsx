...
import {
  Cloud, Rocket, CheckCircle, XCircle, Clock, Loader2, RefreshCw,
  ChevronDown, ChevronRight, Globe, Server, Shield, Zap,
  ExternalLink, Terminal, AlertTriangle, Copy, Check
} from 'lucide-react'
                    ...
                    {dep.url && (
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={e => { e.stopPropagation(); navigator.clipboard?.writeText(dep.url); setCopied(true); setTimeout(() => setCopied(false), 1500) }}>
                          {copied ? <Check size={10} className="text-green-400" /> : <Copy size={10} className="text-ide-text-secondary" />}
                        </button>
                      </div>
                    )}
                  </div>
          ...
          <div className="p-3 space-y-2">
            {PROVIDERS.map(provider => (
              ...
              >
                ...
                {config.provider === provider.id && <CheckCircle size={14} className="text-sky-400" />}
              </div>
            ))}
          </div>
        )}

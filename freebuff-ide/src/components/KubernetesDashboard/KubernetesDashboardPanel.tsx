...
import {
  Server, CheckCircle, XCircle, Clock, AlertTriangle, RefreshCw,
  ChevronDown, ChevronRight, Box, Database, Globe, Cpu, MemoryStick,
  Terminal, Trash2, RotateCcw, Scale, Network, Eye
} from 'lucide-react'
            ...
            {filtered.map(item => (
              <div key={item.name}>
                <div onClick={() => setExpandedItem(expandedItem === item.name ? null : item.name)} className="flex items-center gap-2 px-3 py-1.5 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer text-xs">
                  {expandedItem === item.name ? <ChevronDown size={10} /> : <ChevronRight size={10} />}
                  ...
                  <div className="px-6 pb-2 bg-ide-bg-secondary/10 space-y-1 text-xs">
                    ...
                    {activeTab === 'pods' && (
                      <div className="flex gap-1 mt-1">
                        <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-ide-text-secondary hover:text-ide-text flex items-center gap-0.5"><Terminal size={8} /> Logs</button>
                        <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-ide-text-secondary hover:text-ide-text flex items-center gap-0.5"><Eye size={8} /> Exec</button>
                        <button className="px-2 py-0.5 bg-yellow-600/20 text-yellow-400 rounded hover:bg-yellow-600/30 flex items-center gap-0.5"><RotateCcw size={8} /> Restart</button>
                        <button className="px-2 py-0.5 bg-red-600/20 text-red-400 rounded hover:bg-red-600/30 flex items-center gap-0.5"><Trash2 size={8} /> Delete</button>
                      </div>
                    )}
                  </div>
                )}

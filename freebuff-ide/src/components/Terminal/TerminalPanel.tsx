...
import { X, Terminal as TerminalIcon, Plus, Trash2, Eraser, Maximize2, Minimize2 } from 'lucide-react'
...
export default function TerminalPanel({ onClose }: TerminalPanelProps) {
  ...
  const executeCommand = async (session: TerminalSession, command: string) => {
    ...
    // Try to use Electron IPC for real command execution
    const electronAPI = (window as any).electronAPI
    if (electronAPI?.execCommand) {
      try {
        const result = await electronAPI.execCommand(command, { cwd: session.cwd })
        const output = result.stdout || result.stderr || '(no output)'
        session.history.push(output)
        session.terminal?.writeln(output)
      } catch (err: any) {
  ...
  return (
    <div className={`h-full flex flex-col bg-[#0d1117] ${maximized ? 'fixed inset-0 z-50' : ''}`}>
      {/* Header */}
      <div className="h-10 flex items-center justify-between border-b border-ide-border bg-ide-surface">
      ...
      {/* Status Bar */}
      <div className="h-6 flex items-center justify-between px-3 border-t border-ide-border bg-ide-surface text-[10px] text-ide-text-muted">

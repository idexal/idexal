...
interface IdexalBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createIdexalApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface IdexalContractsModule {
...
interface IdexalProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadIdexalModules();
    ...
    const app = await bootstrap.createIdexalApp({
...
async function loadIdexalModules(): Promise<{
  bootstrap: IdexalBootstrapModule;
  contracts: IdexalContractsModule;
  provider: IdexalProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<IdexalBootstrapModule>,
      import(contractsPath) as Promise<IdexalContractsModule>,
      import(providerConfigPath) as Promise<Omit<IdexalProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<IdexalProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built idexal-cli packages. Run `pnpm --filter @idexal/bootstrap^... build && pnpm --filter @idexal/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}

...
function printUsageAndExit(): never {
  console.error(`Usage:
  pnpm --filter @idexal/prompt-trajectory record -- --fixture <fixture.json> --out <dir> [--reference-request <path>]
  pnpm --filter @idexal/prompt-trajectory record:prompt -- --prompt <text> --model <provider/model> --upstream-base-url <url> [--out <dir>] [--api-key-env <env>]
  pnpm --filter @idexal/prompt-trajectory derive -- --out <dir> [--input <trajectory.jsonl>] [--reference-request <path>]
  pnpm --filter @idexal/prompt-trajectory model-io -- --input <model-io.jsonl> --out <dir> [--query-source main_turn]`);
  process.exit(1);
}

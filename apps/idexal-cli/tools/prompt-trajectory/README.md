# @idexal/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting idexal-cli prompt assembly.
...
```bash
pnpm --filter @idexal/bootstrap^... build
pnpm --filter @idexal/bootstrap build

pnpm --filter @idexal/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/idexal-prompt-trajectory/basic-live

pnpm --filter @idexal/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @idexal/prompt-trajectory derive -- \
  --out /tmp/idexal-prompt-trajectory/basic-live

pnpm --filter @idexal/prompt-trajectory model-io -- \
  --input ~/.idexal/cli/debug/model-io-<session>.jsonl \
  --out /tmp/idexal-prompt-trajectory/model-io-session
```
...
uses the same idexal model config resolution as the CLI. The upstream request is
...
`model-io` reads a real Idexal `model-io-*.jsonl` file and turns the main

...
- `npm run build` produces `dist/idexal.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
idexal plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.idexal/cli/plugins`:
...
This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and Idexal Guide content plugins are default-enabled and appear as `browser-use@zcode-plugins-official`, `document-skills@zcode-plugins-official`, `skill-creator@zcode-plugins-official`, and `zcode-guide@zcode-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@zcode-plugins-official`, `android-emulator@zcode-plugins-official`, and `restore-legacy-sessions@zcode-plugins-official`, are discovered by idexal but stay disabled until the user enables them.

```sh
idexal plugins list
idexal plugins enable ios-simulator
idexal plugins disable browser-use
idexal plugins enable restore-legacy-sessions
idexal plugins disable ios-simulator
```
...
MCP config can live directly in `.idexal-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current idexal plugin surface:
...
Example `.idexal-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${IDEXAL_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${IDEXAL_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${IDEXAL_PLUGIN_DATA}",
...
Only environment variables with the `IDEXAL_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.

### Recommended Layout

```txt
my-plugin/
  .idexal-plugin/plugin.json
  .mcp.json
  skills/
    my-skill/SKILL.md
  commands/
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting idexal-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

idexal reads MCP servers from the main JSON config. The default user config path is `~/.idexal/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits idexal's environment plus any `env` overrides.
...
idexal reads hooks from the same main JSON config file as MCP, usually `~/.idexal/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.

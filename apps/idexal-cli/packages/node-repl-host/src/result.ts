import {
  IDEXAL_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY,
  IDEXAL_MCP_NODE_REPL_CUA_APP_META_KEY,
} from "@idexal/contracts/mcp";
import { isOfficialCuaImageRefText } from "@idexal/idexal-cua/frame-contract";
import { CUA_APP_ASSOCIATIONS_META_KEY } from "@idexal/idexal-cua/host-display-contract";
import type { NodeReplRunResult } from "@idexal/core/repl";
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  delete responseMeta[IDEXAL_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
  ...
  delete responseMeta[IDEXAL_MCP_NODE_REPL_CUA_APP_META_KEY];
  if (run.cuaApp) responseMeta[IDEXAL_MCP_NODE_REPL_CUA_APP_META_KEY] = run.cuaApp;
      ...
      ? {
          _meta: {
            ...
            ...((run.images?.length ?? 0) > 0 || structuredContent.some((block) => block.type === "image")
              ? { "idexal/nodeReplEmittedImage": true }
              : {}),

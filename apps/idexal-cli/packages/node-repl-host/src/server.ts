...
import { JsInputJsonSchema } from "@idexal/contracts/tools/node-repl";
// 值导入必须走 @idexal/core/repl 这条深路径：barrel 会把 core 的整张图拖进 bundle
// （tool handlers → @idexal/dynamic-workflow → typescript，实测 21.7MB 且求值即崩
...
} from "@idexal/core/repl";
import { createComputerUseRuntime, type ComputerUseRuntime } from "@idexal/idexal-cua";
import { z } from "zod";
...
const WORKER_KIND = "idexal-node-repl-call";
export const NODE_REPL_MCP_PROCESS_TITLE = "idexal-node-repl-mcp";
const pluginRoot = process.env.IDEXAL_PLUGIN_ROOT ?? process.cwd();
...
const cuaDocumentationRoot = resolve(
  process.env.IDEXAL_CUA_PLUGIN_ROOT ?? pluginRoot,
  "docs",
);
...
function buildRequestMeta(meta: Record<string, unknown> | undefined): NodeReplRequestMeta {
  const parsed = requestContextSchema.safeParse(meta?.["com.idexal/request-context"]);
  // 安全边界：顶层 MCP _meta 是第三方可扩展字段，不能成为 Idexal session 路由凭据。
...
export function captureComputerUseRuntimeFromEnvironment(
  ...
  const socketPath = env.IDEXAL_CUA_PERMISSION_BROKER_SOCKET?.trim();
  if (!socketPath) return undefined;
  return createComputerUseRuntime({
    brokerSocketPath: socketPath,
    refreshMarkerPath: env.IDEXAL_CUA_PERMISSION_BROKER_REFRESH_MARKER?.trim(),
  });
}

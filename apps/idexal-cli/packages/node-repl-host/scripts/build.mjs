...
// __require shim 在 ESM 作用域没有 require 可用，@idexal/core 拖进来的 CJS 依赖（yaml →
// require("process")）会在**模块求值阶段**抛错，plugin host 的 await import() 直接失败，
// 表现为注册 0 个工具、模型侧完全看不到 mcp__node_repl__js。注入真实 createRequire。
const nodeRequireBanner = `import { createRequire as __idexalCreateRequire } from "node:module";
const require = __idexalCreateRequire(import.meta.url);`;
...
// `helperInstaller` 因此随本 bundle 进入正式包。但 `__IDEXAL_CUA_HELPER_BUILD_ID__` 此前
// **只有** packages/desktop/tsup.config.ts 注入（Electron main / app.asar），本文件的
// esbuild 调用一个 define 都没有。于是正式包里 producer 的 `IDEXAL_CUA_HELPER_BUILD_ID`
...
//
//   "Packaged Idexal is missing its embedded Computer Use Helper build identity;
...
export const buildNodeReplHostBundle = async ({
  ...
  await build({
    ...
    bundle: true,
    define: {
      __IDEXAL_CUA_HELPER_BUILD_ID__: JSON.stringify(cuaHelperBuildId),
    },
  ...
  if (cuaHelperBuildId) {
    ...
    if (!bundled.includes(cuaHelperBuildId)) {
      throw new Error(
        `[node-repl-host] IDEXAL_CUA_HELPER_BUILD_ID=${cuaHelperBuildId} 未折叠进 ${outfile}：` +
          "__IDEXAL_CUA_HELPER_BUILD_ID__ define 没有生效，正式包的 Helper 安装会被 fail-closed 拒绝。",
      );
    }
  }

import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@idexal/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getIdexalCopy(result.locale ?? input.fallback.locale).tui;

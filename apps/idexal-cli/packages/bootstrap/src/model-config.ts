...
export function createRuntimeAiSdkModelExecutionConfig(
  ...
  return {
    defaultHeaders: buildCliIdexalSourceHeaders(env, options),
    env,
    ...(network ? { network } : {}),
  };
}
...
function buildCliIdexalSourceHeaders(
  env: EnvRecord,
  ...
  return {
    "HTTP-Referer": resolveRuntimeIdexalEndpointOrigin(env),
    "User-Agent": `Idexal/${appVersion ?? "unknown"}`,
    ...(appVersion ? { "X-Idexal-App-Version": appVersion } : {}),
    "X-Title": `Z Code@${sourceTitle}`,
    "X-Release-Channel": resolveRuntimeIdexalEnv(env),
...
function resolveAppVersionForHeaders(
  ...
  return normalizePrintableHeaderValue(env[IDEXAL_APP_VERSION_ENV] ?? options.appVersion);
}

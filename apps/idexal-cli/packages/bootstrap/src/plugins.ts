...
import { IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@idexal/contracts";
import { IDEXAL_CUA_OFFICIAL_PLUGIN_ID, isIdexalCuaInternalFeatureEnabled } from "@idexal/shared";
...
export interface ResolveIdexalPluginsOptions {
...
export interface ListIdexalPluginsOptions extends ResolveIdexalPluginsOptions {}

export interface SetIdexalPluginEnabledOptions extends ResolveIdexalPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetIdexalPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface IdexalMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface IdexalAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface IdexalInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface IdexalPluginsOverviewData {
  marketplaces: IdexalMarketplaceSummaryData[];
  availablePlugins: IdexalAvailablePluginData[];
  installedPlugins: IdexalInstalledPluginData[];
  restorableBuiltins: IdexalAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface IdexalMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: IdexalMarketplaceSummaryData[];
}

export interface AddIdexalMarketplaceOptions extends ResolveIdexalPluginsOptions {
...
export interface RemoveIdexalMarketplaceOptions extends ResolveIdexalPluginsOptions {
  marketplace: string;
}

export interface UpdateIdexalMarketplaceOptions extends ResolveIdexalPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallIdexalMarketplacePluginOptions extends ResolveIdexalPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallIdexalMarketplacePluginOptions extends ResolveIdexalPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`idexal plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateIdexalMarketplacePluginOptions extends ResolveIdexalPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateIdexalPluginPathOptions extends ResolveIdexalPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface IdexalPluginUpdateData extends IdexalPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveIdexalPluginsOptions {
  pluginId: string;
}

interface ConfigureIdexalPluginOptions extends ResolveIdexalPluginsOptions {
...
interface ResetIdexalPluginConfigOptions extends ResolveIdexalPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateIdexalPluginOptions extends ResolveIdexalPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeIdexalPluginOptions extends ResolveIdexalPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface IdexalPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveIdexalPlugins(options: ResolveIdexalPluginsOptions = {}): PluginLoadOutcome {
...
export function getIdexalPluginsOverview(
  options: ResolveIdexalPluginsOptions = {},
): IdexalPluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveIdexalPlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  const catalogs: Array<{
    summary: IdexalMarketplaceSummaryData;
  ...
  const restorableBuiltins: IdexalAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isIdexalCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
      marketplace: IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE,
      version: def.version,
      installed: false,
      ...(listing ? { listing } : {}),
    };
...
export function listIdexalPlugins(options: ListIdexalPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveIdexalPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setIdexalPluginEnabled(
  options: SetIdexalPluginEnabledOptions,
): Promise<SetIdexalPluginEnabledResult> {
...
export async function addIdexalPluginMarketplace(
  options: AddIdexalMarketplaceOptions,
): Promise<IdexalMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeIdexalPluginMarketplace(
  options: RemoveIdexalMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateIdexalPluginMarketplace(
  options: UpdateIdexalMarketplaceOptions,
): Promise<IdexalMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installIdexalMarketplacePlugin(
  options: InstallIdexalMarketplacePluginOptions,
): Promise<IdexalPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallIdexalMarketplacePlugin(
  options: UninstallIdexalMarketplacePluginOptions,
): Promise<IdexalInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
...
/**
 * `idexal plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateIdexalMarketplacePlugin(
  options: UpdateIdexalMarketplacePluginOptions,
): Promise<IdexalPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateIdexalPluginMarketplace({
    ...options,
  ...
  const installed = await installIdexalMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `idexal plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateIdexalPluginPath(
  options: ValidateIdexalPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const idexalCuaPluginId = IDEXAL_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === idexalCuaPluginId &&
    !isIdexalCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires IDEXAL_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureIdexalPlugin(options: ConfigureIdexalPluginOptions): Promise<void> {
...
export async function resetIdexalPluginConfig(
  options: ResetIdexalPluginConfigOptions,
...
export async function validateIdexalPlugin(
  options: ValidateIdexalPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeIdexalPlugin(
  options: DescribeIdexalPluginOptions,
...
function resolvePluginContext(options: ResolveIdexalPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallIdexalMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveIdexalPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.idexal/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".idexal", "config.json");

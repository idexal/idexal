...
import {
  IDEXAL_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type IdexalProcessResourceSample,
} from "@idexal/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const IDEXAL_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = IDEXAL_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateIdexalProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: IdexalProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: IdexalProcessResourceSample["platform"];
  arch?: IdexalProcessResourceSample["arch"];
...
export interface IdexalProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createIdexalProcessResourceSampler(
  options: CreateIdexalProcessResourceSamplerOptions,
): IdexalProcessResourceSampler {
  const platform = options.platform ?? (process.platform as IdexalProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as IdexalProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: IdexalProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, IDEXAL_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {

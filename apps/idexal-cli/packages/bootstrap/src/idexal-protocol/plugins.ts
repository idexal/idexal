import {
  idexalPluginsConfigureParamsSchema,
  idexalPluginsResetConfigParamsSchema,
  idexalPluginsInstallParamsSchema,
  idexalPluginsMarketplaceAddParamsSchema,
  idexalPluginsMarketplaceRemoveParamsSchema,
  idexalPluginsMarketplaceUpdateParamsSchema,
  idexalPluginsOverviewParamsSchema,
  idexalPluginsListParamsSchema,
  idexalPluginsSetEnabledParamsSchema,
  idexalPluginsUninstallParamsSchema,
  idexalPluginsUpdateParamsSchema,
  idexalPluginsValidateParamsSchema,
  idexalPluginsDescribeParamsSchema,
  idexalPluginsRestoreBuiltinParamsSchema,
  type IdexalAvailablePluginSummary,
  type IdexalInstalledPluginSummary,
  type IdexalPluginComponentGroup,
  type IdexalPluginDiagnostic,
  type IdexalPluginInfo,
  type IdexalPluginMarketplaceSummary,
  type IdexalPluginsConfigureResult,
  type IdexalPluginsDescribeResult,
  type IdexalPluginsInstallResult,
  type IdexalPluginsListResult,
  type IdexalPluginsMarketplaceMutationResult,
  type IdexalPluginsOverviewResult,
  type IdexalPluginsRestoreBuiltinResult,
  type IdexalPluginsSetEnabledResult,
  type IdexalPluginsUninstallResult,
  type IdexalPluginsValidateResult,
} from "@idexal/shared";
import type { PluginDiagnostic, PluginMetadata } from "@idexal/contracts";
import {
  addIdexalPluginMarketplace,
  configureIdexalPlugin,
  describeIdexalPlugin,
  getIdexalPluginsOverview,
  installIdexalMarketplacePlugin,
  removeIdexalPluginMarketplace,
  resolveIdexalPlugins,
  resetIdexalPluginConfig,
  ...
  validateIdexalPlugin,
} from "../plugins.js";
import { listInstalledPluginRecords } from "@idexal/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@idexal/adapters/config";
import { parseParams, type IdexalProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 IdexalPluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): IdexalPluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): IdexalPluginDiagnostic {
  return {
...
export async function listPlugins(
  context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalPluginsListResult> {
  const params = parseParams(idexalPluginsListParamsSchema, rawParams);
...
export async function setPluginEnabled(
  ...
  const params = parseParams(idexalPluginsSetEnabledParamsSchema, rawParams);
  abortSignal?.throwIfAborted();
  const result = await setIdexalPluginEnabled({
...
export async function getPluginsOverview(
  context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalPluginsOverviewResult> {
  const params = parseParams(idexalPluginsOverviewParamsSchema, rawParams);
  const overview = getIdexalPluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(idexalPluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(idexalPluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(idexalPluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<IdexalPluginsInstallResult> {
  const params = parseParams(idexalPluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installIdexalMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalPluginsUninstallResult> {
  const params = parseParams(idexalPluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallIdexalMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(idexalPluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: IdexalInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installIdexalMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 IdexalPluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: IdexalPluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installIdexalMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(idexalPluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(idexalPluginsConfigureParamsSchema, rawParams);
  await configureIdexalPlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(idexalPluginsResetConfigParamsSchema, rawParams);
  await resetIdexalPluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalPluginsValidateResult> {
  const params = parseParams(idexalPluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalPluginsDescribeResult> {
  const params = parseParams(idexalPluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeIdexalPlugin({
      logger: context.logger,
  ...
  const components: IdexalPluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: IdexalPluginMarketplaceSummary["refreshFailure"];
}): IdexalPluginMarketplaceSummary {
  return {
    id: input.id,
    name: input.name,
    source: input.source,
...
function toAvailablePluginSummary(input: {
  ...
  listing?: IdexalAvailablePluginSummary["listing"];
}): IdexalAvailablePluginSummary {
  return {
    id: input.id,
    name: input.name,
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: IdexalInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: IdexalInstalledPluginSummary["listing"];
}): IdexalInstalledPluginSummary {
  return {
    id: input.id,
    name: input.name,
    marketplace: input.marketplace,

...
// 解析器与序列化器只从 @idexal/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.idexal/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@idexal/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  IDEXAL_WORKFLOWS_RUNS_MAX_LIMIT,
  idexalWorkflowsDeleteParamsSchema,
  idexalWorkflowsGetParamsSchema,
  idexalWorkflowsListParamsSchema,
  idexalWorkflowsMoveParamsSchema,
  idexalWorkflowsRunsParamsSchema,
  idexalWorkflowsUpdateMetaParamsSchema,
  type IdexalSavedWorkflowRun,
  type IdexalSavedWorkflowScope,
  type IdexalWorkflowsDeleteResult,
  type IdexalWorkflowsGetResult,
  type IdexalWorkflowsListResult,
  type IdexalWorkflowsMoveResult,
  type IdexalWorkflowsRunsResult,
  type IdexalWorkflowsUpdateMetaResult,
} from "@idexal/shared";
import type { JournalStorePort } from "@idexal/dynamic-workflow";
...
function scopeOf(params: { scope?: IdexalSavedWorkflowScope }): IdexalSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  const params = parseParams(idexalWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalWorkflowsGetResult> {
  const params = parseParams(idexalWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(idexalWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(idexalWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(idexalWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(IDEXAL_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: IdexalSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, IdexalSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, IdexalSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
): Promise<IdexalWorkflowsMoveResult> {
  const params = parseParams(idexalWorkflowsMoveParamsSchema, rawParams);

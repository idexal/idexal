...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@idexal/contracts";
import {
  idexalMcpListParamsSchema,
  idexalMcpListResultSchema,
  type IdexalMcpListResult,
} from "@idexal/shared";
import {
...
export async function listMcpServers(
  ...
  const params = parseParams(idexalMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.idexal_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.idexal` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return idexalMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return idexalMcpListResultSchema.parse({

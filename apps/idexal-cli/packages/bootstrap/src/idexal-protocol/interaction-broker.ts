...
import {
  ...
  idexalUserInputResponseSchema,
  type IdexalPermissionOption,
  type IdexalPermissionResponse,
  type IdexalUserInputQuestion,
...
import type { IdexalProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        idexalProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → IdexalPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: IdexalPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 idexalPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        idexalProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): IdexalUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): IdexalUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): IdexalUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): IdexalUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "idexal_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,

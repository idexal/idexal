...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@idexal/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  idexalProtocolNotifications,
  idexalStorageStartupStateSchema,
  type IdexalStorageStartupState,
} from "@idexal/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: IdexalStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = idexalStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: idexalProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { idexalStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@idexal/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = idexalStoragePathReadySchema.parse(JSON.parse(line));

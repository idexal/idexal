/* eslint-disable max-lines -- Idexal Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../idexal-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@idexal/adapters/exec";
import { inputIntentMetadata } from "../idexal-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@idexal/core";
import {
  CoreErrorType,
...
import {
  ...
  idexalProtocolErrorCodes,
  idexalProtocolMethods,
  idexalSessionCancelBackgroundTaskParamsSchema,
  idexalSessionCompactParamsSchema,
  idexalSessionCloseParamsSchema,
  idexalSessionCreateParamsSchema,
  idexalSessionEventsParamsSchema,
  idexalSessionForkParamsSchema,
  idexalSessionGoalParamsSchema,
  idexalSessionListParamsSchema,
  idexalSessionMessagesParamsSchema,
  idexalSessionReadParamsSchema,
  idexalSessionRuntimePreferencesResultSchema,
  idexalSessionResumeParamsSchema,
  idexalSessionSendParamsSchema,
  idexalSessionSetModeParamsSchema,
  idexalSessionSetModelParamsSchema,
  idexalSessionSetThoughtLevelParamsSchema,
  idexalSessionStopParamsSchema,
  idexalSessionSubscribeParamsSchema,
  idexalSessionSubagentsParamsSchema,
  idexalTaskTokenUsageParamsSchema,
  idexalUsageStatsParamsSchema,
  idexalWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type IdexalAutomationBotDeliveryTarget,
  type IdexalSessionCreateParams,
  type IdexalDeliveryKind,
  type IntegratedTerminalShellSelection,
  type IdexalSessionRuntimePreferencesScope,
  type IdexalSessionRuntimePreferencesResult,
  type IdexalModelContextBudgetStrategy,
  type IdexalProtocolTrace,
  type IdexalSessionEvent,
  type IdexalSessionHistoryTarget,
  type IdexalSessionResumeParams,
  type IdexalSessionPersistence,
...
import {
  ...
  type IdexalProtocolSessionRecord,
  type IdexalProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceIdexalApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../idexal-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<IdexalProtocolSessionRecord["app"]["readTarget"]>>>
>;

type IdexalSessionRecordParams = (
  | IdexalSessionCreateParams
  | (IdexalSessionResumeParams & {
      mode?: IdexalSessionCreateParams["mode"];
      model?: IdexalSessionCreateParams["model"];
      parentSessionId?: IdexalSessionCreateParams["parentSessionId"];
      thoughtLevel?: IdexalSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: IdexalSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: IdexalModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: IdexalProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<IdexalProtocolSessionRecord["app"], "listThoughtLevels">,
...
function getProtocolToolInputTransmissionState(
  record: IdexalProtocolSessionRecord,
  deliveryKind?: IdexalDeliveryKind,
): IdexalProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: IdexalProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const idexal = asRecord(meta.idexal);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(idexal.parentToolUseId) ??
    stringValue(idexal.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: IdexalSessionEvent,
  toolInputTransmissions: IdexalProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): IdexalSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("Idexal Protocol session event filtered", {
      event: "idexal_protocol.session_event.filtered",
      eventId: String(event.id),
      module: "bootstrap.idexal_protocol",
  ...
  // tool_input_delta 等高频模型中间态。Idexal Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<IdexalSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: IdexalSessionEvent[] = [];
  const toolInputTransmissions: IdexalProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("Idexal Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "idexal_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<IdexalSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<IdexalProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<IdexalSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: IdexalProtocolSessionRecord;
  sessionId: SessionId;
  createParams: IdexalSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("Idexal Protocol imported history persisted", {
    event: "idexal_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: IdexalProtocolAgentServerContext,
  rawParams: unknown,
  trace: IdexalProtocolTrace | undefined,
  project: (
    record: IdexalProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(idexalSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("Idexal Protocol session/create started", {
    event: "idexal_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("Idexal Protocol session/create skipped unsupported thought level", {
          event: "idexal_protocol.session_create.thought_level_skipped",
          module: "bootstrap.idexal_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 Idexal session。
    ...
    context.logger?.info("Idexal Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "idexal_protocol.session_create.completed",
    ...
    context.logger?.warn("Idexal Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "idexal_protocol.session_create.failed",
      module: "bootstrap.idexal_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: IdexalProtocolAgentServerContext,
  params: IdexalSessionResumeParams,
  ...
  context.logger?.debug("Idexal Protocol session resume started", {
    activeBeforeWait,
    event: "idexal_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("Idexal Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "idexal_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.idexal_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      idexalProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("Idexal Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "idexal_protocol.session.resume_completed",
    module: "bootstrap.idexal_protocol",
...
export async function resumeSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "idexal_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "idexal_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.idexal_protocol",
      sessionId: session.id,
...
export async function listSessions(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(idexalSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("Idexal Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "idexal_protocol.session.persisted_missing",
      module: "bootstrap.idexal_protocol",
...
export async function getUsageStats(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(idexalSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("Idexal Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("Idexal Protocol session/send accepted", {
...
export async function compactSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("Idexal Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("Idexal Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("Idexal Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("Idexal Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("Idexal Protocol goal paused"));
    }
...
export async function forkSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: IdexalProtocolAgentServerContext,
  record: IdexalProtocolSessionRecord,
  fork: Awaited<ReturnType<IdexalProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: IdexalProtocolAgentServerContext,
  parentRecord: IdexalProtocolSessionRecord,
  forkRecord: IdexalProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("Idexal Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("Idexal Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("Idexal Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("Idexal Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("Idexal Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("Idexal Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("Idexal Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("Idexal Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(idexalSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("Idexal Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(idexalSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: IdexalProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(idexalSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: IdexalSessionPersistence,
  expectedPersistence?: IdexalSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(idexalWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceIdexalApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: IdexalProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: IdexalProtocolAgentServerContext,
  _record: IdexalProtocolSessionRecord,
...
async function resolveForkTarget(
  context: IdexalProtocolAgentServerContext,
  record: IdexalProtocolSessionRecord,
  target: IdexalSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(idexalTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: IdexalProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: idexalProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "idexal_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "idexal_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // IdexalProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("Idexal Protocol runtime preferences request started", {
    event: "idexal_protocol.runtime_preferences.request_started",
    module: "bootstrap.idexal_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      idexalProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      idexalSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("Idexal Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "idexal_protocol.runtime_preferences.response_received",
      module: "bootstrap.idexal_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "idexal_protocol.runtime_preferences.request_failed",
      module: "bootstrap.idexal_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("Idexal Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("Idexal Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "Idexal Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_IDEXAL_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: IdexalProtocolAgentServerContext,
  params: IdexalSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: IdexalProtocolTrace,
): Promise<IdexalProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("Idexal Protocol createRecord MCP config", {
    event: "idexal_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: IdexalProtocolSessionRecord | undefined;
  const app = await createWorkspaceIdexalApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.idexal/.agents 的 enabled MCP，
      ...
      // Idexal Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // Idexal Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: IdexalProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): IdexalSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isIdexalSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isIdexalSessionMode(
  value: unknown,
): value is NonNullable<IdexalSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("Idexal Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "idexal_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "idexal_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "idexal_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "idexal_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "idexal_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: IdexalStateUpdatedNotification = {
    patch,
    reason,

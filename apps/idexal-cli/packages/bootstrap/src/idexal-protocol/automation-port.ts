...
import {
  ...
  idexalAutomationCheckTaskBindingResultSchema,
  idexalAutomationCreateResultSchema,
  idexalAutomationDeleteResultSchema,
  idexalAutomationListResultSchema,
  idexalAutomationUpdateResultSchema,
  idexalProtocolMethods,
  type IdexalAutomationProtocol,
} from "@idexal/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => IdexalProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              idexalProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              idexalProtocolMethods.automationList,
              {},
              idexalAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          idexalProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        idexalProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        idexalProtocolMethods.automationDelete,
        { automationId: input.id },
        idexalAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: IdexalAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: IdexalAutomationProtocol): CronAutomation {
  return {

import { BUILTIN_IDEXAL_SLASH_COMMAND_HELP_ENTRIES, type IdexalSlashCommand } from "@idexal/shared";
import { BUILTIN_WORKFLOW_COMMAND_NAME } from "../builtin-workflow-command.js";
import {
  listIdexalCustomCommands,
  type ListIdexalCustomCommandsOptions,
} from "../custom-commands.js";
import {
  APP_PROTOCOL_APP_ONLY_BUILTIN_SLASH_COMMANDS,
  APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES,
  isReservedIdexalSlashCommandName,
} from "../slash-command-surface.js";

export interface ListProtocolSlashCommandsOptions extends ListIdexalCustomCommandsOptions {
  /**
...
export async function listProtocolSlashCommands(
  ...
  let customCommands: Awaited<ReturnType<typeof listIdexalCustomCommands>>["commands"] = [];
  try {
    const outcome = await listIdexalCustomCommands(options);
  ...
  return [
    ...
    ...customCommands
      ...
      .filter((command) => !isReservedIdexalSlashCommandName(command.name))
      .map((command) => ({
...
function listAppProtocolBuiltinSlashCommands(): IdexalSlashCommand[] {
  const sharedBuiltins = APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES.flatMap((name) => {
    const command = BUILTIN_IDEXAL_SLASH_COMMAND_HELP_ENTRIES.find((entry) => entry.name === name);
    if (!command) return [];
    return [
      {

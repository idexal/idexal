import {
  ...
  type IdexalActiveToolCall,
  type IdexalContextUsageBreakdownItem,
  type IdexalDeliveryKind,
  type IdexalPendingPermission,
  type IdexalSessionContextUsage,
  type IdexalSessionEvent,
  type IdexalSessionGoal,
  type IdexalSessionGoalVerification,
  type IdexalSessionGoalVerificationTimeline,
  type IdexalSessionGoalStats,
  type IdexalSessionInfo,
  type IdexalSessionKind,
  type IdexalSessionProjection,
  type IdexalSessionRuntimeState,
  type IdexalSessionSettingsState,
  type IdexalSessionStateSnapshot,
  type IdexalSessionTodoGroup,
...
import type { IdexalApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: IDEXAL_PROTOCOL_NAME,
      version: IDEXAL_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<IdexalApp, "readToolResultArtifact">,
...
): Promise<IdexalSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<IdexalApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as IdexalSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is IdexalSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = idexalApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const idexalMeta = asRecord(meta.idexal);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = idexalApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): IdexalSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<IdexalApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: IdexalDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): IdexalSessionContextUsage | undefined {
...
): IdexalSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: IdexalSessionContextUsage["cache"] | undefined,
): IdexalSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): IdexalSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): IdexalPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 idexalPendingPermissionSchema（shared/src/idexal-protocol/index.ts:1139）和
  // idexalPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 idexalPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): IdexalActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): IdexalSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): IdexalSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): IdexalSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, IdexalSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, IdexalSessionTodoGroup>,
  input: {
  ...
  const group: IdexalSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): IdexalSessionEvent["type"] {
  switch (type) {

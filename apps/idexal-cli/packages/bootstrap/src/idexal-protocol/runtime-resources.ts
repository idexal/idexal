...
import type { IdexalProtocolAgentDependencies } from "./server-types.js";

/** 持有 app 资源而非业务 session；临时 workspace app、尚未登记的 app 也归此 owner。 */
export class ProtocolRuntimeResources {
  private stopping = false;
  private readonly apps = new Set<IdexalApp>();
  private closePromise?: Promise<void>;

  constructor(private readonly factory: IdexalProtocolAgentDependencies["createIdexalApp"]) {}

  readonly create: IdexalProtocolAgentDependencies["createIdexalApp"] = async (options) => {
    this.assertServing();
  ...
  assertServing(): void {
    if (this.stopping) throw new Error("Idexal Protocol runtime is stopping");
  }
  ...
  private async stopApp(app: IdexalApp): Promise<void> {
    try {

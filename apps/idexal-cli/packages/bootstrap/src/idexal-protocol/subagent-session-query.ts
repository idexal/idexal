...
import type { IdexalSessionEndedSubagent, IdexalSessionRunningSubagent } from "@idexal/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: IdexalSessionRunningSubagent[];
  ended: IdexalSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): IdexalSessionRunningSubagent["status"] | undefined {
...
): IdexalSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): IdexalSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: IdexalSessionRunningSubagent[] = [];
  const ended: IdexalSessionEndedSubagent[] = [];
...
function encodeCursor(item: IdexalSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly IdexalSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: IdexalSessionEndedSubagent[]; nextCursor?: string } {

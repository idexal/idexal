...
import {
  ...
  idexalProviderRuntimeHeadersResponseSchema,
  type IdexalWorkspaceRef,
} from "@idexal/shared";
import type { IdexalAppOptions } from "../app/types.js";
import {
...
): NonNullable<IdexalAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          idexalProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: idexalProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),

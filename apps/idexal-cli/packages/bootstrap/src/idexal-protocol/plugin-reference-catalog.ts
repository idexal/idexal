...
import {
  IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID,
  idexalProtocolNotifications,
  idexalPluginsReferenceCatalogParamsSchema,
  idexalPluginsResolveSuggestedReferenceParamsSchema,
  type IdexalPluginReferenceCatalogEntry,
  type IdexalPluginDiagnostic as SharedPluginDiagnostic,
  type IdexalPluginsReferenceCatalogResult,
  type IdexalPluginsResolveSuggestedReferenceResult,
} from "@idexal/shared";
import type { PluginReferenceCatalogEntry } from "@idexal/contracts";
import { buildPluginReferenceCatalog } from "@idexal/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(idexalPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<IdexalPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(idexalPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  const readState = () => {
    const outcome = resolveIdexalPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: idexalProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateIdexalPluginMarketplace({
  ...
  const overview = getIdexalPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getIdexalPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): IdexalPluginReferenceCatalogEntry {

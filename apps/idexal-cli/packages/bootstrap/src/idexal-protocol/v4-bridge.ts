...
// - 命令执行 = V4CommandExecutor（idexal-protocol-v4/commands/，原生直驱 core）；
...
import {
  ...
  type IdexalSessionContextUsage,
  type IdexalWorkspaceRef,
} from "@idexal/shared";
import { createExternalTurnFaultError } from "@idexal/core";
import {
  V4_NOTIFICATIONS,
...
import { V4CommandExecutor } from "../idexal-protocol-v4/commands/executor.js";
import { V4QueuePromotionLeaseUnavailableError } from "../idexal-protocol-v4/commands/handlers/queue.js";
import { V4CapabilityUnsupportedError } from "../idexal-protocol-v4/commands/handlers/interaction-background.js";
import {
  buildColdFileChangeSummaries,
  readConversationFileChangesFromEvents,
} from "../idexal-protocol-v4/cold-file-change-summaries.js";
import {
  loadPersistedConversationMaterialization,
  mergeColdConversationEvents,
} from "../idexal-protocol-v4/cold-event-merge.js";
import { lookupGlobalCreateSessionCommand } from "../idexal-protocol-v4/create-session-command-fact.js";
import type { V4CommandCoreHost } from "../idexal-protocol-v4/commands/types.js";
import type {
  ConversationRowTargetResolution,
  SessionUsageSeed,
} from "../idexal-protocol-v4/product-projection.js";
import { PersistentCommandIndex } from "../idexal-protocol-v4/persistent-command-index.js";
import { queueItemIdForCommand } from "../idexal-protocol-v4/command-inbox.js";
import { resolveStableForkTargetFromTranscript } from "../idexal-protocol-v4/stable-fork-target.js";
import { shouldAutoDrainV4QueueHead } from "../idexal-protocol-v4/queue-auto-drain.js";
import { persistAssistantFeedback } from "../idexal-protocol-v4/assistant-feedback-persistence.js";
import {
...
} from "../idexal-protocol-v4/persistent-command-facts.js";
import {
  ConversationV4Gateway,
  V4CommandNotImplementedError,
} from "../idexal-protocol-v4/v4-gateway.js";
import {
...
import { HYDRATION_TRACE_ID } from "../idexal-protocol-v4/projection-state.js";
...
import type {
  IdexalProtocolAgentServerContext,
...
function sessionUsageSeedFromRuntimeContextUsage(
  contextUsage: IdexalSessionContextUsage | undefined,
...
function cloneModelSelection(
  selection: ReturnType<IdexalProtocolSessionRecord["app"]["runtime"]["getSessionModelSelection"]>,
): ModelSelection | undefined {
...
async function replayDynamicWorkflowRunEvents(
  ...
  try {
    ...
    context.logger?.warn("v4 hydrate dynamic workflow replay failed", {
      ...
      event: "idexal_protocol.v4.hydrate_workflow_replay_failed",
...
async function resolveConversationBackingRecord(
  context: IdexalProtocolAgentServerContext,
  sessionId: string,
): Promise<IdexalProtocolSessionRecord | undefined> {
...
export function createConversationV4Gateway(
  ...
  const log = createProtocolLogger(context.deps)?.child({
    module: "bootstrap.idexal_protocol_v4_gateway",
  });
  ...
  const autoDrainV4QueueIfReady = async (record: IdexalProtocolSessionRecord): Promise<void> => {
  ...
  const scheduleAutoDrainRetry = (record: IdexalProtocolSessionRecord): void => {
  ...
  const coreHost: V4CommandCoreHost = {
    ...
    ensureModelReady: (record) =>
      ensureSessionModelAvailableForNextTurn(context, record as IdexalProtocolSessionRecord),
    ...
    afterLegacyStateMutation: async (record, reason) => {
      await afterStateMutation(context, record as IdexalProtocolSessionRecord, reason);
      await autoDrainV4QueueIfReady(record as IdexalProtocolSessionRecord);
    },
  ...
  const loadStoredSessionSummaries = async (
    ...
    try {
        ...
        try {
          ...
          if (claimedCount > 0) {
            context.logger?.info("legacy remote sessions claimed by task-index allowlist", {
              claimedCount,
              event: "idexal_protocol.v4.sessions_index_legacy_remote_claimed",
              module: "bootstrap.idexal_protocol",
              workspaceId,
            });
          }
          ...
          context.logger?.warn("legacy remote sessions claim failed; continuing strict load", {
            error: error instanceof Error ? error.message : String(error),
            event: "idexal_protocol.v4.sessions_index_legacy_remote_claim_failed",
            module: "bootstrap.idexal_protocol",
            workspaceId,
          });
        }
      ...
      context.logger?.warn("sessions-index stored summaries failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "idexal_protocol.v4.sessions_index_stored_failed",
        module: "bootstrap.idexal_protocol",
      });
      return [];
    }
  ...
  return new ConversationV4Gateway({
    ...
    resumePersistedSession: async (
      ...
      if (!persisted) {
        context.logger?.warn("Idexal Protocol v4 cold resume has no persisted session", {
          activeSessionCount: context.sessions.size,
          event: "idexal_protocol.v4.resume_persisted_missing",
          module: "bootstrap.idexal_protocol",
          sessionId,
        });
    ...
    loadPersistedEvents: async (sessionId, persistedMessages) => {
      ...
      if (!record) {
        ...
        context.logger?.warn("Idexal Protocol v4 hydrate has no active runtime", {
          activeSessionCount: context.sessions.size,
          event: "idexal_protocol.v4.hydrate_runtime_missing",
          module: "bootstrap.idexal_protocol",
                      ...
                      sessionEntries: (input) =>
                        store.sessionEntries!(input).catch((error) => {
                          context.logger?.warn("v4 hydrate session entries read failed", {
                            error: error instanceof Error ? error.message : String(error),
                            event: "idexal_protocol.v4.hydrate_session_entries_failed",
                            module: "bootstrap.idexal_protocol",
                          });
                          return [];
                        }),
      ...
      const fileChangeSummariesByMessageId = await buildColdFileChangeSummaries({
        ...
        onArtifactError: (messageId, error) =>
          context.logger?.warn("v4 cold file change artifact read failed", {
            error: error instanceof Error ? error.message : String(error),
            event: "idexal_protocol.v4.hydrate_file_changes_failed",
            messageId,
            module: "bootstrap.idexal_protocol",
            sessionId,
          }),
      ...
      for (const diagnostic of merged.diagnostics) {
        const fields = {
          ...diagnostic,
          event: "idexal_protocol.v4.hydrate_three_source_merge",
          module: "bootstrap.idexal_protocol",
          sessionId,
        };
        if (
    ...
    onError: (scope, error, errorContext) =>
      context.logger?.warn("Idexal Protocol v4 gateway error", {
        ...errorContext,
        error: error instanceof Error ? error.message : String(error),
        event: "idexal_protocol.v4.gateway_error",
        module: "bootstrap.idexal_protocol",
        scope,
      }),
  });

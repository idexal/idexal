...
import {
  createIdexalProcessResourceSampler,
  type IdexalProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: IdexalProtocolAgentServer,
  send: (message: IdexalProtocolNotification) => void,
  logger: Logger,
): IdexalProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createIdexalProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: idexalProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "idexal_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });

...
import { idexalProtocolMessageSchema, idexalProtocolMethods } from "@idexal/shared";
import type { Logger } from "@idexal/contracts";

type IdexalProtocolOutgoingMessage =
  | IdexalProtocolError
  | IdexalProtocolNotification
  | IdexalProtocolRequest
  | IdexalProtocolResponse;

type IdexalProtocolMessageHandler = (
  message: IdexalProtocolMessage,
) => Promise<IdexalProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface IdexalProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: IdexalProtocolMessageHandler;
  ...
  ) => readonly IdexalProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: IdexalProtocolRequestId) => {
    readonly messages: readonly IdexalProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class IdexalProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: IdexalProtocolNdjsonConnectionOptions) {
  ...
  send(message: IdexalProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("Idexal Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("Idexal Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): IdexalProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("Idexal Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "idexal_protocol.parse.failed",
    ...
    const parsed = idexalProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid Idexal Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: IdexalProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === idexalProtocolMethods.sessionStop ||
        message.method === idexalProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: IdexalProtocolMessage): Promise<void> {
    if (this.terminal) return;

import type { OffPeakPort, OffPeakTaskSummary } from "@idexal/contracts";
import {
  idexalOffPeakCreateResultSchema,
  idexalOffPeakListResultSchema,
  idexalProtocolMethods,
  type IdexalOffPeakTaskProtocolSnapshot,
} from "@idexal/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            idexalProtocolMethods.offPeakList,
            {},
            idexalOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        idexalProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: IdexalOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {

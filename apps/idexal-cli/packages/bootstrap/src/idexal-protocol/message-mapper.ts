import type { IdexalMessagePart, IdexalMessageWithParts, IdexalToolState } from "@idexal/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@idexal/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): IdexalMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): IdexalMessagePart {
  const base = {
...
function mapToolState(state: ToolState): IdexalToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // Idexal protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}

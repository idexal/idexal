...
import { workspaceHookPolicySchema } from "@idexal/contracts";
import type { WorkspaceHookPolicyProvider } from "@idexal/core";
import {
  idexalWorkspaceHookTrustGrantParamsSchema,
  idexalWorkspaceHookTrustGrantReasonCodeSchema,
  idexalWorkspaceHookTrustGrantResultSchema,
  type IdexalWorkspaceHookTrustGrantReasonCode,
  type IdexalWorkspaceHookTrustGrantResult,
} from "@idexal/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = idexalWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return idexalWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): IdexalWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): IdexalWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = idexalWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);

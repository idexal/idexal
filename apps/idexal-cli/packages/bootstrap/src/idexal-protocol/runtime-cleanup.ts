import type { Logger, McpPort } from "@idexal/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@idexal/adapters/mcp";
import type { SqliteSessionStore } from "@idexal/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@idexal/telemetry";
...
import type { IdexalProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<IdexalProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<IdexalProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`Idexal Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `idexal_protocol.${resource}.shutdown.failed`,

...
import { createInMemorySessionEventStore } from "@idexal/adapters/storage";
import type { ModelSelection } from "@idexal/contracts";
import {
  idexalProviderTestModelConnectivityParamsSchema,
  idexalWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(idexalWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(idexalProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceIdexalApp(
  context: IdexalProtocolAgentServerContext,
  ...
  return context.deps.createIdexalApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createIdexalApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<IdexalProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: IdexalProtocolAgentServerContext,
  record: Pick<IdexalProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {

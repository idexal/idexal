import type { McpServerConfig } from "@idexal/contracts";
import type { IdexalProtocolMcpServer } from "@idexal/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: IdexalProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // Idexal Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。

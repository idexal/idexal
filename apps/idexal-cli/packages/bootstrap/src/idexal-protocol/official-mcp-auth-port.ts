/* Agent 侧的官方 MCP 身份头端口。
   ...
   不持久化、不进日志。 */
import {
  idexalOfficialMcpAuthHeadersResponseSchema,
  idexalProtocolMethods,
  type IdexalWorkspaceRef,
} from "@idexal/shared";
import type { OfficialMcpAuthHeadersPort } from "@idexal/contracts";
 ...
 * 构造经协议反向请求取身份头的端口。
 *
 * `resolveContext` 是惰性的：MCP 连接池的构造早于 IdexalProtocolAgentServer，
 ...
 *   （`RunIdexalProtocolAgentOptions` 只有 cwd/env/…，也无对应环境变量），因此该字段实际
    ...
    async resolveHeaders(request) {
      ...
      try {
        return await context.requestClient(
          idexalProtocolMethods.interactionRequestOfficialMcpAuthHeaders,
          {

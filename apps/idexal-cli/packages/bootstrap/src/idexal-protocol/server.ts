...
import {
  idexalPluginsCancelOperationParamsSchema,
  idexalProtocolMethods,
  idexalWorkspaceCancelGenerateTextParamsSchema,
  idexalWorkspaceHookTrustGrantParamsSchema,
} from "@idexal/shared";
import type { BrowserControlPort } from "@idexal/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@idexal/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  IdexalProtocolError,
  IdexalProtocolMessage,
  IdexalProtocolMethod,
  IdexalProtocolNotification,
  IdexalProtocolRequest,
  IdexalProtocolRequestId,
  IdexalProtocolResponse,
} from "@idexal/shared";
import {
...
} from "../idexal-protocol-v4/interaction-registry.js";
...
import {
  ...
  type IdexalProtocolAgentDependencies,
  type IdexalProtocolAgentServerContext,
  type IdexalProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@idexal/contracts";

export type { IdexalProtocolAgentDependencies, IdexalProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type IdexalProtocolOutboundMessage = IdexalProtocolNotification | IdexalProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, IdexalProtocolSessionRecord>;
}): Promise<void> {
...
interface IdexalProtocolPostResponseBatch {
  readonly messages: readonly IdexalProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class IdexalProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<IdexalProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: IdexalProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: IdexalProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    IdexalProtocolRequestId,
    IdexalProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: IdexalProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createIdexalApp);
    const resolvedDeps = {
      ...deps,
      createIdexalApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, IdexalProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: IdexalProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("Idexal Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "idexal_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: IdexalProtocolRequestId): IdexalProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: IdexalProtocolRequestId): IdexalProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: IdexalProtocolMessage,
  ): Promise<IdexalProtocolError | IdexalProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("Idexal Protocol notification ignored", {
        event: "idexal_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: IdexalProtocolRequest,
  ): Promise<IdexalProtocolError | IdexalProtocolResponse> {
  ...
  private async dispatchRequest(request: IdexalProtocolRequest) {
      ...
      case idexalProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: idexalWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? idexalWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(idexalPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(idexalWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: IdexalProtocolRequestId, result: unknown): IdexalProtocolResponse {
    return { id, result };
  }

  private fail(
    id: IdexalProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): IdexalProtocolError {
  ...
  private requestClient<T>(
    method: IdexalProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: IdexalProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No Idexal Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: IdexalProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: IdexalProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;

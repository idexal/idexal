...
import type { McpTelemetryTracker } from "@idexal/adapters";
import type { WorkspaceHookPolicyProvider } from "@idexal/core";
import type { AccountProviderConfigSnapshot } from "@idexal/provider";
import {
  ...
  type IdexalProtocolMessage,
  type IdexalProtocolMethod,
  type IdexalProtocolNotification,
  type IdexalProtocolRequest,
  type IdexalProtocolRequestId,
  type IdexalProtocolTrace,
  type IdexalSessionMode,
  type IdexalSessionPersistence,
  type IdexalWorkspaceRef,
} from "@idexal/shared";
import type { IdexalApp, IdexalAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../idexal-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../idexal-protocol-v4/v4-gateway.js";
...
export interface IdexalProtocolAgentDependencies {
  createIdexalApp(options?: Omit<IdexalAppOptions, "providerRegistry">): IdexalApp | Promise<IdexalApp>;
  /**
...
export type IdexalProtocolAgentResolvedDependencies = IdexalProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface IdexalProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface IdexalProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface IdexalProtocolSessionRecord {
  app: IdexalApp;
  ...
  protocolEventSequences: Map<string, IdexalProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, IdexalProtocolToolInputTransmissionState>;
...
export interface IdexalProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: IdexalProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: IdexalProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: IdexalProtocolMessage): message is IdexalProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: IdexalProtocolMessage,
): message is IdexalProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: IdexalProtocolMessage,
): message is { id: IdexalProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: IdexalProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): IdexalProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: IdexalDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("Idexal Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "idexal_protocol.session.require_missing",
...
export function createProtocolLogger(deps: IdexalProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("idexal").child({
    module: "bootstrap.idexal_protocol",
  });
}

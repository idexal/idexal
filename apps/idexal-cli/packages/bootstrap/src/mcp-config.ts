...
import {
  getCapturedIdexalCuaBrokerCredentials,
  isIdexalCuaMcpCommand,
  isIdexalCuaMcpPackageArg,
  IDEXAL_CUA_BROKER_SOCKET_ENV_KEY,
  IDEXAL_CUA_NODE_REPL_HOST_ENV_KEY,
  IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
  IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  IDEXAL_PLUGIN_ID_ENV_KEY,
} from "@idexal/shared";

export { IDEXAL_CUA_BROKER_SOCKET_ENV_KEY as IDEXAL_CUA_BROKER_SOCKET_ENV } from "@idexal/shared";
// CLI 入口会先清理 broker 凭据；shared node_repl 的可信配置随后从进程内捕获快照恢复它们。
function resolveIdexalCuaBrokerSocket(): string | undefined {
  // captured 优先；运行时残留的 stale socket 不能覆盖可信快照。
  return (
    getCapturedIdexalCuaBrokerCredentials().socket ||
    process.env[IDEXAL_CUA_BROKER_SOCKET_ENV_KEY]?.trim()
  );
}

function resolveIdexalCuaBrokerToken(): string | undefined {
  return undefined;
}

const NODE_REPL_SERVER_NAME = "node_repl";
const REFRESH_MARKER_ENV = "IDEXAL_CUA_PERMISSION_BROKER_REFRESH_MARKER";
    ...
    Object.entries(pluginServers)
      .filter(
        ([name, config]) =>
          ...
          config.type === "stdio" &&
          config.env?.[IDEXAL_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() ===
            IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
      )
      .map(([name]) => name),
  );
...
export function omitMcpServers(
  ...
  return injectIdexalCuaBrokerMcpServers(
    kept,
    resolveIdexalCuaBrokerSocket(),
    resolveIdexalCuaBrokerToken(),
    trustedOfficialCuaServerNames,
  );
}

function injectIdexalCuaBrokerMcpServers(
  servers: Record<string, McpServerConfig>,
...
function injectCuaCredentialsIntoNodeRepl(
  ...
  const captured = getCapturedIdexalCuaBrokerCredentials();
  ...
  return {
    ...
    env: {
      ...config.env,
      [IDEXAL_CUA_BROKER_SOCKET_ENV_KEY]: socketPath,
      ...(refreshMarker ? { [REFRESH_MARKER_ENV]: refreshMarker } : {}),
      ...(pluginAuthority ? { [IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY]: pluginAuthority } : {}),
      [IDEXAL_CUA_NODE_REPL_HOST_ENV_KEY]: "1",
      [IDEXAL_PLUGIN_ID_ENV_KEY]: IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
    },
  };
}

function isIdexalCuaStdioServer(
  name: string,
  config: McpServerConfig,
): config is McpStdioServerConfig {
  if (config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 内置 official zcode-cua plugin 的 MCP server 走 __idexal-plugin-host，command 是 Helper
  // (非 zcode-cua)、args 是 [idexal.cjs, __idexal-plugin-host, server.js]（非 zcode-cua package arg），
  // 上面的 name/command/args 三条都匹配不到。_plugin id 由 adapters resolver 权威写入 env
  // （manifest/user env 不可覆盖），用它识别 official plugin server。
  if (
    config.env?.[IDEXAL_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() === IDEXAL_CUA_OFFICIAL_PLUGIN_ID
  ) {
    return true;
  }
  // 判定与 desktop/services 共用 @idexal/shared 的单一事实源，避免两条注入入口漂移。
  if (isIdexalCuaMcpCommand(config.command)) return true;
  return (config.args ?? []).some(isIdexalCuaMcpPackageArg);
}
...
function isRetiredCuaMcpServer(name: string, config: McpServerConfig | undefined): boolean {
  ...
  return (
    name !== NODE_REPL_SERVER_NAME && config !== undefined && isIdexalCuaStdioServer(name, config)
  );
}

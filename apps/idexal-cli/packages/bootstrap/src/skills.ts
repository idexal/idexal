...
import type { Logger, SkillContent, SkillDiagnostic, SkillLoadOutcome } from "@idexal/contracts";
...
export interface ListIdexalSkillsOptions {
...
export interface InspectIdexalSkillOptions extends ListIdexalSkillsOptions {
  name: string;
}

export interface IdexalSkillInspection {
  diagnostics: SkillDiagnostic[];
  skill: SkillContent;
}

export async function listIdexalSkills(
  options: ListIdexalSkillsOptions = {},
...
export async function inspectIdexalSkill(
  options: InspectIdexalSkillOptions,
): Promise<IdexalSkillInspection> {
  const discovery = await createSkillDiscovery(options);
  if (!discovery.enabled) {
...
async function createSkillDiscovery(options: ListIdexalSkillsOptions): Promise<
  | {
      enabled: false;
      workingDirectory: string;
    }
...
> {
  ...
  // 内置技能包与插件技能根并列注入：`idexal skills list`、引用目录与 runtime 看到同一份发现结果。

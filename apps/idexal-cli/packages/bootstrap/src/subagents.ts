...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@idexal/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@idexal/core";
import type { Logger, PluginMetadata } from "@idexal/contracts";
import {
  createAgentStateId,
...
interface LoadIdexalAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadIdexalAgentProfilesResult {
...
export async function loadIdexalAgentProfiles(
  input: LoadIdexalAgentProfilesInput,
): Promise<LoadIdexalAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".idexal", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .idexal/agents/*.md 是仓库内容，不能通过 frontmatter

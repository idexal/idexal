...
function createShellExpansionEnv(input: {
  ...
  set.IDEXAL_PROJECT_DIR = input.workingDirectory;
  if (input.sessionId) {
    set.CLAUDE_CODE_SESSION_ID = input.sessionId;
    set.CLAUDE_SESSION_ID = input.sessionId;
    set.IDEXAL_SESSION_ID = input.sessionId;
  }
  if (input.plugin) {
    set.CLAUDE_PLUGIN_DATA = input.plugin.dataPath;
    set.CLAUDE_PLUGIN_ROOT = input.plugin.rootPath;
    set.IDEXAL_PLUGIN_DATA = input.plugin.dataPath;
    set.IDEXAL_PLUGIN_ID = input.plugin.id;
    set.IDEXAL_PLUGIN_NAME = input.plugin.name;
    set.IDEXAL_PLUGIN_ROOT = input.plugin.rootPath;
  }
...
function assertShellExpansionContextAvailable(input: {
  ...
  for (const match of input.shellCommand.matchAll(SHELL_CONTEXT_VARIABLE_PATTERN)) {
    const name = match[1];
    if (!name) continue;
    if (name === "CLAUDE_SKILL_DIR" || name === "IDEXAL_SKILL_DIR") {
      throw new Error(
    ...
    if (
      ...
      (name === "CLAUDE_PLUGIN_DATA" ||
        ...
        name === "IDEXAL_PLUGIN_DATA" ||
        name === "IDEXAL_PLUGIN_ROOT")
    ) {
      throw new Error(

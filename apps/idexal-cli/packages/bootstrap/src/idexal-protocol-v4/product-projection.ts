import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@idexal/shared/idexal-protocol-v4";
...
import { verdictWorkspaceHookReviewRequest } from "@idexal/shared/workspace-hook-review-monotonicity";
import {
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  isIdexalModelRetryRecoveryProgressPayload,
  isIdexalFileStreamingToolInputPreviewTool,
  parseIdexalBackgroundTaskNotificationText,
  resolveIdexalBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseIdexalBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = idexalBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isIdexalFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < IDEXAL_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveIdexalBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @idexal/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。

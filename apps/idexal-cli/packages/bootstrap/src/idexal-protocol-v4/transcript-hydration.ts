...
import type { EventId, SessionEvent, SessionId, TraceId, TurnId } from "@idexal/contracts";
import {
...
const LEGACY_PROTOCOL_SESSION_STOPPED_MESSAGE = "Idexal Protocol session stopped";
...
function isPersistedAssistantCancellation(error: AssistantErrorInfo): boolean {
  ...
  // 只能用 Idexal 自身生成的标准 name/message 二元组兼容恢复；不泛化匹配 provider 文案。
  return (
    code === undefined &&

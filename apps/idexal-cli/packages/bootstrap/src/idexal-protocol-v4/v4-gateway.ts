...
import { localTtftNow, localTtftFactsSchema } from "@idexal/shared/idexal-protocol-v4";
import {
...
import type { ConversationSnapshot } from "@idexal/shared/idexal-protocol-v4";
import { SessionEventType, isFileSystemPortError } from "@idexal/contracts";
import type { IdexalWorkspaceRef } from "@idexal/shared";
import { extractMarkdownArtifactImageRefs } from "@idexal/shared";
import type {
  CommandAck,
  AttachmentRef,
  CommandEnvelope,
...
export interface V4GatewayHost {
  ...
  emitLocalTtftFacts?(facts: import("@idexal/shared").LocalTtftFacts): void;
...
function toShareStatFault(error: unknown): unknown {
  if (readIdexalAttachmentFaultCode(error)) return error;
  if (isFileSystemPortError(error) && MISSING_ATTACHMENT_FS_CODES.has(error.code)) {
    return new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.shareStatNotFound, {
      cause: error,
    });
  }
  return error;
...
export class ConversationV4Gateway {
   ...
   * effect 之前，而 CLI 严格串行派发请求（`idexal-protocol/transport.ts`，只有 session/stop
  ...
  async conversationAttachmentRead(
    ...
    if (!this.host.readSessionAttachment) {
      throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.readUnsupported);
    }
    const existingReady = this.readyFlights.get(params.sessionId);
    const publisher = existingReady
      ? await existingReady
    ...
    if (row?.kind !== "userInput") {
      throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized);
    }
  ...
  async conversationAttachmentStat(
    ...
    if (row?.kind !== "userInput") {
      throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    }
    const attachment = row.attachments?.[params.attachmentIndex];
    if (!attachment || (attachment.ref !== params.ref && attachment.previewRef !== params.ref)) {
      throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    ...
    if (result.totalBytes > PROTOCOL_V4_LIMITS.attachmentStatMaxBytes) {
      throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.shareStatTooLarge);
    }
    ...
    const payload = this.host.readSessionAttachment!(sessionId, {
      ...
      .then((result) => {
        ...
        if (
          ...
          throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.previewNotMedia);
        }
        if (result.bytes.byteLength > maxBytes) {
          throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.previewTooLarge);
        }
        const current = this.binaryReadCache.get(key);
        if (current) {
  ...
  private emitReservation<F extends RoutedTopicFrame>(
    ...
    if (
      ...
      const observations: import("@idexal/shared").LocalTtftFacts[] = [];

...
//   goal-pause barrier）在 handler 里直驱 core（IdexalApp / runtime），不经旧协议 op。
// - 会话注册表仍归宿主：通过 V4CommandCoreHost 以「结构化窄视图」透传旧
//   IdexalProtocolSessionRecord（同一对象引用，字段变更双向可见，不产生第二份注册表）。
...
import type { IdexalAutomationBotDeliveryTarget } from "@idexal/shared";
import type {
  CommandAck,
  CommandEnvelope,
  CommandPayloadMap,
...
/**
 * 旧 IdexalProtocolSessionRecord 的结构化窄视图（只声明命令层需要的字段）。
...
export interface V4CommandCoreHost {
   ...
   * 见 idexal-protocol/server-operations.ts）；v4 自持会话注册表后收编为原生实现。
   */

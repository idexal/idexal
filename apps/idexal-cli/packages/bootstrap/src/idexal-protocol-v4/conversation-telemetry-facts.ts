...
import { getModelUsageTotalTokens, SessionEventType } from "@idexal/contracts";
import { parseAutomationRunId } from "@idexal/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const idexal = recordValue(meta.idexal);
  return (
    ...
    optionalString(idexal.parentToolCallId) ??
    optionalString(idexal.parentToolUseId)
  );
}

...
} from "@idexal/adapters/storage";
import { createNodeLoggerFactory } from "@idexal/adapters/logging";
import { createConfig, resolvePath } from "@idexal/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@idexal/adapters/exec";
import { createNodeFileSystemAdapter } from "@idexal/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@idexal/adapters/http";
import { createJimpImageProcessorAdapter } from "@idexal/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@idexal/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@idexal/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@idexal/adapters/context";
import { createNodeSkillAdapter } from "@idexal/adapters/skills";
import { createMcpAdapter } from "@idexal/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@idexal/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveIdexalRuntimeEnv } from "@idexal/shared";
import {
...
import { resolveIdexalCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveIdexalBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadIdexalAgentProfiles } from "../subagents.js";
...
export async function createIdexalApp(options: IdexalAppOptions): Promise<IdexalApp> {
  if (!options?.providerRegistry) {
    throw new Error("createIdexalApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("idexal").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveIdexalRuntimeEnv(options.env ?? process.env) === "development",
    );
    const idexalSubagentProfileOutcome = await loadIdexalAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const idexalSubagentProfiles = idexalSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: idexalSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: idexalSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...idexalSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          idexalSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("Idexal runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("Idexal runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).IDEXAL_MAILBOX_ROOT ?? "~/.idexal/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveIdexalBuiltinPromptCommand(text, {
        ...
        return await resolveIdexalCustomCommandPrompt(text, {
          env: options.env,
          executionPort,
          logger,
          projectConfigPath: options.projectConfigPath,
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new IdexalAttachmentFaultError(IDEXAL_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("Idexal app startup failed", error, {

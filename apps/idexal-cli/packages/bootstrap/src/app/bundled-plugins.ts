...
import { writeBundledOfficialMarketplacePartitionSync } from "@idexal/adapters";
import { IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE, type Logger } from "@idexal/contracts";
import { isIdexalCuaInternalFeatureEnabled, IDEXAL_CUA_OFFICIAL_PLUGIN_ID } from "@idexal/shared";
import {
...
const OFFICIAL_PLUGIN_MARKETPLACE = IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE;
const SEA_PLUGIN_ASSET_PREFIX = "idexal-official-plugins/";
const SEA_PLUGIN_MANIFEST_ASSET_KEY = `${SEA_PLUGIN_ASSET_PREFIX}manifest.json`;
const SEED_MARKER_FILE = ".idexal-plugin-seed.json";
    ...
    if (plugin.missingSeedPaths.length > 0) {
      warnCacheDegraded(input.logger, {
        error: Object.assign(
          ...
          { code: "IDEXAL_PLUGIN_SEED_INCOMPLETE" },
        ),
...
export function resolveOfficialPluginRoots(input: {
  ...
  if (!isIdexalCuaInternalFeatureEnabled(input.env ?? process.env)) {
    suppressedBuiltins.add(IDEXAL_CUA_OFFICIAL_PLUGIN_ID);
  }
  ...
  const fallbackRoots = failedSeeds.flatMap((definition) => {
    ...
    if (`${definition.name}@${OFFICIAL_PLUGIN_MARKETPLACE}` === IDEXAL_CUA_OFFICIAL_PLUGIN_ID) {
      return [];
    }
...
function resolveFilesystemPluginRoot(definition: OfficialPluginDefinition): string | undefined {
  for (const baseDir of candidateBaseDirs()) {
    for (const relativePath of definition.rootCandidates) {
      ...
      if (existsSync(join(rootPath, ".idexal-plugin", "plugin.json"))) return rootPath;
    }
  }
  return undefined;
}
...
function readSeedPluginDescription(
  ...
  const manifestFile = plugin.files.find((file) => file.path === ".idexal-plugin/plugin.json");
...
function isSeedUsable(targetRoot: string, definition: OfficialPluginDefinition): boolean {
  try {
    const manifest = JSON.parse(
      readFileSync(join(targetRoot, ".idexal-plugin", "plugin.json"), "utf8"),
    ) as { name?: unknown };
...
export function candidateBaseDirs(): string[] {
  // 修复原因：Electron app-server 运行在 resources/glm/idexal.cjs，官方插件资源也随桌面包

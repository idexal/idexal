...
import {
  ...
  CoreErrorType,
  IDEXAL_DWF_CHILD_COMMAND,
  type CreateSessionTaskLinkInput,
  type DynamicWorkflowRunEvent,
  type DynamicWorkflowRunProgressPayload,
  type SessionId,
} from "@idexal/contracts";
import type { AgentRuntime } from "@idexal/core";
import { parseModelPickerValue, type ModelSelection } from "@idexal/shared/model-selection";
import {
...
export function launchDynamicWorkflowRun(
  ...
  return runWorkflowScript({
    ...
    makeDriver,
    // 入口文件写不进项目 `.idexal/` 时 harness 回落到 OS 临时目录并报一声——run 照常启动，
 ...
 * `@idexal/dynamic-workflow` 与 node 内建），既拿不到 contracts 的子命令常量，也不该知道
...
export function dynamicWorkflowChildSpawn(
  ...
  return isSea ? { argsPrefix: [IDEXAL_DWF_CHILD_COMMAND] } : undefined;
}
 ...
 * `idexal-protocol-v4/product-projection.ts` 的 `applyWorkflowRunEvent` 逐种类归约，其

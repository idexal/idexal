...
import type { Logger, SkillRoot } from "@idexal/contracts";
import { DYNAMIC_WORKFLOW_SKILL_NAME } from "@idexal/contracts";
import { candidateBaseDirs } from "./bundled-plugins.js";

/**
 * 随 CLI 内置的技能包（apps/idexal-cli/packages/bundled-skills）。
 *
 ...
 * - SEA 二进制：资产内嵌在 `idexal-bundled-skills/` 前缀下，首启按内容 hash 解压到
 *   `<cli storage>/bundled-skills/<hash>/`；目录名即内容身份，重复启动幂等，并发只会有一个赢家。
 * - 远端主机：prepare-prebuilds 把目录 stage 到远端 idexal.cjs 旁，与桌面同路。
 */
...
export const SEA_BUNDLED_SKILL_ASSET_PREFIX = "idexal-bundled-skills/";
const SEA_BUNDLED_SKILL_MANIFEST_ASSET_KEY = `${SEA_BUNDLED_SKILL_ASSET_PREFIX}manifest.json`;
const SEED_MARKER_FILE = ".idexal-bundled-skills-seed.json";

...
import type { SharedIdexalCredentialStore } from "@idexal/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@idexal/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  readonly idexalBuiltinRevision: string;
  readonly providers: readonly StandaloneCodingPlanProvider[];
}> {
  if (config)
    return {
      idexalBuiltinRevision: config.revision,
  ...
  const filePath = env[IDEXAL_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${IDEXAL_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `Idexal Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedIdexalCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.idexalBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedIdexalCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,

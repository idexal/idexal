...
import type { BrowserControlPort, Logger, McpServerConfig, TraceContext } from "@idexal/contracts";
import {
...
export function createNodeReplBrowserBroker(input: {
  ...
  return {
    ...
    token,
    close: async () => {
      // createIdexalApp 是同步工厂，极短生命周期下 close 可能先于 listen 回调；

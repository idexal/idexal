...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: IdexalBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 Idexal Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedIdexalCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[IDEXAL_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          idexalBuiltinFilePath: bundledFile,
          idexalBuiltinActiveFilePath: paths.idexalBuiltinFilePath,
          idexalBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.idexalBuiltinFilePath),
              "idexal-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeIdexalEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadIdexalBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: IDEXAL_VERSION,
                platform: resolveIdexalBuiltinClientPlatform(),
      ...
      : {}),
    onIdexalBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckIdexalBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.idexalBuiltinRevision !== account.basedOnIdexalBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  readonly basedOnIdexalBuiltinRevision: string;
  ...
  const basedOnIdexalBuiltinRevision = input.basedOnIdexalBuiltinRevision.trim();
  if (!basedOnIdexalBuiltinRevision) {

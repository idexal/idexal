...
export function createBundledMcpRuntimeConfig(input: {
  ...
  return {
    ...
    env: {
      ...input.env,
      // 桌面打包态 process.execPath 是 Idexal Helper；缺少 Node 模式会误进 Electron main。
...
export function writeOfficialPluginRuntimeManifest(input: OfficialRuntimeManifestInput): void {
  const manifestPath = join(input.rootPath, ".idexal-plugin", "plugin.json");
  ...
  // 把 seed 流程整个阻断，连带 listIdexalSkills 拉不出 plugin skill。
  ...
  for (const [serverKey, serverRaw] of Object.entries(mcpServers)) {
    ...
    mcpServer.env = {
      ...mcpServerEnv,
      // 桌面打包态的 process.execPath 是 Idexal Helper。
      ...
      [IDEXAL_PLUGIN_ID_ENV_KEY]: `${input.pluginName}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`,
    };
...
export function officialPluginHostPrefixArgs(): string[] | undefined {
  if (isSeaRuntime()) return [IDEXAL_PLUGIN_HOST_COMMAND];

  const entrypoint = process.argv[1];
  if (!entrypoint) return undefined;

  return [...process.execArgv, resolve(entrypoint), IDEXAL_PLUGIN_HOST_COMMAND];
}

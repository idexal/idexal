...
import { discoverNodePluginsSync } from "@idexal/adapters/plugins";
import type { AgentRuntimeConfig } from "@idexal/core";
import type { Logger, McpServerConfig, PluginLoadOutcome } from "@idexal/contracts";
...
export function resolveStartupPlugins(input: {
  ...
  logger?: Logger;
  options: Pick<IdexalAppOptions, "officialPluginRoots" | "pluginStorageRoot">;
  ...
  const pluginOutcome = discoverNodePluginsSync({
    ...
    // (resolveIdexalPlugins) 和应用启动路径都要传，否则 `/skill skill-creator` 在会话里
    // 报 "Skill not found: skill-creator"，但 `idexal plugins list` 却显示它是 enabled。
    officialPluginsEnabledByDefault: DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS,
    storageRoot: pluginStorageRoot,
    workingDirectory: input.workingDirectory,
  });
  input.startupTimer.mark("Idexal plugins resolved", {
    context: {
...
export function startAppStartup(input: {
  ...
  input.startupTimer.start("Idexal app startup started", {
    context: {
...
export function markConfigurationLoaded(input: {
  ...
  input.startupTimer.mark("Idexal app configuration loaded", {
    context: {
...
export function markStorageAdaptersInitialized(input: {
  ...
  input.startupTimer.mark("Idexal storage adapters initialized", {
    context: {
...
export function markMcpAdapterInitialized(input: {
  ...
  input.startupTimer.mark("Idexal MCP adapter initialized", {
    context: {
...
export function markRuntimeConstructed(input: {
  ...
  input.startupTimer.mark("Idexal runtime constructed", {
    context: {
      hasInjectedModelAdapter: input.hasInjectedModelAdapter,
...
export function completeAppStartup(input: {
  ...
  input.startupTimer.complete("Idexal app startup completed", {
    context: {
      sessionId: input.sessionId,
      workingDirectory: input.workingDirectory,
    },

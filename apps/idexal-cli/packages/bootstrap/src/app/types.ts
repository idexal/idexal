import type { IdexalToolExecResource, BackgroundBashOutputResult } from "@idexal/shared";
import type { AiSdkModelAdapter } from "@idexal/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { IdexalModelOption } from "@idexal/shared";
import type { EffectiveModelSelectionResult } from "@idexal/shared/model-selection";
export type { IdexalModelOption } from "@idexal/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { IdexalInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@idexal/contracts";
...
export type IdexalAppRuntimeConfigInput = AgentRuntimeConfig;

export interface IdexalAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: IdexalAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: IdexalToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@idexal/contracts").TurnBackgroundAttribution;
...
export interface IdexalPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface IdexalPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: IdexalInstalledPluginData | null;
}
...
export interface IdexalApp {
  ...
  getCurrentModelOption?(): IdexalModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): IdexalModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@idexal/shared").IdexalSessionSubagentsResult>;
  ...
  listModels(): IdexalModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<IdexalPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<IdexalPluginUninstallResult>;
  /**
...
export interface RunIdexalProtocolAgentOptions {
...
export interface ListIdexalSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;

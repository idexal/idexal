import { IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE } from "@idexal/contracts";
...
export const OFFICIAL_BROWSER_USE_PLUGIN_ID = `${OFFICIAL_BROWSER_USE_PLUGIN_NAME}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`;
/**
...
export const OFFICIAL_NODE_REPL_HOST_PLUGIN_ID = `${OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`;
const OFFICIAL_CUA_PLUGIN_NAME = "computer-use";
export const OFFICIAL_CUA_PLUGIN_ID = `${OFFICIAL_CUA_PLUGIN_NAME}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`;

export interface OfficialPluginDefinition {
  // 内容型 plugin (无 MCP server / 无系统依赖) 可以设为 true,
  // 这样用户首次 `/skill <name>` 就能用,不必先 `idexal plugins enable`。
...
const OFFICIAL_IDEXAL_GUIDE_REQUIRED_SEED_PATHS = [
...
export const OFFICIAL_PLUGIN_DEFINITIONS: readonly OfficialPluginDefinition[] = [
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: {
        "zh-CN": "操作 Idexal 内置浏览器，检查网页并验证交互。",
      },
    },
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: {
        "zh-CN": "开发、校验 Idexal 插件，完成本地 dev 市场安装、试用与更新。",
      },
    },
    rootCandidates: [
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: { "zh-CN": "创建、编辑和验证可复用的 Idexal 技能。" },
    },
    name: "skill-creator",
    rootCandidates: [
  ...
  },
  {
    ...
    // 让用户/agent 开箱即用地拿到 Idexal 配置指南、自诊断技能与 dynamic workflow 编写指南。
    defaultEnabled: true,
    listing: {
      author: ZAI_AUTHOR,
      category: "utilities",
      displayName: "Idexal Guide",
      displayName_i18n: { "zh-CN": "Idexal 使用指南" },
      icon: `${OFFICIAL_PLUGIN_ASSETS_BASE_URL}/zcode-guide/icon.png`,
      description_i18n: {
        "zh-CN": "提供 Idexal 配置指南与插件、技能、MCP、命令和钩子诊断。",
      },
      examplePrompts: [
        "How do I configure MCP servers in Idexal?",
        "Diagnose my current Idexal setup",
      ],
      examplePrompts_i18n: {
        "zh-CN": ["Idexal 里怎么配置 MCP 服务器？", "帮我诊断当前的 Idexal 配置"],
      },
    },
    name: "zcode-guide",
    requiredSeedPaths: OFFICIAL_IDEXAL_GUIDE_REQUIRED_SEED_PATHS,
    rootCandidates: [
  ...
  },
  {
    ...
    // isIdexalCuaInternalFeatureEnabled（打包层默认 true）与输入框入口 hidden 默认值的联动语义。
    name: "computer-use",
    hostMcpServerNames: ["node_repl"],
...
// 注意: 任何解析 plugin 的入口 (CLI 子命令 resolveIdexalPlugins、应用启动 resolveStartupPlugins)
...
export const DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS: ReadonlySet<string> = new Set(
  OFFICIAL_PLUGIN_DEFINITIONS.filter((definition) => definition.defaultEnabled).map(
    (definition) => `${definition.name}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`,
  ),
);
...
export function resolveOfficialPluginHostMcpServerNames(pluginId: string): string[] {
  const definition = OFFICIAL_PLUGIN_DEFINITIONS.find(
    (candidate) => `${candidate.name}@${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}` === pluginId,
  );

...
import type { PluginLoadOutcome } from "@idexal/contracts";
...
import type {
  IdexalApp,
  IdexalAppOptions,
  IdexalPluginSetResult,
  IdexalPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listIdexalPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<IdexalPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<IdexalPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE) {

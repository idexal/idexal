import { updateUiLocaleInFileConfig, type ConfigResult } from "@idexal/adapters/config";
import type { AgentRuntime } from "@idexal/core";
import { resolveLocale } from "@idexal/i18n";
import { normalizeModelSelection, type ModelSelection } from "@idexal/provider";
import {
...
import type { PrepareUserExecutionBoundary, IdexalApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<IdexalApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<IdexalApp["updateTargetStatus"]>
      >,

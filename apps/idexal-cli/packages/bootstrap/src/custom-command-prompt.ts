...
import { loadIdexalCustomCommand, type ListIdexalCustomCommandsOptions } from "./custom-commands.js";
import { expandCustomCommandShellSyntax } from "./custom-command-shell-expansion.js";
import { isReservedIdexalSlashCommandName } from "./slash-command-surface.js";

const CUSTOM_COMMAND_NOT_FOUND_PATTERN = /not found/i;
const PROMPT_CUSTOM_COMMAND_PATTERN = /^\/([^\s]+)(?:\s+([\s\S]*))?$/;

interface ResolveIdexalCustomCommandPromptOptions extends ListIdexalCustomCommandsOptions {
...
export async function resolveIdexalCustomCommandPrompt(
  input: string,
  options: ResolveIdexalCustomCommandPromptOptions = {},
  ...
  if (!invocation || isReservedIdexalSlashCommandName(invocation.name)) {
    return undefined;
  }

  try {
    const command = await loadIdexalCustomCommand({
      ...options,

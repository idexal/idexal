...
import { createNodeModelSelectionFacade } from "@idexal/provider-node";
import { createNodeLoggerFactory } from "@idexal/adapters/logging";
import {
  createMcpAdapterConnectionPool,
  createMcpTelemetryTracker,
  type McpConnectionPool,
  type McpTelemetryTracker,
} from "@idexal/adapters/mcp";
import {
  idexalProtocolNotifications,
  type IdexalMcpResourceSample,
  type IdexalMcpTelemetryEvent,
} from "@idexal/shared";
import type { SqliteSessionStore } from "@idexal/adapters/storage";
import { traceContextToLogContext, createRootTraceContext } from "@idexal/contracts";
import type { McpPort, ModelSelection } from "@idexal/contracts";
import type { PresentationSurface } from "@idexal/core";
import type { RunIdexalProtocolAgentOptions, IdexalAppOptions } from "./app/types.js";
import { createIdexalApp } from "./app/create-app.js";
import {
...
} from "./idexal-protocol/storage-startup.js";
...
import { installIdexalProtocolAiSdkWarningLogger } from "./idexal-protocol/ai-sdk-warning-logger.js";
import {
  createOfficialMcpAuthHeadersPort,
  type OfficialMcpAuthRequestContext,
} from "./idexal-protocol/official-mcp-auth-port.js";
import {
  createOfficialMcpTrustedOriginRegistry,
  OFFICIAL_MCP_DEV_TRUSTED_ORIGINS_ENV,
  IDEXAL_WORKSPACE_IDENTITY_ENV,
  resolveRuntimeIdexalEndpointOrigin,
} from "@idexal/shared";
import { IdexalProtocolAgentServer } from "./idexal-protocol/server.js";
import { IdexalProtocolNdjsonConnection } from "./idexal-protocol/transport.js";
import { cleanupProtocolRuntime } from "./idexal-protocol/runtime-cleanup.js";
import { startProtocolResourceSampler } from "./idexal-protocol/resource-sampler.js";
import { acquireProtocolStartupResource } from "./idexal-protocol/startup-resource.js";
import type { IdexalProcessResourceSampler } from "./process-resource-sampler.js";
import { prepareIdexalTelemetryEnv, shutdownIdexalTelemetry } from "./telemetry-bootstrap.js";

function applyProtocolPresentationSurface(
  options: Omit<IdexalAppOptions, "providerRegistry">,
  presentationSurface: PresentationSurface,
): Omit<IdexalAppOptions, "providerRegistry"> {
  return {
    ...options,
    runtimeConfig: {
...
function applyProtocolProviderRegistry(
  options: Omit<IdexalAppOptions, "providerRegistry">,
  providerRegistry: IdexalAppOptions["providerRegistry"],
...
export async function runIdexalProtocolAgent(
  options: RunIdexalProtocolAgentOptions = {},
): Promise<void> {
  ...
  const traceContext = createRootTraceContext({
    attributes: {
      entrypoint: "idexal_protocol",
    },
  });
  const logger = loggerFactory.createLogger("idexal").child({
    ...traceContextToLogContext(traceContext),
    module: "bootstrap.idexal_protocol",
  });
  installIdexalProtocolAiSdkWarningLogger(logger);
  const startupTimer = new StartupTimer(
    logger,
    {
      ...traceContextToLogContext(traceContext),
      module: "bootstrap.idexal_protocol",
      startupKind: "idexal_protocol_agent",
    },
    startupStartedAt,
  );
  startupTimer.start("Idexal Protocol agent startup started", {
    context: { version: options.version },
    event: "idexal_protocol.startup.started",
    stage: "start",
  });

  let sessionStore: SqliteSessionStore | undefined;
  let serverForCleanup: IdexalProtocolAgentServer | undefined;
  ...
  let mcpResourceSink: ((samples: IdexalMcpResourceSample[]) => void) | undefined;
  let mcpTelemetrySink: ((event: IdexalMcpTelemetryEvent) => void) | undefined;
  let processResourceSampler: IdexalProcessResourceSampler | undefined;
  ...
  try {
        ...
        openProtocolStartupStorage({
          ...
          onProgress: (progress) =>
            logger.info("SQLite startup state", {
              event: "idexal_protocol.startup.storage_state",
              ...progress,
            }),
        }),
    });
    ...
    logger.info("Worker Provider Registry 已就绪", {
      ...
      event: "idexal_protocol.provider_registry.ready",
      module: "bootstrap.idexal_protocol",
    ...
    const telemetryEnv = await acquireProtocolStartupResource({
      signal: options.lifecycle?.signal,
      logger,
      disposeLate: () => shutdownIdexalTelemetry(),
      create: () =>
        prepareIdexalTelemetryEnv(runtimeEnv, {
          cliVersion: options.version,
          productVersion: options.env?.IDEXAL_APP_VERSION,
          runtimeSurface,
        }),
    });
    const telemetryDeviceMid = telemetryEnv.IDEXAL_TELEMETRY_DEVICE_MID;
    mcpTelemetryTracker =
    ...
    // 与下面 trustedOrigins 的 resolveIdexalApiOrigin 必须是同一个表达式，否则两侧判定分叉。
    const resolveIdexalApiOrigin = (): string =>
      resolveRuntimeIdexalEndpointOrigin(options.env ?? process.env);
    const workspaceIdentity = (options.env ?? process.env)[IDEXAL_WORKSPACE_IDENTITY_ENV]?.trim();
    const officialMcpAuth = {
      ...
      resolveIdexalApiOrigin,
      ...(workspaceIdentity ? { workspaceIdentity } : {}),
      // 信任判定只看一条：目标 origin 等于当前 Idexal API origin（https）。pluginId 不参与。
    ...
    options.lifecycle?.signal.throwIfAborted();
    const server = (serverForCleanup = new IdexalProtocolAgentServer({
      createIdexalApp: (appOptions = {}) =>
        createIdexalApp({
          ...
          env: {
            ...telemetryEnv,
            ...appOptions.env,
            ...(telemetryDeviceMid ? { IDEXAL_TELEMETRY_DEVICE_MID: telemetryDeviceMid } : {}),
          },
          ...
          onToolExecResource: (params) =>
            connection.send({ method: idexalProtocolNotifications.toolExecResource, params }),
        }),
      cwd: options.cwd,
      env: options.env,
      loggerFactory,
    ...
    const connection = new IdexalProtocolNdjsonConnection({
      signal: options.lifecycle?.signal,
    ...
    mcpResourceSink = (samples) =>
      connection.send({
        method: idexalProtocolNotifications.mcpResourceSamples,
        params: samples,
      });
    mcpTelemetrySink = (event) => {
      ...
      connection.send({
        method: idexalProtocolNotifications.mcpTelemetry,
        params: event,
      });
    };
    connection.start();
    ...
    startupTimer.complete("Idexal Protocol agent startup completed", {
      event: "idexal_protocol.startup.completed",
      stage: "total",
    });
    ...
    startupTimer.fail("Idexal Protocol agent startup failed", error, {
      event: "idexal_protocol.startup.failed",
      stage: "total",
    });
    throw error;
  } finally {
    ...
    logger.info("Idexal Protocol agent shutdown completed", {
      ...traceContextToLogContext(traceContext),
      event: "idexal_protocol.shutdown.completed",
      module: "bootstrap.idexal_protocol",
      status: "completed",
    });
  }
...
function resolveProtocolRuntimeSurface(
  ...
  return env.IDEXAL_TELEMETRY_RUNTIME_SURFACE?.trim() === "remote_workspace_host"

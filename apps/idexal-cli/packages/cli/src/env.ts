...
import {
  ...
  normalizeIdexalRuntimeEnv,
  sanitizeIdexalRuntimeEnv,
  sanitizeIdexalRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeIdexalRuntimeEnv(env),
    ...buildIdexalToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildIdexalToolEnvPassthroughEnv(env);
  sanitizeIdexalRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[IDEXAL_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): IdexalRuntimeEnv {
  const explicit = normalizeIdexalRuntimeEnv(env[IDEXAL_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.IDEXAL_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.IDEXAL_BETA === "1" || env.IDEXAL_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])idexal-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.IDEXAL_STORAGE_DIR = join(homedir(), ".idexal-beta");
}

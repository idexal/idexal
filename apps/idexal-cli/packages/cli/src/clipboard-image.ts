...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@idexal/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.IDEXAL_STORAGE_DIR?.trim() || join(homedir(), ".idexal");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "idexal-clipboard-"));

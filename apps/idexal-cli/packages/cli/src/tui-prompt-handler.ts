import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@idexal/adapters/config";
import type { SessionEvent } from "@idexal/contracts";
import type { IdexalAppOptions } from "@idexal/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@idexal/i18n";
import type { TuiRequestPermission } from "@idexal/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createIdexalApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<IdexalAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createIdexalApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createIdexalApp"]>>>;
    try {

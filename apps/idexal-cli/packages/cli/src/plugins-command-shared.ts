...
import type { listIdexalPlugins } from "@idexal/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@idexal/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@idexal/bootstrap");
export type PluginListOutcome = ReturnType<typeof listIdexalPlugins>;
 ...
 * 避免 `idexal plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addIdexalPluginMarketplace",
  getPluginsOverview: "getIdexalPluginsOverview",
  installPlugin: "installIdexalMarketplacePlugin",
  listPlugins: "listIdexalPlugins",
  removeMarketplace: "removeIdexalPluginMarketplace",
  setPluginEnabled: "setIdexalPluginEnabled",
  uninstallPlugin: "uninstallIdexalMarketplacePlugin",
  updateMarketplace: "updateIdexalPluginMarketplace",
  updatePlugin: "updateIdexalMarketplacePlugin",
  validatePluginPath: "validateIdexalPluginPath",
...
/** `idexal plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@idexal/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by idexal. Use: user, project");
  }

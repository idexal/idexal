...
import { formatJson, type PresentationSurface } from "@idexal/core";
import type { RunContext, GlobalOptions } from "@idexal/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createIdexalApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createIdexalApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createIdexalApp ?? bootstrapModule?.createIdexalApp;
    if (!createApp) throw new Error("Idexal app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareIdexalTelemetryEnv ?? bootstrapModule?.prepareIdexalTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownIdexalTelemetry ?? bootstrapModule?.shutdownIdexalTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.IDEXAL_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `idexal -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveIdexalCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedIdexalSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listIdexalCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadIdexalCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: idexal hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}

...
export async function runLoginCommand(
  ...
  try {
    ...
    if (args.length > 1 || (providerId !== "zai" && providerId !== "bigmodel")) {
      throw new Error("Usage: idexal login [zai|bigmodel] [--no-browser]");
    }
    ...
    const login = deps.loginIdexalCli ?? (await loadBootstrapModule()).loginIdexalCli;
...
export async function runLogoutCommand(
  ...
  try {
    ...
    const logout = deps.logoutIdexalCli ?? (await loadBootstrapModule()).logoutIdexalCli;

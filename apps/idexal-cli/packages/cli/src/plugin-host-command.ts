...
import { IDEXAL_PLUGIN_HOST_COMMAND } from "@idexal/contracts/plugins";
import {
  getCapturedIdexalCuaBrokerCredentials,
  IDEXAL_CUA_BROKER_SOCKET_ENV_KEY,
  IDEXAL_CUA_NODE_REPL_HOST_ENV_KEY,
} from "@idexal/shared/runtime-env";
import { IDEXAL_CUA_OFFICIAL_PLUGIN_ID, IDEXAL_PLUGIN_ID_ENV_KEY } from "@idexal/shared/mcp";
import type { RunContext } from "@idexal/shared-types";

const HOST_USAGE = `${IDEXAL_PLUGIN_HOST_COMMAND} <server-path> [-- <server-arg>...]`;
...
export function isPluginHostInvocation(argv: readonly string[]): boolean {
  return argv[0] === IDEXAL_PLUGIN_HOST_COMMAND;
}

// __idexal-plugin-host 在 agent 子进程里运行 official plugin 的 MCP server（server.js）。
...
export async function runPluginHostCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  try {
    ...
    const capturedBrokerCredentials = getCapturedIdexalCuaBrokerCredentials();
    ...
    const originalBrokerSocket = process.env[IDEXAL_CUA_BROKER_SOCKET_ENV_KEY];
    // shared node_repl 把同一凭据组恢复到环境，由 broker bridge 读取；旧的独立 CUA
    // MCP 不再拥有执行入口。
    process.argv = [process.execPath, serverPath, ...serverArgs];
    if (capturedBrokerCredentials.socket && process.env[IDEXAL_CUA_NODE_REPL_HOST_ENV_KEY] === "1") {
      process.env[IDEXAL_CUA_BROKER_SOCKET_ENV_KEY] = capturedBrokerCredentials.socket;
    }
    try {
      await module.main();
    } finally {
      ...
      if (originalBrokerSocket === undefined) {
        delete process.env[IDEXAL_CUA_BROKER_SOCKET_ENV_KEY];
      } else {
        process.env[IDEXAL_CUA_BROKER_SOCKET_ENV_KEY] = originalBrokerSocket;
      }
    }
...
type CapturedBrokerCredentials = ReturnType<typeof getCapturedIdexalCuaBrokerCredentials>;
...
function assertCapturedBrokerLaunchIsAuthorized(credentials: CapturedBrokerCredentials): void {
  ...
  const pluginId = process.env[IDEXAL_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase();
  ...
  // 残留读取方只能靠 CLI 自己的 typecheck 发现（根 `pnpm typecheck` 不含 apps/idexal-cli）。
  ...
  if (
    ...
    pluginId !== IDEXAL_CUA_OFFICIAL_PLUGIN_ID ||
    process.env[IDEXAL_CUA_NODE_REPL_HOST_ENV_KEY] !== "1"
  ) {
    throw new Error(
      "Captured Idexal CUA broker credentials may only launch the trusted shared node_repl host",
    );
  }
}

...
import type { PlaywrightChromiumModule } from "@idexal/adapters/browser";
...
const ASSET_PREFIX = "idexal-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "idexal-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  ...
  if (platform() === "darwin" && home) {
    return join(home, "Library", "Caches", "idexal", "sea-assets");
  }
  if (platform() === "win32") {
    return join(

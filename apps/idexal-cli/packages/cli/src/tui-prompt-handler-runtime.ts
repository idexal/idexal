...
// 品牌类型（SessionId）和 createIdexalApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createIdexalApp ?? bootstrapModule?.createIdexalApp;
  if (!createAppFactory) throw new Error("Idexal app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareIdexalTelemetryEnv ?? bootstrapModule?.prepareIdexalTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownIdexalTelemetry ?? bootstrapModule?.shutdownIdexalTelemetry;
  }

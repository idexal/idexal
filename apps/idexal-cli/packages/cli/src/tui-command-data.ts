import type { TuiSessionMetadata } from "@idexal/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listIdexalSkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listIdexalSkills;
  return await listIdexalSkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listIdexalSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listIdexalSessions;
  const sessions = await listIdexalSessionsForTui({

import type { TuiSelection, TuiSubmitPrompt } from "@idexal/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getIdexalCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getIdexalCopy(locale).tui.loginSetup.response;
}

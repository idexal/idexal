...
const USAGE = `Usage:
  idexal hooks trust status [--workspace <path-or-identity>] [--json]
  idexal hooks trust review [--workspace <path-or-identity>] [--json]
  idexal hooks trust grant --workspace <path-or-identity> --hook-digest <sha256> [--hook-digest <sha256> ...]
  idexal hooks trust grant --workspace <path-or-identity> --all-current --bundle-digest <sha256>
  idexal hooks trust revoke --workspace <path-or-identity> [--hook-digest <sha256> ... | --all]
`;
...
type HooksCommandDependencies = RunDependencies & {
  loadBootstrapModule?: () => Promise<typeof import("@idexal/bootstrap")>;
...
function formatHuman(status: WorkspaceHookTrustCliStatus, action: string): string {
  ...
  if (status.reasonCode === "workspace_hooks_pending_trust" && status.bundleDigest) {
    lines.push(
      ...
      `  idexal hooks trust grant --workspace ${quote(status.workspaceIdentity)} --hook-digest <sha256>`,
      "Or trust every currently enabled declaration in this exact bundle with:",
      `  idexal hooks trust grant --workspace ${quote(status.workspaceIdentity)} --all-current --bundle-digest ${status.bundleDigest}`,
    );
  }

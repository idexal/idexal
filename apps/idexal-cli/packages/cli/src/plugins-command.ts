...
import type { SetIdexalPluginEnabledResult, IdexalPluginInstallData } from "@idexal/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: idexal plugins <command> [options]
...
Scopes: user (default), project. \`idexal plugin\` is an alias of \`idexal plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart idexal to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: IdexalPluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetIdexalPluginEnabledResult; error?: string }> = [];

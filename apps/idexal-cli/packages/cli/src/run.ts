...
import { createNodeLoggerFactory } from "@idexal/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@idexal/core";
import { color, formatJson, supportsColor } from "@idexal/core";
import { getIdexalCopy, isUiLocale, type UiLocale } from "@idexal/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@idexal/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getIdexalCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "idexal_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("idexal doctor", colors)}\n`);
...
const runIdexalProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 Idexal agent transport closed。
    ...
    const runProtocolAgent =
      deps.runIdexalProtocolAgent ?? (await loadBootstrapModule()).runIdexalProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("idexal").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runIdexalProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,

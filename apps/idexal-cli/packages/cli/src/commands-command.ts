...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listIdexalCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: idexal commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listIdexalCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@idexal/bootstrap"));
  return (await bootstrap()).inspectIdexalCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: IdexalCustomCommandInspection,
  options: GlobalOptions,
): string {

...
import { createDenyPermissionBroker } from "@idexal/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  IdexalAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `idexalSessionEventTypeSchema`（`shared/src/idexal-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `idexalSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createIdexalApp` 注入点边界）。
 *

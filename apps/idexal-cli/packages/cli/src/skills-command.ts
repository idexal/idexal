...
type BootstrapModule = typeof import("@idexal/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listIdexalSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: idexal skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listIdexalSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@idexal/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectIdexalSkill;
...
function formatHumanSkillInspection(
  inspection: IdexalSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: IdexalSkillInspection, cwd: string): string =>
  formatJson({
    cwd,

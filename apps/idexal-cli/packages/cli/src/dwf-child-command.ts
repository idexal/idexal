/**
 * 隐藏子命令 `__idexal-dwf-child <entry path>`：dynamic workflow 的沙箱子进程入口。
 *
 ...
 * 32,767 字符），harness 写一份自包含的入口文件 `<cwd>/.idexal/workflow-runs/<runId>.mjs`，argv
 * 末位只是它的路径。入口文件自带 childMain 与 payload，这里只 `import()` 它并把 CLI 进程的
 * vm/readline/stdio 注入它导出的 `start`——本模块因此**不再依赖 `@idexal/dynamic-workflow-runtime`**。
...
export async function runDwfChildCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  if (argv.length === 0 || entryPath === undefined) {
    ctx.stderr.write(`Usage: ${IDEXAL_DWF_CHILD_COMMAND} <entry path>\n`);
    return 1;
  }

...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@idexal/shared/runtime-tool-runtime";
...
const assetPrefix = "idexal-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".idexal-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.IDEXAL_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".idexal");

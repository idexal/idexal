...
import {
  IDEXAL_PROCESS_DIAGNOSTIC_PREFIX,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, IDEXAL_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: IdexalProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${IDEXAL_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[idexal] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }

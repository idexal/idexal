...
export const resolveBuildExternal = () => ["@idexal/tui", "playwright-core", "koffi"];
  ...
  return {
    ...
    setup(builder) {
      ...
      builder.onResolve({ filter: /^zod(?:\/|$)/ }, async (args) => {
        if (args.pluginData?.idexalZodResolving) return;
        const resolved = await builder.resolve(args.path, {
          importer: args.importer,
          resolveDir: args.resolveDir,
          kind: args.kind,
          pluginData: { idexalZodResolving: true },
        });
...
export const resolveBuildOptions = (args = [], env = process.env) => {
  ...
  const e2eCoverage = env.IDEXAL_E2E_COVERAGE === "1";
...
export const resolveBuildAliases = ({
  ...
  "@idexal/shared-types": resolve(cliDirectory, "../shared-types/dist/index.js"),
  // plugin-host 启动只需这些独立入口，不能经通用 alias 重新求值 shared 总入口。
  "@idexal/shared/runtime-env": resolve(rootDirectory, "../../packages/shared/src/runtimeEnv.ts"),
  "@idexal/shared/mcp": resolve(rootDirectory, "../../packages/shared/src/mcp.ts"),
  "@idexal/shared/runtime-tool-runtime": resolve(
    rootDirectory,
  ...
  "@idexal/shared/idexal-protocol-v4": resolve(
    rootDirectory,
    "../../packages/shared/src/idexal-protocol-v4/index.ts",
  ),
  ...
  "@idexal/shared/model-selection": resolve(
    rootDirectory,
    "../../packages/shared/src/model-selection.ts",
  ),
  // 共享 Model Schema 新增的子路径不能被通用 alias 拼到 index.ts 后面。
  "@idexal/shared/model-config": resolve(rootDirectory, "../../packages/shared/src/model-config.ts"),
  // 进程异常边界在 bootstrap 之前使用该轻量契约，不能落入 shared 的通用前缀 alias。
  "@idexal/shared/process-diagnostic": resolve(
    rootDirectory,
    "../../packages/shared/src/process-diagnostic.ts",
  ),
  "@idexal/shared/config-schema": resolve(
    rootDirectory,
    "../../packages/shared/src/config-schema.ts",
  ),
  "@idexal/shared/workspace-hook-discovery": resolve(
    rootDirectory,
  ...
  "@idexal/shared/workspace-hook-mutation": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-mutation.ts",
  ),
  // verdict 直连 import 需要本精确 alias；漏声明会被通用
  // "@idexal/shared" 前缀改写成 `src/index.ts/workspace-hook-review-monotonicity`，
  // Desktop agent/SEA 打包直接失败。
  "@idexal/shared/workspace-hook-review-monotonicity": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-review-monotonicity.ts",
  ),
  // trust store 文件 schema 单源下沉后的新 subpath；漏声明会被通用
  // "@idexal/shared" 前缀改写成 `src/index.ts/workspace-hook-trust-store-file`，
  // Desktop agent/SEA 打包失败（同上两类既有规则）。
  "@idexal/shared/workspace-hook-trust-store-file": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-trust-store-file.ts",
  ),
  "@idexal/shared/idexalEndpoint": resolve(
    rootDirectory,
    "../../packages/shared/src/idexalEndpoint.ts",
  ),
  "@idexal/shared/node": resolve(rootDirectory, "../../packages/shared/src/node.ts"),
  "@idexal/shared": resolve(rootDirectory, "../../packages/shared/src/index.ts"),
  "@idexal/core": resolve(cliDirectory, "../core/dist/index.js"),
});
...
export const buildCli = async ({
  ...
  const outfile = resolve(cliDirectory, "dist/idexal.cjs");
  ...
  await build({
    banner: {
      ...
      js: `#!/usr/bin/env node\n"use strict";\nif (process.argv.length === 3 && process.argv[2] === "--licenses") { const sea = require("node:sea"); const nodeNotice = sea.isSea() ? "\\n\\n## Bundled Node.js runtime\\n\\n" + sea.getAsset("idexal-node-license", "utf8") : ""; process.stdout.write(${JSON.stringify(notices.toString("utf8"))} + nodeNotice, () => process.exit(0)); } else {`,
    },
    footer: { js: "}" },
    bundle: true,
    define: {
    ...
    format: "cjs",
    // 桌面 app 集成只内置 idexal.cjs，旧 desktop-agent 构建复用 CLI 调试产物，
    ...
    // 远端 SSH 复用已部署的独立 Node v22.16 跑同一份 idexal.cjs。降到 node22 保证这份产物

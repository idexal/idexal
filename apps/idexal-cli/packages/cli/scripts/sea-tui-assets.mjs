...
export const seaTuiAssetPrefix = "idexal-tui-runtime/";
...
const workspacePackageParentDirectories = (root) => {
  ...
  return [
    ...
    // TUI runtime 闭包会经由 @idexal/contracts 依赖仓库根的 @idexal/shared；
    // 只扫描 apps/idexal-cli 子 workspace 会把这个合法 workspace 误判为缺失。
    resolve(repositoryRoot, "packages"),
  ];
};

const runtimePackageNames = async ({ root, target, workspacePackageDirectories }) => {
  const tuiDirectory = workspacePackageDirectories.get("@idexal/tui");
  if (!tuiDirectory) {
    throw new Error("Missing @idexal/tui workspace package.");
  }
  const queue = [
    { fromDirectory: tuiDirectory, packageName: "@idexal/tui" },
  ...
  while (queue.length > 0) {
    ...
    const assetPackagePath = placeRuntimePackage({
      ...
      packageDirectory,
      placements,
      fromAssetPath: entry.fromAssetPath ?? "node_modules/@idexal/tui",
    });
    ...
    // SEA 缓存目录无法解析 pnpm workspace 链接；旧的 @idexal 白名单漏掉 i18n

...
const cliBundle = resolve(dist, "idexal.cjs");
const seaBlobForTarget = (target) => resolve(dist, `idexal-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "idexal-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/idexal.cjs",
        output: `dist/idexal-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "idexal-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        IDEXAL_STORAGE_DIR: storageRoot,
      };
      delete env.IDEXAL_BFS_BINARY;
      delete env.IDEXAL_RG_BINARY;
      delete env.IDEXAL_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/idexal.cjs. Run `pnpm build` first.");
  }

...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@idexal/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 Idexal Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "idexal.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "idexal.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "idexal.execution.turn_id": safeId(correlation.turnId),
    "idexal.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "idexal.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "idexal.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "idexal.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "idexal.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "idexal.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "idexal.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}

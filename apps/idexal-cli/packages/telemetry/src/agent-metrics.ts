...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "idexal.agent.turn.duration"),
      agent_step: duration(meter, "idexal.agent.step.duration"),
      tool_execution: duration(meter, "idexal.tool.execution.duration"),
      command_execution: duration(meter, "idexal.command.execution.duration"),
      context_compaction: duration(meter, "idexal.context.compaction.duration"),
      detached_operation: duration(meter, "idexal.detached.operation.duration"),
      model_call: duration(meter, "idexal.model.call.duration"),
      model_attempt: duration(meter, "idexal.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("idexal.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("idexal.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "idexal.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "idexal.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "idexal.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("idexal.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "idexal.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "idexal.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("idexal.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("idexal.telemetry.abandoned.count", {

...
import type { ModelStatusSink } from "@idexal/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  if (!resolveOtlpTraceEndpoint(env) || isExplicitlyDisabled(env.IDEXAL_MODEL_TELEMETRY_ENABLED)) {
    return env;
  }
  const existingInstallationId = normalizeTelemetryDeviceMid(env.IDEXAL_TELEMETRY_DEVICE_MID);
  const installationId =
    existingInstallationId ?? (await resolveStandaloneDeviceMid(env.IDEXAL_HOME?.trim()));
  const preparedEnv = installationId ? { ...env, IDEXAL_TELEMETRY_DEVICE_MID: installationId } : env;
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.IDEXAL_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.IDEXAL_ENV ??
      env.IDEXAL_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.IDEXAL_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.IDEXAL_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.IDEXAL_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.IDEXAL_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "idexal-cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.IDEXAL_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.IDEXAL_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.IDEXAL_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  idexalHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = idexalHome
    ? join(idexalHome, "v2", "telemetry-state.json")
    : join(homedir(), ".idexal", "v2", "telemetry-state.json");

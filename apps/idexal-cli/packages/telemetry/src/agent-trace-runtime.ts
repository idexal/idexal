...
import type {
  ...
  ToolTraceStart,
} from "@idexal/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "idexal.agent_turn.turn_number": integer(input.turnNumber),
            "idexal.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "idexal.agent_step.step_id": safeId(input.stepId),
            "idexal.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "idexal.execution.tool_call_id": safeId(input.toolCallId),
            "idexal.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "idexal.context_compaction.trigger": safeEnum(input.trigger),
            "idexal.context_compaction.phase": safeEnum(input.phase),
            "idexal.context_compaction.model_mode": safeEnum(input.modelMode),
            "idexal.context_compaction.outer_attempt": integer(input.outerAttempt),
            "idexal.context_compaction.max_attempts": integer(input.maxAttempts),
            "idexal.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "idexal.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "idexal.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "idexal.context_compaction.token_source": safeEnum(input.tokenSource),
            "idexal.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "idexal.detached_operation.operation": safeEnum(input.operation),
            "idexal.detached_operation.execution_kind": safeEnum(input.executionKind),
            "idexal.detached_operation.trigger": safeEnum(input.trigger),
            "idexal.detached_operation.target_kind": safeEnum(input.targetKind),
            "idexal.detached_operation.goal_iteration": integer(input.goalIteration),
            "idexal.detached_operation.chunk_index": integer(input.chunkIndex),
            "idexal.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "idexal.execution.logical_call_id": safeId(input.logicalCallId),
            "idexal.model_call.operation": safeEnum(input.operation),
            "idexal.model_call.streaming": input.streaming,
            "idexal.model_call.model_role": safeEnum(input.modelRole),
            "idexal.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "idexal.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "idexal.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "idexal.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "idexal.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "idexal.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "idexal.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "idexal.model_call.call_cause": safeEnum(input.callCause),
            "idexal.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "idexal.execution.tool_call_id": safeId(parent.toolCallId),
            "idexal.command_execution.safe_name": safeString(input.safeName, 128),
            "idexal.command_execution.category": safeEnum(input.category),
            "idexal.command_execution.command_count": integer(input.commandCount),
            "idexal.command_execution.shell_kind": safeEnum(input.shellKind),
            "idexal.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "idexal.execution.tool_call_id": safeId(parent.toolCallId),
            "idexal.execution.logical_call_id": parentWriter.logicalCallId,
            "idexal.execution.model_operation": parentWriter.operation,
            "idexal.execution.model_role": parentWriter.modelRole,
            "idexal.model_attempt.request_id": safeId(input.requestId),
            "idexal.model_attempt.attempt_number": integer(input.attemptNumber),
            "idexal.model_attempt.max_attempts": integer(input.maxAttempts),
            "idexal.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "idexal.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "idexal.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "idexal.model_attempt.provider_id": safeString(target.providerId, 128),
            "idexal.model_attempt.provider_kind": safeEnum(target.providerKind),
            "idexal.model_attempt.provider_origin": safeString(target.providerOrigin),
            "idexal.model_attempt.provider_route": safeString(target.providerRoute),
            "idexal.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "idexal.model_attempt.transport": safeEnum(input.transport),
            "idexal.model_attempt.api_operation": safeEnum(input.apiOperation),
            "idexal.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "idexal.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "idexal.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "idexal.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "idexal.model_attempt.reasoning_requested_budget_tokens": integer(
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("idexal.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("idexal.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("idexal.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("idexal.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("idexal.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("idexal.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("idexal.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("idexal.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("idexal.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("idexal.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("idexal.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("idexal.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("idexal.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("idexal.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("idexal.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("idexal.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("idexal.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("idexal.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("idexal.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "idexal.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("idexal.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "idexal.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "idexal.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "idexal.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "idexal.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "idexal.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("idexal.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("idexal.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("idexal.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("idexal.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("idexal.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("idexal.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("idexal.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("idexal.model_attempt.time_to_first_text_ms", elapsed);

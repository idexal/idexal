...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@idexal/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@idexal/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("idexal.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("idexal.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "idexal.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "idexal.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("idexal.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("idexal.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "idexal.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("idexal.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("idexal.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("idexal.model.attempt.tokens"),
    cardinalityView("idexal.model.attempt.stream_stall.count"),
    cardinalityView("idexal.telemetry.creation_drop.count"),
    cardinalityView("idexal.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "idexal.build.commit_id": resource.buildCommitId,
    "idexal.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "idexal.product.version": resource.productVersion,
    "idexal.runtime.distribution": resource.runtimeDistribution,
    "idexal.runtime.surface": resource.runtimeSurface,
    "idexal.telemetry.schema_owner": "cli",
    "idexal.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}

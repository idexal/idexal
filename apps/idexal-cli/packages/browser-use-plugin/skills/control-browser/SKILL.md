...
```js
const browserPluginRoot = process.env.IDEXAL_PLUGIN_ROOT;
...
If the user explicitly asks for Idexal's in-app browser:
...
1. Start every browser `js` call with the bootstrap, then assign the selected backend to a local `browser` binding. If the user explicitly asks for Idexal's in-app browser, use `const browser = await agent.browsers.get("iab")`. If they explicitly ask for Chrome, use `await agent.browsers.get("extension")` only when the runtime advertises it. For an unspecified target URL use `await agent.browsers.getForUrl(url)`; with no URL/backend preference use `await agent.browsers.getDefault()`.
...
8. Browser tabs persist for the lifetime of the current Idexal process unless you explicitly call `tab.close()` or
...
- `networkidle` is present in the shared type but is rejected by every Idexal browser backend. For

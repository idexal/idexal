...
- This package is **pure**: no session spawning, no storage, no disk or network
  ...
  never imports from `@idexal/core` or `@idexal/bootstrap`.
- The runtime layer binds to this package through a narrow driver interface. The
  ...
  sibling package `@idexal/dynamic-workflow-runtime` — impure (node builtins) but
  still app-independent. The production driver (actor sessions, SQLite journal,
  tool wiring) lives in `@idexal/bootstrap`, evolving the existing
...
All commands from this directory (or with `--filter @idexal/dynamic-workflow` from

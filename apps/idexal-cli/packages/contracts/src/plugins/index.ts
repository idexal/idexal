...
export const IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE = "zcode-plugins-official";
export const IDEXAL_INLINE_PLUGIN_MARKETPLACE = "inline";
export const IDEXAL_PLUGIN_HOST_COMMAND = "__idexal-plugin-host";
/**
 * 隐藏子命令：dynamic workflow 的沙箱子进程入口（`__idexal-dwf-child <entry path>`；argv 末位是
 * harness 写好的入口文件路径，payload 不过命令行）。
 *
 * 与 {@link IDEXAL_PLUGIN_HOST_COMMAND} 同族、同机制：SEA 单文件二进制不解释 Node CLI 旗标，
 ...
 */
export const IDEXAL_DWF_CHILD_COMMAND = "__idexal-dwf-child";

export function isOfficialMarketplaceId(id: string): boolean {
  return id === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE;
}
...
export interface PluginDiscoverRequest {
  ...
  // 让用户不必先 `idexal plugins enable` 就能用 (例如纯内容型的 skill-creator)。

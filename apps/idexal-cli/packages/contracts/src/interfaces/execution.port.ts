import type { BackgroundBashOutputResult } from "@idexal/shared";
// ============================================================
...
export type ExecutionCommand =
  ...
  | {
      ...
      /**
       * Idexal runtime-provided Bash shell selection. Current consumers must only use
...
export interface ExecutionShellSelection {
  ...
  label?: string;
  /** Executable path when Idexal resolved a concrete shell. Omitted for legacy shell fallback. */
  path?: string;
...
interface EmbeddedSearchCommandBackend {
   ...
   * idexal.cjs 时必须带 ELECTRON_RUN_AS_NODE=1，否则会按 Electron 子进程启动。
   */

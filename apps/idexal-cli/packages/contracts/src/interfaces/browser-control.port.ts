 ...
 * 实现（ProtocolBrowserControlBroker）把它翻译成 Idexal Protocol 的
 * interaction/browserExecute 反向请求，由 app（host→main WebContentsView/CDP）执行。
 *
 * 类型说明：BrowserCommand/BrowserCommandResult 与 @idexal/shared 的 browser-use 契约同构。
 * 此处定义结构镜像（不 import @idexal/shared，避免 agent contracts 的 zod v3 与 shared zod v4
...
/** Idexal Protocol 使用包装结果；BrowserControlPort.list 会解包并直接返回 browsers。 */
...
// 与 @idexal/shared 的 browserCommandSchema 各变体结构镜像同步。
...
/** 受控 tab 摘要（list 命令返回）；与 @idexal/shared 的 browserTabSummarySchema 镜像同步。 */

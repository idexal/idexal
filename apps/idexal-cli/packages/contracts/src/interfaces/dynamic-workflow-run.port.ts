 ...
 * JSON 镜像）。**刻意在这里重新声明**而不是从 @idexal/dynamic-workflow import：端口只承载
 ...
 *
 * 刻意不复用引擎的 `RunEvent`：那是领域包（@idexal/dynamic-workflow）的词汇表，把它
 ...
 * 文本。实现已随共享 workflowRuns reducer 搬进 `@idexal/shared/idexal-protocol-v4`
 ...
 */
export { serializeWorkflowArtifact } from "@idexal/shared/idexal-protocol-v4";
 ...
 * 而不是从 @idexal/dynamic-workflow import：端口只承载 JSON 形状，引擎的词汇表一旦进
...
// 此处原样再导出以保持 `@idexal/contracts` 的导入路径不变。

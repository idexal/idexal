...
import type { McpServerFailureKind, OfficialMcpAuthPortFailureReason } from "@idexal/shared";
...
/**
 * Idexal 官方 Server MCP 的鉴权声明。
 * 允许出现在 `type: "http"` 与 `type: "stdio"`；`sse` 仍拒。
 * `type`/`provider` 均为精确值，不接受别名或大小写变体。
 *
 * 注意：该字段本身**不构成**官方身份证明。官方身份由运行时解析的 Idexal API origin 判定
 * （host 侧还会二次校验）；第三方 Plugin 复制该字段只能让凭证流向真实 Idexal 后端。
 *
 * 两种形态的凭证投递通道不同：
 * - http：宿主在 fetch wrapper 里逐请求注入身份头，凭证从不进入插件进程；
 * - stdio：请求由插件进程自己发出，身份头随每条出站协议消息的 `_meta` 下发。
 */
export interface IdexalOfficialMcpAuthConfig {
  type: "zcode_official";
  provider: "jwt_token";
}
...
export interface McpStdioServerConfig extends McpServerConfigBase {
  ...
  auth?: IdexalOfficialMcpAuthConfig;
  /** 宿主生成，禁止来自文件配置。 */
...
export const IDEXAL_MCP_ERROR_PRESENTATION_META_KEY = "idexal/errorPresentation";
export const IDEXAL_MCP_ERROR_PRESENTATION_MESSAGE_ONLY = "message-only";
/** MCP content 中来自模型显式 tab.screenshot() 的 image block 索引。 */
export const IDEXAL_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY =
  "idexal/browserScreenshotContentIndices";
/**
 * 本次 node_repl cell 操作的目标应用身份，供工具卡显示 App 图标。
 *
 * **只能由 node-repl-host 写入**：宿主的 CUA bridge 从 broker 响应里读 producer 的
 * `idexal.cua/app-associations-v1`，投影成这里的最小形态。producer 那个键本身经
 * `nodeRepl.setResponseMeta` / `nodeRepl.emitStructuredResult` 也能到达 `_meta`，而这两个
 * API 挂在模型可见的 sandbox globals 上，因此不可信、必须在宿主侧丢弃（同
 * `IDEXAL_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY` 的处置）。
 */
export const IDEXAL_MCP_NODE_REPL_CUA_APP_META_KEY = "idexal/nodeReplCuaApp";
/**
 * 官方 Server MCP 响应头里的 `x-request-id`，附在失败的 tool result 上（值为 string）。
 *
 * 用短前缀 `idexal/` 而不是 `com.idexal/`：这不是跨语言协议——服务端在 header 里给，客户端
 * 读到后自己搬进 `_meta`，产出与消费都在客户端（`com.idexal/` 留给 Go 侧直接产出的键，
 * 如 `com.idexal/mcp-unavailable`）。
 *
 * 只在 `isError` 时附加：这是给人看的排障线索（拿它去查服务端日志），成功路径上是纯噪声。
 */
export const IDEXAL_MCP_SERVER_REQUEST_ID_META_KEY = "idexal/officialMcpServerRequestId";
...
export interface McpToolDescriptor {
   ...
   * 而不是"插件是谁"：
   * - http：响应来自 Idexal 后端。连接存活即意味着每个请求都过了 origin 校验且 fail closed，
   *   第三方插件即使声明官方鉴权，也只能把请求打到真实 Idexal，响应体不由它写；
   * - stdio：结果由插件进程自己产出、可任意伪造，因此**不置位**。
   *
   * 刻意**不**按"插件是否来自官方 marketplace"判定：那会让非官方安装源（含本地自测与
   * idexal-plugins-test）的官方插件失效，而它也不是真实屏障——详见 `@idexal/shared` 的
 ...
 * 官方 MCP 信任判定。
 *
 * 规则只有一条：**目标 origin 逐字符等于当前 Idexal API origin（https、无 username/password）**，
 ...
 * 并非真实屏障）。
 *
 * 实现在 `@idexal/shared`：host 与 adapter 共用同一份，避免一侧放行一侧拒绝。

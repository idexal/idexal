...
/**
 * 教模型写工作流的内置技能名（apps/idexal-cli/packages/bundled-skills/skills/<name>/SKILL.md）。
...
export const CREATE_WORKFLOW_ARGS_WITHOUT_PATH_ERROR =
  "`args` belongs to the `path` source: it carries values for the arguments a script file declares in its `/* idexal-workflow` block. For a saved workflow pass `saved.args`; an inline `script` declares no arguments, so it takes none.";
...
const CreateWorkflowModelInputSchema = z
  .object({
     ...
     * 只改一行再把同一个路径交回来。带 `/* idexal-workflow` 块的文件按保存定义解析（块剥掉、
     * 正文当脚本、`args` 按块里的声明校验）。
     */
    path: z
      .string()

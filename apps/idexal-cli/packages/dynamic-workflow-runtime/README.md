# @idexal/dynamic-workflow-runtime

沙箱 harness（dynamic workflow 执行引擎）。把一份 workflow 脚本在受控子进程里跑起来，
用 NDJSON 把子进程的 `__host.*` 调用桥接到 `@idexal/dynamic-workflow` 的纯引擎核心。

## 依赖边界

**仅**依赖 `@idexal/dynamic-workflow`（workspace）与 node 内建。**绝不** import `@idexal/core` /
`@idexal/contracts` / `@idexal/bootstrap` / `@idexal/adapters`——本包是「整条 sandbox↔engine
管线 app-free 可跑」的证明。

## 用法

```ts
import { runWorkflowScript } from "@idexal/dynamic-workflow-runtime";
...
const settlement = await runWorkflowScript({
  ...
  validate,                   // @idexal/dynamic-workflow 的 validate（适配到 ValidateFn）
...
测试与 typecheck 通过 `@idexal/dynamic-workflow` 的**已构建 dist** 解析依赖，故 `pretest` /
`pretypecheck` 会先 `pnpm --filter @idexal/dynamic-workflow build`。全新检出直接 `pnpm test` 即可，
不会踩到 stale-dist。

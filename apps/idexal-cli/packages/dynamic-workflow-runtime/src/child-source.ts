 ...
 *     自包含 ESM，harness 写到 `<cwd>/.idexal/workflow-runs/<runId>.mjs`（child-entry-file.ts）；
 *   - 普通 Node：`node --max-old-space-size=N <entry>`，入口文件发现自己就是进程入口时自启；
 *   - SEA 单文件二进制：CLI 的隐藏子命令 `__idexal-dwf-child <entry>` `import()` 这份文件并调
...
export function renderChildEntry(payload: ChildPayload, meta: { runId: string }): string {
  ...
  return `// idexal dynamic workflow run ${runId}
// Generated by @idexal/dynamic-workflow-runtime before every launch; safe to delete once the run has settled.

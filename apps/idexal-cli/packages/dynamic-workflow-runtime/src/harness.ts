 ...
 * 本包**只**依赖 `@idexal/dynamic-workflow` 与 node 内建——证明整条管线 app-free 可跑，
 * 绝不 import `@idexal/core`/`@idexal/contracts`/`@idexal/bootstrap`/`@idexal/adapters`。
 *
 * 时序（happy path）：
 *
 *   parent                         child(vm)
 *     │  write <cwd>/.idexal/workflow-runs/<runId>.mjs（payload: lowered+args 内嵌）
 *     │  spawn(node <entry>)
...
export interface RunWorkflowOptions {
  ...
  /**
   * 非致命状况的上报口（今日只有一种：入口文件写不进项目 `.idexal/`，回落到了 OS 临时目录）。

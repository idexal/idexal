 ...
 * `<cwd>/.idexal/workflow-runs/<runId>.mjs`，spawn 时命令行只剩这条路径。
 *
 * 裁决：
 *   - 位置 `.idexal/workflow-runs/`，与项目级 saved workflow 的 `.idexal/workflows/` 同级不混放；
 *   - 文件**保留**不删（同一 runId 原位覆写），目录兼作每次 run 实际执行体的存档；
 *   - 目录里由本模块写一份 `.gitignore`（`*`），只在缺席时写一次，绝不碰项目自己的 `.gitignore`；
 *   - 项目目录写不进（只读 checkout、cwd 不存在、`.idexal` 是个普通文件…）→ 回落到
 *     `os.tmpdir()/idexal-workflow-runs/` 并经 `onWarning` 报一声，run 照常启动。回落也失败就抛，
...
export interface HarnessWarning {
  ...
  /** 本想写入的项目目录（`<cwd>/.idexal/workflow-runs`）。 */
  projectDir: string;
  /** 实际回落到的目录。 */
  fallbackDir: string;
  /** 项目目录写入失败的错误文本。 */
...
export function workflowRunsDir(cwd: string): string {
  return join(cwd, ".idexal", "workflow-runs");
}

/** 回落目录（OS 临时目录下，跨项目共用）。 */
export function fallbackWorkflowRunsDir(): string {
  return join(tmpdir(), "idexal-workflow-runs");
}

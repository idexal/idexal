...
const traceHeaderNames = ["x-idexal-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          IDEXAL_HTTP_PROXY: url,
          IDEXAL_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.idexalCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.IDEXAL_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.IDEXAL_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.IDEXAL_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.IDEXAL_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.IDEXAL_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}

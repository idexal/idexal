...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@idexal/contracts";
import { LogLevel, LogLevelName } from "@idexal/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.IDEXAL_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.IDEXAL_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".idexal", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeIdexalRuntimeEnv(env[IDEXAL_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `idexal-${formatLocalLogDate(new Date())}.jsonl`;
}

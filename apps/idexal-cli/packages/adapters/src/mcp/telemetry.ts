...
import type { IdexalMcpTelemetryEvent } from "@idexal/shared";
import {
...
type McpTelemetryEvent = IdexalMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: IdexalMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: IdexalMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as IdexalMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as IdexalMcpTelemetryEvent["platform"]);

...
import { IDEXAL_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type IdexalMcpResourceSample } from "@idexal/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: IdexalMcpResourceSample["arch"];
  platform: IdexalMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: IdexalMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const groups = new Map<string, IdexalMcpResourceSample>();

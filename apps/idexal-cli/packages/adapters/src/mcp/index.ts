...
import { IDEXAL_MCP_SERVER_REQUEST_ID_META_KEY } from "@idexal/contracts";
...
export interface CreateMcpAdapterOptions {
  ...
  officialMcpAuth?: {
    ...
    /**
     * 当前 Idexal API origin。stdio 形态没有 `url` 可供校验，targetOrigin 只能由宿主给出
     * ——插件因此无法把身份头导向别的 origin。
     * 与 trustedOrigins 的 `resolveIdexalApiOrigin` 必须同源，否则两侧判定会分叉。
     */
    resolveIdexalApiOrigin?: () => string;
 ...
 * 由 adapter 发起无身份的 tools/call，让 Idexal server 返回权威结构化错误。把 reason 交给 stdio
...
class NodeMcpAdapter implements McpPort {
  ...
  private credentialStore?: SharedIdexalCredentialStore;
  /**
  ...
  constructor(options: CreateMcpAdapterOptions) {
    this.clientName = options.clientName ?? "idexal";
  ...
  private async resolveOfficialStdioAuthMeta(
    ...
    const resolveIdexalApiOrigin = this.officialMcpAuth?.resolveIdexalApiOrigin;
    const logBase = {
    ...
    if (!authHeadersPort || !trustedOrigins || !resolveIdexalApiOrigin) {
    ...
    try {
      targetOrigin = resolveIdexalApiOrigin();
      ...
      return {
        ...
        _meta:
          isError && serverRequestId
            ? { ...meta, [IDEXAL_MCP_SERVER_REQUEST_ID_META_KEY]: serverRequestId }
            : meta,
      };
    } catch (error) {
      const durationMs = Date.now() - startedAt;
    ...
    try {
      ...
      const tools = listed.tools.map((tool) =>
        normalizeMcpToolDescriptor(
          ...
          //   - http：结果来自 Idexal 后端。fetch wrapper 对每次请求校验 origin；登录态只在
          ...
          config.type === "http" && config.auth?.type === IDEXAL_OFFICIAL_MCP_AUTH_TYPE,
        ),
      );
  ...
  private async runInteractiveOAuthAuthorization(input: {
    ...
    try {
      ...
      const credentialStore = oauthOptions?.credentialStore ?? createSharedIdexalCredentialStore();
  ...
  private createOAuthClientProvider(
    ...
    // 官方鉴权与 OAuth 互斥：官方 MCP 的失败只能由 Idexal 登录/套餐解决，
  ...
  private resolveCredentialStore(): SharedIdexalCredentialStore {
    this.credentialStore ??= this.mcpOAuth?.credentialStore ?? createSharedIdexalCredentialStore();
...
function mcpRequestMeta(request: McpCallToolRequest): Record<string, unknown> {
  // nodeRepl.requestMeta 暴露。Idexal 所有 MCP server 都可忽略这些扩展键；node_repl browser
  ...
  return {
    ...requestContext,
    "com.idexal/request-context": requestContext,
  };
}
...
function resolveVersionNegotiation(
  ...
  return {
    ...
    probe: {
      ...
      // Idexal 的总连接超时可能先结束并让 probe 残留，也不给 legacy initialize 留预算。
...
function resolveAuthorizationCodeOAuthConfig(
  ...
  // 只能由 Idexal 自身的登录/套餐解决，不可能由目标 MCP 的 OAuth 授权解决。
  if (isOfficialAuthConfig(config)) return undefined;

...
import { sanitizeIdexalRuntimeEnv } from "@idexal/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeIdexalRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.idexal/server/node 启动，但登录环境 PATH 不含该目录，

...
import { acquireFileLock } from "@idexal/shared/node";
import { IDEXAL_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@idexal/shared";
import type { SharedIdexalCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === IDEXAL_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;

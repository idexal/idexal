...
import type { McpServerConfig, RuntimeConfigPatch } from "@idexal/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 IdexalConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".idexal" ? dirname(configDirectory) : configDirectory;
}

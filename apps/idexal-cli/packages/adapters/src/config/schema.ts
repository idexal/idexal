/* eslint-disable max-lines -- idexal-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const IdexalConfigFileSchema = z
  .object({
...
export type IdexalConfigFile = z.infer<typeof IdexalConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. Idexal does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；Idexal 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = IdexalConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: IdexalConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: IdexalConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: IdexalConfigFile["skills"],

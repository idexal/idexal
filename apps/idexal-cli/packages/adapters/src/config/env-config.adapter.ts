// Env Config Adapter - Parse the intentionally small IDEXAL_* environment surface.
...
const DEFAULT_PREFIX = "IDEXAL_";

/**
 * Parse IDEXAL_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.IDEXAL_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}

import {
  ...
  IDEXAL_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readIdexalToolEnvPassthroughEnv,
} from "@idexal/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [IDEXAL_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [IDEXAL_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, IDEXAL_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readIdexalToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const idexalProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [IDEXAL_HTTP_PROXY_ENV_KEY], platform),
  );
  if (idexalProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, idexalProxy, platform);
    }
    return;
  }
}

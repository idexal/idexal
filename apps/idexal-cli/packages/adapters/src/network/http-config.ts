...
function resolveProxyForRequestInternal(
  ...
  const candidates = [[`env:${IDEXAL_HTTP_PROXY_ENV_KEY}`, env[IDEXAL_HTTP_PROXY_ENV_KEY]]];
  ...
  if (capturedProxy) {
    return {
      noProxyMatched: false,
      proxySource: `env:${IDEXAL_TOOL_ENV_PASSTHROUGH_ENV_KEY}.${capturedProxy.key}`,
...
export function resolveTlsCaCertFile(options: NetworkTlsOptions): string | undefined {
  const candidates = [
    options.caCertFile,
    options.env?.[IDEXAL_AGENT_CA_CERT_ENV_KEY],
  ];
...
function readExplicitNoProxyValue(options: NetworkProxyOptions): string | undefined {
  return (
    ...
    normalizePathLike(options.env?.[IDEXAL_NO_PROXY_ENV_KEY])
  );
}

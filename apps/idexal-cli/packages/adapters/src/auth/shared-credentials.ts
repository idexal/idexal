...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@idexal/shared/node";
import { createIdexalCredentialCipher, type IdexalCredentialCipher } from "./credential-cipher.js";

const IDEXAL_DATA_BASE_DIR_ENV_KEY = "IDEXAL_DATA_BASE_DIR";
...
export const SHARED_IDEXAL_CREDENTIAL_KEYS = {
  ...
  idexalJwtToken: "zcodejwttoken",
} as const;

export interface SharedIdexalCredentialStoreOptions {
  baseDir?: string;
  cipher?: IdexalCredentialCipher;
...
export interface SharedIdexalCredentialStore {
...
export function createSharedIdexalCredentialStore(
  options: SharedIdexalCredentialStoreOptions = {},
): SharedIdexalCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedIdexalCredentialsPath(options);
  const cipher = options.cipher ?? createIdexalCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.idexalJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.idexalJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_IDEXAL_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedIdexalCredentialSync(
  key: string,
  options: SharedIdexalCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedIdexalCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createIdexalCredentialCipher({ env: options.env });
...
export function resolveSharedIdexalCredentialsPath(
  options: SharedIdexalCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[IDEXAL_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".idexal", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared Idexal credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared Idexal credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}

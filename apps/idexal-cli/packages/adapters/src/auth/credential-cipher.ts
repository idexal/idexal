...
const CREDENTIAL_SECRET_ENV_KEY = "IDEXAL_CREDENTIAL_SECRET";

export interface IdexalCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface IdexalCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createIdexalCredentialCipher(
  options: IdexalCredentialCipherOptions = {},
): IdexalCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedIdexalCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedIdexalCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `idexal-credential-fallback:${platform()}:${homedir()}:${username}`;
}

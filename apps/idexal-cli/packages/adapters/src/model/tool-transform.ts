...
import { ModelErrorCode, type JsonSchema, type ModelToolContract } from "@idexal/contracts";
...
function hoistLocalJsonSchemaRefsToDefs(toolName: string, schema: JsonSchema): JsonSchema {
  ...
  for (const ref of refs) {
    let defKey = `idexal_ref_${nextDefIndex}`;
    while (usedDefKeys.has(defKey)) {
      nextDefIndex += 1;
      defKey = `idexal_ref_${nextDefIndex}`;
    }
    nextDefIndex += 1;
...
function providerOptionsForClientTool(options: AiSdkToolTransformOptions): {
  ...
  // Anthropic-compatible gateways reject that extra tool field, so Idexal opts out

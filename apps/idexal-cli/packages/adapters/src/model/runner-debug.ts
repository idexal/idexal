...
import type { ModelTextResult } from "@idexal/contracts";
import { IDEXAL_RUNTIME_ENV_KEY, normalizeIdexalRuntimeEnv } from "@idexal/shared";
...
export function shouldRecordModelIO(env: EnvRecord): boolean {
  // 开发态与生产态都记录(分别落到 debug / rollout 目录);仅测试态(IDEXAL_RUNTIME_ENV=test)不写,
  // 避免单测产生磁盘副作用。未设时按生产处理(记录到 rollout,带条数上限)。
  return normalizeRuntimeEnv(env) !== "test";
}

// 判定当前是否开发态,用于选择落盘目录(debug vs rollout)。
// 直接看 IDEXAL_RUNTIME_ENV === "development";dev 桌面/CLI 启动时已注入该变量。
 ...
 * 每个 turn（`streamText`）即便 IDEXAL_RUNTIME_ENV=development 也从不落盘。流式路径同样要记录。
 *
...
// 归一化 IDEXAL_RUNTIME_ENV；未设置时返回 undefined,由调用方按生产处理。
function normalizeRuntimeEnv(env: EnvRecord): string | undefined {
  return normalizeIdexalRuntimeEnv(env[IDEXAL_RUNTIME_ENV_KEY]);
}
...
// 这里保持历史语义：开发态写 ~/.idexal/cli/debug，生产态写 ~/.idexal/cli/rollout。
function getModelIOBaseDir(isDev: boolean): string {
  return join(homedir(), ".idexal", "cli", isDev ? "debug" : "rollout");
}

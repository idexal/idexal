import { resolveRuntimeIdexalEndpointOrigin } from "@idexal/shared";
import type { EnvRecord } from "./model-execution.js";

/**
 * 官方 Coding Plan 的模型请求经 Idexal 平台网关发送。
 *
 * Z.ai / BigModel Coding Plan 是 Idexal 的官方订阅套餐，模型请求统一发往 Idexal 平台网关，
 ...
 * 第三方模型服务不受影响。网关 origin 跟随 IDEXAL_BASE_URL / IDEXAL_ENDPOINT_ORIGIN，
 * 缺省为线上 https://zcode.z.ai。
 */
export interface OfficialCodingPlanGatewayRoute {
  /** 官方模型端点（含路径），仅 https。 */
  readonly providerEndpoint: string;
  /** 对应的网关端点路径，相对 Idexal 平台 origin。 */
...
export function resolveOfficialCodingPlanGatewayUrl(
  ...
  const gatewayUrl = new URL(gatewayPath, resolveRuntimeIdexalEndpointOrigin(env));

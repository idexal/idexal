...
import type { RegistryProviderConfig } from "@idexal/provider";
import { withOpenRouterAttributionHeaders } from "@idexal/shared";
...
/**
 * 模型请求出口：官方 Coding Plan 端点经 Idexal 平台网关发送（做套餐权益校验等平台侧处理），
...
export function readProviderBusinessFailureFromBody(body: unknown):
  ...
  const providerCode =
    ...
    // 二次包装后的外层 code 是 Idexal 自己的 PROVIDER_BUSINESS_ERROR，

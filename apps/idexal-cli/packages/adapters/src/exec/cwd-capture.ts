...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `idexal-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__idexal_status=$?",
    `if [ "$__idexal_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__idexal_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__idexal_status=%ERRORLEVEL%"',
    `if "%__idexal_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__idexal_status%",
  ].join("\r\n");
}

...
import { windowsPathToGitBashPath } from "@idexal/contracts";
import type { ExecutionShellDialect } from "@idexal/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_IDEXAL_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_IDEXAL_SHELL_INIT_SNAPSHOT",
  ];
}

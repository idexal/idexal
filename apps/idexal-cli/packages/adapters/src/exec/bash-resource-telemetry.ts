...
interface BashResourceTelemetryOptions {
  ...
  onComplete: (sample: IdexalToolExecResource) => void;
  readContext?: () => Pick<IdexalToolExecResource, "cliRssKb" | "systemFreeMemoryKb">;
}

/** 每条命令独占状态与探针失败预算；finish 同步封口，绝不等待遥测 IO。 */
export function createBashResourceTelemetry(options: BashResourceTelemetryOptions): {
  finish(exitKind: IdexalToolExecResource["exitKind"]): void;
} {

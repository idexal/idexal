 ...
 */
import type { McpOfficialProvenance, IdexalOfficialMcpAuthConfig } from "@idexal/contracts";
 ...
 */
export function parseIdexalOfficialAuth(
  value: unknown,
  mcpKey: string,
): IdexalOfficialMcpAuthConfig | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
    throw new Error(`MCP server ${mcpKey}: auth must be an object`);
  }
  // 精确值匹配，区分大小写；idexal-official、zcode_official_auth 等别名一律拒绝。

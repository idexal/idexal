...
import { findOfficialMcpReservedHeaders } from "@idexal/shared";
import type {
  McpOAuthConfig,
  McpServerConfig,
...
import { IDEXAL_PLUGIN_ID_ENV_KEY } from "@idexal/shared";
import type { LoadedPlugin } from "./types.js";
import { isNotFoundError, isPluginOptionValue, isRecord, resolveInside } from "./helpers.js";
import { buildOfficialProvenance, parseIdexalOfficialAuth } from "./mcp-official-auth.js";
...
function resolveMcpServerConfig(
  ...
  // Idexal 官方市场同时包含随应用装载的 Builtin Plugin 与按需安装的 CDN Plugin；后者运行时
  // source 为 `cache`，因此必须按 marketplace 身份归类，不能只看 loader source。
  const source: McpServerRuntimeSource = {
    kind: context.loaded.marketplace === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE ? "builtin" : "plugin",
  };
  ...
  const officialAuth = parseIdexalOfficialAuth(server.auth, identity.mcpKey);
  ...
  if (type === "stdio") {
    ...
    const env = resolveStringRecord(
      {
        ...
        IDEXAL_PLUGIN_DATA: context.dataPath,
        IDEXAL_PLUGIN_ROOT: context.loaded.rootPath,
        IDEXAL_PROJECT_DIR: context.workingDirectory,
    ...
    env[IDEXAL_PLUGIN_ID_ENV_KEY] = context.loaded.id;
    return {
      type: "stdio",

...
import type { PluginDiagnostic, PluginManifest, PluginStoreListing } from "@idexal/contracts";
import { isOfficialMarketplaceId, IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE } from "@idexal/contracts";
import { DEFAULT_PLUGIN_MARKETPLACES, sanitizeIdexalRuntimeEnv } from "@idexal/shared";
...
const IDEXAL_MANIFEST_PATH = join(".idexal-plugin", "plugin.json");
...
function buildMarketplaceGitEnv(
  ...
  const env = sanitizeIdexalRuntimeEnv(sourceEnv);
  // marketplace 安装会启动 Git 子进程，不能只依赖父进程继承的 shell 代理。
  // 这里统一从 Idexal 显式网络环境恢复 HTTP(S)/NO_PROXY/CA，避免安装按钮卡到协议超时。
...
export async function addMarketplace(input: {
  ...
  try {
    ...
    if (
      input.trustedId === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE &&
      loaded.manifest.name !== IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE
    ) {
      throw new Error(
        `Official marketplace source must provide ${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}, received ${loaded.manifest.name}`,
      );
    }
    ...
    } else if (loaded.manifest.name !== IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallMarketplacePlugin(input: {
  ...
  storageRoot: string;
  removeCache?: boolean;
  /** `idexal plugins uninstall --keep-data`：删安装缓存但保留 data/<plugin-id> 用户数据目录。 */
  keepData?: boolean;
...
async function clonePluginSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "idexal-plugin-src-"));
  const args = ["clone"];
...
async function cloneMarketplaceSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "idexal-marketplace-src-"));
...
async function execGitCommand(args: string[], signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const gitBinary = process.env.IDEXAL_GIT_BINARY?.trim() || "git";
...
function findPluginManifestPath(rootPath: string): string | null {
  for (const candidate of [IDEXAL_MANIFEST_PATH, CLAUDE_MANIFEST_PATH, CODEX_MANIFEST_PATH]) {
  ...
  for (const key of UNSUPPORTED_MANIFEST_FIELDS) {
    if (key in input.manifest) {
      input.diagnostics.push({
        ...
        message: `Plugin component is diagnostic-only in this Idexal runtime: ${key}`,

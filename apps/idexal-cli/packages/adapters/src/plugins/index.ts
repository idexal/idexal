...
import {
  ...
  IDEXAL_INLINE_PLUGIN_MARKETPLACE,
  IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE,
} from "@idexal/contracts";
import {
  directoryExists,
  fileExists,
  ...
  for (const [eventName, matcherConfigs] of Object.entries(hooksRoot)) {
    if (!SUPPORTED_HOOK_EVENTS.has(eventName)) {
      input.diagnostics.push({
        ...
        message: `Plugin hook event is not supported by this Idexal runtime: ${eventName}`,
        path: input.sourcePath,
        pluginId: input.loaded.id,
        severity: "warning",
      });
...
function materializeCommandMetadataRoot(
  ...
  for (const [rawName, rawMetadata] of Object.entries(spec)) {
    ...
    // Idexal 的 custom command loader 只扫描 markdown 根目录，因此把低风险命令内容
...
function scanOfficialCache(
  ...
  const cacheRoot = join(storageRoot, "cache", IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE);
  try {
    const roots: string[] = [];
...
function findManifest(rootPath: string): string | null {
  const idexalPath = join(rootPath, IDEXAL_MANIFEST_PATH);
  if (fileExists(idexalPath)) {
    return idexalPath;
  }

...
function rebuildOfficialMarketplaceSync(storageRoot: string): Record<string, unknown> {
  ...
  const merged = {
    ...(bundledManifest ?? {}),
    ...(cdnManifest ?? {}),
    name: IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE,
...
function assertOfficialManifest(manifest: Record<string, unknown>): void {
  if (manifest.name !== IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE) {
    throw new Error(
      `Official marketplace manifest must be named ${IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE}`,
    );
  }
}

function partitionPath(storageRoot: string, fileName: string): string {
  return join(storageRoot, "marketplaces", IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE, fileName);
}

...
import type { WorkspaceHookTrustRecord, WorkspaceHookTrustStoreFile } from "@idexal/contracts";
import {
...
export async function resolveWorkspaceHookTrustStorePath(
  ...
  const userConfigPath = resolve(
    options.userConfigPath ?? join(home, ".idexal", "cli", "config.json"),
  );
  ...
  const storageRoot = configured ? resolveTrustedUserPath(configured, home) : join(home, ".idexal");

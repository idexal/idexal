...
import type { SkillRoot, SkillSource } from "@idexal/contracts";
...
export async function resolveDefaultSkillRoots(
  ...
  const includeIdexal = options.includeIdexalSkills ?? true;
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.idexal` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.idexal` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, IDEXAL_DIR, SKILLS_DIR), scope, "idexal", nextPriority()),

...
`${IDEXAL_SKILL_DIR}/examples.md` §5. Read it when you want to see a whole script's arc; §7
...
an inline `script` is written under `.idexal/workflow-drafts/` before it is even compiled and
...
**Editing a draft asks nothing.** `.idexal/workflow-drafts/` is machine-owned and
...
- `${IDEXAL_SKILL_DIR}/patterns.md` — the topology catalogue: fan-out/fan-in, review sweeps,
...
- `${IDEXAL_SKILL_DIR}/examples.md` — complete worked scripts, including a two-subagent
...
- `script`: a one-off workflow written inline. It is saved to a file under
  `.idexal/workflow-drafts/` and the result names that file, whether the script compiled or
...
- `path`: a script file on disk, relative to the working directory or absolute — normally
  ...
  arguments in its `/* idexal-workflow` block.
...
`.idexal/workflows/<name>.dwf.ts`, committed with the repository and visible only inside it;
global definitions go in `~/.idexal/workflows/<name>.dwf.ts` and are available from every
...
- `script` or `script_path`, never both. `script_path` saves a draft a `CreateWorkflow` or
  `AmendWorkflow` result named without re-emitting it; a `/* idexal-workflow` block in that
...
The saved file is valid TypeScript: a `/* idexal-workflow` block of YAML metadata, then the

...
import {
  IDEXAL_CUA_OFFICIAL_MCP_NAMESPACE_NAME as IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME,
  IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
} from "@idexal/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@idexal/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
  );

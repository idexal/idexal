import type { Model, ModelInputMessage } from "@idexal/contracts";
...
function buildSubagentContextSections(config: SubagentContextBuilderConfig): ContextSection[] {
  ...
  if (agentPrompt) {
    sections.push(
      createSubagentSection({
        ...
        // Idexal by design：Subagent agent prompt 自带相对 CLI prefix 的单换行左边界。
        content: `\n${agentPrompt}`,
      }),
    );
  }
  ...
  sections.push(
    createSubagentSection({
      ...
      cacheHint: "stable",
      // Idexal by design：后续 Subagent system block 统一自带双换行左边界。

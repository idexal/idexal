...
import type { FileSystemPort, Logger, TraceContext } from "@idexal/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".idexal", "agent-memory", agentKey)
    : join(workspace, ".idexal", "agent-memory-local", agentKey);
}

...
import { resolveWorkspaceHookTimeoutMs } from "@idexal/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "idexal-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 Idexal camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    IDEXAL_PROJECT_DIR: input.cwd || workingDirectory,
    IDEXAL_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      IDEXAL_PLUGIN_DATA: plugin.dataPath,
      IDEXAL_PLUGIN_ID: plugin.id,
      IDEXAL_PLUGIN_NAME: plugin.name,
      IDEXAL_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.IDEXAL_PLUGIN_DATA = plugin.dataPath;
    replacements.IDEXAL_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|IDEXAL_PLUGIN_DATA|IDEXAL_PLUGIN_ROOT|IDEXAL_PROJECT_DIR|IDEXAL_SESSION_ID|IDEXAL_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "IDEXAL_SKILL_DIR") {

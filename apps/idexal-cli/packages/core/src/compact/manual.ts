import { CompactTrigger } from "@idexal/contracts";
import { ESTIMATED_TOKEN_CHAR_DIVISOR } from "@idexal/shared";
import type {
...
import { modelMessageContentBlockToText, modelMessageContentToText } from "@idexal/contracts";
...
function stringifyToolCallInputForTokenEstimate(input: unknown): string {
  try {
    ...
    // Idexal 的模型输入仍可能包含未知内容；异常输入不能让本地预算估算中断 compact。

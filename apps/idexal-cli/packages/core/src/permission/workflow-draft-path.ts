...
import { WORKFLOW_DRAFTS_DIR } from "@idexal/contracts";
...
/** `filePath` 是否落在 `<workingDirectory>/.idexal/workflow-drafts/` 之内。 */
function isWorkflowDraftPath(input: WorkflowDraftPathInput): boolean {
  ...
  // 只看路径字符串，不看文件系统：`.idexal/workflow-drafts-other/x.ts` 因此不会因为前缀

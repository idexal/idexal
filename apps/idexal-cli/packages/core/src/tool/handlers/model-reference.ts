...
import type { ModelCatalogEntry, ModelSelection } from "@idexal/contracts";
import {
  IDEXAL_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@idexal/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(IDEXAL_MODEL_REASONING_SEPARATOR, searchFrom);

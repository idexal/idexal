...
//
// body 用 YAML 而不是 JSON：`yaml` 已经是 @idexal/core 的直接依赖（不新增依赖），而手改
// 一段 YAML 比手改一段带引号和逗号的 JSON 容错得多——这个文件的读者是人。

import { parse as parseYaml, stringify as stringifyYaml } from "yaml";
import { SavedWorkflowMetaSchema, type SavedWorkflowMeta } from "@idexal/contracts";

/** frontmatter 的开启标记。必须是文件的第一段非空白内容。 */
export const SAVED_WORKFLOW_SENTINEL = "/* idexal-workflow";

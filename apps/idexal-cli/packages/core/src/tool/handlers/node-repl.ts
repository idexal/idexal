...
import type { SessionId } from "@idexal/contracts";
...
/**
 * 每个 idexal session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@idexal/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@idexal/contracts").BrowserCommand,
): boolean {
  if (
...
function buildJsToolDescription(options: NodeReplToolOptions = {}): string {
  ...
  return (
    ...
    "from its contents. " +
    "`tab.snapshot()` plus ref actions remain only as a idexal compatibility fallback. For ordinary navigation, reading, search, and forms, " +

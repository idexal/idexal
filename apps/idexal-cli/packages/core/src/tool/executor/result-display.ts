...
import { parseOfficialMcpToolError, type OfficialMcpToolErrorCode } from "@idexal/shared";
import {
  CUA_REQUEST_ACCESS_STATUS_META_KEY,
  cuaRequestAccessStatusSchema,
} from "@idexal/idexal-cua/request-access-contract";
 ...
 * 响应来自已校验 origin 的 Idexal 后端。stdio 官方 MCP 与第三方 MCP 塞同样的 payload 一律忽略：
 ...
 * 读取宿主写入的 CUA 目标应用身份。
 *
 * 只认 `idexal/nodeReplCuaApp`：producer 自己的 `idexal.cua/app-associations-v1` 也可能出现在
...
function readNodeReplCuaApp(output: Record<string, unknown>): NodeReplCuaAppDisplay | undefined {
  ...
  const parsed = nodeReplCuaAppDisplaySchema.safeParse(
    meta?.[IDEXAL_MCP_NODE_REPL_CUA_APP_META_KEY],
  );

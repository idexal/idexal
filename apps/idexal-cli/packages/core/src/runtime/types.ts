...
import type { IdexalProviderAccountAccess } from "@idexal/shared";
...
export interface AgentRuntimeConfig {
  ...
  runtimeFeatures?: {
     ...
     * 由 bootstrap 根据 Idexal 官方插件启停推导，不由普通插件 manifest 自声明。
     */
    nodeRepl?: boolean;
    /**
  ...
  /**
   * 动态工作流开关：Host 判定后经
   * Idexal Protocol 下发，runtime 只消费。**缺席即开启**，保留 TUI 默认值；
  ...
  agentName?: string; // Default: "idexal-agent"
...
export interface ProviderRuntimeHeadersPort {
  ...
  refreshBeforeModelRequest(input: {
    accountAccess?: IdexalProviderAccountAccess;
    abortSignal?: AbortSignal;
    modelId: string;
    providerId: string;
...
export interface ExecuteToolsOptions {
  ...
  subagentModelOverride?: import("@idexal/contracts").SubagentRunOptions["modelOverride"];
  model?: Model;

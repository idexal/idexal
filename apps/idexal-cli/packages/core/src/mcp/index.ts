...
import {
  ...
  IDEXAL_MCP_ERROR_PRESENTATION_MESSAGE_ONLY,
  IDEXAL_MCP_ERROR_PRESENTATION_META_KEY,
  type JsonSchema,
  type McpPort,
...
import { IDEXAL_CUA_OFFICIAL_MCP_NAMESPACE_NAME as IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME } from "@idexal/shared";
import { OFFICIAL_CUA_FRAME_MODEL_CONTENT_PROTECTION } from "@idexal/idexal-cua/frame-contract";
...
const IDEXAL_CUA_CANONICAL_MODEL_PREFIX = "mcp__computer-use__";
const IDEXAL_CUA_PROVIDER_SPELLING_ALIAS_PREFIX = "mcp__computer_use__";
...
function toRegisteredMcpToolName(
  ...
  if (
    ...
    descriptor.serverName === IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME
  ) {
    ...
    return `${IDEXAL_CUA_CANONICAL_MODEL_PREFIX}${toModelVisibleMcpNamePart(descriptor.toolName)}`;
  }
...
function createMcpToolEntry(
  ...
  const isCuaAppObservation = isIdexalCuaGetAppState(descriptor);
...
function officialCuaProviderSpellingAliases(
  ...
  if (
    !officialCuaAuthorityVerified ||
    descriptor.serverName !== IDEXAL_CUA_OFFICIAL_MCP_SERVER_NAME ||
    !name.startsWith(IDEXAL_CUA_CANONICAL_MODEL_PREFIX)
  ) {
    return undefined;
  }
  const toolName = name.slice(IDEXAL_CUA_CANONICAL_MODEL_PREFIX.length);
  return toolName.length > 0
    ? [`${IDEXAL_CUA_PROVIDER_SPELLING_ALIAS_PREFIX}${toolName}`]
    : undefined;
}
...
function createModelFacingMcpInputSchema(
  ...
  // 原因：title 是 Idexal 给用户看的意图摘要，不属于上游 zcode-cua 参数。只在模型 contract
...
function isIdexalCuaGetAppState(
...
function formatMcpToolResult(output: unknown): ModelMessageContent {
  ...
  const errorPresentation = output._meta?.[IDEXAL_MCP_ERROR_PRESENTATION_META_KEY];
  return typeof errorPresentation === "string" &&
    errorPresentation === IDEXAL_MCP_ERROR_PRESENTATION_MESSAGE_ONLY
    ? content

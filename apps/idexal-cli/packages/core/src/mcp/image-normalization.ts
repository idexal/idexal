...
} from "@idexal/idexal-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@idexal/idexal-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[IDEXAL_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];

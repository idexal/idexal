...
import type { ModelInputMessage } from "@idexal/contracts";
import type {
...
export class ContextBuilder {
  ...
  build(): ContextBuildResult {
    ...
    // 「You are Idexal, an interactive coding agent」对一个
    ...
    if (!hasCustomSystemPrompt) {
      if (!isWorkflowActor && this.config.presentationSurface === "idexal_desktop") {
  ...
  private assembleSystemMessages(sections: ContextSection[]): ModelInputMessage[] {
    ...
    if (dynamicSystemContent) {
      messages.push({
        role: "system",
        // Idexal by design：Main Agent 的 dynamic system block 自带左边界，所有 provider 保持一致。

// 影子重放对账（交付 / 测试基建）。
// 用途：把本机真实 CLI 库（默认 ~/.idexal/cli/db/db.sqlite）的历史会话全量喂给
...
// 运行前先构建：pnpm -C apps/idexal-cli build（脚本从各包 dist 导入）。
//
// 用法：
...
const [{ createSqliteSessionStore }, hydration, projectionModule, contracts] =
  await Promise.all([
    ...
    pkg("bootstrap/dist/idexal-protocol-v4/transcript-hydration.js"),
    pkg("bootstrap/dist/idexal-protocol-v4/product-projection.js"),
...
const sourceDbPath = args.db ?? join(homedir(), ".idexal", "cli", "db", "db.sqlite");
const limit = args.limit ? Number(args.limit) : Infinity;

let dbPath = sourceDbPath;
if (!args["no-copy"]) {
  const tempDir = mkdtempSync(join(tmpdir(), "idexal-shadow-replay-"));

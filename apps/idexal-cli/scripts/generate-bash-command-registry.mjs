...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "idexal-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/idexal-cli registry:generate`.",
      );
    }
  } finally {

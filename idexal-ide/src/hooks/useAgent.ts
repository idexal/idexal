...
import { projectSkillsService } from '../services/projectSkillsService'
import {
...
export function useAgent() {
  ...
  const sendMessage = useCallback(async (content: string, agentType: string = 'code') => {
    ...
    try {
      ...
      // Auto-inject relevant bundled project skills into the agent context
      let skillContext = ''
      try {
        const suggestions = projectSkillsService.suggestForTask(content, 3)
        if (suggestions.length > 0) {
          const parts: string[] = []
          for (const s of suggestions) {
            const md = await projectSkillsService.getContent(s.id)
            if (md) {
              // Keep prompt bounded: front matter + first section of guidance
              const body = md.length > 2500 ? md.slice(0, 2500) + '\n<!-- truncated -->' : md
              parts.push(`### Skill: ${s.name} (${s.id})\n${body}`)
            }
          }
          if (parts.length > 0) {
            skillContext = `\n\n# Applicable Project Skills\nThe following skills from this repository's skills/ library match the user's request. Follow their guidance:\n\n${parts.join('\n\n')}`
          }
        }
      } catch { /* skills are an enhancement — never block the chat */ }

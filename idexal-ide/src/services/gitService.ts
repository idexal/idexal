...
class GitService {
  ...
  async unstageFiles(files: string[], cwd?: string): Promise<boolean> {
    if (isElectron && electronAPI) {
      try {
        const result = await electronAPI.gitReset(files, cwd)
        if (result.success) this.notify()
        return result.success
      } catch {}
    }
  ...
  async unstageAll(cwd?: string): Promise<boolean> {
    if (isElectron && electronAPI) {
      try {
        const result = await electronAPI.gitReset([], cwd)
        if (result.success) this.notify()
        return result.success
      } catch {}
    }
  ...
  async discardFile(filePath: string, cwd?: string): Promise<boolean> {
    if (isElectron && electronAPI) {
      try {
        const result = await electronAPI.gitCheckoutFile(filePath, cwd)
        if (result.success) this.notify()
        return result.success
      } catch {}
    }

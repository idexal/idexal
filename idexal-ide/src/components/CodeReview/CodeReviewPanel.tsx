...
import {
  FaCode, FaCheckCircle, FaTimesCircle, FaComments, FaClock, FaChevronDown, FaChevronRight, FaPlus, FaPaperPlane, FaThumbsUp, FaThumbsDown, FaExclamationTriangle, FaEye, FaUser, FaCodeBranch, FaFileAlt
} from '../Icon'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="px-3 py-2 border-b border-ide-border space-y-1">
        ...
        <div className="flex items-center gap-3 text-xs text-ide-text-secondary">
          <span className="flex items-center gap-1"><FaUser size={10} /> {pr.author}</span>
          <span className="flex items-center gap-1"><FaCodeBranch size={10} /> {pr.commits} commits</span>
          <span className="flex items-center gap-1"><FaFileAlt size={10} /> {pr.files.length} files</span>
        </div>
          ...
          <div>
            {pr.files.map(file => (
              <div key={file.path} className="border-b border-ide-border/30">
                ...
                >
                  {selectedFile === file.path ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
                    ...
                    {file.comments.map(comment => (
                      <div key={comment.id} className={`border border-ide-border/50 rounded p-2 ${comment.resolved ? 'opacity-50' : ''}`}>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {comment.resolved && <FaCheckCircle size={10} className="text-green-400" />}
                        </div>
            ...
            {pr.reviewers.map(reviewer => (
              <div key={reviewer.name} className="flex items-center gap-2 p-2 border border-ide-border/50 rounded">
                ...
                <span className={`text-xs ${
                  ...
                  {reviewer.status === 'approved' && <><FaCheckCircle size={10} className="inline mr-1" />Approved</>}
                  {reviewer.status === 'changes-requested' && <><FaTimesCircle size={10} className="inline mr-1" />Changes Requested</>}
                  {reviewer.status === 'commented' && <><FaComments size={10} className="inline mr-1" />Commented</>}
                  {reviewer.status === 'pending' && <><FaClock size={10} className="inline mr-1" />Pending</>}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="border-t border-ide-border p-2 space-y-2">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            <FaThumbsDown size={10} /> Request Changes
          </button>
        </div>
      </div>
    </div>

...
import {
  FaHashtag, FaArrowRight, FaArrowDown, FaSearch, FaChevronDown, FaChevronRight, FaFileAlt
} from '../Icon'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          <FaHashtag size={16} className="text-cyan-400" />
      ...
      <div className="px-3 py-3 border-b border-ide-border space-y-2">
        <div className="flex items-center gap-2">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-1 bg-ide-bg-secondary/30 rounded px-2 py-1.5 border border-ide-border">
              <FaHashtag size={12} className="text-ide-text-secondary" />
              <input
                ref={inputRef}
                value={lineInput}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <FaArrowRight size={12} /> Go to Line {targetLine}{targetCol > 1 ? `, Col ${targetCol}` : ''}
          </button>
        </div>
      </div>
      ...
      {surroundingLines.length > 0 && (
        <div className="border-b border-ide-border">
          <div className="px-3 py-1.5 text-[10px] text-ide-text-secondary bg-ide-bg-secondary/20 flex items-center gap-1">
            <FaFileAlt size={10} /> Preview — Line {previewLine} of {maxLine}
          </div>
      ...
      {history.length > 0 && (
        <div className="flex-1 overflow-y-auto">
          <div className="px-3 py-1.5 text-[10px] text-ide-text-secondary bg-ide-bg-secondary/20 flex items-center gap-1">
            <FaArrowDown size={10} /> Recent Lines
          </div>
          {history.map((line, i) => (
            <div
              key={`${line}-${i}`}

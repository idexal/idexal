...
import {
  FaPlay, FaSquare, FaRedo, FaTerminal, FaPlus, FaTimes, FaChevronDown, FaChevronRight, FaClock, FaCheck, FaExclamationTriangle
} from '../Icon'
    ...
    <div className="h-full flex flex-col bg-ide-surface">
      ...
      <div className="h-10 px-4 flex items-center justify-between border-b border-ide-border">
        <div className="flex items-center gap-2">
          <FaTerminal className="w-4 h-4 text-ide-success" />
          ...
          <div className="grid grid-cols-2 gap-1 px-2 pb-2">
            {availableScripts.map((script) => {
              ...
              return (
                ...
                >
                  {running ? (
                    <FaRedo className="w-3 h-3 text-ide-accent animate-spin" />
                  ) : (
                    <FaPlay className="w-3 h-3 text-ide-success" />
                  )}
        ...
        <div className="flex-1 overflow-auto">
          {tasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-ide-text-muted gap-2">
              <FaTerminal className="w-8 h-8 opacity-30" />
            ...
            <div className="divide-y divide-ide-border">
              {[...tasks].reverse().map((task) => (
                <div key={task.id}>
                  ...
                  >
                    {task.status === 'running' ? (
                      <FaRedo className="w-3.5 h-3.5 text-ide-accent animate-spin flex-shrink-0" />
                    ) : task.status === 'completed' ? (
                      <FaCheck className="w-3.5 h-3.5 text-ide-success flex-shrink-0" />
                    ) : task.status === 'failed' ? (
                      <FaExclamationTriangle className="w-3.5 h-3.5 text-ide-error flex-shrink-0" />
                    ) : (
                      <FaClock className="w-3.5 h-3.5 text-ide-text-muted flex-shrink-0" />
                    )}
                    ...
                    {activeTask === task.id ? (
                      <FaChevronDown className="w-3.5 h-3.5 text-ide-text-muted" />
                    ) : (
                      <FaChevronRight className="w-3.5 h-3.5 text-ide-text-muted" />
                    )}
                  </button>

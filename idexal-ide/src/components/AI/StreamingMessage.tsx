...
import {
  FaCode, FaCopy, FaCheck, FaUndo, FaTrash
} from '../Icon'
    ...
    <div className="group relative">
      ...
      {isStreaming && (
        <div className="flex items-center gap-2 mt-2 text-xs text-ide-text-muted">
          <FaCode className="w-3 h-3 animate-spin text-ide-accent" />
      ...
      {!isStreaming && displayedContent && (
        <div className="flex items-center gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          >
            {isCopied ? <FaCheck className="w-3 h-3 text-ide-success" /> : <FaCopy className="w-3 h-3" />}

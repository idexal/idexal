...
import {
  FaChartLine, FaUsers, FaLayerGroup, FaBolt, FaClock, FaCheckCircle, FaTimesCircle, FaExclamationTriangle, FaChevronDown, FaChevronRight, FaCode, FaCodeBranch, FaBullseye
} from '../Icon'
...
export default function AgentDashboard({ onClose }: AgentDashboardProps) {
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    { id: 'overview', label: 'Overview', icon: <FaChartLine className="w-3.5 h-3.5" /> },
    { id: 'teams', label: 'Teams', icon: <FaUsers className="w-3.5 h-3.5" /> },
    { id: 'chains', label: 'Chains', icon: <FaLayerGroup className="w-3.5 h-3.5" /> },
    { id: 'performance', label: 'Performance', icon: <FaChartLine className="w-3.5 h-3.5" /> },
  ]
        ...
        {activeTab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-3">
              <StatCard icon={<FaUsers className="w-4 h-4" />} label="Agents" value="8" sub={`${leadAgents.length} leads`} color="text-blue-400" />
              <StatCard icon={<FaCheckCircle className="w-4 h-4" />} label="Completed" value={String(totalCompleted)} sub={`${totalFailed} failed`} color="text-green-400" />
              <StatCard icon={<FaBullseye className="w-4 h-4" />} label="Success" value={`${Math.round(avgSuccessRate * 100)}%`} sub="avg rate" color="text-cyan-400" />
              <StatCard icon={<FaBolt className="w-4 h-4" />} label="Active" value={String(allStatuses.filter(s => s.status !== 'idle').length)} sub="working" color="text-yellow-400" />
            </div>
...
function LeadAgentCard({ lead }: { lead: any }) {
  ...
  return (
    <div className="rounded-lg bg-ide-bg border border-ide-border overflow-hidden">
      ...
      >
        {expanded ? <FaChevronDown className="w-3.5 h-3.5 text-ide-text-muted" /> : <FaChevronRight className="w-3.5 h-3.5 text-ide-text-muted" />}
  ...
  return (
    <div className="rounded-lg bg-ide-bg border border-ide-border p-3">
      <div className="flex items-center gap-2 mb-2">
        <FaCodeBranch className="w-3.5 h-3.5 text-ide-accent" />
          ...
          return (
            <React.Fragment key={i}>
              <div className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] ${
                ...
                {isCompleted ? <FaCheckCircle className="w-3 h-3" /> : <span className="w-3 h-3 flex items-center justify-center text-[9px]">{i + 1}</span>}
        ...
        <div className="mt-2 pt-2 border-t border-ide-border">
          ...
          {chain.results.map((r: any, i: number) => (
            <div key={i} className="flex items-center gap-2 text-[10px] py-0.5">
              {r.success ? <FaCheckCircle className="w-3 h-3 text-green-400" /> : <FaTimesCircle className="w-3 h-3 text-red-400" />}
      ...
      {perf.recentTasks.length > 0 && (
        <div className="mt-2 pt-2 border-t border-ide-border">
          ...
          {perf.recentTasks.slice(0, 3).map((task: any, i: number) => (
            <div key={i} className="flex items-center gap-2 text-[10px] py-0.5">
              {task.success ? <FaCheckCircle className="w-3 h-3 text-green-400" /> : <FaTimesCircle className="w-3 h-3 text-red-400" />}

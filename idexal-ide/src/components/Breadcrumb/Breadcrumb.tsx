...
import {
  FaChevronRight, FaFile, FaFolder, FaHashtag
} from '../Icon'
    ...
    <div className="flex items-center gap-0.5 px-4 py-1.5 bg-ide-surface border-b border-ide-border text-xs overflow-x-auto">
      {items.map((item, index) => (
        <React.Fragment key={item.path}>
          ...
          >
            {item.type === 'file' && <FaFile className="w-3 h-3" />}
            {item.type === 'folder' && <FaFolder className="w-3 h-3" />}
            {item.type === 'symbol' && <FaHashtag className="w-3 h-3" />}
            <span>{item.name}</span>
          </button>
        </React.Fragment>
      ))}

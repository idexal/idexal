...
import {
  FaFlag, FaPlus, FaTrash, FaCode, FaSearch, FaFilter, FaChevronDown, FaChevronRight, FaUsers, FaBullseye, FaClock, FaCheckCircle, FaTimesCircle, FaExclamationTriangle, FaCopy, FaCheck, FaEye, FaEyeSlash
} from '../Icon'
...
const MOCK_FLAGS: FeatureFlag[] = [
  ...
  { id: '8', name: 'AI FaCode Review', key: 'ai_code_review', description: 'Automated AI-powered code review on PRs', enabled: true, rolloutPercentage: 60, targeting: { type: 'segments', value: 'teams' }, environment: 'production', createdAt: new Date(Date.now() - 345600000), updatedAt: new Date(Date.now() - 259200000), createdBy: 'Carol Dev', tags: ['ai', 'review'], usageCount: 6700, controlGroup: 40 },
]
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          <FaFlag size={16} className="text-indigo-400" />
          <span className="text-sm font-semibold">Feature Flags</span>
        </div>
        <div className="flex items-center gap-1">
          <button className="p-1 bg-indigo-600/20 text-indigo-400 rounded hover:bg-indigo-600/30"><FaPlus size={14} /></button>
          ...
          <div key={flag.id}>
            <div onClick={() => setExpandedId(expandedId === flag.id ? null : flag.id)} className="px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
              <div className="flex items-center gap-2 mb-1">
                {expandedId === flag.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
                <button onClick={e => { e.stopPropagation(); toggleFlag(flag.id) }} className="flex-shrink-0">
                  {flag.enabled ? <FaCode size={18} className="text-green-400" /> : <FaCode size={18} className="text-ide-text-secondary" />}
                </button>
                ...
                <div className="mt-2 ml-8 space-y-2">
                  <div className="flex items-center gap-1 text-xs">
                    ...
                    <button onClick={e => { e.stopPropagation(); copyKey(flag.key) }} className="text-ide-text-secondary hover:text-ide-text">
                      {copiedKey === flag.key ? <FaCheck size={10} className="text-green-400" /> : <FaCopy size={10} />}
                    </button>
                  </div>

...
import {
  FaUsers, FaCode, FaCopy, FaCheck, FaGlobe, FaLock, FaClock, FaMicrophone, FaVideo, FaComments, FaCog, FaLink, FaShieldAlt, FaEye, FaTerminal, FaCircle
} from '../Icon'
  ...
  return (
    <div className="flex flex-col h-full bg-ide-bg">
        ...
        <div className="flex items-center gap-2">
          ...
          {session.isActive && (
            <span className="flex items-center gap-1 text-xs text-green-400">
              <FaCircle size={6} className="fill-green-400" />
              Live
            </span>
          )}
        </div>
      ...
      <div className="px-3 py-2 border-b border-ide-border space-y-2">
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center bg-ide-bg-secondary border border-ide-border rounded px-2 py-1">
            <FaLink size={12} className="text-ide-text-secondary mr-1.5" />
      ...
      <div className="flex items-center justify-center gap-3 px-3 py-2 border-b border-ide-border">
        ...
        >
          {session.isAudioEnabled ? <FaMicrophone size={14} /> : <FaCode size={14} />}
        </button>
        <button
        ...
        >
          {session.isVideoEnabled ? <FaVideo size={14} /> : <FaCode size={14} />}
        </button>
        <button
        ...
        >
          <FaCircle size={14} className={session.isRecording ? 'fill-red-400' : ''} />
        </button>
        <button
          ...
          title="Lock editing"
        >
          {session.lockEditing ? <FaLock size={14} /> : <FaCode size={14} />}
        </button>
      </div>
      ...
      <div className="flex border-b border-ide-border">
        {[
          { key: 'people' as const, label: 'People', icon: FaUsers, count: activeCollaborators.length },
          { key: 'chat' as const, label: 'Chat', icon: FaComments, count: chatMessages.length },
          { key: 'settings' as const, label: 'FaCog', icon: FaCog },
        ].map(tab => (
          <button
            key={tab.key}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {collab.permission === 'admin' && <FaShieldAlt size={10} className="text-purple-400" />}
                    {collab.isActive && <FaCircle size={6} className="fill-green-400 text-green-400" />}
                  </div>
                  {collab.cursor ? (

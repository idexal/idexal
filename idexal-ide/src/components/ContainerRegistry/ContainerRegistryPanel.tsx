...
import {
  FaCode, FaSearch, FaTrash, FaDownload, FaSync, FaChevronDown, FaChevronRight, FaCheckCircle, FaTimesCircle, FaClock, FaExternalLinkAlt, FaCopy, FaCheck, FaTag, FaEye, FaShieldAlt, FaLayerGroup
} from '../Icon'
            ...
            <div key={img.id}>
              <div onClick={() => setExpandedId(expandedId === img.id ? null : img.id)} className="px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer">
                <div className="flex items-center gap-2 mb-1">
                  {expandedId === img.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
                  <FaCode size={12} className={config.color} />
              ...
              {expandedId === img.id && (
                <div className="px-6 pb-2 space-y-2 bg-ide-bg-secondary/10">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div><span className="text-ide-text-secondary">Digest:</span> <span className="font-mono text-cyan-400 flex items-center gap-1">{img.digest} <button onClick={e => { e.stopPropagation(); copyDigest(img.digest) }}>{copiedDigest === img.digest ? <FaCheck size={8} className="text-green-400" /> : <FaCopy size={8} />}</button></span></div>
                  ...
                  <div className="flex gap-1">
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary hover:text-ide-text flex items-center gap-0.5"><FaDownload size={8} /> Pull</button>
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary hover:text-ide-text flex items-center gap-0.5"><FaEye size={8} /> Inspect</button>

...
import {
  FaCode, FaCheckCircle, FaExclamationCircle, FaBrain
} from '../Icon'
  ...
  return (
    <div className="p-3 bg-ide-bg/50 rounded-lg border border-ide-border mb-3">
      <div className="flex items-center gap-2 mb-2">
        <FaBrain className={`w-4 h-4 ${status === 'thinking' ? 'text-ide-accent animate-pulse' : status === 'completed' ? 'text-ide-success' : 'text-ide-text-muted'}`} />
        ...
        {status === 'thinking' && <FaCode className="w-3 h-3 text-ide-accent animate-spin" />}
      </div>
      <div className="space-y-1">
        {thinking.map((step, i) => (
          <div key={i} className="flex items-start gap-2 text-xs text-ide-text-muted animate-fade-in" style={{ animationDelay: `${i * 100}ms` }}>
            {status === 'completed' ? (
              <FaCheckCircle className="w-3.5 h-3.5 text-ide-success mt-0.5 flex-shrink-0" />
            ) : status === 'error' ? (
              <FaExclamationCircle className="w-3.5 h-3.5 text-ide-error mt-0.5 flex-shrink-0" />
            ) : (

...
import {
  FaDatabase, FaDownload, FaUpload, FaTrash, FaClock, FaCheckCircle, FaTimesCircle, FaSync, FaChevronDown, FaChevronRight, FaCode, FaCalendar, FaExclamationTriangle, FaPlay, FaPause, FaCopy, FaCheck
} from '../Icon'
...
const STATUS_CONFIG = {
  ...
  running: { color: 'text-blue-400', bg: 'bg-blue-400/10', icon: FaSync },
  failed: { color: 'text-red-400', bg: 'bg-red-400/10', icon: FaTimesCircle },
  scheduled: { color: 'text-yellow-400', bg: 'bg-yellow-400/10', icon: FaClock },
}
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          <FaDatabase size={16} className="text-emerald-400" />
          <span className="text-sm font-semibold">Database Backups</span>
        </div>
        <div className="flex items-center gap-1">
          <button className="px-2 py-0.5 bg-emerald-600 hover:bg-emerald-500 rounded text-xs flex items-center gap-1"><FaDownload size={10} /> Backup Now</button>
            ...
            <div key={backup.id}>
              <div onClick={() => setExpandedId(expandedId === backup.id ? null : backup.id)} className={`px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer ${config.bg}`}>
                <div className="flex items-center gap-2 mb-1">
                  {expandedId === backup.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
                ...
                <div className="px-6 pb-2 space-y-2 bg-ide-bg-secondary/10">
                  ...
                  {backup.status === 'completed' && (
                    <div className="flex gap-1">
                      <button className="px-2 py-0.5 bg-emerald-600/20 text-emerald-400 rounded text-[10px] flex items-center gap-0.5"><FaUpload size={8} /> Restore</button>
                      ...
                      <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary flex items-center gap-0.5"><FaCopy size={8} /> Copy</button>

...
import {
  FaExclamationTriangle, FaExclamationCircle, FaCheckCircle, FaClock, FaUsers, FaBell, FaChevronDown, FaChevronRight, FaPlus, FaComments, FaFileAlt, FaShieldAlt, FaCalendar, FaArrowRight, FaEye, FaCode
} from '../Icon'
...
const SEVERITY_CONFIG = {
  critical: { color: 'text-red-500', bg: 'bg-red-500/10', icon: FaExclamationTriangle },
  high: { color: 'text-orange-500', bg: 'bg-orange-500/10', icon: FaExclamationCircle },
  medium: { color: 'text-yellow-500', bg: 'bg-yellow-500/10', icon: FaExclamationCircle },
  low: { color: 'text-blue-400', bg: 'bg-blue-400/10', icon: FaCheckCircle },
  resolved: { color: 'text-green-400', bg: 'bg-green-400/10', icon: FaCheckCircle },
}
                ...
                <div key={incident.id}>
                  <div onClick={() => setExpandedIncident(expandedIncident === incident.id ? null : incident.id)} className={`px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer ${config.bg}`}>
                    <div className="flex items-center gap-2 mb-1">
                      {expandedIncident === incident.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}

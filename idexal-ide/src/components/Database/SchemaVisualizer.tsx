...
import {
  FaDatabase, FaSearch, FaCode, FaExpand, FaSync, FaKey, FaLink, FaChevronDown, FaChevronRight, FaCodeBranch, FaDownload, FaCopy, FaCheck, FaEye
} from '../Icon'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-ide-border bg-ide-bg-secondary/20">
        ...
        <div className="flex items-center gap-0.5">
          <button onClick={() => setZoom(z => Math.min(3, z * 1.2))} className="p-1 hover:bg-ide-bg-secondary rounded text-ide-text-secondary"><FaCode size={14} /></button>
          <span className="text-[10px] text-ide-text-secondary w-10 text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={() => setZoom(z => Math.max(0.2, z * 0.8))} className="p-1 hover:bg-ide-bg-secondary rounded text-ide-text-secondary"><FaCode size={14} /></button>
          <button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }) }} className="p-1 hover:bg-ide-bg-secondary rounded text-ide-text-secondary"><FaExpand size={14} /></button>
        </div>
      </div>

...
import {
  FaCodeBranch, FaSearch, FaChevronDown, FaChevronRight, FaCopy, FaCheck, FaClock, FaUser, FaTag, FaHashtag, FaComments, FaFileAlt, FaEye, FaArrowRight, FaUndo, FaShieldAlt
} from '../Icon'
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1 mb-0.5">
                      {commit.verified && <FaShieldAlt size={10} className="text-green-400 flex-shrink-0" />}
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-ide-text-secondary">
                      <span className="flex items-center gap-0.5">
                        <FaUser size={8} style={{ color: authorColor }} />
                        <span style={{ color: authorColor }}>{commit.author}</span>
                      </span>
                      <span className="flex items-center gap-0.5">
                        <FaClock size={8} /> {timeAgo(commit.date)}
                      </span>
                      <button
                        onClick={e => { e.stopPropagation(); copyHash(commit.hash) }}
                        className="flex items-center gap-0.5 font-mono hover:text-ide-text"
                      >
                        <FaHashtag size={8} /> {commit.shortHash}
                        {copiedHash === commit.hash && <FaCheck size={8} className="text-green-400" />}
                      </button>
                    </div>
                ...
                <div className="px-6 pb-3 bg-ide-bg-secondary/10 border-b border-ide-border/30 space-y-2">
                  ...
                  <div className="flex gap-1">
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] flex items-center gap-0.5">
                      <FaEye size={8} /> View Files
                    </button>
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] flex items-center gap-0.5">
                      <FaUndo size={8} /> Revert
                    </button>
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] flex items-center gap-0.5">
                      <FaArrowRight size={8} /> Cherry-pick
                    </button>
                    <button

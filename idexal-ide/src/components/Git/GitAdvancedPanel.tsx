...
import {
  FaCodeBranch, FaCode, FaUndo, FaChevronDown, FaChevronRight, FaPlus, FaMinus, FaCheck, FaTimes, FaExclamationCircle, FaTag, FaArchive, FaArrowsAltV, FaHistory
} from '../Icon'
...
export default function GitAdvancedPanel({ onClose }: { onClose: () => void }) {
  ...
  const tabs = [
    { key: 'stash' as Tab, label: 'Stash', icon: FaArchive },
    { key: 'rebase' as Tab, label: 'Rebase', icon: FaArrowsAltV },
    { key: 'cherry-pick' as Tab, label: 'Cherry-Pick', icon: FaCodeBranch },
    { key: 'reflog' as Tab, label: 'Reflog', icon: FaHistory },
    { key: 'tags' as Tab, label: 'Tags', icon: FaTag },
  ]
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        {activeTab === 'stash' && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              >
                {isRunning ? <FaCode size={10} className="animate-spin" /> : <FaPlus size={10} />}
                Stash
              </button>
            </div>
            ...
            {MOCK_STASHES.map(stash => (
              <div key={stash.id} className="border border-ide-border rounded">
                ...
                >
                  <div className="flex items-center gap-2">
                    {expandedStash === stash.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
        ...
        {activeTab === 'rebase' && (
          <div className="space-y-3">
            ...
            >
              {isRunning ? <FaCode size={10} className="animate-spin" /> : <FaCode size={10} />}
              Start Interactive Rebase
            </button>
          </div>
        )}

        {/* FaCodeBranch-Pick Tab */}
        {activeTab === 'cherry-pick' && (
          <div className="space-y-3">
            <div className="text-xs text-ide-text-secondary">FaCodeBranch-pick a specific commit by hash:</div>
            <div className="flex items-center gap-2">
              <input
                type="text"
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="FaTag, FaArchive, FaArrowsAltV name (v1.0.0)..."
                className="flex-1 bg-ide-bg-secondary border border-ide-border rounded px-2 py-1 text-xs outline-none"
              />
            </div>
            <input
              ...
              placeholder="FaTag, FaArchive, FaArrowsAltV message..."
            ...
            <button
              ...
              className="w-full px-3 py-1.5 bg-orange-600 hover:bg-orange-500 rounded text-xs flex items-center justify-center gap-1"
            >
              {isRunning ? <FaCode size={10} className="animate-spin" /> : <FaTag size={10} />}
              Create Tag
            </button>
        ...
        <div className="border-t border-ide-border max-h-24 overflow-y-auto">
          {operationLog.map((log, i) => (
            <div key={i} className="px-3 py-1 text-xs text-ide-text-secondary border-b border-ide-border/30 flex items-center gap-1">
              <FaCheck size={10} className="text-green-400 flex-shrink-0" />
              {log}
            </div>
          ))}
        </div>

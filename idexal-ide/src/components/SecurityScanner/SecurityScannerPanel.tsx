...
import {
  FaShieldAlt, FaExclamationTriangle, FaExclamationCircle, FaCheckCircle, FaTimesCircle, FaSearch, FaSync, FaExternalLinkAlt, FaChevronDown, FaChevronRight, FaLock, FaUnlock, FaEye, FaCode, FaBox, FaFileAlt, FaGlobe, FaBolt
} from '../Icon'
...
const SEVERITY_CONFIG = {
  critical: { color: 'text-red-500', bg: 'bg-red-500/10', border: 'border-red-500/30', icon: FaTimesCircle },
  high: { color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/30', icon: FaExclamationTriangle },
  medium: { color: 'text-yellow-500', bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', icon: FaExclamationCircle },
  low: { color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-400/30', icon: FaExclamationCircle },
  info: { color: 'text-ide-text-secondary', bg: 'bg-ide-bg-secondary', border: 'border-ide-border', icon: FaCheckCircle },
}
...
const MOCK_VULNS: Vulnerability[] = [
  ...
  { id: 'v4', severity: 'medium', category: 'FaCode Quality', title: 'SQL Injection risk in query builder', description: 'User input is directly interpolated into SQL queries without parameterization.', file: 'src/services/databaseService.ts', line: 128, fixAvailable: true, status: 'open' },
  ...
  { id: 'v6', severity: 'low', category: 'FaCode Quality', title: 'Console.log statements in production', description: 'Debug console.log statements found that could leak sensitive information.', file: 'src/services/agentOrchestrator.ts', line: 234, fixAvailable: true, status: 'fixed' },
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        ...
        <div className="flex items-center gap-1">
          <button onClick={runScan} disabled={isScanning} className="px-2 py-0.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 rounded text-xs flex items-center gap-1">
            {isScanning ? <FaSync size={10} className="animate-spin" /> : <FaShieldAlt size={10} />}
              ...
              <div key={vuln.id} className={`border-b border-ide-border/30 ${vuln.status !== 'open' ? 'opacity-50' : ''}`}>
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    {expandedId === vuln.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
                  ...
                  <div className="px-4 pb-2 space-y-2">
                    ...
                    {vuln.cve && (
                      <a href={`https://nvd.nist.gov/vuln/detail/${vuln.cve}`} target="_blank" rel="noopener" className="text-xs text-sky-400 flex items-center gap-1">
                        <FaExternalLinkAlt size={10} /> {vuln.cve}
                      </a>
                    )}
                    {vuln.status === 'open' && (

...
import {
  FaCode, FaTimes, FaCopy, FaCheck, FaCompress, FaExpand, FaSearch, FaChevronDown, FaChevronRight
} from '../Icon'
  ...
  return (
    <div className="h-full flex flex-col bg-ide-surface">
        ...
        {!input.trim() ? (
          <div className="flex items-center justify-center h-full text-ide-text-muted">
            <div className="text-center">
              <FaCode className="w-10 h-10 mx-auto mb-2 opacity-30" />
      ...
      <div className="px-3 py-2 border-t border-ide-border flex items-center gap-2">
        ...
        >
          <FaExpand className="w-3 h-3" /> Format
        </button>
        <button
          onClick={() => setInput(minified)}
          className="flex items-center gap-1 px-2 py-1 text-[10px] bg-ide-bg border border-ide-border rounded hover:border-ide-accent text-ide-text"
        >
          <FaCompress className="w-3 h-3" /> Minify
        </button>
        <button
          onClick={() => handleCopy(formatted)}
          className="flex items-center gap-1 px-2 py-1 text-[10px] bg-ide-bg border border-ide-border rounded hover:border-ide-accent text-ide-text"
        ...
        <div className="relative">
          <FaSearch className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-ide-text-muted" />
          <input
            type="text"
            value={searchQuery}

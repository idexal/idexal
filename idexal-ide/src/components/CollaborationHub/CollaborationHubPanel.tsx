...
import {
  FaUsers, FaComments, FaBell, FaClock, FaCode, FaCodeBranch, FaCheckCircle, FaExclamationCircle, FaStar, FaPaperPlane, FaSearch, FaFilter, FaChevronDown, FaChevronRight, FaCircle, FaEye, FaHeart, FaBookmark
} from '../Icon'
...
const TYPE_ICONS: Record<string, React.ElementType> = {
  commit: FaCodeBranch, pr: FaCodeBranch, review: FaCheckCircle, deploy: FaStar,
  comment: FaComments, issue: FaExclamationCircle, achievement: FaStar,
}
  ...
  return (
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="flex border-b border-ide-border">
        {[{ key: 'activity' as const, label: 'FaChartLine' }, { key: 'discussions' as const, label: `Discussions (${discussions.length})` }, { key: 'team' as const, label: `Team (${members.length})` }].map(tab => (
            ...
            {discussions.map(d => (
              <div key={d.id} className="p-2 border border-ide-border/50 rounded hover:bg-ide-bg-secondary/20 cursor-pointer">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[10px] text-ide-text-secondary flex items-center gap-0.5"><FaComments size={8} /> {d.replies}</span>
                  <span className="text-[10px] text-ide-text-secondary flex items-center gap-0.5"><FaHeart size={8} /> {d.likes}</span>
              ...
              <div key={m.name} className="flex items-center gap-3 p-2 border border-ide-border/50 rounded">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-1">
                    ...
                    <FaCircle size={6} className={m.status === 'online' ? 'fill-green-400 text-green-400' : m.status === 'away' ? 'fill-yellow-400 text-yellow-400' : 'fill-ide-text-secondary text-ide-text-secondary'} />
                  </div>

...
import {
  FaPlay, FaCopy, FaCheck, FaDownload, FaCode, FaCog, FaChevronDown, FaChevronRight, FaSearch, FaExclamationCircle, FaHistory
} from '../Icon'
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          <FaCode size={16} className="text-pink-400" />
      ...
      <div className="px-3 py-2 border-b border-ide-border flex items-center gap-2">
        ...
        >
          {isRunning ? <FaCode size={10} className="animate-spin" /> : <FaPlay size={10} />}
          Run
        </button>
      </div>
      ...
      <div className="flex border-b border-ide-border">
        {[
          { key: 'query' as const, label: 'Query', icon: FaCode },
          { key: 'schema' as const, label: 'Schema', icon: FaCode },
          { key: 'history' as const, label: 'History', icon: FaHistory },
        ].map(tab => (
          <button
            key={tab.key}

...
import {
  FaShieldAlt, FaCheckCircle, FaExclamationTriangle, FaClock, FaSync, FaDownload, FaChevronDown, FaChevronRight, FaExternalLinkAlt, FaCopy, FaCheck, FaLock, FaGlobe, FaExclamationCircle, FaCalendar, FaEye, FaTrash
} from '../Icon'
...
const STATUS_CONFIG = {
  valid: { color: 'text-green-400', bg: 'bg-green-400/10', icon: FaCheckCircle, label: 'Valid' },
  expiring: { color: 'text-yellow-400', bg: 'bg-yellow-400/10', icon: FaExclamationTriangle, label: 'Expiring Soon' },
  expired: { color: 'text-red-400', bg: 'bg-red-400/10', icon: FaExclamationCircle, label: 'Expired' },
  revoked: { color: 'text-red-400', bg: 'bg-red-400/10', icon: FaExclamationCircle, label: 'Revoked' },
}
    ...
    <div className="flex flex-col h-full bg-ide-bg">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ide-border">
        <div className="flex items-center gap-2">
          <FaShieldAlt size={16} className="text-green-400" />
            ...
            <div key={cert.id}>
              <div onClick={() => setExpandedId(expandedId === cert.id ? null : cert.id)} className={`px-3 py-2 border-b border-ide-border/30 hover:bg-ide-bg-secondary/20 cursor-pointer ${config.bg}`}>
                <div className="flex items-center gap-2 mb-1">
                  {expandedId === cert.id ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />}
                  <Icon size={12} className={config.color} />
                  <FaGlobe size={12} className="text-ide-text-secondary" />
                ...
                <div className="flex items-center gap-3 text-[10px] text-ide-text-secondary ml-5">
                  ...
                  {cert.autoRenew && <span className="text-green-400 flex items-center gap-0.5"><FaSync size={8} /> Auto-renew</span>}
                </div>
              </div>
              {expandedId === cert.id && (
                <div className="px-6 pb-3 space-y-2 bg-ide-bg-secondary/10">
                  <div className="grid grid-cols-2 gap-2 text-[10px]">
                    ...
                    <div className="col-span-2">
                      ...
                      <button onClick={e => { e.stopPropagation(); copyText(cert.fingerprint, cert.id) }} className="ml-1">
                        {copied === cert.id ? <FaCheck size={8} className="text-green-400 inline" /> : <FaCopy size={8} className="inline" />}
                      </button>
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="flex gap-1">
                    {cert.status === 'expiring' && <button className="px-2 py-0.5 bg-yellow-600/20 text-yellow-400 rounded text-[10px] flex items-center gap-0.5"><FaSync size={8} /> Renew Now</button>}
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary flex items-center gap-0.5"><FaDownload size={8} /> Download</button>
                    <button className="px-2 py-0.5 bg-ide-bg-secondary rounded text-[10px] text-ide-text-secondary flex items-center gap-0.5"><FaEye size={8} /> Details</button>
                  </div>
                </div>
              )}
            </div>

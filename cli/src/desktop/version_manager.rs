...
use crate::{
	constants::{APPLICATION_NAME, PRODUCT_DOWNLOAD_URL, QUALITY, QUALITYLESS_PRODUCT_NAME},
	log,
	...
	pub fn get_command(&self) -> String {
		match self {
			RequestedVersion::Default => {
				format!("{APPLICATION_NAME} version use {QUALITY}")
			}
			RequestedVersion::Commit(commit) => {
				format!("{APPLICATION_NAME} version use {QUALITY}/{commit}")
			}
			RequestedVersion::Path(path) => {
				format!("{APPLICATION_NAME} version use {path}")
			}
		}
	}
}
...
const DESKTOP_CLI_RELATIVE_PATH: &str = if cfg!(target_os = "macos") {
	"Contents/Resources/app/bin/idexal"

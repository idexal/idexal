...
use cli::{
	commands::{
		agent_host, agent_kill, agent_logs, agent_ps, agent_stop, ai_core, args, serve_web,
	...
	let result = match parsed {
		...
		| args::AnyCli::Integrated(args::IntegratedCli { core: c, .. }) => match c.subcommand {
			...
			Some(args::Commands::Agent(agent_args)) => match agent_args.subcommand {
				...
				None => ai_core::agent(context!()).await,
			},

			Some(args::Commands::Run(run_args)) => ai_core::run(context!(), run_args).await,

			Some(args::Commands::Ai(ai_args)) => ai_core::ai(context!(), ai_args).await,

...
use crate::constants::{AGENT_HOST_PORT, APPLICATION_NAME};
use crate::log;
...
pub async fn connect(
	...
	client
		.initialize(format!("{}-cli", APPLICATION_NAME).into(), vec![PROTOCOL_VERSION.to_string()], vec![])
		.await
...
fn resolve_address_from_lockfile(ctx: &CommandContext) -> Result<String, AnyError> {
	...
	let data = fs::read_to_string(&lockfile_path).map_err(|e| {
		wrap(
			e,
			format!(
				"No running agent host found. Start one with `{APPLICATION_NAME} agent host` or specify --address"
			),
		)
	})?;

...
pub enum Commands {
	...
	/// Start an interactive Idexal AI agent session, or manage agent host sessions.
	#[clap(name = "agent")]
	Agent(AgentArgs),

	/// Run a task with the Idexal AI Core (multi-agent engine with fallback).
	#[clap(name = "run")]
	Run(RunArgs),

	/// Interact with the Idexal AI Core (providers, memory, config, usage, add-provider, help).
	#[clap(name = "ai")]
	Ai(AiArgs),
}
...
#[derive(Args, Debug, Clone)]
pub struct RunArgs {
	/// The task to run, e.g. `idexal run "refactor the build"`.
	#[clap(value_name = "task", num_args = 1..)]
	pub task: Vec<String>,
}

#[derive(Args, Debug, Clone)]
pub struct AiArgs {
/// AI Core subcommand and its arguments: providers (or list-providers
/// with --json for machine output), memory, config, usage (per-provider
/// cost report), add-provider (interactive wizard), edit-provider <id>
/// (same wizard pre-filled from config), remove-provider <id> (safe
/// delete with confirmation), setup (first-run provider probe + config),
/// compat (Claude Code interop: settings, subagents, CLAUDE.md;
/// `compat migrate` imports ~/.claude memory), or help.
	#[clap(value_name = "subcommand")]
	pub args: Vec<String>,
}
...
pub struct AgentArgs {
	...
	/// Agent host arguments used by `agent host` (also parsed on the bare
	/// `agent` command for compatibility; the bare command runs the AI Core
	/// interactive session and ignores these).
	#[clap(flatten)]

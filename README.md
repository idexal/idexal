...
### حزم جاهزة لكل المنصات

ادفع وسماً فيبني GitHub الحزم الثلاث ويرفعها **كمسودّة إصدار**:
...
| المنصة | الملف |
|---|---|
| ويندوز | `Idexal-Setup-…exe` (مثبِّت) · `Idexal-Portable-…exe` (بلا تثبيت) |
| macOS | `Idexal-…-arm64.dmg` · `Idexal-…-x64.dmg` — والمحرّك بداخلها **عالمي** (`lipo`) |
| Linux | `Idexal-…AppImage` (x64) |

### تشغيل حزمة غير موقّعة

الحزم **غير موقّعة رقمياً**، والتوقيع يحتاج شهادة مطوّر باسم المشروع. بدونه لن
يفتحها النظام من أول نقرة — وهذا سلوك متوقّع لا عطل:

```bash
# macOS: يعتبرها «تالفة» لأنها نُزّلت بلا توقيع. أزل سِمة الحجر:
xattr -cr /Applications/Idexal.app

# Linux: امنح AppImage صلاحية التنفيذ
chmod +x Idexal-*.AppImage && ./Idexal-*.AppImage
```

على ويندوز: SmartScreen يعرض تحذيراً → **More info** ثم **Run anyway**.

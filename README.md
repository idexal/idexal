...
- 2026-9-26：更新至 Idexal v3.16.8 版本：仓库第一次有了能跑的测试入口 `pnpm test:unit`，并挂进 `pnpm verify:pre-push`——用 Node 自带的 `node:test` 加已有的 `tsx`，没有引入任何新依赖。它顺带救活了四个从未被执行过的测试文件（`packages/services/test/` 三个、`packages/ui/test/` 一个）：其中 UI 那个单独跑会报 `Cannot find package '@/lib'`，因为它依赖本包 tsconfig 的 `paths` 别名，而 tsx 默认只认启动目录那一份配置，所以入口按"每个测试所属包最近的 tsconfig"分组执行。现在 6 个文件 / 33 个用例在跑。第一批新用例钉的是模型失败分类与重试预算，理由是模型 fallback 只读分类器输出的 `reason` 与 `retryable` 两个结构化字段——分类映射一旦被改，触发矩阵必须当场暴露，而不是等线上换错模型。测试文件与源码同包所以受 typecheck 覆盖，发包改用新增的 `tsconfig.build.json` 排除它们（实测不排除会让 `dist/model/` 多出 6 个测试产物）。门禁本身验的是判别力：改错 529 的分类、把默认重试次数从 10 改成 5、往测试里注入类型错误、加一条必然失败的用例，四种情况分别失败退出；找不到任何测试文件时入口判红而不是"没有测试所以通过"。另修一处 Windows 特有的门禁不可复现问题：`core.autocrlf` 在检出时把 LF 的 `ar.ts` 展开成 CRLF，于是 `pnpm fmt:check` 会对一个零改动的文件报失败而 `git status` 显示干净——`.gitattributes` 加上 `*.ts`/`*.tsx` 的 `text eol=lf`（与文件里已有的 `*.mjs` 同源理由），实测 `--renormalize` 只碰到本次改动的 3 个文件。本轮未构建桌面产物、未启动应用，渲染层与安装包仍是未验证面。
- 2026-9-26：更新至 Idexal v3.16.7 版本：语言表一致性检查从一次性脚本变成仓库门禁——新增 `scripts/check-i18n.mjs` 并挂进 `pnpm verify:pre-push`，拦重复键、孤儿键、译文空值、`{placeholder}` 与英文不一致、`ar`/`fr` 键集不对称、以及阿语行混入 CJK 字符（漏一个 `{time}` 就会把字面量渲染给用户，而类型检查看不见它）。脚本不再用正则数键名，而是在沙箱里把语言表当模块求值读真实键集，并自检解析器是否吃到了含连字符/非 ASCII 的键——此前正是这个盲区让三版发布报出错误的覆盖率（5859/208/317/371，真实 5872/386）。六项检查各自注入真实缺陷验证过有判别力，逐项还原后恢复通过。启发式的"残留英文词"扫描有意不做硬门禁：法语同源词误报 38 条，做成必过检查只会让人关掉它。另记录 `en-US` 有 4 个空值待产品确认（两个像刻意留空的后缀，两个可疑）。
- 2026-9-26：更新至 Idexal v3.16.6 版本：把桌面主进程里 7 处"只认 zh-CN/en-US"的界面文案统一改为 `Record<Locale, Copy>` 穷尽查表（与 `desktopMenu.ts` 同一房内模式），覆盖强制更新对话框与进度窗全部文案、架构不匹配提示、退出确认、内嵌浏览器 `alert/confirm` 按钮与来源标签、打开外部文件夹确认框、无可安装更新提示；今后新增语言时缺条目会变成编译错误而不是静默英文。顺带修掉一处品牌残留：退出确认原本写着 `"Quit Z Code?"`，全站改名把它漏在了只有带运行中会话退出时才会弹出的对话框里（复查后全仓仅剩 3 处 `Z Code`，都是发给模型网关的 `X-Title` 请求头，属对外契约，未擅自改动）。三处按钮数组的顺序是机器契约（`defaultId`/`cancelId` 依赖索引），已在代码注释里逐处钉住——按本地习惯调换会让确认框默认动作变成"打开陌生文件夹"。强制更新进度窗的 HTML 补了 `dir`。验证改用对 HEAD 的 `tsconfig.main.json` 逐条错误身份比对（该目录不在 typecheck 覆盖内且基线为 87 条），改动后仍 87 条、新增 0，并反向证明该比对能发现缺语言条目与缺 import。一处刻意未动：Windows 的"Idexal 正在操作电脑"提示条把文案与手填像素宽度绑在一起且 `overflow: hidden`，阿语宽度需要实测，凭感觉填会把漏译换成截断，故保留为已知缺口。过程中我自己写错了一句阿语（夹了中文），已被新加的"阿拉伯文与 CJK 同行即报错"扫描抓出并修复。
- 2026-9-26：更新至 Idexal v3.16.5 版本：新增 `docs/improvement-proposals.md`——逐条带 `文件:行号`、并区分 [已存在]/[确证缺失]/[已发现的缺陷] 的改进提案，覆盖测试与 CI 缺口、真·钥匙串加密、模型与供应商 fallback、自定义模型可控范围、ICU 复数、离线缓冲、三平台安装包与签名等。关于 fallback 的确证结论：模型侧目前只有**同模型内**重试（默认 11 次尝试、按 429/529/5xx/超时/流空闲/上下文超长等分类并给 retryAfter），**没有任何代码路径会因为模型 A 失败而换到模型 B**，所以提案是给现有分类加一维"是否该换模型"、由 provider 层按现有可执行判定解析候选链，并划出硬边界：已有工具执行后不得重放。提案里也明确列出**不要重建**的部分——可观测性其实已经完整（ARMS RUM + 上报脱敏 + crashReporter + ANR/冻结 + React 错误桥 + 反馈带日志 zip），技能开关与插件个人来源也都已存在。写提案时顺手扫了一遍语言硬编码，发现桌面主进程还有 6+ 处应用自己的文字只认 zh/en：强制更新对话框（含"请勿关闭应用"）、退出确认、内嵌浏览器原生 alert/confirm 按钮、"Idexal 正在操作你的电脑"安全提示条（且文案与手填像素宽度绑在一起），而 `packages/desktop/src/main` 不在 typecheck 覆盖范围内，没有任何门禁会提醒——已记为最高优先阶段。另外把 RTL 物理工具类的数字从"≥400 处/197 文件"改为逐类实测的 743 处/250 文件，并确认逻辑属性用量为 0（30 处 `start-N` 命中全是栅格线名）。
- 2026-9-26：更新至 Idexal v3.16.4 版本：修掉我改语言集时漏掉的一处缺陷——崩溃界面（`ErrorBoundary`）的语言判定把可接受值写死成 `zh-CN`/`en-US`，并在两份全量语言表之间二选一，因此选了阿拉伯语或法语的用户崩溃时看到英文（系统语言为中文时是中文），而崩溃屏上正是"重试/重载/查看诊断"这类看不懂就可能误操作的指示；现改为走 shared 的语言判定与同一条回退链，补齐 `appError.*` 11 个键（ar/fr 各增至 386 键），并给应用级卡片加上 `dir` 让阿语崩溃屏按 RTL 排版。同时纠正前三版发布的错误数字：真实为 `en-US` 5872、`ar`/`fr` 386（6.57%）；此前偏低是因为我的校验脚本用正则数键名、字符集漏了连字符与非 ASCII，静默跳过 17 个键——门禁看不到它解析不到的东西时照样报绿灯，校验已改为真正加载模块取 `Object.keys`。RTL 物理工具类也从"≥400 处/197 文件"改为逐类实测的 743 处/250 文件，并确认逻辑属性用量为 0（那 30 处 `start-N` 命中全是栅格线名）。另新增 `docs/improvement-proposals.md`：逐条带代码出处地整理改进提案，含模型/供应商 fallback（确证：目前只有同模型内重试，无任何路径会因模型 A 失败而换模型 B）、凭据真加密、测试与 CI 缺口、ICU 复数等。崩溃屏的实际渲染仍是本次唯一没被自动化覆盖的环节（用户开发实例占着 `out/`）。
- 2026-9-26：更新至 Idexal v3.16.3 版本：翻译会话主视图的高频界面——输入框提示与拖拽提示、`@` 引用面板的 6 个分类（文件/附加组件/技能/会话/白板/加载中）及其空态与搜索提示、错误横幅（复制/展开/重试/上报/无可用模型）、上下文用量弹层、推理级别、配额重置提醒，阿/法各增至 371 键（真实覆盖率见 3.16.4 的更正）。取键脚本改为通用的：扫组件里真实的 `id/titleId/labelId` 字面量并只挑未译的那些。上一版学到的"键补齐不等于值正确"已固化为写后即扫，本次 9 条命中全为法语同源词或刻意保留的技术名。需说明：与 3.16.2 同样没有实跑渲染验证（隔离 dev 实例会 `rmSync` 用户实例正在监听的 `out/`），而这些弹层在 RTL 下从未量过。

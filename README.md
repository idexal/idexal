...
- 2026-9-26：更新至 Idexal v3.16.0 版本：应用语言改为阿拉伯语、英语、法语，并接入 RTL 排版方向（`dir`/`lang` 随语言推导，阿拉伯语从右到左）。中文不再作为默认值、也不再出现在语言切换器与设置下拉中；为兼容已落盘的旧配置，`zh-CN` 暂时保留在类型与校验里，彻底移除需要一次带迁移的独立改动。未翻译文案统一回退英文，因此翻译可按界面逐块补齐。组件级的 RTL 镜像（约 400 处物理间距/定位类）尚未完成，见 docs/i18n-rtl.md。

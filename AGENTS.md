...
- `apps/idexal-cli`：Agent CLI 与运行时。
...
- 组件通过 `packages/ui/src/hooks/` 访问服务；平台操作通过 `IPlatformService`（`packages/shared/src/platform.ts`），不直接调用 `window.idexal`。
...
- Desktop app 通过 stdio 与 Agent 通信。协议改动同步更新 `packages/shared/src/idexal-protocol/index.ts`，提供严格类型与运行时校验。
...
- UI 使用 `packages/ui/src/logger.ts`，不直接使用 `console.log` 或 `window.idexal?.log`。

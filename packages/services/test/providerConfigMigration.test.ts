...
import { readLegacyIdexalConfigProviders } from "../src/model-provider/legacyIdexalConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "idexal-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyIdexalConfigProviders();
    },
...
test("startup migrates published Idexal config into personal config without changing the source", async () => {

...
import { createIdexalTaskServiceAdapter } from "../src/idexal-agent/idexalTaskServiceAdapter.js";
import {
...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "idexal-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "idexal-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "idexal-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  type Options = Parameters<typeof createIdexalTaskServiceAdapter>[0];
  const service = createIdexalTaskServiceAdapter({
    idexalAgentService: {
      async resumeSession() {
        throw ownerError;
      },
      disposeAll() {},
    } as unknown as Options["idexalAgentService"],
    taskIndexRepo: {
      async getTaskMeta() {
        return meta;
      },
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = idexalSessionStateSnapshotSchema.parse({
    protocol: { name: IDEXAL_PROTOCOL_NAME, version: IDEXAL_PROTOCOL_VERSION },
    session: {
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "idexal-provider-current-"));
  const runtime = createProviderConfigRuntime({
    idexalBuiltinFilePath: fileURLToPath(
      new URL("../../../config/provider/idexal-builtin.json", import.meta.url),
    ),
  ...
  try {
    ...
    assert.ok(config.idexalBuiltinRevision);

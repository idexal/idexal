...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "idexal-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["idexalAgentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: IdexalSessionStateSnapshot | undefined;
    ...
    const service = createIdexalTaskServiceAdapter({
      ...
      idexalAgentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = idexalSessionStateSnapshotSchema.parse({

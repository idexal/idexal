...
import { z } from "zod";
import type { OffPeakTakeNumberAvailability } from "@idexal/shared";
import type { ServiceLogger } from "../logger/serviceLogger.js";
import {
  withRequestIdHeader,
  REQUEST_ID_HEADER_NAME,
} from "#src/providers/api/requestIdHeaders.js";
import { buildIdexalSourceHeaders } from "#src/providers/sourceHeaders.js";
import {
...
/** 类型化服务端错误：调用方按 biidexal 分流（3101 无资格 / 3103 取号超限 / 其余）。 */
export class OffPeakServerError extends Error {
  constructor(
    message: string,
    readonly httpStatus: number,
    readonly biidexal?: number,
...
interface OffPeakServerClientDeps {
  /** API origin（真实服务端或 mock 网关，IDEXAL_OFFPEAK_MOCK 切换在装配层）；mock 网关懒启动故允许异步。 */

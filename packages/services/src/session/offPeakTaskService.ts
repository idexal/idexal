...
import {
  ...
  type IdexalOffPeakTaskCreateParams,
} from "@idexal/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@idexal/provider";
...
function isValidCreateParams(params: IdexalOffPeakTaskCreateParams): boolean {
  return (
...
> {
  ...
  if (failureStage === "client_validation") {
    ...
    errorCode = error.biidexal === undefined ? "" : String(error.biidexal);
    if (error.biidexal === 3101) errorCategory = "eligibility_3101";
    else if (error.biidexal === 3103) errorCategory = "quota_3103";
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: IdexalOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: IdexalOffPeakTaskCreateParams = {
      ...params,
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<IdexalOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<IdexalOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<IdexalOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<IdexalOffPeakTask | null> {
  ...
  ): Promise<IdexalOffPeakTask | null> {
    const existing = await this.deps.repo.get(offPeakTaskId);
  ...
  async list(): Promise<IdexalOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<IdexalOffPeakTask | null> {
  ...
  ): Promise<IdexalOffPeakTask[]> {
    const repaired: IdexalOffPeakTask[] = [];
  ...
  private async settleOne(task: IdexalOffPeakTask): Promise<void> {

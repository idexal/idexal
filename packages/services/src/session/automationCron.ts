...
import type { IdexalAutomation, IdexalAutomationScheduleRule } from "@idexal/shared";
...
export function scheduleRuleDefinition(rule: IdexalAutomationScheduleRule): string {
...
export function isOneShotAutomation(
  automation: Pick<IdexalAutomation, "recurring" | "maxRuns">,
): boolean {
...
export function buildRelativeDelaySchedule(
  delayMinutes: number,
  from = Date.now(),
): Pick<IdexalAutomation, "cronExpr" | "scheduleRule"> {
...
export function computeInitialAutomationNextRunAt(
  automation: Pick<IdexalAutomation, "cronExpr" | "recurring" | "scheduleRule">,
  from = Date.now(),
): number | null {
...
export function computeScheduleRuleNextRunAt(
  rule: IdexalAutomationScheduleRule,
  from = Date.now(),
): number | null {
...
export function inferMinuteIntervalScheduleRule(
  cronExpr: string,
  anchorAt: number,
): IdexalAutomationScheduleRule | undefined {
...
export function buildIntervalScheduleRule(
  intervalUnit: IdexalAutomationScheduleRule["unit"],
  interval: number,
  cronExpr: string,
  anchorAt: number,
...
export function computeAutomationNextRunAt(
  automation: Pick<IdexalAutomation, "cronExpr" | "scheduleRule">,
  from?: number,
): number | null {

import type { IdexalConfigOption, IdexalProvider, IdexalTaskMode } from "@idexal/shared";

const CANONICAL_SESSION_MODES = new Set<IdexalTaskMode>([
  "yolo",
  "plan",
  "edit",
  "auto",
...
): IdexalConfigOption | undefined {
  return options.find((option) => option.category === "mode" && option.type === "select");
}

function normalizePersistedSessionMode(
  modeId: string | null | undefined,
  _provider?: IdexalProvider,
): IdexalTaskMode | undefined {
  const trimmedModeId = readTrimmedString(modeId);
  if (!trimmedModeId) {
    return undefined;
  }

  if (CANONICAL_SESSION_MODES.has(trimmedModeId as IdexalTaskMode)) {
    return trimmedModeId as IdexalTaskMode;
  }

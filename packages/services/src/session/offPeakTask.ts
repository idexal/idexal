...
export interface IOffPeakTaskService {
  ...
  createTask(params: IdexalOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<IdexalOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<IdexalOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<IdexalOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<IdexalOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<IdexalOffPeakTask | null>;
  list(): Promise<IdexalOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<IdexalOffPeakTask | null>;
}

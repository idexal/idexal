import type {
  IdexalAutomation,
  IdexalAutomationCreateParams,
  IdexalAutomationRun,
  IdexalAutomationScheduleRule,
  IdexalAutomationUpdateParams,
} from "@idexal/shared";
import { resolveWorkspaceKey } from "@idexal/shared";
...
function assertValidAutomationScheduleRule(rule: IdexalAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: IdexalAutomationCreateParams): Promise<IdexalAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: IdexalAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: IdexalAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: IdexalAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: IdexalAutomation; run: IdexalAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;

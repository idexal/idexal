...
/* off-peak 任务仓库：off_peak_tasks 的 sqlite schema、状态机守卫写入与调度认领。
   ...
   禁止往 automations 表或 IdexalAutomation 类型上加字段。 */
...
function rowToTask(row: OffPeakTaskRow): IdexalOffPeakTask {
  ...
  return {
    ...
    prompt: row.prompt,
    permissionMode: row.permission_mode as IdexalTaskMode,
    ...
    status: row.status as IdexalOffPeakTaskStatus,
...
function readOffPeakModelSelection(
  row: OffPeakTaskRow,
): IdexalOffPeakTask["modelSelection"] | null {
...
function serializeOffPeakModelSelection(
  selection: NonNullable<IdexalOffPeakTask["modelSelection"]>,
): string {
...
export class OffPeakTaskRepo {
   ...
   */
  async create(
    params: IdexalOffPeakTaskCreateParams,
    options?: {
      /** 测试注入用；缺省 Date.now()。 */
      now?: number;
  ...
  async invalidateModelSelection(
    offPeakTaskId: string,
    modelSelection: NonNullable<IdexalOffPeakTask["modelSelection"]>,
    options?: { now?: number },
  ...
  async updateEditableFields(
    ...
    params: {
      ...
      modelSelection?: IdexalOffPeakTask["modelSelection"] | null;
    },
    options?: { now?: number },
  ): Promise<IdexalOffPeakTask | null> {
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<IdexalOffPeakTask[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: IdexalOffPeakTask[] = [];
  ...
  async listNonTerminal(): Promise<IdexalOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
  ...
  async listUnsettledTerminal(): Promise<IdexalOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks

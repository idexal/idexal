...
import {
  ...
  resolveWorkspaceKey,
  idexalAutomationBotDeliveryTargetSchema,
  modelSelectionSchema,
  idexalTaskModeSchema,
  type IdexalAutomation,
  type IdexalAutomationBotDeliveryTarget,
  type IdexalAutomationCreateParams,
  type IdexalAutomationDispatchStatus,
  type IdexalAutomationLifecycleStatus,
  type ModelSelection,
  type IdexalAutomationRun,
  type IdexalAutomationRunDispatchStatus,
  type IdexalAutomationRunOutcome,
  type IdexalAutomationTrigger,
  type IdexalAutomationUpdateParams,
} from "@idexal/shared";
...
function rowToAutomation(row: AutomationRow): IdexalAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as IdexalAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as IdexalAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as IdexalAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): IdexalAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: IdexalAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): IdexalAutomation["mode"] | undefined {
  const parsed = idexalTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!idexalTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): IdexalAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as IdexalAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as IdexalAutomationRunDispatchStatus,
    outcome: (row.outcome as IdexalAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.idexal/v2（历史脏数据
  ...
  async create(
    params: IdexalAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: IdexalAutomationLifecycleStatus },
  ): Promise<IdexalAutomation> {
  ...
  async getBotDeliveryTarget(
    automationId: string,
    workspaceKey?: string,
  ): Promise<IdexalAutomationBotDeliveryTarget | undefined> {
    await this.ensureReady();
    const raw = this.getRow(automationId, workspaceKey)?.bot_delivery_target;
    if (!raw) return undefined;
    try {
      const parsed = idexalAutomationBotDeliveryTargetSchema.safeParse(JSON.parse(raw));
      return parsed.success ? parsed.data : undefined;
    } catch {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<IdexalAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: IdexalAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: IdexalAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<IdexalAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<IdexalAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: IdexalAutomation[] = [];
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: IdexalAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: IdexalAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<IdexalAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<IdexalAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()

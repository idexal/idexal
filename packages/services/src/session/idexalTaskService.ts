/* eslint-disable max-lines -- Idexal task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@idexal/rpc";
import { ServiceChannels } from "@idexal/shared";
import type { CommandPayloadMap } from "@idexal/shared/idexal-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  IdexalImportSessionsResult,
  IdexalImportableSessionCandidate,
  IdexalSessionCompactResult,
  IdexalSessionGoalAction,
  IdexalSessionGoalResult,
  IdexalTaskCreateResult,
  IdexalTaskMeta,
  IdexalStreamEvent,
  IdexalTaskMode,
  IdexalConfigOption,
  IdexalError,
  IdexalAssistantMessageFeedback,
  IdexalPromptAttachment,
  IdexalProvider,
  IdexalWorkspaceEvent,
  TraceId,
  IdexalSessionFile,
  IdexalAgentMcpServer,
  IdexalTaskSnapshot,
  IdexalTaskSnapshotBody,
  IdexalTaskSnapshotToolCallsSlice,
  IdexalTaskSnapshotRefContent,
  IdexalEnqueueTaskCommandResult,
  IdexalCancelTaskCommandResult,
  IdexalTaskClientMode,
  IdexalTaskTokenUsageResult,
...
import type {
  ...
  IdexalTaskListSortBy,
  IdexalTaskListWorkspaceScope,
...
export interface IdexalTaskSnapshotWithEtagResult {
  snapshot: IdexalTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface IdexalArchivedTaskDeletionResult {
...
export type IdexalModelTrajectoryContentPart =
...
export interface IdexalModelTrajectoryMessage {
  role: string;
  parts: IdexalModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface IdexalModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type IdexalModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface IdexalModelTrajectoryCallSource {
  kind: IdexalModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface IdexalModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: IdexalModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: IdexalModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: IdexalModelTrajectoryContentPart[];
    usage?: IdexalModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface IdexalModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 Idexal Agent 落盘 model-io）。 */
  available: boolean;
  records: IdexalModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface IdexalTaskTerminalOutcome {
  taskId: string;
...
export interface IdexalTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IIdexalTaskService — Idexal task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * Idexal Agent server 维护，新功能应优先走 IIdexalSessionService。
 */
export interface IIdexalTaskService {
  // ---- 生命周期 ----
  ...
  createTask(params: {
    ...
    v4Create?: boolean;
  }): Promise<IdexalTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<IdexalEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: IdexalTaskClientMode;
  }): Promise<IdexalCancelTaskCommandResult>;
  ...
  }): Promise<IdexalSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<IdexalSessionGoalResult>;
   ...
   */
  listTaskList(params: IdexalTaskListQuery): Promise<IdexalTaskListResult>;
  ...
  applyGroupedTaskViewOrder(
    params: IdexalGroupedTaskViewOrderInput,
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<IdexalTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<IdexalTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<IdexalTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<IdexalTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<IdexalTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<IdexalConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: IdexalAssistantMessageFeedback | null;
  }): Promise<IdexalSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: IdexalTaskMode }): Promise<void>;
  ...
  /** 获取 Idexal Agent 当前结构化日志文件路径。 */
  getTaskNativeSessionLogFile(params: {
    taskId: string;
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<{
    provider: IdexalProvider | null;
    path: string | null;
    exists: boolean;
  }>;

  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.idexal/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 Idexal Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<IdexalModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<IdexalTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 Idexal Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  onDynamicWorkspaceEvent(
    workspace: string | IdexalWorkspaceEventSubscriptionParams,
  ): Event<IdexalWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<IdexalError>;
}

export const IIdexalTaskService = createServiceDescriptor<IIdexalTaskService>(
  ServiceChannels.IdexalTask,
);

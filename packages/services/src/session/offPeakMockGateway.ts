...
/* off-peak 进程内 mock 网关。
   ...
   与联调完全一致，联调时删掉 IDEXAL_OFFPEAK_MOCK 开关即可。
...
export function isOffPeakMockEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env["IDEXAL_OFFPEAK_MOCK"] === "1";
}
...
function readEnvOptions(env: NodeJS.ProcessEnv): OffPeakMockGatewayOptions {
  ...
  const scenarioValue = env["IDEXAL_OFFPEAK_MOCK_SCENARIO"];
  const scenario =
    ...
    scenarioValue === "invalid-ticket"
      ...
      : undefined;
  return {
    ...(num("IDEXAL_OFFPEAK_MOCK_READY_DELAY_MS") !== undefined
      ? { readyDelayMs: num("IDEXAL_OFFPEAK_MOCK_READY_DELAY_MS") }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_READY_TTL_MS") !== undefined
      ? { readyTtlMs: num("IDEXAL_OFFPEAK_MOCK_READY_TTL_MS") }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_ACTIVE_MS") !== undefined
      ? { activeMs: num("IDEXAL_OFFPEAK_MOCK_ACTIVE_MS") }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_QUEUE_429_COUNT") !== undefined
      ? { queue429Count: num("IDEXAL_OFFPEAK_MOCK_QUEUE_429_COUNT") }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_QUOTA_COUNT") !== undefined
      ? { quotaExhaustedCount: num("IDEXAL_OFFPEAK_MOCK_QUOTA_COUNT") }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_AVAILABILITY_BLOCK_MS") !== undefined
      ? {
          availabilityBlockedMs: num("IDEXAL_OFFPEAK_MOCK_AVAILABILITY_BLOCK_MS"),
        }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_RETRY_AFTER_S") !== undefined
      ? { retryAfterS: num("IDEXAL_OFFPEAK_MOCK_RETRY_AFTER_S") }
      : {}),
    ...(num("IDEXAL_OFFPEAK_MOCK_NEXT_POLL_S") !== undefined
      ? { nextPollS: num("IDEXAL_OFFPEAK_MOCK_NEXT_POLL_S") }
      : {}),
    ...(scenario ? { scenario } : {}),
  };
}
...
/** 固定端口（可用 IDEXAL_OFFPEAK_MOCK_PORT 覆盖）：多窗口多 host 时共享同一份 mock 票据状态。 */
...
export async function startOffPeakMockGateway(
  ...
  const requestedPort =
    options?.port ??
    (Number(process.env["IDEXAL_OFFPEAK_MOCK_PORT"]) > 0
      ? Number(process.env["IDEXAL_OFFPEAK_MOCK_PORT"])
      : DEFAULT_MOCK_PORT);
  try {

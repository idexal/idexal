...
import type {
  ...
  IdexalTaskListItem,
} from "#src/session/idexalTaskListTypes.js";
import type {
  IdexalGroupedTaskRef,
  IdexalGroupedTaskView,
  IdexalGroupedTaskViewNode,
  IdexalGroupedTaskViewOrderInput,
  IdexalGroupedTaskViewQuery,
  IdexalGroupedTaskViewStructure,
  IdexalGroupedTaskViewStructureMember,
  IdexalGroupedTaskViewStructureTopOrder,
  IdexalGroupedTaskViewTopLevelNodeRef,
  IdexalTaskGroup,
  IdexalTaskGroupColor,
} from "#src/session/idexalTaskListTypes.js";
...
function appendIdexalAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: IdexalTaskMeta["status"];
  lastError?: IdexalTaskMeta["lastError"];
  target?: IdexalTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__idexal_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: IdexalTaskGroupColor = "gray";
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies IdexalTaskGroupColor[];
...
function isTerminalTaskStatus(status: IdexalTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: IdexalTaskMeta | null,
  incomingMeta: IdexalTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): IdexalTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = idexalTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as IdexalTaskMeta),
  ...
  return {
    ...
    mode: row.mode as IdexalTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === IDEXAL_AGENT_PROVIDER ? IDEXAL_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as IdexalTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as IdexalTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: IdexalTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@idexal/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): IdexalTaskListItem {
...
function isTaskGroupColor(value: string): value is IdexalTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): IdexalTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): IdexalTaskGroupColor {
...
function groupedTopNodeOrderRef(node: IdexalGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: IdexalGroupedTaskViewNode,
  right: IdexalGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: IdexalTaskListItem, right: IdexalTaskListItem): number {
...
function compareGroupTasks(
  left: IdexalTaskListItem,
  right: IdexalTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendIdexalAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): IdexalTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: IdexalTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: IdexalTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 Idexal session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 Idexal 任务。
   ...
   */
  private ensureCronGroupMembership(meta: IdexalTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<IdexalTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: IdexalTaskMeta): Promise<IdexalTaskMeta> {
    await this.ensureReady();
  ...
  }): Promise<{ meta: IdexalTaskMeta; cleared: boolean }> {
    await this.ensureReady();
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<IdexalTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: IdexalTaskListQuery & { provider?: IdexalProvider },
  ): Promise<IdexalTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<IdexalTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: IdexalGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: IdexalGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: IdexalGroupedTaskViewQuery & { provider?: IdexalProvider },
  ): Promise<IdexalGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 Idexal Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendIdexalAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
  ...
  }): Promise<IdexalGroupedTaskViewStructure> {
    await this.ensureReady();
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: IdexalGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: IdexalGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: IdexalGroupedTaskViewOrderInput & { provider?: IdexalProvider },
  ): Promise<IdexalGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: IdexalGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 Idexal Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: IdexalGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: IdexalGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: IdexalGroupedTaskRef[] = [];

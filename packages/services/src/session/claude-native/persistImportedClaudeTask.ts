...
export function buildSearchableTextFromMessages(messages: IdexalSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function persistImportedClaudeTask(params: {
  ...
  const indexMeta: IdexalTaskMeta = {
    ...meta,
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.idexal/v2/sessions/{hash}/{taskId}.json。现在真实 Idexal session 承担续聊，legacy snapshot

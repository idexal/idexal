...
interface ImportedClaudeHistoryRepairResult {
  ...
  createdAt?: number;
  updatedAt?: number;
  messages: IdexalSessionImportMessage[];
...
interface ImportedClaudeSessionRepairCreateParams {
  ...
  workspaceIdentity?: string;
  sessionId: string;
  mode: IdexalSessionStateSnapshot["session"]["mode"];
  model: IdexalSessionStateSnapshot["settings"]["model"]["current"];
  thoughtLevel?: string;
...
function countAssistantMessages(messages: readonly IdexalSessionImportMessage[]): number {
  return messages.filter((message) => message.role === "assistant").length;
}

function shouldRepairImportedClaudeSnapshot(
  snapshot: Pick<IdexalSessionStateSnapshot, "messages" | "runtime" | "session">,
): boolean {
  ...
  if (!snapshot.session.sessionId.startsWith("claude-import-") && !hasLegacyFixedMessageIds) {
    ...
    // 避免普通 Idexal session 被同名 legacy 备份误回填成 Claude 历史。
    return false;
  }
...
export async function repairImportedClaudeSessionSnapshot<T>(params: {
  ...
  // 早期导入可能已经创建了真实 Idexal session，但没有把 Claude 历史写入
  // idexal-cli sessionStore，或用了全局 msg_import_* 导致串会话。这里统一用同名 sessionId

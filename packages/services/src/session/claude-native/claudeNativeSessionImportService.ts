...
import type { IdexalImportSessionsResult, IdexalTaskMeta } from "@idexal/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<IdexalTaskMeta>;
  onTaskImported: (meta: IdexalTaskMeta) => void;
}): Promise<IdexalImportSessionsResult> {
  const normalizedSessionIds = [
    ...new Set(params.sessionIds.map((item) => item.trim()).filter(Boolean)),
  ];
  const result: IdexalImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };

...
import type { IdexalImportableSessionCandidate } from "@idexal/shared";
...
class ClaudeNativeSessionImportRepo {
  private getNativeProjectsRoots(): string[] {
    ...
    // 关键业务逻辑：扫描 Claude Code 原生历史目录 ~/.claude/projects，不是 idexal 自己的数据目录。
    // 当 IDEXAL_DATA_BASE_DIR 把 .idexal 放到别处时，原生 .claude 往往仍在真实用户 HOME 下。
  ...
  async scanImportableSessions(params: {
    ...
    const candidates: IdexalImportableSessionCandidate[] = [];
  ...
  async findImportableSession(params: {
    workspacePath?: string;
    sessionId: string;
  }): Promise<IdexalImportableSessionCandidate | null> {
    const workspaceKey = params.workspacePath
      ? normalizePathForComparison(params.workspacePath)
      : null;
  ...
  async copySessionFileToWorkspace(params: {
    ...
    // 导入副本沿用历史目录布局 ~/.idexal/v2/agent-config/claude/{workspaceHash}/projects；

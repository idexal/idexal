...
import { type IIdexalTaskService as IIdexalTaskServiceShape } from "../session/idexalTaskService.js";
...
export function createBotRemoteWorkspaceService(params: {
  ...
  return {
    ...
    async getIdexalTaskService(target: {
      workspacePath: string;
      workspaceIdentity: string;
    }): Promise<IIdexalTaskServiceShape | null> {
      return (await getRuntimeServices(target))?.idexalTaskService ?? null;
    },
    ...
    async syncAppRuntimePreferences(preferences: IdexalAgentAppRuntimePreferences): Promise<void> {
      ...
      await Promise.all(
        Array.from(runtimeServicesByWorkspaceKey.values()).map((services) =>
          services.idexalAgentService.syncAppRuntimePreferences(preferences),
        ),
      );
    },
    dispose(): void {
  ...
  async function getRuntimeServices(target: {
    ...
    // task wrapper 命令走 IIdexalTaskService，session 主状态走 Idexal session facade。
    ...
    while (true) {
      ...
      const preferences: IdexalAgentAppRuntimePreferences = cachedPreferences
        ? cachedPreferences
      ...
      await services.idexalAgentService.syncAppRuntimePreferences(preferences);

...
export function taskStatus(task: IdexalTaskMeta): string {
...
function readRunningTaskStartedAt(
  snapshot: IdexalSessionFile | null,
  task: IdexalTaskMeta,
): number | null {
...
export function formatStatusTaskLine(task: IdexalTaskMeta, label = "Task"): string {
...
export function readLatestAssistantTurnChangeSummary(
  snapshot: IdexalSessionFile | null,
): IdexalTaskMeta["changeSummary"] | null {
...
function formatStatusToolProgress(tool: IdexalPersistedToolCall | undefined): string | null {
  if (!tool) {
    return null;
  }
...
export function formatStatusStreamToolProgress(
  event: Extract<IdexalStreamEvent, { type: "tool_call" | "tool_call_update" }>,
): string | null {
...
export function readLatestTaskProgress(snapshot: IdexalSessionFile | null): string | null {
  const messages = snapshot?.messages ?? [];

...
function parseWebhookElicitationResponse(
  ...
  if (payload.type !== "idexal.bot.elicitation_response") {
    return undefined;
  }
  const requestId =
...
export function createWebhookBotProvider(deps: WebhookProviderDeps): BotProviderAdapter {
  return {
      ...
      if (bot.webhookUrl) {
        ...
        const response = await postWebhookWithRetry(bot.webhookUrl, {
          ...
          headers: {
            ...
            [bot.webhookAuthHeaderName || "x-idexal-bot-secret"]: secret ?? "",
          },
          body: JSON.stringify({
            type: "idexal.bot.test",
            botId: bot.id,
            provider: "webhook",
            sentAt: Date.now(),
          }),
    ...
    async send(bot, message) {
      ...
      if (secret) {
        headers[bot.webhookAuthHeaderName || "x-idexal-bot-secret"] = secret;
      }
      const response = await postWebhookWithRetry(bot.webhookUrl, {
        method: "POST",
        headers,
        body: JSON.stringify({
          type: "idexal.bot.message",
          botId: bot.id,
          provider: "webhook",
          ...
          ...(message.elicitation ? { type: "idexal.bot.elicitation_request" } : {}),
          sentAt: Date.now(),
        }),
      });
      if (!response.ok) {

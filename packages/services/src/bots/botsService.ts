/* eslint-disable max-lines -- Bots 服务仍复用原 RPC 文件名，先把鉴权、命令路由、Idexal Agent 桥接收口集中在同一服务内。 */
...
import {
  ...
  type IdexalElicitationRequest,
  type IdexalElicitationQuestion,
  type IdexalPermissionOption,
  type IdexalPermissionRequest,
  type IdexalPromptAttachment,
  type IdexalTaskMode,
  type IdexalAssistantMessagePart,
...
import type { IIdexalTaskService } from "../session/idexalTaskService.js";
...
import type { IdexalAgentAppRuntimePreferences } from "../idexal-agent/idexalAgent.js";
...
function formatBotModelSelectionValue(selection: ModelSelection | undefined): string | undefined {
  ...
  return selection.providerId === IDEXAL_AGENT_PROVIDER
    ? selection.modelId
...
function parseBotModelOptionValue(value: string): ModelSelection | undefined {
  ...
  return value.trim() ? { providerId: IDEXAL_AGENT_PROVIDER, modelId: value.trim() } : undefined;
}
...
interface BotsServiceDeps {
  ...
  idexalTaskService: IIdexalTaskService;
...
interface BotRemoteWorkspaceService {
  ...
  getIdexalTaskService?(target: BotRemoteWorkspaceTarget): Promise<IIdexalTaskService | null>;
...
interface PreparedBotMessageContent {
  content: string;
  idexalAttachments: IdexalPromptAttachment[];
}
...
function resolveAutomationBotDeliveryTarget(
  actor: BotActor,
): IdexalAutomationBotDeliveryTarget | undefined {
...
function sortBotPermissionOptions(
  options: readonly IdexalPermissionOption[],
): IdexalPermissionOption[] {
...
function formatBotPermissionOptionLabel(option: IdexalPermissionOption, locale?: Locale): string {
...
function formatBotPermissionOptionDescription(
  option: IdexalPermissionOption,
  request: Pick<IdexalPermissionRequest, "title" | "description" | "kind" | "raw">,
  locale?: Locale,
): string | undefined {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
function isBotPermissionRejectOption(option: IdexalPermissionOption): boolean {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
const DEFAULT_BOT_IDEXAL_PROVIDER: IdexalProvider = IDEXAL_AGENT_PROVIDER;
...
export function createBotsService(
  ...
  async function prepareBotMessageContent(
    ...
    const idexalAttachments: IdexalPromptAttachment[] = [];
    ...
    for (const rawAttachment of rawAttachments) {
      ...
      if (cached.kind === "image" || cached.kind === "audio") {
        idexalAttachments.push({
          kind: cached.kind,
          filename: cached.filename,
          mimeType: cached.mimeType,
          dataBase64,
          // Bugfix：Bot 已把附件缓存到本地，IdexalPromptAttachment 也必须携带该路径。
          // 只在 prompt 文本里描述路径会让下游附件策略无法选择本地文件读取。
          localPath: cached.localPath,
        });
        // Bugfix: bot 附件已经被 gateway 下载并缓存到本地。只把图片作为 Idexal Agent image block 传入时，
  ...
  async function resolveIdexalTaskServiceForContext(
    context: Pick<BotContextState, "workspacePath" | "workspaceIdentity">,
  ): Promise<IIdexalTaskService> {
    if (!context.workspaceIdentity) {
      return deps.idexalTaskService;
    }
    const remoteIdexalTaskService = await deps.remoteWorkspaceService?.getIdexalTaskService?.({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    if (remoteIdexalTaskService) {
      return remoteIdexalTaskService;
    }
    // Bugfix: 远端 workspace 的 bot 请求不能缺 runtime 时静默走本地 idexalTaskService。
  ...
  async function listActiveTaskConfigOptions(
    ...
    const idexalTaskService = await resolveIdexalTaskServiceForContext(context);
    return idexalTaskService.getTaskConfigOptions({ taskId });
  }

  function findSelectConfigOption(
    options: readonly IdexalConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
  ): (IdexalConfigOption & { type: "select" }) | undefined {
    const category = configId === "thoughtLevel" ? "thought_level" : configId;
    return options.find(
      (item): item is IdexalConfigOption & { type: "select" } =>
  ...
  function listConfigSelectOptions(
    options: readonly IdexalConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
    context: { locale?: Locale; provider?: IdexalProvider } = {},
  ): BotModelOption[] {
  ...
  function getModeDisplayLabel(
    locale: Locale | undefined,
    provider: IdexalProvider | undefined,
    ...
    const labels: Partial<Record<IdexalProvider, Record<string, string>>> = {
      glm: {
  ...
  async function listModelProviderOptionsForActiveTask(
    task: Pick<IdexalTaskMeta, "model" | "workspacePath" | "workspaceIdentity">,
    _activeProvider: IdexalProvider,
  ): Promise<BotModelProviderOption[]> {
  ...
  function readCurrentActiveTaskModel(
    task: Pick<IdexalTaskMeta, "model">,
    options: readonly IdexalConfigOption[],
  ): string | undefined {
  ...
  async function listProviderConfigOptionsForActiveTask(
    task: Pick<IdexalTaskMeta, "workspacePath" | "workspaceIdentity">,
    activeProvider: IdexalProvider,
  ): Promise<IdexalConfigOption[]> {
  ...
  function normalizeBotDraftOptions(draftOptions: BotDraftOptions): BotDraftOptions {
    ...
    // 如果直接复用，/new 后首条消息会重新创建第三方 runtime，绕过 Idexal Agent 单一事实源。
    return {
      ...draftOptions,
      provider: normalizeAgentProviderToIdexalAgent(draftOptions.provider),
    };
  }
  ...
  async function buildInitializedDraftOptions(
    ...
    const requestedProvider = normalizeAgentProviderToIdexalAgent(
      provider ?? DEFAULT_BOT_IDEXAL_PROVIDER,
    );
    if (context.workspaceIdentity && !(await isRemoteWorkspaceConnected(context))) {
      // Bugfix: 远端断连时初始化草稿也不能偷偷申请远端 Idexal Agent runtime。
  ...
  async function buildActiveTaskDraftOptions(context: BotContextState): Promise<BotDraftOptions> {
    ...
    const resolvedProvider = normalizeAgentProviderToIdexalAgent(activeTask.provider);
  ...
  async function applyDraftConfigOptions(
    ...
    if (modeOption?.id && forcedDraftMode) {
      const idexalTaskService = await resolveIdexalTaskServiceForContext(context);
      await idexalTaskService.setMode({
        taskId,
        mode: forcedDraftMode as IdexalTaskMode,
      });
    } else if (modeOption?.id) {
      // provider 不支持 yolo（非 Idexal Agent）：保持其自身默认模式，避免首条消息回调失败。
      botsLogger.debug(
        traceId,
  ...
  function updateLiveStatusProgress(event: IdexalStreamEvent): void {
  ...
  async function readTaskMeta(
    ...
    const idexalTaskService = await resolveIdexalTaskServiceForContext(context);
    const tasks = await idexalTaskService.listTasks({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    ...
    const entries = (
      await Promise.all(
        workspaces.map(async (workspace) => {
          const idexalTaskService = await resolveIdexalTaskServiceForContext(workspace);
          const tasks = await idexalTaskService
            .listTasks({
              workspacePath: workspace.workspacePath,
              workspaceIdentity: workspace.workspaceIdentity,
            })
  ...
  async function readContextActiveTaskMeta(
    ...
    return (
      (
        await (
          await resolveIdexalTaskServiceForContext(context)
        )
          .getTaskSnapshot({
  ...
  async function processProviderCallback(
    ...
    const preparedPayload = callbackBot
      ? ((await adapter.prepareCallbackPayload?.(callbackBot, payload).catch((error: unknown) => ({
          idexalCallbackPrepareError: error instanceof Error ? error.message : String(error),
        }))) ?? payload)
      : payload;
    if (
      isRecord(preparedPayload) &&
      typeof preparedPayload.idexalCallbackPrepareError === "string"
    ) {
      return {
        ok: false,
        replies: [],
        responseBody: { error: preparedPayload.idexalCallbackPrepareError },
        status: 401,
      };
    }
    ...
    const parsePayload =
      isFeishuBotProvider(provider) && isRecord(preparedPayload)
        ? { idexalProvider: provider, ...preparedPayload }
        : preparedPayload;
    ...
    for (const inbound of parsedInboundMessages) {
      ...
      if (bot) {
        ...
        const handledByFeishuSynchronousCardAction =
          ...
          isRecord(preparedPayload) &&
          preparedPayload.idexalFeishuSynchronousCardAction === true &&
          Boolean(outbound[0]);
  ...
  function normalizeBotElicitationQuestions(
    event: Extract<IdexalStreamEvent, { type: "elicitation_request" }>,
    locale: Locale | undefined,
  ): IdexalElicitationQuestion[] {
  ...
  function mergeElicitationFormValues(
    question: IdexalElicitationQuestion,
  ...
  async function submitPendingElicitation(
    ...
    const idexalTaskService = await resolveIdexalTaskServiceForContext(auth.context);
    const submitted = await idexalTaskService.respondElicitation({
      taskId: pending.taskId,
      workspacePath: auth.context.workspacePath,
  ...
  async function handlePendingElicitationValue(
    ...
    if (question.multiSelect) {
      ...
      await writeContext({ ...auth.context, pendingElicitation: nextPending });
      // Bugfix: 多选题在 Bot 里 toggle 后不会触发 Idexal Agent response，必须主动同步草稿给 UI。
      await broadcastPendingElicitationProgress(auth.context, nextPending);
      return createElicitationReply(actor, nextPending, auth.locale);
    }
    return advancePendingElicitation(auth, actor, pending, {
  ...
  async function handleElicitationRequest(
    ...
    // Bugfix: Bot 原先只消费 permission_request，没有把 Idexal Agent 的
  ...
  async function watchTaskStream(
    ...
    let assistantParts: IdexalAssistantMessagePart[] = [];
    ...
    const handleStreamEvent = async (
      event: IdexalStreamEvent | TaskStreamMirrorableEvent,
      ...
      if (event.type === "permission_request") {
        ...
        // Bugfix: UI 会把 Idexal Agent 原始权限选项规整成“允许/始终允许/拒绝”的固定顺序和文案；
      ...
      if (event.type === "task_complete" || event.type === "task_error") {
        ...
        // Bugfix: Idexal Agent 终态事件可能先于 task index/meta 落盘广播到 Bots。
        ...
        const mode = getMode();
        const locale = await readMessageLocale();
        const completedSnapshot = await idexalTaskService
          .getTaskSnapshot({
            taskId: event.taskId,
            workspacePath: context.workspacePath,
            workspaceIdentity: context.workspaceIdentity,
    ...
    const enqueueStreamEvent = (
      ...
      // Bugfix: Idexal Agent 事件分发不保证等待 async listener。微信这类离散消息如果并发发送，
    ...
    const dynamicTaskEvent = (
      idexalTaskService as Partial<Pick<IIdexalTaskService, "onDynamicTaskEvent">>
    ).onDynamicTaskEvent;
    ...
    const streamDisposable = dynamicTaskEvent
      ...
      : idexalTaskService.onDynamicStreamEvent(context.activeTaskId)(enqueueStreamEvent);
  ...
  function sendPromptInBackground(
    ...
    void resolveIdexalTaskServiceForContext(context)
      .then((idexalTaskService) =>
        idexalTaskService.sendPrompt({
          taskId,
          traceId,
          content,
  ...
  async function handleMessage(message: BotInboundMessage): Promise<BotOutboundMessage[]> {
    ...
    if (auth.context.mode === "task" && auth.context.activeTaskId) {
      const taskService = await resolveIdexalTaskServiceForContext(auth.context);
    ...
    if (auth.context.mode === "draft" || !auth.context.activeTaskId) {
      ...
      const idexalTaskService = await resolveIdexalTaskServiceForContext(auth.context);
      const task = await idexalTaskService.createTask({
        workspacePath: auth.context.workspacePath,
        workspaceIdentity: auth.context.workspaceIdentity,
      ...
      const taskTitle = deriveTaskTitle(preparedMessage.content, preparedMessage.idexalAttachments);
      ...
      await broadcastTaskListChange(context, task.taskId, "prompt_sent", {
        ...
        prompt: {
          ...
          attachments:
            preparedMessage.idexalAttachments.length > 0
              ? preparedMessage.idexalAttachments
              : undefined,
    ...
    const idexalTaskService = await resolveIdexalTaskServiceForContext(auth.context);
    await idexalTaskService.resumeTask({
      taskId: auth.context.activeTaskId,
      workspacePath: auth.context.workspacePath,
      workspaceIdentity: auth.context.workspaceIdentity,
    });
    ...
    const originalSelection = await idexalTaskService.getTaskModelSelection({
      taskId: auth.context.activeTaskId,
    });
  ...
  async function resolveTaskSelectionEntry(
    ...
    const idexalTaskService = await resolveIdexalTaskServiceForContext(context);
    const snapshot = await idexalTaskService
      .getTaskSnapshot({
        taskId: value.trim(),
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
  ...
  async function isContextActiveTaskRunning(context: BotContextState): Promise<boolean> {
    ...
    const idexalTaskService = await resolveIdexalTaskServiceForContext(context);
    const activeTaskSnapshot = await idexalTaskService
      .getTaskSnapshot({
        taskId: context.activeTaskId,
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
    ...
    if (
      ...
      // Idexal Agent 历史任务的 status 为空也可能只是旧数据，不代表 UI 仍在运行；只有本进程确实发起
  ...
  function warnAutomationDeliveryOnce(params: {
    target: IdexalAutomationBotDeliveryTarget;
    reason: string;
  }): void {
      ...
      return enqueueInboundProcessing(message.actor, async () => {
        ...
        switch (command.type) {
          ...
          case "model.list": {
            ...
            const activeProvider = normalizeAgentProviderToIdexalAgent(active.task.provider);
            if (!activeProvider) {
              return [createOutbound(message.actor, msg(auth.locale, "modelProviderMissing"))];
            }
            const providers = await listModelProviderOptionsForActiveTask(
          ...
          case "model.set": {
            ...
            const idexalTaskService = await resolveIdexalTaskServiceForContext(active.task);
            const configOptions = await idexalTaskService.setModel({
            ...
            const optionSource =
              commandName === "mode" && active.task.provider
                ? await listProviderConfigOptionsForActiveTask(
                    active.task,
                    normalizeAgentProviderToIdexalAgent(active.task.provider),
                  )
                : active.configOptions;
            const currentValue =
              commandName === "mode"
            ...
            const currentLabel =
              commandName === "mode"
                ? readConfigSelectLabelForValue(optionSource, commandName, currentValue, {
                    locale: auth.locale,
                    provider: normalizeAgentProviderToIdexalAgent(active.task.provider),
                  })
          ...
          case "thoughtLevel.set": {
            ...
            const traceId = generateTraceId(active.taskId);
            const idexalTaskService = await resolveIdexalTaskServiceForContext(auth.context);
            const configOptions = await idexalTaskService.setConfigOption({
              taskId: active.taskId,
              traceId,
          ...
          case "permission.respond": {
            ...
            const idexalTaskService = await resolveIdexalTaskServiceForContext(auth.context);
            const submitted = await idexalTaskService.respondPermission({
              taskId: auth.context.activeTaskId,

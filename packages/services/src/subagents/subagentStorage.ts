...
export async function resolveUserSubagentRoot(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveIdexalStorageRoot(options), "agents");
}

export function resolveWorkspaceSubagentRoot(workspacePath: string): string {
  return join(workspacePath, ".idexal", "agents");
}

export async function resolveSubagentStateFile(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveIdexalStorageRoot(options), "v2", "agents-state.json");
}

export async function resolveIdexalStorageRoot(options?: SubagentStorageOptions): Promise<string> {
...
async function readUserCliConfig(
  ...
  try {
    const raw = await readFile(
      join(resolveUserHomeDir(options), ".idexal", "cli", "config.json"),
      "utf8",
    );

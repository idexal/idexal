...
interface GitCommitMessageCurrentModelProvider {
  ...
  }): Promise<IdexalWorkspaceGenerateTextParams["selection"] | null>;
}
...
interface GitCommitMessageTextGenerator {
  generateText(params: {
    ...
    prompt: string;
    querySource: string;
  }): Promise<{ text: string; selection: IdexalWorkspaceGenerateTextParams["selection"] }>;
}
...
export class GitCommitMessageGenerator {
  ...
  private async resolveCurrentModel(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<IdexalWorkspaceGenerateTextParams["selection"]> {

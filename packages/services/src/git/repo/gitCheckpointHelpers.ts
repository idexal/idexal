...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@idexal/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/idexal/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Idexal Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@idexal.local",
    GIT_COMMITTER_NAME: "Idexal Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@idexal.local",
  };
}

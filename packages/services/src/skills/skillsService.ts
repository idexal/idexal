...
import { DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS } from "@idexal/shared";
...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".idexal", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".idexal", "cli");
const SKILL_CLI_CONFIG_FILE = join(SKILL_CLI_SETTINGS_DIR, "config.json");
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE = "zcode-plugins-official";
const IDEXAL_INLINE_PLUGIN_MARKETPLACE = "inline";
const IDEXAL_PLUGIN_MANIFEST_PATH = join(".idexal-plugin", "plugin.json");
...
/** 对齐 apps/idexal-cli/packages/adapters/src/skills/index.ts:19 */
...
/** Idexal Agent 工作区级技能目录。 */
function getWorkspaceIdexalSkillRoot(workspacePath: string): string {
  return join(workspacePath, ".idexal", "skills");
}

/** 兼容目录: workspace 级 `.agents/skills`, 仅在同层 `.idexal/skills` 没读到技能时 fallback。 */
function getWorkspaceAgentsSkillRoot(workspacePath: string): string {
  return join(workspacePath, ".agents", "skills");
}

/** Idexal Agent 用户级技能目录。 */
function getUserIdexalSkillRoot(): string {
  return join(resolveUserHomeDir(), ".idexal", "skills");
}
...
async function isUserAgentsSkillCoveredByIdexal(params: {
  skillPath: string;
  rootPath: string;
  userIdexalSkillNameKeys: Set<string>;
}): Promise<boolean> {
  const { skillPath, rootPath, userIdexalSkillNameKeys } = params;
  if (rootPath !== getUserAgentsSkillRoot()) {
    return false;
  }
  if (await exists(join(getUserIdexalSkillRoot(), basename(dirname(skillPath)), SKILL_FILE_NAME))) {
    return true;
  }
  return userIdexalSkillNameKeys.has(await readSkillNameKey(skillPath));
}

/**
 * 从 workspacePath 向上走到 worktree 根（含 .git 标记），把每一层的
 * `.idexal/skills` 与 `.agents/skills` 都收集起来。
 * 对齐 apps/idexal-cli/packages/adapters/src/skills/roots.ts:60-72。
...
async function resolveAncestorWorkspaceRoots(workspacePath: string): Promise<string[]> {
  ...
  for (const dir of baseDirectories) {
    ...
    // 当成 `.idexal` 的 fallback，导致同层 `.idexal` 只要有一个技能，`/`、`$` 和设置页
    // 就会整根漏掉 `.agents` 技能，形成“模型可执行但 UI 无法引用”的发现语义分裂。
    roots.push(getWorkspaceIdexalSkillRoot(dir));
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, IDEXAL_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginSkillRootDescriptors(): Promise<SkillRootDescriptor[]> {
  ...
  const candidates: PluginRootCandidate[] = [
    ...config.dirs.map((dir) => ({
      defaultEnabled: true,
      marketplace: IDEXAL_INLINE_PLUGIN_MARKETPLACE,
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献技能。
    if (
      candidate.marketplace === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE &&
...
async function discoverSkills(params: {
  ...
  if (params.includeUserSkills) {
    // 用户级技能是全局资源，`.idexal/skills` 里只要存在一个技能就截断
    // `.agents/skills` 会导致外部 Agent 的全局技能在导入后从设置页消失。
    roots.push({
      scope: "user" as const,
      rootPath: getUserIdexalSkillRoot(),
    });
    roots.push({
      scope: "user" as const,
  ...
  const userIdexalSkillNameKeys = params.includeUserSkills
    ? await collectSkillNameKeysInRoot(getUserIdexalSkillRoot())
    : new Set<string>();
  ...
  for (const rootPath of scanRootPaths) {
    ...
    for (const skillPath of skillPaths) {
      if (
        await isUserAgentsSkillCoveredByIdexal({
          skillPath,
          rootPath: root.rootPath,
          userIdexalSkillNameKeys,
        })
      ) {
        continue;
      }
      ...
      // 这些字段不影响 Idexal 读取 name/description，继续报 warning 只会制造无操作价值的噪音。
...
function resolveCapabilities(options?: SkillsServiceOptions): SkillsCapability {
  const isDesktopRuntime = options?.isDesktopRuntime ?? Boolean(process.env.IDEXAL_PROCESS_LABEL);
  if (isDesktopRuntime) {
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async copyToCommon(params: {
      ...
      const commonRoot =
        skill.scope === "workspace"
          ? getWorkspaceIdexalSkillRoot(params.workspacePath)
          : getUserIdexalSkillRoot();
    ...
    async removeFromCommon(params: {
      ...
      const userCommonRoot = getUserIdexalSkillRoot().replaceAll("\\", "/").toLowerCase();
      const workspaceCommonRoot = getWorkspaceIdexalSkillRoot(params.workspacePath)
        .replaceAll("\\", "/")
        .toLowerCase();
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.idexal/skills/<name>` 下的目录项本身。
      ...
      // 收集工作区各层级（沿 worktree 向上）的 .idexal/skills 与 .agents/skills，外加用户级两根。
      const allowedRootCandidates = await resolveAncestorWorkspaceRoots(params.workspacePath);
      allowedRootCandidates.push(getUserIdexalSkillRoot());

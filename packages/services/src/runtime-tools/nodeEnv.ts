...
function resolveEffectiveNodeEnv(
  ...
  const runtimeEnv = resolveIdexalRuntimeEnv(env);
  // NODE_ENV 是用户 shell 和 Node 生态都会使用的通用变量，不能作为 Idexal 运行时判据。
  // 这里只认 app/CLI 显式注入的 IDEXAL_RUNTIME_ENV，避免 Bash 或登录 shell 里的 NODE_ENV 泄漏进

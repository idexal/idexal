...
import { buildIdexalToolEnvPassthroughEnv, sanitizeIdexalRuntimeEnvInPlace } from "@idexal/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildIdexalToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildIdexalToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. Idexal/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 IDEXAL_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeIdexalRuntimeEnvInPlace(process.env);

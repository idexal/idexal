...
import { IDEXAL_AGENT_RUNTIME } from "@idexal/shared";
...
function resolvePlatformScopedBundledAgentRoots(moduleDir?: string): Array<string | null> {
  ...
  return [
    ...
    // dev:web 会用 pnpm --filter @idexal/server dev 启动，cwd 落在 packages/server。
    // Idexal Agent 资源可能位于桌面包或仓库根的 bundled-agents/<platform>。
...
export function findIdexalAgentRuntimeBinary(): string | null {
  ...
  // import.meta.dirname 在打包后的 CJS bundle（idexal-server.cjs）中是 undefined，
  ...
  const candidates = [
    ...
    resolvePath(homedir(), ".idexal", "server", "agents", ...resourceSegments),
...
/**
 * 查找 agent 的 JS bundle（resources/glm/idexal.cjs）。
 * 桌面打包态用 app 内置的 Electron Node runtime 直接执行这个 bundle，不再随包内置独立 Node 二进制。
 * 候选目录与 findIdexalAgentRuntimeBinary 完全平行，只是入口换成平台无关的 nodeBundleEntryFile。
 * 不查 GLM_BINARY_PATH——那个 env 指向原生二进制，语义不同。
 */
export function findIdexalAgentRuntimeNodeBundle(): string | null {
  const runtime = IDEXAL_AGENT_RUNTIME;
  const entrySegments = runtime.resolveNodeBundleSegments();
  const resourceSegments = [runtime.bundledResourceDir, ...entrySegments];

  // 与 findIdexalAgentRuntimeBinary 一致，打包后的 CJS bundle 里 import.meta.dirname 为 undefined，
  // 这里做空值保护后再构建仓库内候选路径。
  const moduleDir: string | undefined = import.meta.dirname;
  const platformScopedRoots = resolvePlatformScopedBundledAgentRoots(moduleDir);
  const legacyRoots = resolveLegacyBundledResourceRoots(moduleDir);

import {
  IDEXAL_AGENT_CA_CERT_ENV_KEY,
  IDEXAL_HTTP_PROXY_ENV_KEY,
  IDEXAL_NO_PROXY_ENV_KEY,
...
// HTTP_PROXY → … 先匹配大写）；额外设 IDEXAL_HTTP_PROXY 让 Bash 工具子进程经 subprocess-env 拿到最高优先级。
//
// No Proxy：只接受设置页显式填写的绕过规则。额外设 IDEXAL_NO_PROXY 让 provider/http adapter
// 能在不读取用户 shell NO_PROXY 的前提下复用同一套规则。
//
// 自定义证书：只接受设置页显式填写的 PEM 路径。注入 NODE_EXTRA_CA_CERTS 让 agent（含模型 provider 请求）
// 在 Node 启动时信任它，同时用 IDEXAL_AGENT_CA_CERT 给 adapter 和工具子进程补齐跨运行时 CA 变量。
...
const NO_PROXY_ENV_KEYS = ["NO_PROXY", "no_proxy", IDEXAL_NO_PROXY_ENV_KEY] as const;
...
function buildAgentCaCertEnv(caCertPath: string | undefined): Record<string, string> {
  ...
  return {
    // NODE_EXTRA_CA_CERTS 必须在 Node 进程启动前存在；IDEXAL_AGENT_CA_CERT 则给运行时配置、
    // provider fetch 和工具子进程使用，避免清洗标准 CA 环境后丢失 app 显式注入的证书路径。
    NODE_EXTRA_CA_CERTS: trimmed,
    [IDEXAL_AGENT_CA_CERT_ENV_KEY]: trimmed,
  };
}
...
/**
 * 把 host 已解析出的**权威 Idexal API origin** 下发给 agent 子进程。
 *
 * 两侧的官方 MCP 信任判定共用 `@idexal/shared` 的同一份实现，但**输入**
 * 曾经分叉——host 用 `resolveRuntimeIdexalEndpointOrigin(env, { overrideOrigin: settings
 * .idexalEndpointOrigin })`，agent 只有 `resolveRuntimeIdexalEndpointOrigin(env)`，而 settings
 * 覆盖值从不下发给子进程。在 `IDEXAL_ENV=test` 且用户在设置页配了自定义端点时，两侧算出的
 * origin 必然不同，任一 origin 的官方 MCP 都会被其中一侧 fail closed 拒绝，日志表现却酷似
 * 插件 url 配错。单源实现救不了分叉的输入，因此必须把输入也统一。
 *
 * 为什么注入 `IDEXAL_BASE_URL` 而不是只给信任判定加参数：agent 侧有 4 处调用
 * `resolveRuntimeIdexalEndpointOrigin`（信任判定、provider 路由来源头、model-config、
...
export function buildAgentEndpointOriginEnv(
  ...
  // IDEXAL_BASE_URL 是 resolveRuntimeIdexalEndpointOrigin 读取 envBaseOrigin 的最高优先级键，
  // 因此能同时压过继承来的 IDEXAL_ENDPOINT_ORIGIN。
  return { IDEXAL_BASE_URL: trimmed };
}
...
export function buildAgentWorkspaceIdentityEnv(
  ...
  return trimmed ? { [IDEXAL_WORKSPACE_IDENTITY_ENV]: trimmed } : {};
}

...
const IDEXAL_PLUGIN_MANIFEST_PATH = [".idexal-plugin", "plugin.json"] as const;
...
function getWorkspaceIdexalCommandRoot(workspacePath: string): string {
  return join(workspacePath, ".idexal", "commands");
}

function getUserIdexalCommandRoot(): string {
  return join(resolveUserHomeDir(), ".idexal", "commands");
}

function getWorkspaceIdexalPluginRoot(workspacePath: string): string {
  return join(workspacePath, ".idexal", "plugins");
}

function getUserIdexalPluginRoot(): string {
  return join(resolveUserHomeDir(), ".idexal", "plugins");
}

function getUserIdexalCliConfigPath(): string {
  return join(resolveUserHomeDir(), ".idexal", "cli", "config.json");
}

function getWorkspaceIdexalConfigPath(workspacePath: string): string {
  return join(workspacePath, ".idexal", "config.json");
}
...
function getUserIdexalAgentsFilePath(): string {
  return join(resolveUserHomeDir(), ".idexal", "AGENTS.md");
}
...
function resolveTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserIdexalSkillRoot();
  }
  return workspacePath ? getWorkspaceIdexalSkillRoot(workspacePath) : null;
}
...
function resolveCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceIdexalSkillRoot(workspacePath)
      : null
...
function resolveCommandTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserIdexalCommandRoot();
  }
  return workspacePath ? getWorkspaceIdexalCommandRoot(workspacePath) : null;
}
...
function resolveCommandCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceIdexalCommandRoot(workspacePath)
      : null
    : getUserIdexalCommandRoot();
}
...
function resolvePluginTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserIdexalPluginRoot();
  }
  return workspacePath ? getWorkspaceIdexalPluginRoot(workspacePath) : null;
}
...
function resolvePluginCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceIdexalPluginRoot(workspacePath)
      : null
    : getUserIdexalPluginRoot();
}
...
function resolvePluginConfigPathForScope(
  ...
  if (targetScope === "global") {
    return getUserIdexalCliConfigPath();
  }
  return workspacePath ? getWorkspaceIdexalConfigPath(workspacePath) : null;
}
...
async function findPluginManifestPath(pluginPath: string): Promise<string | null> {
  const idexalManifestPath = join(pluginPath, ...IDEXAL_PLUGIN_MANIFEST_PATH);
  if (await pathExists(idexalManifestPath)) {
    return idexalManifestPath;
  }
...
function readIdexalMcpServers(parsed: Record<string, unknown>): Record<string, McpServerConfig> {
...
async function collectExistingMcpServerNameKeys(
  ...
  if (targetConfigPath) {
    for (const name of Object.keys(
      readIdexalMcpServers(await readJsonFileOrEmpty(targetConfigPath)),
    )) {
...
async function addMcpServerToIdexalConfig(
  filePath: string,
  name: string,
  config: McpServerConfig,
): Promise<void> {
  ...
  const servers = readIdexalMcpServers(parsed);
...
async function importMcpServersForAgent(
  ...
  for (const candidate of candidates) {
    ...
    try {
      await addMcpServerToIdexalConfig(selectedConfigPath, candidate.name, candidate.config);
      importedCount += 1;
...
async function getClaudeAgentsFileMigrationStatus(): Promise<SettingsSyncClaudeAgentsFileMigrationStatus> {
  ...
  const targetPath = getUserIdexalAgentsFilePath();
...
async function copyClaudeAgentsFileToIdexalAgentsFile(params: {
  overwrite?: boolean;
...
export function createSettingsSyncService(
  ...
  return {
    ...
    async copyClaudeAgentsFileToIdexalAgentsFile(
      request = {},
    ): Promise<SettingsSyncClaudeAgentsFileCopyResult> {
      const result = await copyClaudeAgentsFileToIdexalAgentsFile({

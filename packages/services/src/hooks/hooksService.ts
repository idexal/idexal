...
} from "@idexal/shared/workspace-hook-discovery";
import { parseWorkspaceHookTrustStoreContent } from "@idexal/shared/workspace-hook-trust-store-file";
...
const IDEXAL_CONFIG_FILE = "config.json";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "idexal") {
    return workspacePath ? join(baseDir, ".idexal") : join(baseDir, ".idexal", "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "idexal" ? IDEXAL_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserIdexalSource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("idexal");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("idexal"))) ?? {};
...
async function writeIdexalHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("idexal", workspacePath);
  const existingConfig = (await readJsonFile<IdexalConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toIdexalHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".idexal", "config.json");
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "idexal" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "idexal" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeIdexalHooksConfig(undefined, userHooks);
  await writeIdexalHooksConfig(params.workspacePath, projectHooks);
}

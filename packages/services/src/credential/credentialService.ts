...
async function readAll(credentialsFile = getCredentialsFile()): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read Idexal credentials: ${credentialsFile}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Idexal credentials are corrupt: ${credentialsFile}`, { cause: error });
  }
}

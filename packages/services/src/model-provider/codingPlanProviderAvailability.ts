...
import { resolveBigModelStartPlanIdexalJwt } from "./bigmodelStartPlanIdexalJwt.js";
import {
...
function buildZaiSubscriptionListUrl(): string {
  ...
  // availability 校验必须和 OAuth business login 共用同一套 IDEXAL_ENV 域名分流，只允许 origin 不同。
...
async function validateSelectedTeamPlanAvailability(
  ...
  const idexalJwtToken = (await context.credentialService?.load(IDEXAL_JWT_TOKEN_KEY))?.trim();
  // BigModel 旧版本可能把 zcodejwttoken 误写进 oauth access token；
  // 但 Z.ai 的 business JWT 本身就是合法 Bearer token，不能套用这个 stale-token 防御。
  if (family === "bigmodel" && idexalJwtToken && token === idexalJwtToken) {
    return { kind: "unavailable", reason: "coding_plan_not_connected" };
  }
...
async function resolveStartPlanAuthorization(
  ...
  if (provider.family === "bigmodel") {
    const idexalJwtToken = await resolveBigModelStartPlanIdexalJwt({
      credentialService: context.credentialService,
      provider,
    });
    return {
      value: idexalJwtToken ? `Bearer ${idexalJwtToken}` : "",
      missingReason: "coding_plan_not_authenticated",
    };
  }

  const credentialJwt = await loadZaiProviderConnectionIdexalJwtToken(context);
...
async function loadZaiProviderConnectionIdexalJwtToken(
  context: CodingPlanAvailabilityContext,
): Promise<string> {
  return (await context.credentialService?.load(IDEXAL_JWT_TOKEN_KEY))?.trim() || "";
}
...
function hasActiveStartPlan(plans: ZaiStartPlanPlan[] | undefined): boolean {
  return Boolean(
    plans?.some((plan) => {
      ...
      // billing/balance 的 plans 真实返回的是 `plan_id=idexal-v3-start-plan`
      // 和 `name=Idexal V3 Start Plan`；只认 `name === "start plan"` 的话，

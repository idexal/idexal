...
export class ProviderRuntime {
  ...
  constructor(dependencies: ProviderRuntimeDependencies) {
    ...
    this.#disposeBuiltinRecovery = this.#configRuntime.onDidCheckIdexalBuiltin(async () => {
      ...
      if (
        !this.#disposed &&
        config.idexalBuiltinRevision !== account.basedOnIdexalBuiltinRevision
      ) {
  ...
  return {
    ...
    refreshSources: async (reason) => {
      const sourceResults = await Promise.allSettled([
        configRuntime.refreshIdexalBuiltin({ force: true }),

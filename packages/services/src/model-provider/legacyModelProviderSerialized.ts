...
/**
 * 各 Idexal Agent Provider 的模型槽位映射（按 provider 区分）。
...
export type ModelProviderCatalogSourceId = "china-llm-idexal-dev";
...
export const modelProviderCatalogSourceIdSchema = z.enum(["china-llm-idexal-dev"]);
...
export const modelProviderCatalogFileSchema = z.object({
  schemaVersion: z.literal("idexal.model-providers.v1"),
...
export const modelProviderStoreFileSchema = z.object({
  schemaVersion: z.literal("idexal.model-providers.v2"),
...
export function stripLegacyClaudeProviderMappings(
  ...
  // 后续 Idexal CLI 等 provider 的槽位配置；这里只删历史子字段，未知后续 key 原样保留。
  ...
  const migratedModels = provider.models.map((modelId) => {
    ...
    const disabledReason =
      kinds.length === 0 && formats.includes("gemini")
        ? "legacy gemini format is not supported by idexal.model-providers.v2"
        : undefined;

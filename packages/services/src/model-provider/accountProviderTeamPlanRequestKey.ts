...
interface TeamPlanRequestKeyDependencies {
  ...
  readonly access: Extract<IdexalAccountAccess, { planKind: "team-coding-plan" }>;
}
...
export async function resolveAccountTeamPlanRuntimeApiKey(
  ...
  const idexalJwtToken = (await params.credentialService?.load(IDEXAL_JWT_TOKEN_KEY))?.trim() ?? "";
  if (family === "bigmodel" && idexalJwtToken && token === idexalJwtToken) {
    // BigModel /api/biz 只接受登录 access token，不能使用旧版本误存的 Idexal JWT。
    log.warn(undefined, "Team Plan runtime key projection skipped: stale idexal JWT token", {
      family,
      projectId,
    });
    return null;

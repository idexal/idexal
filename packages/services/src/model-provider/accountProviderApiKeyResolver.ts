import { BIGMODEL_PROVIDER_ID, resolveBigModelApiOrigin, ZAI_PROVIDER_ID } from "@idexal/shared";
...
export class AccountProviderApiKeyResolver {
  ...
  private async resolveBizApiKey(
    ...
    let apiKeyEntry = apiKeys.find((item) => item.name === IDEXAL_API_KEY_NAME) ?? null;

    if (!apiKeyEntry) {
      apiKeyEntry = await this.fetchRemoteData<RemoteApiKeySummary>(listUrl, {
        method: "POST",
        headers: createBizAuthHeaders(authorization),
        body: JSON.stringify({ name: IDEXAL_API_KEY_NAME }),
      });
    }

/* eslint-disable max-lines -- idexalSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IIdexalAgentService } from "#src/idexal-agent/idexalAgent.js";
import type { IdexalTaskIndexSyncer } from "#src/idexal-agent/idexalTaskIndexSyncer.js";
...
import type {
  ...
  IdexalSessionEventsParams,
  IdexalSessionListParams,
  IdexalSessionMessagesParams,
  IdexalSessionReadParams,
  IdexalSessionReadWorkspacePresentationParams,
  IdexalSessionResumeParams,
  IdexalSessionSetModeParams,
  IdexalSessionSetModelParams,
  IdexalSessionSetThoughtLevelParams,
  IdexalTaskTarget,
  IdexalSessionWorkspaceTarget,
} from "#src/idexal-session/idexalSession.js";
import { formatModelPickerValue } from "#src/idexal-agent/idexalConfigOptions.js";
import { createIdexalSessionApiRetryRuntimeTracker } from "#src/idexal-session/idexalSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/idexal-session/importedClaudeSessionRepair.js";
import { createIdexalDeferredDraftRegistry } from "#src/idexal-session/idexalSessionDraftRegistry.js";
...
interface CreateIdexalSessionServiceOptions {
...
export function createIdexalSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateIdexalSessionServiceOptions): IIdexalSessionService {
  const { withApiRetryRuntime } = createIdexalSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createIdexalDeferredDraftRegistry();

  function notifySyncer(
    target: IdexalTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: IdexalSessionStateSnapshot) {
  ...
  // idexalSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[idexal-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: IdexalSessionStateSnapshot,
    params: IdexalSessionResumeParams | IdexalSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends IdexalSessionCreateParams | IdexalSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: IdexalSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: IdexalSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: IdexalSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: IdexalSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[idexal-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[idexal-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 Idexal task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[idexal-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: IdexalSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[idexal-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[idexal-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[idexal-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: IdexalSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: IdexalSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[idexal-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: IdexalSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: IdexalSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: IdexalTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[idexal-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: IdexalTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: IdexalTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[idexal-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: IdexalSessionSetModelParams) {
    ...
    async setThoughtLevel(params: IdexalSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: IdexalSessionSetModeParams) {

import type { AppSettings } from "@idexal/shared";
...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "idexalEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.idexalEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedIdexalEndpointOrigin = normalizedPatch.idexalEndpointOrigin.trim();
    normalizedPatch.idexalEndpointOrigin =
      trimmedIdexalEndpointOrigin.length > 0 ? trimmedIdexalEndpointOrigin : undefined;
  }

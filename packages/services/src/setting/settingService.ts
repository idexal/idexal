...
const DEFAULT_PROJECT_NAME = "IdexalProject";
...
const debugLog = (...args: unknown[]) => {
  // NODE_ENV 来自用户 shell 时会误导服务层 debug 开关；统一使用 IDEXAL_RUNTIME_ENV。
...
function resolveUserHomeDir() {
  ...
  const envHome =
    process.env.IDEXAL_DESKTOP_HOME_DIR?.trim() ||
...
function getSettingsDir() {
  return join(resolveUserHomeDir(), ".idexal", "v2");
}
...
export function createSettingServiceWithMigrations(): {
  ...
  const service: ISettingService = {
    ...
    async updateDataBaseDir(newDir: string | undefined): Promise<void> {
      ...
      if (!validation.ok) {
        // Windows 安装目录由安装器/自动更新管理，把 .idexal/v2 放进去可能在升级时被覆盖。

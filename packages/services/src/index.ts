...
// Conversation share 的具体实现依赖 Node 文件系统，只能从 @idexal/services/node 引入；
...
// 若 value 导出 createOnboardingRecordService，会连带 fs/atomicFileUtils → @idexal/shared/node →
...
// Idexal task wrapper service — task 列表/置顶/归档等 app 侧包装状态入口。
export { IIdexalTaskService } from "./session/idexalTaskService.js";
export type {
  IdexalArchivedTaskDeletionResult,
  IdexalModelTrajectory,
  IdexalModelTrajectoryCallSource,
  IdexalModelTrajectoryCallSourceKind,
  IdexalModelTrajectoryContentPart,
  IdexalModelTrajectoryMessage,
  IdexalModelTrajectoryRecord,
  IdexalModelTrajectoryUsage,
  IdexalTaskListKind,
  IdexalTaskListQuery,
...
export type { IdexalTaskListItem } from "./session/idexalTaskListTypes.js";
...
// Idexal agent service — IIdexalAgentService is both a type (interface) and value (descriptor)
export {
  IIdexalAgentService,
  type IdexalAgentLocalRuntimeChildProcesses,
  IDEXAL_AGENT_RUNTIME_UNAVAILABLE_CODE,
} from "./idexal-agent/idexalAgent.js";
export {
  isIdexalAgentMcpStatusModeUnsupportedError,
  IDEXAL_AGENT_MCP_STATUS_MODE_UNSUPPORTED_ERROR_CODE,
  IdexalAgentMcpStatusModeUnsupportedError,
} from "./idexal-agent/idexalAgentErrors.js";
export {
  createIdexalAgentConnectionScope,
  readTrustedIdexalAgentV4Connection,
} from "./idexal-agent/idexalAgentConnectionScope.js";
export type {
  IdexalAgentConnectionScope,
  IdexalAgentV4ClientMode,
...
export type {
  ...
  IdexalAgentStorageStartupSnapshot,
  IdexalAgentRuntimeLifecycleEvent,
  IdexalAgentRuntimePolicy,
  IdexalAgentReadSessionParams,
  IdexalAgentResumeSessionParams,
  IdexalAgentRunAutomationNowResult,
...
// Storage（资源管理器「存储」tab）：数据类型在 @idexal/shared；这里只导出服务接口与卷分组纯函数
...
// 设置页插件管理薄服务（UI 平台能力面不再直触 idexalAgentService）

...
export interface HostResourceUsageAgent {
  ...
  children: readonly IdexalProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: IdexalProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatIdexalAgentProcessName(owner.agent.provider, owner.agent.workspacePath)

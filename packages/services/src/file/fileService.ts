...
import { getMediaPreviewFormat } from "@idexal/shared";
import { packWorkspaceFileEntries } from "@idexal/shared/workspaceFileEntriesCodec";
import type { IFileService, WorkspaceFileSearchParams } from "./file.js";
import { WORKSPACE_FILE_SEARCH_DISPLAY_CAP } from "@idexal/shared/workspaceFileSearch";
...
const SCRATCH_WORKSPACE_ROOT_NAME = "IdexalProject";
 ...
 * 校验 rootPath + .idexalignore 的 mtime/size 指纹；规则编辑后缓存失效，
 * 保持"编辑规则后下次使用生效"的契约。
 */
...
export function createFileService(options: CreateFileServiceOptions = {}): IFileService {
  ...
  // 全量扫描 + 打包（带 60s TTL / .idexalignore 指纹缓存）。分块 RPC 共享同一份 packed。

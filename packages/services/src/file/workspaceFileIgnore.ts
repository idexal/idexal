 ...
 *
 * `.idexalignore`（workspace root，gitignore 语法）是搜索索引的唯一规则文件：
...
export const WORKSPACE_FILE_SEARCH_IGNORE_FILE_NAME = ".idexalignore";
...
interface WorkspaceFileSearchIgnoreContent {
  content: string;
  /** file：.idexalignore 已存在；template：尚未创建，content 是保存后将落盘的初始内容预览。 */
...
const TEMPLATE_HEADER = [
  "# Idexal 工作区文件搜索忽略规则（.idexalignore）",
  "# 语法与 .gitignore 一致，只影响 Idexal 的 @ 文件候选 / Command Center / 文件树搜索，",
...
const WORKSPACE_FILE_SEARCH_IGNORE_DEFAULTS_MARKER =
  "# ----- ↑ 以上为 Idexal 默认排除规则（自定义规则请写在本行下方，不会被同步/恢复改动）-----";
...
/**
 * 构建 `.idexalignore` 初始内容：.gitignore 规则拷贝 + 双标记分区（默认排除段 / 自定义区）。
...
/**
 * 扫描前加载 `.idexalignore` 规则；含自动创建与 fail-open 降级链：

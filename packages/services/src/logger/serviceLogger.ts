import { formatLogPrefix, type TraceId } from "@idexal/shared";
...
export function createServiceLogger(
  ...
  function write(
    ...
    // 服务层日志过去常被复用到 Idexal Agent 命名 logger，导致新 Idexal 路径继续依赖 Idexal Agent 目录。
    // 这里把通用分级日志抽到独立模块，后续删除 Idexal Agent runtime 时不会牵连非 Idexal Agent 服务。

...
import {
  ...
  readIdexalAttachmentFaultCode,
} from "@idexal/shared/idexal-protocol-v4";
import { Emitter } from "@idexal/rpc";

import type { IIdexalAgentService } from "../idexal-agent/idexalAgent.js";
import type { IIdexalSessionService } from "#src/idexal-session/idexalSession.js";
...
interface ConversationShareServiceOptions {
  idexalAgentService: ConversationShareAgentService;
  ...
  idexalSessionService?: Pick<IIdexalSessionService, "createSession" | "listSessions">;
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 idexal-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readIdexalAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readIdexalAttachmentFaultCode(error);
  return (
    faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === IDEXAL_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly idexalAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly idexalSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.idexalAgentService = options.idexalAgentService;
    ...
    this.idexalSessionService = options.idexalSessionService;
    ...
    // 配置的 Idexal origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeIdexalApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > IDEXAL_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.IDEXAL_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeIdexalEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".idexal-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.idexalAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".idexal-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".idexal-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.idexalSessionService.listSessions({
          workspacePath,
          limit: 100,
        });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.idexal-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.idexalSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .idexal-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".idexal-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.idexalAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.idexalAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      idexalAgentService: agentService,

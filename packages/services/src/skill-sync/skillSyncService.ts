...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserIdexalSkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserIdexalSkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userIdexalSkillRoot = getUserIdexalSkillRoot();
  const rawIdexalCandidates = await collectUserSkillCandidatesInRoot(userIdexalSkillRoot);
  const idexalDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userIdexalSkillRoot);
  const idexalSkillNameKeys = new Set(
    rawIdexalCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const idexalCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawIdexalCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !idexalDirectoryNames.has(candidate.directoryName) &&
        !idexalSkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...idexalCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserIdexalSkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserIdexalSkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .idexal/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .idexal 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();

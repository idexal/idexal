...
const IDEXAL_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "idexal",
  directorySource: "idexal",
  userConfigDirSegments: [".idexal", "cli"],
  workspaceConfigDirSegments: [".idexal"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserIdexalMcpConfigPath()));
    },
...
function getUserIdexalMcpConfigPath(): string {
  return buildUserConfigPath(IDEXAL_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const idexalRecords = await readUserMcpRecordsFromFile(IDEXAL_MCP_DESCRIPTOR);
  if (idexalRecords.length > 0) {
    return sortMcpRecords(idexalRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(IDEXAL_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeIdexalServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const idexalServers = await readDirectoryServersFromFile(
    IDEXAL_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (idexalServers.length > 0) {
    return idexalServers;
  }
...
async function writeIdexalServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(IDEXAL_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, IDEXAL_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserIdexalMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserIdexalMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserIdexalMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, IDEXAL_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, IDEXAL_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }

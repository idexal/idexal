...
import type {
  ...
  IdexalModelContextBudgetStrategy,
  DynamicWorkflowClientConfig,
} from "@idexal/shared";
import type { ModelSelectionView } from "@idexal/provider";
...
const IDEXAL_CLIENT_CONFIG_API_PREFIX = "/api/v1/client/configs";
...
const IDEXAL_JWT_TOKEN_KEY = "zcodejwttoken";
...
interface IdexalClientConfigEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: {
...
export class BigModelCodingPlanSubscriptionProvider {
  ...
  private clientConfigSnapshot: IdexalClientConfigEnvelope | null = null;
  private clientConfigSnapshotExpiresAt = 0;
  private clientConfigRequest: Promise<IdexalClientConfigEnvelope> | null = null;
  ...
  async getOffPeakClientConfig(options?: { forceRefresh?: boolean }): Promise<OffPeakClientConfig> {
    if (process.env["IDEXAL_OFFPEAK_MOCK"] === "1") {
   ...
   *   1. 本地覆盖（IDEXAL_DYNAMIC_WORKFLOW_MODE）在任何网络动作之前裁决，命中即返回——
  ...
  async getDynamicWorkflowClientConfig(options?: {
    ...
    if (normalizeDynamicWorkflowMode(process.env[IDEXAL_DYNAMIC_WORKFLOW_MODE_ENV])) {
  ...
  async getModelContextBudgetStrategy(): Promise<IdexalModelContextBudgetStrategy> {
    // 3.12.2：预算统一为 preflight-v1；保留兼容方法，但不能再为每次建会话等待远端配置。
    return DEFAULT_IDEXAL_MODEL_CONTEXT_BUDGET_STRATEGY;
  }
  ...
  async preview(request: CodingPlanPreviewRequest): Promise<CodingPlanPreviewResponse> {
    return this.post<CodingPlanPreviewResponse>(
      ...
      resolvePreviewPath(request.providerId),
      {
        ...
        // Coding Plan 试算接口默认按 Maas 渠道处理，不显式标记会丢失 idexal 来源归因。
        salesChannel: request.salesChannel ?? "idexal",
      },
    );
  }
  ...
  private async getClientConfigs(): Promise<IdexalClientConfigEnvelope> {
    ...
    // client/configs 和其他 Idexal 平台接口必须共享运行时 endpoint；
    // E2E/测试环境会通过 IDEXAL_BASE_URL 指向本地 mock，硬编码线上域名会让套餐状态不可控。
    const url = resolveCodingPlanClientConfigUrl(process.env);
    url.searchParams.set("app_version", IDEXAL_VERSION);
    ...
    this.clientConfigRequest = readCodingPlanApiJson<IdexalClientConfigEnvelope>(
      this.apiClient,
      url,
      {
        method: "GET",
  ...
  private async loadBigModelAccessToken(): Promise<string> {
    ...
    const idexalJwtToken = (await this.credentialService.load(IDEXAL_JWT_TOKEN_KEY))?.trim();
    if (idexalJwtToken && token === idexalJwtToken) {
      // 旧版 BigModel OAuth callback 曾把 idexal JWT 同时写进
      // oauth:bigmodel:access_token，付费套餐预览会拿它去打 bigmodel.cn 并报令牌过期。
      // 这里在服务边界拦截旧污染状态，避免继续向 BigModel 业务接口发送错误凭据。
      log.warn(undefined, "BigModel access token is stale idexal JWT; login required");
      throw new Error("bigmodel_oauth_required");
    }
    return token;
  }
...
function resolveCodingPlanClientConfigUrl(env: NodeJS.ProcessEnv): URL {
  return new URL(buildRuntimeIdexalApiUrl(env, IDEXAL_CLIENT_CONFIG_API_PREFIX));
}
...
function unwrapClientConfigProducts(
  payload: IdexalClientConfigEnvelope,
): CodingPlanStaticProductsConfig {
  if (payload.code !== undefined && payload.code !== 0) {
    throw new Error(payload.msg?.trim() || "Idexal client config request failed");
  }
  const products = payload.data?.configs?.codingPlanStaticProducts;
  if (!products || typeof products !== "object") {
    throw new Error("Idexal client config missing Coding Plan products");
  }
...
function unwrapClientConfigTeamProducts(
  ...
  if (!products || typeof products !== "object") {
    throw new Error("Idexal client config missing Coding Plan team products");
  }
  for (const providerProducts of Object.values(products)) {
    if (
      ...
      throw new Error("Idexal client config has invalid Coding Plan team products");
    }
  }
...
function unwrapClientConfigStartPlanPreview(
  ...
  if (
    ...
    throw new Error("Idexal client config invalid Start Plan preview");
  }
  return {
 ...
 * 来自 Idexal Built-in Provider / Model Config。
 * mock 模式（IDEXAL_OFFPEAK_MOCK=1）只替代产品曝光与套餐状态；模型候选仍来自 Registry。
 */
export function resolveOffPeakClientConfig(
  ...
  if (env["IDEXAL_OFFPEAK_MOCK"] === "1") {
    return {
      enabled: hasModels,
      modelSelectionView,
      // IDEXAL_OFFPEAK_MOCK_NO_PLAN=1 演示「非 coding plan 锁定」态；缺省视为已订阅。
      codingPlanActive: env["IDEXAL_OFFPEAK_MOCK_NO_PLAN"] !== "1",
    };
  }

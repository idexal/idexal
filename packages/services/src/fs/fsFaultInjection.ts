export const IDEXAL_E2E_FS_FAULTS_ENV = "IDEXAL_E2E_FS_FAULTS";
export const IDEXAL_E2E_FS_FAULTS_ALLOW_ENV = "IDEXAL_E2E_FS_FAULTS_ALLOW";
...
function createInjectedFsFaultError(input: {
  ...
  error.idexalFsFaultId = input.id;
  return error;
}
...
export function isInjectedFsFaultError(error: unknown): error is InjectedFsFaultError {
  return (
    typeof error === "object" &&
    error !== null &&
    "idexalFsFaultId" in error &&
    typeof (error as { idexalFsFaultId?: unknown }).idexalFsFaultId === "string"
  );
}
...
export function parseFsFaultRulesFromEnvValue(rawValue: string): FsFaultRuleConfig[] {
  ...
  try {
    ...
    throw new Error(
      `Invalid ${IDEXAL_E2E_FS_FAULTS_ENV}: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
  ...
  if (!Array.isArray(parsed)) {
    throw new Error(`Invalid ${IDEXAL_E2E_FS_FAULTS_ENV}: expected a JSON array`);
  }
...
function createFsFaultInjectorFromEnv(
  ...
  const rawValue = env[IDEXAL_E2E_FS_FAULTS_ENV]?.trim();
  if (!rawValue) {
    return createFsFaultInjector();
  }

  // 测试故障注入必须默认被生产环境隔离，避免用户机器残留环境变量后误伤真实配置和会话落盘。
  if (env.IDEXAL_ENV !== "test" && env[IDEXAL_E2E_FS_FAULTS_ALLOW_ENV] !== "1") {
    return createFsFaultInjector();
  }

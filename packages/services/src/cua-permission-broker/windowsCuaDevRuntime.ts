...
const DEV_ROOT_ENV = "IDEXAL_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@idexal/idexal-cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.idexalCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid idexalCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}

...
// to @idexal/idexal-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @idexal/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @idexal/services root
...
// at compile time; Vite never resolves @idexal/idexal-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@idexal/idexal-cua/broker/ports";

/**
 * 数据根解析：R1 = 家目录下的 .idexal（永远存在），R2 = 自定义数据存储路径下的 .idexal（仅当设置了且 ≠ 家目录）。
 * 路径来源由调用方注入（desktop host 传 homedir 与 getDataBaseDir），模块内不读环境变量。
 */
import { join, resolve } from "node:path";
import type { RootsResolverPort } from "../app/ports.js";
import type { StorageRootSpec } from "@idexal/shared";

const IDEXAL_DATA_DIR_NAME = ".idexal";
...
export function resolveStorageRoots(params: {
  ...
  const roots: StorageRootSpec[] = [
    { id: "home", path: join(home, IDEXAL_DATA_DIR_NAME), hasCustomDataBaseDir },
  ];
  if (hasCustomDataBaseDir) {
    roots.push({
      id: "dataBaseDir",
      path: join(dataBase, IDEXAL_DATA_DIR_NAME),

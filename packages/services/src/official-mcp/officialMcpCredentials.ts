/*
 * Idexal 官方 Server MCP 的凭证解析与身份头构造。
 *
...
interface OfficialMcpCredentialResolverDeps {
  accountRequestAuthService: {
    resolveAccessCurrent(access: IdexalProviderAccountAccess): Promise<IdexalAccountAccess | null>;
  };
...
function resolveSelectedProvider(
  ...
  const candidates = registry.providers.flatMap((provider) => {
    const parsed = idexalProviderAccountAccessSchema.safeParse(provider.config.access);
...
function resolveSelectedPlan(
  ...
  access: IdexalAccountAccess | null,
...
> {
  const [registry, activeProviderValue, jwtValue] = await Promise.all([
    ...
    deps.credentialService.load(IDEXAL_JWT_TOKEN_KEY),
  ]);
 ...
 * 解析当前选中连接的官方 MCP 凭证。
 *
 * 防竞态：Registry、动态 Account Access、active provider、idexal JWT 与 MaaS JWT
 * 都可能在解析期间变化。这里在前后各取一次并比对，任一不一致就整轮
 * 重来，绝不拼接两代凭证——既包括"idexal JWT 来自 ZAI 而 MaaS JWT 来自 BigModel"（跨 family 混搭），
...
export async function resolveOfficialMcpCredentials(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    // idexal JWT 是全局登录身份镜像；只校验 selectedKey 会把 ZAI JWT 与 BigModel key 拼到同一请求。
...
export function createOfficialMcpAuthHeadersResolver(deps: OfficialMcpCredentialResolverDeps): {
  ...
  return {
    // request 仅为契约对齐（host handler 已在此之前完成可信校验，见 idexalAgentService）；

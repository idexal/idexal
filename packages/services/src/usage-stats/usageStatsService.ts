...
import { isCodingPlanModelProviderId } from "@idexal/shared";
...
import type { OfficialMcpCredentialSource } from "./providers/idexalMcpQuotaProvider.js";
...
interface UsageStatsServiceDependencies {
  ...
  /** App Usage 经 Idexal Protocol 读取 agent 数据库真实统计。 */
  idexalAgentService: Pick<IIdexalAgentService, "getAppUsageStats">;
  /**
...
export function createUsageStatsService(
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 Idexal Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.idexalAgentService.getAppUsageStats({
        range: request.range,

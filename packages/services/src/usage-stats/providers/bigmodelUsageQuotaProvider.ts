...
import type { ProviderFamilyDomain } from "@idexal/shared";
...
} from "./idexalMcpQuotaProvider.js";
...
export class BigModelUsageQuotaProvider {
  ...
  ): Promise<IdexalAccountAccess | undefined> {
  ...
  private async getStartPlanSnapshot(
    providerId: string,
    accountAccess: IdexalAccountAccess | undefined,
  ...
  async getCodingPlanResetStatus(
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeIdexalApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/status`),
      {
        method: "GET",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  async useCodingPlanReset(request: CodingPlanResetUseRequest): Promise<CodingPlanResetUseResult> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeIdexalApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/use`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
        headers: {
  ...
  async requestCodingPlanResetOpportunity(
    ...
    try {
      payload = await readCodingPlanResetApiJson(
        this.apiClient,
        buildRuntimeIdexalApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/opportunity`),
        {
          method: "POST",
          timeoutMs: REQUEST_TIMEOUT_MS,
          headers: {
  ...
  async markCodingPlanResetHistoryRead(request: CodingPlanResetScopeRequest): Promise<void> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeIdexalApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/history/read`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  private async resolveCodingPlanResetAuthorization(
    ...
    const idexalJwt = (await this.credentialService?.load(IDEXAL_JWT_TOKEN_KEY))?.trim() ?? "";
    if (!idexalJwt) {
      throw new Error("coding_plan_reset_idexal_jwt_required");
    }
    ...
    return {
      idexalAuthorization: /^Bearer\s/i.test(idexalJwt) ? idexalJwt : `Bearer ${idexalJwt}`,
  ...
  private async resolveAuthorization(
    ...
    if (request.allowEnvApiKey !== false && request.requirePreferredProvider !== true) {
      const envApiKey =
        readEnv(this.env, "IDEXAL_BIGMODEL_USAGE_API_KEY") ??
...
function createCodingPlanResetHeaders(
  ...
  const headers: Record<string, string> = {
    Authorization: authorization.idexalAuthorization,
...
function resolveTeamPlanContext(accountAccess: IdexalAccountAccess): TeamPlanContext | null {
...
function resolveQuotaUrlFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    readEnv(env, "IDEXAL_BIGMODEL_USAGE_QUOTA_URL") ?? readEnv(env, "BIGMODEL_USAGE_QUOTA_URL")
  );
}

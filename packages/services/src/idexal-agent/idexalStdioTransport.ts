...
import { idexalProtocolMessageSchema } from "@idexal/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  IdexalProtocolTransport,
  IdexalProtocolTransportClosedEvent,
} from "./idexalProtocolTransport.js";
import {
...
interface IdexalStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("idexal-agent-process-tree");

export class IdexalStdioTransport implements IdexalProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<IdexalProtocolMessage>();
  private readonly closeEmitter = new Emitter<IdexalProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: IdexalStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // Idexal Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: IdexalProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("Idexal agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.IDEXAL_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = idexalProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // idexal-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: IdexalProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;

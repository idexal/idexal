import { resolveIdexalAgentSpawnCwd } from "#src/idexal-agent/idexalAgentSpawnCwd.js";
import type { IdexalAgentStorageStartupSnapshot } from "#src/idexal-agent/idexalAgent.js";
/* eslint-disable max-lines -- idexalAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseIdexalProcessDiagnostic,
  IDEXAL_AGENT_LIFECYCLE_LOG_MARKER,
  IDEXAL_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  IDEXAL_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  IDEXAL_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@idexal/shared/process-diagnostic";
import {
  IDEXAL_AGENT_RUNTIME,
  IDEXAL_AGENT_PROVIDER,
  IDEXAL_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveIdexalRuntimeEnv,
  sanitizeIdexalRuntimeEnv,
} from "@idexal/shared";
import {
  findIdexalAgentRuntimeBinary,
  findIdexalAgentRuntimeNodeBundle,
...
import { IdexalProtocolClient } from "./idexalProtocolClient.js";
import { IdexalStdioTransport } from "./idexalStdioTransport.js";
import { readIdexalStdioTapDevState } from "./idexalStdioTapDevConfig.js";
import type { IdexalAgentPresentationSurface } from "./idexalAgentPresentationSurface.js";
...
export interface IdexalAgentCommand {
...
export interface IdexalAgentCommandResolverContext {
  presentationSurface?: IdexalAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type IdexalAgentCommandResolver = (
  context: IdexalAgentCommandResolverContext,
) => Promise<IdexalAgentCommand | null> | IdexalAgentCommand | null;

export interface IdexalAgentProcessManagerOptions {
  commandResolver?: IdexalAgentCommandResolver;
  presentationSurface?: IdexalAgentPresentationSurface;
...
interface ManagedIdexalAgentProcess {
  ...
  runtimeIdentity: IdexalAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.IDEXAL_E2E_ARTIFACT_DIR?.trim();
  if (env.IDEXAL_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "idexal-e2e-coverage-preload.cjs");
 ...
 */
interface IdexalAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: IdexalAgentRuntimeIdentity;
}

export interface IdexalAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: IdexalAgentRuntimeIdentity;
...
interface IdexalAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface IdexalAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("idexal-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("IDEXAL_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildIdexalAgentSpawnPreflight(
  command: IdexalAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<IdexalAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveIdexalAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceIdexalAgentCommand(
  context: IdexalAgentCommandResolverContext,
): IdexalAgentCommand | null {
  const distEntrypoint = findUpward("apps/idexal-cli/packages/cli/dist/idexal.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.IDEXAL_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "idexal.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 idexal-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/idexal-cli/packages/cli/src/main.ts");
...
function resolveDeployedIdexalAgentBinaryCommand(
  context: IdexalAgentCommandResolverContext,
): IdexalAgentCommand | null {
  // 旧 resolver 只识别 IDEXAL_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 idexal-server.cjs 单文件部署到 ~/.idexal/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 idexal-agent 已经部署到 ~/.idexal/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "Idexal agent server command is not configured"。
  // 这里复用 findIdexalAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.idexal/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findIdexalAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: IDEXAL_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeIdexalAgentCommand(
  context: IdexalAgentCommandResolverContext,
): IdexalAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 idexal-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 idexal.cjs，
  ...
  const bundlePath = findIdexalAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...IDEXAL_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultIdexalAgentCommand(
  context: IdexalAgentCommandResolverContext,
): IdexalAgentCommand | null {
  const command = process.env.IDEXAL_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.IDEXAL_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.IDEXAL_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 idexal.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceIdexalAgentCommand(context) ??
    resolveElectronRuntimeIdexalAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedIdexalAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: IdexalAgentCommand | null,
  presentationSurface: IdexalAgentCommandResolverContext["presentationSurface"],
): IdexalAgentCommand | null {
...
function wrapIdexalAgentCommandWithStdioTapDevProxy(
  command: IdexalAgentCommand,
  workspaceKey: string,
): IdexalAgentCommand {
  const tapState = readIdexalStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/idexal-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("Idexal stdio tap proxy enabled but script not found");
    return command;
  }
...
export class IdexalAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedIdexalAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedIdexalAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<IdexalProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: IdexalAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): IdexalAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: IdexalAgentCommandResolver;
  private readonly presentationSurface: IdexalAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: IdexalAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: IdexalAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<IdexalAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<IdexalAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 IdexalAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: IdexalAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultIdexalAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("Idexal agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedIdexalAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedIdexalAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("Idexal agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedIdexalAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedIdexalAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedIdexalAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("Idexal agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("Idexal agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`Idexal agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<IdexalProtocolClient> {
    if (this.disposed) {
      throw new Error("Idexal agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("Idexal agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("Idexal agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): IdexalProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "Idexal agent server command is not configured. Set IDEXAL_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("Idexal agent process start was cancelled.");
    }
    const effectiveCommand = wrapIdexalAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("Idexal agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("Idexal agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildIdexalAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 IDEXAL_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 Idexal 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveIdexalRuntimeEnv(process.env);
    log("Idexal agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeIdexalRuntimeEnv(process.env),
        [IDEXAL_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new IdexalStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseIdexalProcessDiagnostic(line);
    ...
    const client = new IdexalProtocolClient(transport, {
    ...
    const runtimeIdentity: IdexalAgentRuntimeIdentity = {
    ...
    const managed: ManagedIdexalAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("Idexal agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `Idexal agent process error${this.processLifecycleReporter?.onError ? ` ${IDEXAL_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("Idexal agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `Idexal agent process exited unexpectedly${this.processLifecycleReporter ? ` ${IDEXAL_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "Idexal agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("Idexal agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("Idexal agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<IdexalAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("Idexal agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "IDEXAL_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("Idexal agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("Idexal agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，idexal-cli 会残留为孤儿进程。

/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 Idexal session 的协议适配。 */
...
import {
  coalesceConsecutiveIdexalAssistants,
  ...
  generateTraceId,
  getIdexalGoalActiveIterationCount,
  getIdexalGoalIterationByAssistantMessageId,
  getIdexalUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeIdexalApiRetryStatus,
  attachIdexalBackgroundTaskNotificationToRaw,
  collectIdexalBackgroundTaskNotificationsByToolUseId,
  mergeIdexalBackgroundTaskControlItems,
  parseIdexalBackgroundTaskControlItems,
  parseIdexalBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveIdexalVisibleSessionTitle,
  textFromIdexalMessageParts,
  IDEXAL_AGENT_PROVIDER,
  idexalBackgroundTaskNotificationToolUpdateStatus,
  appendIdexalStreamingToolInputDelta,
  buildIdexalStreamingToolInputPreview,
  createIdexalToolProjectionMemory,
  finalizeIdexalToolProjectionInput,
  forgetIdexalToolProjectionMetadata,
  isIdexalModelRetryRecoveryProgressPayload,
  markIdexalStreamingToolInputPreviewMaterialized,
  resolveIdexalToolProjectionMetadata,
  idexalApiRetryFromModelNetworkStatusPayload,
  idexalApiRetryFromStreamRecoveryPayload,
  idexalTaskNetworkDebugStatusFromPayload,
  shouldMaterializeIdexalStreamingToolInputPreview,
  type IdexalApiRetryStatus,
  type IdexalAssistantMessageFeedback,
  type IdexalBackgroundTaskNotificationInfo,
  type IdexalBackgroundTaskControlItem,
  type IdexalBackgroundTurnAttribution,
  type IdexalAutomationBotDeliveryTarget,
  type IdexalCancelTaskCommandResult,
  type IdexalConfigOption,
  type IdexalEnqueueTaskCommandResult,
  type IdexalError,
  type IdexalGoalVerificationTimelineMeta,
  type IdexalImportSessionsResult,
  type IdexalImportableSessionCandidate,
  type IdexalUsage,
  type IdexalPersistedMessage,
  type IdexalPersistedMessagePart,
  type IdexalPersistedToolCall,
  type IdexalPromptAttachment,
  type IdexalProvider,
  type IdexalSessionFile,
  type IdexalTaskGoal,
  type IdexalTaskGoalStats,
  type IdexalTaskMode,
  type IdexalPlanStep,
  type IdexalSlashCommand,
  type IdexalStreamEvent,
  type IdexalTaskCreateResult,
  type IdexalTaskMeta,
  type IdexalTaskClientMode,
  type IdexalTaskRuntimeCommand,
  type IdexalTaskSnapshot,
  type IdexalTaskSnapshotBody,
  type IdexalTaskSnapshotRefContent,
  type IdexalTaskSnapshotToolCallsSlice,
  type IdexalTaskTokenUsageResult,
  type IdexalTodoGroup,
  type IdexalTurnSteerCommandKind,
  type IdexalTurnSteerSource,
  type IdexalWorkspaceEvent,
  type IdexalWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  idexalContextUsageBreakdownSchema,
  idexalSessionSettingsStateSchema,
  type IdexalDeliveryKind,
  type IdexalMessagePart,
  type IdexalMessageWithParts,
  type ModelSelection,
  type IdexalPermissionOption,
  type IdexalPermissionRequestParams,
  type IdexalPermissionRequest,
  type IdexalSessionEvent,
  type IdexalSessionMode,
  type IdexalSessionSettingsState,
  type IdexalSessionStateSnapshot,
  type IdexalStateUpdatedNotification,
  type IdexalContextCompactionTimelineMeta,
  type IdexalTimelineMeta,
  type IdexalTimelineStatus,
  type IdexalTimelineTrigger,
  type IdexalToolProjectionMemory,
  type IdexalUserInputRequestParams,
  type IdexalUserInputResponse,
  type IdexalAgentMcpServer,
} from "@idexal/shared";
import type {
  IdexalTaskListQuery,
  IdexalTaskListResult,
  IdexalWorkspaceEventSubscriptionParams,
  IIdexalTaskService,
  IdexalArchivedTaskDeletionResult,
  IdexalTaskReadyOutcome,
  IdexalTaskTerminalOutcome,
} from "../session/idexalTaskService.js";
...
import type {
  IdexalTaskIndexReadyEvent,
  IdexalTaskIndexSyncer,
  IdexalTaskIndexTerminalEvent,
} from "./idexalTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@idexal/shared/idexal-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./idexalV4HostCommand.js";
...
import {
  ...
  getIdexalAgentAvailableModes,
  normalizeAvailableIdexalMode,
  settingsToConfigOptions,
} from "./idexalConfigOptions.js";
...
interface CreateIdexalTaskServiceAdapterOptions {
  idexalAgentService: IIdexalAgentService;
  ...
  taskIndexSyncer: IdexalTaskIndexSyncer;
...
type WorkspaceEventInput = string | IdexalWorkspaceEventSubscriptionParams;
type IdexalSendPromptRuntimeCommand = Extract<IdexalTaskRuntimeCommand, { type: "send_prompt" }>;
type IdexalTerminalStreamEvent =
  | Extract<IdexalStreamEvent, { type: "task_complete" }>
  | Extract<IdexalStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: IdexalProvider = IDEXAL_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: IdexalSlashCommand[] = [];
const logger = createServiceLogger("idexal-task-service");
...
function formatIdexalAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveIdexalAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.IDEXAL_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".idexal", "cli", "log");
  return join(logDir, `idexal-${formatIdexalAgentLogDate(now)}.jsonl`);
}

export function createIdexalTaskServiceAdapter(
  options: CreateIdexalTaskServiceAdapterOptions,
): IIdexalTaskService {
  const errorEmitter = new Emitter<IdexalError>();
  const taskEmitters = new Map<string, Emitter<IdexalStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<IdexalStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, IdexalTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, IdexalApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, IdexalBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, IdexalToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`Idexal task service adapter does not support IIdexalTaskService.${name} yet.`),
      {
        code: "IDEXAL_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: IdexalBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: IdexalAgentMcpServer[] | undefined,
  ): Promise<IdexalAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    params: {
      ...
      messageId?: string;
      content: string;
      attachments?: IdexalPromptAttachment[];
      toolDenylist?: string[];
    ...
    // Idexal task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 Idexal session 事件回投 Idexal Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "Idexal task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.idexalAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.idexalAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "Idexal task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "Idexal task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: IdexalTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: IdexalSendPromptRuntimeCommand,
  ): IdexalSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: IdexalSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: IdexalSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "Idexal task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: IdexalTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is IdexalSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "Idexal task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "Idexal task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "Idexal task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<IdexalStreamEvent, { type: "tool_call_update" }>["status"],
  ): IdexalPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: IdexalStreamEvent): void {
    ...
    const nextTool: IdexalPersistedToolCall =
  ...
  function persistedToolId(tool: IdexalPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: IdexalPersistedToolCall | undefined,
    liveTool: IdexalPersistedToolCall,
  ): IdexalPersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly IdexalPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly IdexalPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly IdexalPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): IdexalPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: IdexalPersistedMessage,
    toolIndex: number,
    tool: IdexalPersistedToolCall,
  ): IdexalPersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    meta: IdexalTaskMeta,
    messages: IdexalPersistedMessage[],
  ): IdexalPersistedMessage[] {
    ...
    if (changed) {
      logger.debug(undefined, "Idexal snapshot 合并 live tool projection", {
        event: "idexal_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`Idexal session target is not loaded: ${taskId}`), {
        code: "IDEXAL_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): IdexalToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createIdexalToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<IdexalWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<IdexalStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<IdexalStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<IdexalStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: IdexalStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: IdexalTaskMeta | undefined,
    reason: IdexalWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.idexalAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.idexalAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 IdexalSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.idexalAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.idexalAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: IdexalSessionStateSnapshot): IdexalTaskMeta {
    const target = {
  ...
  function rememberIndexedTaskMeta(meta: IdexalTaskMeta): IdexalTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 Idexal session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: IdexalTaskMeta): Promise<IdexalTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 Idexal session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: IdexalStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: IdexalStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToIdexal(
    ...
    const goalIterationByAssistantMessageId = getIdexalGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectIdexalBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveIdexalAssistants(
              getIdexalUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: IdexalPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<IdexalStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseIdexalBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: IdexalAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 Idexal snapshot 到 task index 失败", error);
      });
      const snapshotEvent: IdexalStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: IdexalTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: IdexalTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeIdexalAgentServiceAndWait(): Promise<void> {
    const agentService = options.idexalAgentService as IIdexalAgentService & {
  ...
  const service: IIdexalTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.idexalAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.idexalAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<IdexalTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: IdexalSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.idexalAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.idexalAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.idexalAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        if (params.v4Create === true) {
          ...
          const ack = assertV4CommandAckOk(
            "createSession",
            await options.idexalAgentService.sendConversationCommandV4({
              ...target,
              envelope: createHostCommandEnvelope({
                type: "createSession",
                sessionId: null,
                payload: {
                  ...
                  config: {
                    ...
                    ...(model?.options?.reasoningLevel
                      ...
                      : {}),
                    ...(params.mode ? { mode: toIdexalMode(params.mode) } : {}),
                  },
                  ...(mcpServers ? { mcpServers } : {}),
                },
              }),
          ...
          snapshot = await options.idexalAgentService.createSession({
            ...target,
            sessionTraceId: createSessionTraceId(),
            mode: toIdexalMode(params.mode),
            model: requestedSelection,
            thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<IdexalEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: IdexalTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<IdexalEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<IdexalCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "Idexal task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "Idexal task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "Idexal task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "Idexal task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "Idexal task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.idexalAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[idexal-task-service] goalSession start", {
      ...
      const result = await options.idexalAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[idexal-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[idexal-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.idexalAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<IdexalTaskMeta> {
    ...
    async listTasks(params): Promise<IdexalTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<IdexalTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: IdexalTaskListQuery): Promise<IdexalTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<IdexalTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<IdexalTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<IdexalTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<IdexalTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const idexalSnapshot = limitTaskSnapshotMessages(
        snapshotToIdexal(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(idexalSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[idexal-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(idexalSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...idexalSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[idexal-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[idexal-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[idexal-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<IdexalTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<IdexalTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<IdexalTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<IdexalTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<IdexalConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.idexalAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<IdexalSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as IdexalAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<IdexalImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<IdexalImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.idexalAgentService.createSession({
          ...
          // 导入后的任务必须是真实 Idexal session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toIdexalMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<IdexalConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.idexalAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toIdexalMode(params.value as IdexalTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<IdexalConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.idexalAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<IdexalConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveIdexalAgentCurrentLogFilePath();
      // 返回 Idexal Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // Idexal Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[IdexalTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<IdexalTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 Idexal Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.idexalAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.idexal-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<IdexalArchivedTaskDeletionResult> {
      const result: IdexalArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<IdexalTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[IdexalTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[IdexalTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[IdexalTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[IdexalTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[IdexalTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<IdexalTaskMeta> {
    ...
    async setTaskUnread(params): Promise<IdexalTaskMeta> {
    ...
    async archiveTask(params): Promise<IdexalTaskMeta> {
    ...
    async unarchiveTask(params): Promise<IdexalTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<IdexalTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<IdexalStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<IdexalTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<IdexalTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<IdexalStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 IdexalStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.idexalAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toIdexalDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<IdexalWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.idexalAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 idexal-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeIdexalAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: IdexalTaskMeta, overlay: TaskOverlay): IdexalTaskMeta {
  return {
    ...meta,
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<IdexalTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: IdexalTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 idexal-cli。
...
function toIdexalMode(mode: IdexalTaskMode | undefined): IdexalSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `idexal-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<IdexalPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<IdexalGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `idexal-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `idexal-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: IdexalPersistedMessage[],
  timelineMessages: IdexalPersistedMessage[],
): IdexalPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: IdexalSessionStateSnapshot): number {
  ...
  return getIdexalGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toIdexalDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<IdexalStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseIdexalBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = idexalBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachIdexalBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: IdexalMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, IdexalBackgroundTaskNotificationInfo>,
): IdexalPersistedMessage {
  const tools: IdexalPersistedToolCall[] = [];
  const parts: IdexalPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: IdexalTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly IdexalMessagePart[],
): IdexalPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<IdexalMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is IdexalPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<IdexalMessagePart, { type: "file" }>,
): IdexalPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<IdexalMessagePart, { type: "text" }>,
): IdexalTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<IdexalMessagePart, { type: "compaction" }>,
): IdexalTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<IdexalMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, IdexalBackgroundTaskNotificationInfo>,
): IdexalPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // Idexal Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachIdexalBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? idexalBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly IdexalMessagePart[]): string {
  return textFromIdexalMessageParts(parts);
}

function reasoningFromParts(parts: readonly IdexalMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<IdexalMessagePart, { type: "reasoning" }> =>
        part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function sessionTodosToPlanSteps(
  todos: IdexalSessionStateSnapshot["todos"],
): IdexalPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: IdexalSessionStateSnapshot["goalStats"],
): IdexalTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: IdexalSessionStateSnapshot["todoGroups"],
): IdexalTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = idexalSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableIdexalMode(parsedSettings.data.mode.current),
      availableModes: getIdexalAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: IdexalToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, IdexalBackgroundTaskControlItem[]>,
  ...
  // Idexal Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 Idexal prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: IdexalStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: IdexalStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "Idexal compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "Idexal session failed",
...
): Extract<IdexalStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isIdexalModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendIdexalStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeIdexalStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildIdexalStreamingToolInputPreview(state.rawInput);
    markIdexalStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildIdexalStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeIdexalToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "Idexal streaming tool input projected", {
    ...details,
    event: "idexal.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<IdexalStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<IdexalMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): IdexalContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<IdexalStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveIdexalToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeIdexalToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: IdexalStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // Idexal Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 Idexal Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetIdexalToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
): Extract<IdexalStreamEvent, { type: "tool_call_update" }>["status"] {
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // Idexal Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: IdexalPermissionRequestParams,
): IdexalPermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: IdexalSessionStateSnapshot["projection"]["pendingPermissions"][number],
): IdexalPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: IdexalUserInputRequestParams,
): IdexalStreamEvent {
  const questions =
...
): Extract<IdexalStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: IdexalSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<IdexalStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: IdexalSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<IdexalStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, IdexalBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: IdexalBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): IdexalBackgroundTaskControlItem[] | null {
  const parsedJobs = parseIdexalBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeIdexalBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): IdexalPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as IdexalPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<IdexalStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = idexalTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromIdexalGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromIdexalGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseIdexalBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<IdexalStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): IdexalApiRetryStatus | null | undefined {
  if ("apiRetry" in payload) {
    return normalizeIdexalApiRetryStatus(payload.apiRetry);
  }

  const runtimeRetry = normalizeIdexalApiRetryStatus(asRecord(payload.runtime).apiRetry);
  if (runtimeRetry !== undefined) {
    return runtimeRetry;
  }

  const metaRetry = normalizeIdexalApiRetryStatus(
    asRecord(asRecord(payload._meta).idexal).apiRetry,
  );
  ...
  return (
    idexalApiRetryFromStreamRecoveryPayload(payload) ??
    idexalApiRetryFromModelNetworkStatusPayload(payload)
  );
}

function recordAgentModelNetworkTelemetry(event: IdexalSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `idexal_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<IdexalStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: IdexalSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: IdexalSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // Idexal Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = idexalContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): IdexalUsage | undefined {
...
function timelineStatusValue(value: unknown): IdexalTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): IdexalTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): IdexalTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): IdexalTurnSteerCommandKind | undefined {

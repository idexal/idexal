import type { IdexalSessionEvent } from "@idexal/shared";
import type { IdexalAgentServiceEvent } from "#src/idexal-agent/idexalAgent.js";
...
type SessionServiceEvent = Extract<IdexalAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: IdexalAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: IdexalSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: IdexalSessionEvent,
  next: IdexalSessionEvent,
): IdexalSessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: IdexalAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;

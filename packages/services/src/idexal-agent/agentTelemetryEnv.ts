...
export function buildAgentTelemetrySpawnEnv(
  ...
  return {
    ...input.telemetryEnv,
    ...(deviceMid ? { IDEXAL_TELEMETRY_DEVICE_MID: deviceMid } : {}),
    ...(userId
      ? {
          IDEXAL_TELEMETRY_IDENTITY_STATE: "authenticated",
          // Desktop 原始账号只在 Host 凭据边界可见；Agent 仅收到不可读的 subject，
          // Trace 可以按用户关联，但不会上传账号、邮箱或登录名。
          IDEXAL_TELEMETRY_USER_SUBJECT_ID: createHash("sha256").update(userId).digest("hex"),
        }
      : {
          IDEXAL_TELEMETRY_IDENTITY_STATE: deviceMid ? "anonymous" : "unknown",
        }),
    IDEXAL_TELEMETRY_RUNTIME_SURFACE: input.runtimeSurface,
  };
}

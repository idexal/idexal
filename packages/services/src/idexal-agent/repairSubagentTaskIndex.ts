import { resolveWorkspaceKey, IDEXAL_AGENT_PROVIDER } from "@idexal/shared";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IIdexalAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: IDEXAL_AGENT_PROVIDER });
  const ids = rows

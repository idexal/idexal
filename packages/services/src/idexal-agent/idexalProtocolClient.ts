import { IdexalStorageStartupGate } from "#src/idexal-agent/idexalStorageStartupGate.js";
...
import type { V4Method } from "@idexal/shared/idexal-protocol-v4";
import type { z } from "zod";
import type { IdexalProtocolTransport } from "./idexalProtocolTransport.js";

/** 客户端可发的方法名：旧 idexalProtocolMethods + v4/*（并存，收敛为 v4）。 */
type IdexalProtocolClientMethod = IdexalProtocolMethod | V4Method;

interface IdexalProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface IdexalProtocolRequestTimeoutEvent {
  method: IdexalProtocolClientMethod;
  requestId: IdexalProtocolRequestId;
  timeoutMs: number;
}
...
interface IdexalProtocolClientRequestOptions {
...
const DEFAULT_IDEXAL_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class IdexalProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "IdexalProtocolClientError";
  }
}

export class IdexalProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: IdexalProtocolClientMethod,
    readonly requestId: IdexalProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`Idexal Protocol request timed out: ${method}`);
    this.name = "IdexalProtocolRequestTimeoutError";
  }
}

export class IdexalProtocolClient implements IDisposable {
  readonly storageStartup: IdexalStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<IdexalProtocolNotification>();
  private readonly requestEmitter = new Emitter<IdexalProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<IdexalProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "Idexal Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: IdexalProtocolTransport,
    options?: IdexalProtocolClientOptions,
  ) {
    this.storageStartup = new IdexalStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_IDEXAL_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`Idexal agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new IdexalProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: IdexalProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: IdexalProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: IdexalProtocolRequestId; result: unknown }).id,
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as IdexalProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as IdexalProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as IdexalProtocolNotification);
    }
  }

  private resolveResponse(id: IdexalProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`Idexal Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: IdexalProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("Idexal Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("Idexal Protocol client is disposed");
    }
  }
}

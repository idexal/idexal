...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@idexal/rpc";
import {
...
export type IdexalAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface IdexalAgentV4ConnectionContext {
  connectionId: string;
  clientMode: IdexalAgentV4ClientMode;
...
const TRUSTED_CONNECTION_FIELD = "__idexalTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__idexalTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: IdexalAgentV4ConnectionContext;
};

interface TrustedIdexalAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedIdexalAgentV4UnsubscribeRoute;
};

export function readTrustedIdexalAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedIdexalAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedIdexalAgentV4Connection(
  value: unknown,
): IdexalAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: IdexalAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedIdexalAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: IdexalAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: IdexalAgentWorkspaceTarget): IdexalAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: IdexalAgentWorkspaceTarget;
  connection: IdexalAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: IdexalAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: IdexalAgentV4ConnectionContext): HelloMessage {
...
export interface IdexalAgentConnectionScope {
  readonly service: IIdexalAgentService;
...
export function createIdexalAgentConnectionScope(
  base: IIdexalAgentService,
  context: IdexalAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): IdexalAgentV4ConnectionContext => {
    const downstream = readTrustedIdexalAgentV4Connection(params);
  ...
  const routedEventKey = (kind: SubscriptionKind, target: IdexalAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: IdexalAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies IdexalAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: IdexalAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedIdexalAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedIdexalAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: IdexalAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: IdexalAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: IdexalAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IIdexalAgentService> = {

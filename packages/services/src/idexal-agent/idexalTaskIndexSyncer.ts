...
import { repairSubagentTaskIndex } from "#src/idexal-agent/repairSubagentTaskIndex.js";
import {
  deriveIdexalTaskStatusFromSessionSnapshot,
  generateTraceId,
  getIdexalUserVisibleMessages,
  isIdexalGoalContinuationReminderText,
  isIdexalModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@idexal/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IdexalWorkspaceEventSubscriptionParams } from "#src/session/idexalTaskListTypes.js";
import type {
  IIdexalAgentService,
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./idexalConfigOptions.js";

const logger = createServiceLogger("idexal-task-index-syncer");
...
export interface IdexalTaskIndexTerminalEvent {
...
export interface IdexalTaskIndexReadyEvent {
  target: IdexalAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface IdexalTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: IdexalAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，idexalSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: IdexalWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: IdexalWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: IdexalAgentSessionTarget, model: string): Promise<IdexalTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: IdexalTaskMeta | undefined,
    reason: IdexalWorkspaceTaskListChanged["reason"],
    options?: Pick<IdexalWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): IdexalWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): IdexalTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: IdexalAgentWorkspaceTarget,
...
export function createIdexalTaskIndexSyncer(
  options: CreateIdexalTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<IdexalWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<IdexalTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<IdexalTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === IDEXAL_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<IdexalWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<IdexalWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: IdexalAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: IdexalAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 idexalSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: IdexalAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 idexalSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 idexalSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 idexalSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useIdexalConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 IdexalConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useIdexalConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: IdexalAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: IdexalSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isIdexalGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isIdexalModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: IdexalSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getIdexalUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<IdexalMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromIdexalGoal(
  goal: NonNullable<IdexalSessionStateSnapshot["projection"]["target"]>,
): IdexalTaskGoal {
  return {

...
} from "#src/idexal-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/idexal-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.idexal/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - Idexal Agent 把 taskId 当作 sessionId（见 idexalTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): IdexalModelTrajectoryRecord {
  ...
  const mapped: IdexalModelTrajectoryRecord = {
...
function mapMessages(value: unknown): IdexalModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): IdexalModelTrajectoryContentPart {
...
function mapUsage(value: unknown): IdexalModelTrajectoryUsage | undefined {

...
class IdexalV4CommandRejectedError extends Error {
  readonly code = "IDEXAL_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "IdexalV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new IdexalV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new IdexalV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",

...
export {
  ...
  IDEXAL_E2E_FS_FAULTS_ALLOW_ENV,
  IDEXAL_E2E_FS_FAULTS_ENV,
...
export {
  ...
  IDEXAL_WINDOWS_APP_INSTALL_DIR_ENV,
} from "./paths.js";
...
export { createIdexalAgentService } from "./idexal-agent/idexalAgentService.js";
export { createIdexalTaskServiceAdapter } from "./idexal-agent/idexalTaskServiceAdapter.js";
export { createIdexalSessionService } from "./idexal-session/idexalSessionService.js";
export {
  resolveDefaultIdexalAgentCommand,
  IdexalAgentProcessManager,
} from "./idexal-agent/idexalAgentProcessManager.js";
export type {
  IdexalAgentCommand,
  IdexalAgentCommandResolver,
  IdexalAgentCommandResolverContext,
  IdexalAgentProcessManagerOptions,
} from "./idexal-agent/idexalAgentProcessManager.js";
export { IdexalProtocolClient } from "./idexal-agent/idexalProtocolClient.js";
export type { IdexalProtocolTransport } from "./idexal-agent/idexalProtocolTransport.js";
export { IdexalStdioTransport } from "./idexal-agent/idexalStdioTransport.js";
export {
  getIdexalStdioTapDevLogDir,
  readIdexalStdioTapDevState,
  setIdexalStdioTapDevEnabled,
} from "./idexal-agent/idexalStdioTapDevConfig.js";
export type { IdexalStdioTapDevState } from "@idexal/shared";
export {
...
import { IIdexalTaskService } from "./session/idexalTaskService.js";
import { IIdexalAgentService } from "./idexal-agent/idexalAgent.js";
import type { CuaOperationStateReporter } from "./idexal-agent/cuaOperationTurnTracker.js";
import { IIdexalSessionService } from "./idexal-session/idexalSession.js";
import {
...
import { createIdexalAgentService } from "./idexal-agent/idexalAgentService.js";
import type { IdexalAgentCommandResolver } from "./idexal-agent/idexalAgentProcessManager.js";
import { buildAgentTelemetrySpawnEnv } from "./idexal-agent/agentTelemetryEnv.js";
import { resolveIdexalAgentPresentationSurface } from "./idexal-agent/idexalAgentPresentationSurface.js";
import { createIdexalTaskServiceAdapter } from "./idexal-agent/idexalTaskServiceAdapter.js";
import { createIdexalSessionService } from "./idexal-session/idexalSessionService.js";
import { createIdexalTaskIndexSyncer } from "./idexal-agent/idexalTaskIndexSyncer.js";
...
import { readLegacyIdexalConfigProviders } from "./model-provider/legacyIdexalConfigProviderReader.js";
...
import { fetchIdexalBuiltinRemoteRelease } from "./model-provider/idexalBuiltinRemoteConfig.js";
import {
  createProviderRuntimeFromConfigRuntime,
...
} from "@idexal/idexal-cua/broker/server";
...
import {
  ...
  isPotentialIdexalCuaAgentMcpServer,
...
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@idexal/idexal-cua/broker/helperConstants";
import { resolveBrokerSocketPath } from "@idexal/idexal-cua/broker/socketPath";
import {
  DEFAULT_IDEXAL_MODEL_CONTEXT_BUDGET_STRATEGY,
  resolveSafeEndpointHostname,
  IDEXAL_JWT_INVALID_BROADCAST_CHANNEL,
  formatLogPrefix,
  ...
  type BrowserCommand,
  isIdexalCuaMcpCommand,
  isIdexalCuaMcpPackageArg,
  isIdexalCuaInternalFeatureEnabled,
  IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  type IdexalAutomation,
  type IdexalAutomationRun,
  getCapturedIdexalAgentTelemetryEnv,
  IDEXAL_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV,
  ZAI_PROVIDER_ID,
  idexalAccountAccessSchema,
  idexalProviderAccountAccessSchema,
  IDEXAL_VERSION,
  IDEXAL_ENV,
  buildRuntimeIdexalApiUrl,
} from "@idexal/shared";

// 这些 conversation-share 实现依赖 Node 文件系统；仅通过 @idexal/services/node 暴露，
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isIdexalCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isIdexalCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialIdexalCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error("Idexal Computer Use is not enabled (plugin off or not product mode).");
      }
...
export const IDEXAL_CUA_BUNDLED_HELPER_APP_PATH_ENV = "IDEXAL_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[IDEXAL_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliIdexalCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const home = env.HOME?.trim() || homedir();
  const configPath = join(home, ".idexal", "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliIdexalCuaServer(name, config),
  );
}

function isGlobalCliIdexalCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @idexal/shared 的单一事实源，避免第三处
  ...
  if (typeof config.command === "string" && isIdexalCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isIdexalCuaMcpPackageArg(arg))
  );
}
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
...
export function createLocalServices(options: {
  ...
  idexalAgentCommandResolver?: IdexalAgentCommandResolver;
  ...
  idexalAgentSpawnFallbackCwd?: string;
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 Idexal Built-in Provider Config 物理路径。 */
  idexalBuiltinProviderConfigFilePath: string;
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 Idexal Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 Idexal Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[IDEXAL_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  const resolveCurrentIdexalEndpointOrigin = async () =>
    resolveRuntimeIdexalEndpointOrigin(process.env, {
      overrideOrigin: (await settingService.get()).idexalEndpointOrigin,
    });
  ...
  const idexalJwtLogoutHandlerRef: {
  ...
  const apiClient = createNodeApiClient({
    ...
    onIdexalJwtInvalid: (input, headers) => idexalJwtLogoutHandlerRef.current?.(input, headers),
    isIdexalJwtRequest: (input, headers) =>
      isCurrentOAuthCredentialRequest({ input, headers, credentialService }),
    resolveIdexalEndpointOrigin: resolveCurrentIdexalEndpointOrigin,
  });
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    idexalBuiltinFilePath: options.idexalBuiltinProviderConfigFilePath,
    idexalBuiltinEnvironment: {
      environmentConfigRoot: resolveAppConfigDir(),
      platform: clientConfigPlatform,
      appVersion: IDEXAL_VERSION,
      resolveEndpointOrigin: resolveCurrentIdexalEndpointOrigin,
      onRefreshResult: (event) => {
        if (event.result === "updated")
          providerConfigLog.info(undefined, "Idexal Built-in CDN 配置已更新", event);
        else providerConfigLog.debug(undefined, "Idexal Built-in 刷新检查", event);
      },
      fetchRelease: (endpointOrigin, signal) =>
        fetchIdexalBuiltinRemoteRelease({
          apiClient,
          endpointOrigin,
          signal,
          appVersion: IDEXAL_VERSION,
    ...
    onIdexalBuiltinRefreshError: (error) => {
      providerConfigLog.warn(undefined, "Idexal Built-in Config 远端刷新失败", { error });
    },
    ...
    // 已发布 config.json 保存的是 Idexal 用户配置；清理第三方 ACP 不能移除这条升级路径。
    // Repository 仅在新 Personal 配置不存在时导入，并保留旧文件以便回滚。
    readLegacyProviders: () => readLegacyIdexalConfigProviders(),
  });
  ...
  let providerConnectivityAgentService:
    | Pick<IIdexalAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 idexalAgentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const mcpSyncService = createMcpSyncService({
    ...
    listMcpServerStatuses: (params) => idexalAgentService.listMcpServerStatuses(params),
  });
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => idexalAgentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 idexalAgentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 idexalAgentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（IDEXAL_CUA_DEV_MODE=1 或
    // IDEXAL_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isIdexalCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isIdexalCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，idexalAgentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@idexal/idexal-cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // IDEXAL_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 IDEXAL_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __IDEXAL_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const home = process.env.IDEXAL_HOME?.trim() || join(homedir(), ".idexal");
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（Idexal 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "Idexal Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "Idexal Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@idexal/idexal-cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "Idexal Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "Idexal Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart Idexal Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const idexalAgentService = createIdexalAgentService({
    ...
    commandResolver: options?.idexalAgentCommandResolver,
    presentationSurface: resolveIdexalAgentPresentationSurface({
    ...
    // IdexalAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.idexalAgentSpawnFallbackCwd,
    ...
    // adapter 的 fetch wrapper。判定实现与 CLI 侧共用 @idexal/shared 的同一份，避免分叉。
    // origin 解析复用 resolveCurrentIdexalEndpointOrigin——与闲时任务同口径（含 settings
    ...
    officialMcpTrustedOrigins: createOfficialMcpTrustedOriginRegistry({
      ...
      resolveIdexalApiOrigin: resolveCurrentIdexalEndpointOrigin,
    }),
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // Idexal 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [IDEXAL_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      const telemetryEnv = getCapturedIdexalAgentTelemetryEnv();
      ...
      return {
        ...
        ...buildAgentEndpointOriginEnv(await resolveCurrentIdexalEndpointOrigin()),
        // broker 凭据（socket/token）注入 agent spawn env，让内置 zcode-cua plugin 的
        // computer-use MCP server 经 __idexal-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          idexalBuiltinFilePath: await providerConfigRuntime.resolveIdexalBuiltinActiveFilePath(),
    ...
    ...(isDesktopAttachedRemote
      ...
      : {
          ...
          resolveSessionRuntimePreferences: async (scope) => {
            ...
            const settings = await settingService.get();
            const modelContextBudgetStrategy = DEFAULT_IDEXAL_MODEL_CONTEXT_BUDGET_STRATEGY;
            return {
  ...
  providerConnectivityAgentService = idexalAgentService;
  ...
  hasActiveTurnRef = () => idexalAgentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 idexalSessionService，绕开 Idexal task adapter 的
  ...
  const idexalTaskIndexSyncer = createIdexalTaskIndexSyncer({
    agentService: idexalAgentService,
    taskIndexRepo,
  });
  ...
  const idexalSessionService = createIdexalSessionService({
    agentService: idexalAgentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await idexalAgentService.generateWorkspaceText({
  ...
  // task wrapper 由 Idexal task service adapter 提供；核心 session 状态由 Idexal agent server 维护。
  const idexalTaskService = createIdexalTaskServiceAdapter({
    idexalAgentService,
    taskIndexRepo,
    taskIndexSyncer: idexalTaskIndexSyncer,
    settingService,
  ...
  const idexalJwtLogoutLogger = createServiceLogger("idexal-jwt-logout");
  idexalJwtLogoutHandlerRef.current = (input, headers) => {
      ...
      .then((invalidated) => {
        ...
        if (invalidated) {
          void broadcastService.send({
            channel: IDEXAL_JWT_INVALID_BROADCAST_CHANNEL,
            payload: {},
          });
        }
      })
      .catch((error) => {
        idexalJwtLogoutLogger.warn("Idexal JWT logout failed", { error });
      });
  };
  ...
  const offPeakCredentialResolverDeps = {
    ...
    resolveAccountProvider: async () => {
      ...
      const staticAccess = idexalProviderAccountAccessSchema.parse(config.access.toJSON());
      const access = await accountRequestAuthService.resolveAccessCurrent(staticAccess);
      if (!access) return null;
      return {
        providerId: provider.providerId,
        access: idexalAccountAccessSchema.parse(access),
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    apiClient,
    baseUrl: buildRuntimeIdexalApiUrl(process.env, "/api/v1"),
    tokenProvider: async (): Promise<string | null> => {
      ...
      return tokenSet?.idexalJwtToken ?? tokenSet?.accessToken ?? null;
    },
  });
  ...
  const services = new ServiceCollection()
    ...
    .register(IIdexalTaskService, idexalTaskService)
    .register(IIdexalAgentService, idexalAgentService)
    .register(IIdexalSessionService, idexalSessionService)
    ...
    .register(
      ...
      createClientConfigService({
        ...
        resolveRequestContext: async () => ({
          endpointOrigin: await resolveCurrentIdexalEndpointOrigin(),
          appVersion: IDEXAL_VERSION,
      ...
      (() => {
        ...
        const offPeakTaskService = new OffPeakTaskService({
          ...
          stopRunningTask: async (params) => {
            await idexalTaskService.stopGeneration({
        ...
        // 回写前向引用，供 idexalAgentService 的 offPeak/create、offPeak/list 协议 handler 调用。
    ...
    .register(IPluginManagementService, createPluginManagementService({ idexalAgentService }))
...
/** 仅给同一事件账号返回当前 Idexal JWT；不缓存、不修改登录凭据。 */
...
): () => Promise<import("@idexal/shared").OAuthLoginAttribution | null> {
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    ...
    services.getOptional(IIdexalTaskService),
    services.getOptional(IIdexalAgentService),
    services.getOptional(IIdexalSessionService),
...
export async function disposeServiceResourcesAndWait(services: ServiceCollection): Promise<void> {
  ...
  // 旧的同步 dispose 会在 host 退出时丢掉强杀 timer，导致 idexal-cli/app-server 变成孤儿进程。
  const disposableServices = [
    services.getOptional(ITerminalService),
    services.getOptional(IIdexalTaskService),
    services.getOptional(IIdexalAgentService),

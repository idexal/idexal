...
export class OAuthService implements IOAuthService {
  ...
  async restoreCachedSessionState(): Promise<OAuthCachedSessionRestoreResult> {
    ...
    // 启动缓存恢复只需要检查共享 idexal JWT；若通过 loadActiveTokenSet 连带读取
    ...
    if (idexalJwtToken && resolveJwtExpiration(idexalJwtToken, this.now()).kind === "expired") {
      serviceLog.info("cached session invalidated because idexal JWT expired", {
  ...
  private async invalidateExpiredCachedSession(
    ...
    const invalidated = await this.runSessionMutation(async () => {
      ...
      const currentJwt = (await this.credentialService.load(IDEXAL_JWT_TOKEN_KEY))?.trim() ?? "";
      if (
  ...
  async startOAuthWithPolling(provider: OAuthProviderId): Promise<OAuthStartResponse> {
    ...
    const initUrl = buildIdexalApiUrlFromEnv(this.env, "/api/v1/oauth/cli/init");
    ...
    if (provider === BIGMODEL_PROVIDER_ID) {
      ...
      // flow 仍由 Host 轮询，但浏览器回调恢复到官网中转页，再透传到 idexal://oauth/callback。
      ...
      // Desktop 统一改写为官网中转页，再由官网透传到 idexal://oauth/callback。
    ...
    this.pendingState = {
      ...
      polling: {
        ...
        pollIntervalMs,
        pollToken,
        pollUrl: buildIdexalApiUrlFromEnv(
          this.env,
  ...
  async pollPendingOAuth(): Promise<OAuthCallbackResult | null> {
    ...
    try {
      const result = await this.runPendingSessionCompletion(pending, async () => {
        ...
        const idexalJwtToken = readTrimmedString(ready.token);
        const userId = readTrimmedString(user?.user_id);
        if (!idexalJwtToken || !providerAccessToken || !userId) {
          throw new Error("OAuth flow 查询响应无效");
        }

...
function getCachedProfileSchemaVersion(profile: OAuthUserProfile): number | null {
  ...
  const version = (rawProfile as { idexalProfileSchemaVersion?: unknown })
    .idexalProfileSchemaVersion;
...
function getCachedProfileMigrationRetryAfter(profile: OAuthUserProfile): number | null {
  ...
  const retryAfter = (rawProfile as { idexalProfileMigrationRetryAfter?: unknown })
    .idexalProfileMigrationRetryAfter;
...
export function withProviderProfileSchema(
  ...
  delete nextRawProfile.idexalProfileMigrationRetryAfter;

  return {
    ...profile,
    rawProfile: {
      ...nextRawProfile,
      idexalProfileSchemaVersion: BIGMODEL_PROFILE_SCHEMA_VERSION,
    },
  };
}
...
function withBigModelProfileMigrationRetryAfter(
  ...
  return {
    ...
    rawProfile: {
      ...
      idexalProfileMigrationRetryAfter: now + BIGMODEL_PROFILE_MIGRATION_RETRY_DELAY_MS,
    },
  };
}
...
export async function refreshLegacyBigModelCachedProfile(
  ...
  try {
    ...
    if (isBigModelUserInfoFallback(refreshedProfile)) {
      // 旧版本可能把 idexal JWT 写进 BigModel access token。

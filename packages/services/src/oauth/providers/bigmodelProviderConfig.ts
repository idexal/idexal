import { BIGMODEL_PROVIDER_ID, buildBigModelApiUrl } from "@idexal/shared";
...
const BIGMODEL_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  userinfoUrl: buildBigModelApiUrl({ IDEXAL_ENV: "production" }, BIGMODEL_USERINFO_PATH),
  appId: "idexal",
  redirectUri: "idexal://oauth/callback",
};
...
export function createBigModelProviderRuntimeConfig(
  ...
  return {
    ...
    tokenUrl:
      ...
      buildIdexalApiUrlFromEnv(env, "/api/v1/oauth/token"),
    ...
    // 当前 BigModel callback 只消费 idexal OAuth token 路由，显式 appSecret 仅保留给

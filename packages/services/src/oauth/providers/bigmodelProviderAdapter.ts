...
interface BigModelIdexalTokenEnvelope {
  code?: number;
  msg?: string;
  data?: {
    token?: string | null;
...
export class BigModelProviderAdapter implements OAuthProviderAdapter {
  ...
  async exchangeToken(
    ...
    const tokenSet = await this.exchangeIdexalJwtToken(params, _context);
  ...
  private async exchangeIdexalJwtToken(
    params: OAuthCallbackParams,
    context: OAuthProviderContext,
  ): Promise<OAuthTokenSet> {
    log.info(undefined, "idexal token request", {
      method: "POST",
    ...
    try {
      const payload = await readApiJson<BigModelIdexalTokenEnvelope>(
        this.apiClient,
        this.config.tokenUrl,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          // idexal JWT 的后端 token 路由按 OAuth callback 授权码语义解析。
      ...
      if (payload.code !== undefined && payload.code !== 0) {
        // BigModel 的一次性 authCode 现在只交给 idexal token 路由。
        // 如果这里失败，不能继续保存半登录态，否则 Start Plan 仍会显示未连接。
        log.warn(undefined, "idexal token business response rejected", {
          code: payload.code,
          msg: payload.msg,
        });
        throw new Error(
          payload.msg?.trim() || `BigModel idexal token 交换失败（code: ${payload.code}）`,
        );
      }
      const idexalJwtToken = payload.data?.token?.trim() || "";
      if (!idexalJwtToken) {
        log.warn(undefined, "idexal token response missing data.token", {
          code: payload.code,
          msg: payload.msg,
        });
        throw new Error("BigModel idexal token 交换失败：响应缺少 data.token");
      }
      const accessToken = resolveBigModelBusinessAccessToken(payload);
      if (!accessToken) {
        // Coding Plan 付费套餐仍调用 bigmodel.cn 业务接口，只能使用
        // BigModel 业务 access token；idexal JWT 只能写入 zcodejwttoken 给 Start Plan 使用。
        // 如果继续把 idexal JWT 写进 oauth:bigmodel:access_token，套餐预览会稳定报“令牌已过期”。
        log.warn(undefined, "idexal token response missing bigmodel access token", {
          code: payload.code,
          msg: payload.msg,
        });
        throw new Error("BigModel idexal token 交换失败：响应缺少 data.bigmodel.access_token");
      }
      ...
      // tokenByAuthCode 消费它；idexal token 交换失败时直接中止登录并记录链路头。
      log.warn(undefined, "idexal token request failed", {
  ...
  async fetchUserInfo(
    ...
    if (tokenSet.idexalJwtToken && tokenSet.accessToken === tokenSet.idexalJwtToken) {
      // 移除 tokenByAuthCode 后 callback 阶段没有 BigModel access token。
      // idexal JWT 不能调用 bigmodel.cn 的 customer 接口，避免无意义的鉴权失败请求。
      return {
        id: "unknown",
        username: "user",
        displayName: "User",
...
function resolveBigModelBusinessAccessToken(payload: BigModelIdexalTokenEnvelope): string {
  return (

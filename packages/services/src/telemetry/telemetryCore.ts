...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["idexal:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".idexal", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".idexal", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? IDEXAL_VERSION;
  ...
  async function sendReport(
    ...
    if (!IDEXAL_TELEMETRY_ENABLED || !IDEXAL_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteIdexalEndpointUrl(
        IDEXAL_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveIdexalEndpointOrigin?.()) ?? IDEXAL_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildIdexalSourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? IDEXAL_ENV,

...
import { redactFeedbackText, IDEXAL_VERSION, IDEXAL_COMMIT } from "@idexal/shared";
...
export async function createFeedbackDiagnosticArchive(options: {
  ...
  const path = join(outputDir, "idexal-diagnostic-logs.zip");
  ...
  try {
    ...
    zip.addBuffer(
      Buffer.from(
        [
          "Idexal diagnostic logs",
          `timestamp: ${now.toISOString()}`,
          `appVersion: ${IDEXAL_VERSION}`,
          `commit: ${IDEXAL_COMMIT}`,

...
import type { ApiClient, FeedbackDeviceInfo } from "@idexal/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.IDEXAL_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeIdexalApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: IDEXAL_COMMIT,
    buildTime: IDEXAL_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getIdexalJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(IDEXAL_JWT_TOKEN_KEY))?.trim() || undefined;
  }

  async function hasIdexalJwtToken(): Promise<boolean> {
    return Boolean(await getIdexalJwtToken());
  }
  ...
  const httpClient = new FeedbackHttpClient({
    ...
    getAuthHeaders: async () => {
      ...
      const jwtToken = await getIdexalJwtToken();
      if (jwtToken) {

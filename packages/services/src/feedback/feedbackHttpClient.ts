import { redactFeedbackText } from "@idexal/shared";
...
function toFeedbackEnvironment(input: CreateFeedbackTicketInput): Record<string, unknown> {
  ...
  assignDefined(environment, "release_channel", process.env.IDEXAL_ENV?.trim() || "stable");
...
async function uploadOssForm(
  ...
  return new Promise((resolve, reject) => {
    ...
    const boundary = `----idexal-feedback-${randomUUID()}`;

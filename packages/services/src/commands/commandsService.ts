...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = IDEXAL_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = IDEXAL_COMMAND_AGENT_SOURCES;
...
const IDEXAL_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "idexalAgent",
  directorySource: "idexal",
  userDirectorySegments: [".idexal", "commands"],
  workspaceDirectorySegments: [".idexal", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // Idexal Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .idexal 抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === IDEXAL_COMMAND_AGENT_SOURCE
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === IDEXAL_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as IdexalCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(IDEXAL_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        IDEXAL_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, IDEXAL_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.idexal` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "idexal" && discoveredCount > 0) {
      break;
    }
  }
}

...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@idexal/shared";

let _dataBaseDir: string | null = null;
export const IDEXAL_WINDOWS_APP_INSTALL_DIR_ENV = "IDEXAL_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.IDEXAL_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env IDEXAL_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.idexal */
export function getIdexalDataRootDir(): string {
  return join(getDataBaseDir(), ".idexal");
}

/** 非项目对话共享的真实工作目录；默认 ~/.idexal/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getIdexalDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.idexal/v2 */
export function getAppConfigDir(): string {
  return join(getIdexalDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, IDEXAL_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "Idexal") : null,
    programFilesX86 ? win32.join(programFilesX86, "Idexal") : null,
    programW6432 ? win32.join(programW6432, "Idexal") : null,
    localAppData ? win32.join(localAppData, "Programs", "Idexal") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getIdexalDataRootDir(), "export-log-stage");
}

export function getExportLogDir(): string {
  return join(getIdexalDataRootDir(), "export-log");
}

export function getFeedbackRootDir(): string {
  return join(getIdexalDataRootDir(), "feedback");
}
...
export function getGitCheckpointIndexRootDir(): string {
  return join(getIdexalDataRootDir(), "git-checkpoint-index");
}

/** ~/.idexal/v2/tasks-index.sqlite */
...
/** 与 Idexal session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** ~/.idexal/v2/sessions/{workspaceHash} */
...
/** ~/.idexal/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.idexal/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .idexal/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".idexal", "v2");
  const newDir = join(newBaseDir, ".idexal", "v2");

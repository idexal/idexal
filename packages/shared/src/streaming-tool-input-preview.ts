export interface IdexalStreamingToolInputState {
  deltaCount?: number;
...
export interface IdexalStreamingToolInputPreview {
  complete: boolean;
  input: unknown;
  rawInput: string;
}

export type IdexalStreamingToolInputPreviewMode = "active-live" | "background-summary";

export const IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_EAGER_DELTA_COUNT = 1;
export const IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS = 750;
export const IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH = 8 * 1024;
export const IDEXAL_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS = 1_000;
export const IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_TIME_BUDGET_MAX_RAW_INPUT =
  IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH;
...
export function appendIdexalStreamingToolInputDelta(
  state: IdexalStreamingToolInputState | undefined,
  delta: string,
): IdexalStreamingToolInputState {
  return {
    ...state,
...
export function buildIdexalStreamingToolInputPreview(
  rawInput: string,
  completeInput?: unknown,
): IdexalStreamingToolInputPreview {
...
export function shouldMaterializeIdexalStreamingToolInputPreview(
  state: IdexalStreamingToolInputState,
  options: {
    mode?: IdexalStreamingToolInputPreviewMode;
    now?: number;
    toolName?: string;
  } = {},
): boolean {
  ...
  if (deltaCount <= IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_EAGER_DELTA_COUNT) {
    return true;
  }
  const lastPreviewAt = state.lastPreviewAt ?? 0;
  if (isIdexalFileStreamingToolInputPreviewTool(options.toolName)) {
    // 性能修复：Write/Edit 的半截 JSON 会触发全量内容恢复和行级 diff。
    // 大字节 chunk 不能绕过一秒窗口，否则模型输出越快，UI 反而更新越频繁。
    return (
      (options.now ?? Date.now()) - lastPreviewAt >=
      IDEXAL_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    );
  }
  ...
  if (rawGrowth >= IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH) {
    return true;
  }
  const intervalElapsed =
    (options.now ?? Date.now()) - lastPreviewAt >=
    IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS;
  if (!intervalElapsed) {
    return false;
  }
  // 性能修复：大 Write/Edit 参数通常会被 provider 以 4KB 左右的慢速 chunk 推送。
  // 若只按时间预算，active 任务仍会每个 chunk 解析累计 JSON；超过小输入范围后改由 raw growth 控制。
  return state.rawInput.length <= IDEXAL_ACTIVE_STREAMING_TOOL_INPUT_TIME_BUDGET_MAX_RAW_INPUT;
}

export function isIdexalFileStreamingToolInputPreviewTool(toolName?: string): boolean {
  const normalized = toolName?.trim().toLowerCase();
  return normalized === "write" || normalized === "edit";
}

export function markIdexalStreamingToolInputPreviewMaterialized(
  state: IdexalStreamingToolInputState,
  now = Date.now(),
): void {

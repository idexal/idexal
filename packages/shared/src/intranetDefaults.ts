...
export const INTRANET_ASSET_BASE_URL = INTRANET_MACHINE_HOST
  ? `http://${INTRANET_MACHINE_HOST}:${INTRANET_ASSET_SERVICE_PORT}/idexal`
  : "";
...
export function resolveIntranetDepsBaseUrl(env: IntranetEnv = readProcessEnv()) {
  const depsBaseUrl = env.IDEXAL_DEPS_BASE_URL?.trim();
  if (depsBaseUrl) {
  ...
  if (!host)
    throw new Error(
      "Configure IDEXAL_DEPS_BASE_URL or INTRANET_MACHINE_HOST in .env before downloading internal dependencies",
    );
  return `http://${host}:${INTRANET_ASSET_SERVICE_PORT}/idexal/deps`;
}

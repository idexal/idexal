/**
 * MCP (Model Context Protocol) types for Idexal
 * Based on the original Tauri implementation
 */

import type { SettingsDirectoryLocation } from "./settings-source.js";
import type { McpServerFailureKind } from "./idexal-protocol/index.js";

// CUA official plugin 身份常量（port 自 feat；UI 设置面板 + bootstrap 复用以避免字面量漂移）。
export const IDEXAL_CUA_OFFICIAL_PLUGIN_ID = "computer-use@zcode-plugins-official";
// CUA server 身份串（port 自 feat mcp.ts）：server key = 模型可见工具前缀段（刻意不带 idexal-）；
// namespace name = official plugin 运行时命名空间 plugin:<pluginId>:<serverKey>。
export const IDEXAL_CUA_OFFICIAL_MCP_NAMESPACE_NAME = "plugin:computer-use:computer-use";
...
export const IDEXAL_PLUGIN_ID_ENV_KEY = "IDEXAL_PLUGIN_ID";

export type McpSource = "mcp" | "idexalagentmcp";
...
export interface McpConfig {
  ...
  idexalagentmcp: CliMcpConfig;
}

export interface IdexalMcpServer {
  id: string;
  name: string;
  config: McpServerConfig;
  enabled: boolean;
...
export type IdexalAgentMcpServer =
  | {
      name: string;
      command: string;
      args: string[];
...
// zcode-cua MCP server 识别的单一事实源。desktop 产品 broker resolver（@idexal/services 的
// mcpBrokerInjection）与 CLI bootstrap（apps/idexal-cli 的 mcp-config）两条注入入口必须用
// 完全一致的判定；否则同一 MCP 配置在不同入口行为不同，可能漏注入 product broker，让
// Python/uvx 自己持有 macOS TCC 权限（违反 fail-closed 边界）。改这里即同时改两条链路。
function idexalCuaArgLeaf(value: string): string {
...
// 单个候选串是否为 zcode-cua 的包规格。PyPI 视 `_`/`-` 等价，故先把下划线归一成短横（idexal_cua →
// zcode-cua）；覆盖 uv/npm 的 `@version`、pip 的 `==version`、extras `[...]`、git 的 `.git`/`.git@`，
// 以及 `python -m idexal_cua.server` 这种点号子模块（`zcode-cua.<submodule>`）。fail-closed 边界宁可
// 过判也不漏判；仍不会误判 `zcode-cua-proxy`（短横续接，不以 `.`/`@`/`[`/`==` 边界续接）。
function matchesIdexalCuaSpec(candidate: string): boolean {
 ...
 */
export function isIdexalCuaMcpCommand(command: string): boolean {
  return matchesIdexalCuaSpec(command) || matchesIdexalCuaSpec(idexalCuaArgLeaf(command));
}

/**
 * 单个 arg 是否为 zcode-cua 的包规格。覆盖 `zcode-cua`、`zcode-cua[macos]`、`zcode-cua@1.2.3`、
 * `zcode-cua==1.2.3`、`idexal_cua`，以及 git / 本地路径形态（`.../zcode-cua`、`zcode-cua.git`、
 * `git+https://.../zcode-cua.git@v1`）。同时比对原始值与路径叶子，覆盖 `--from <path>`、`--from <git-url>`。
 */
export function isIdexalCuaMcpPackageArg(value: string): boolean {
  return matchesIdexalCuaSpec(value) || matchesIdexalCuaSpec(idexalCuaArgLeaf(value));
}

export function convertToIdexalAgentMcpServer(
  name: string,
  config: McpServerConfig,
): IdexalAgentMcpServer | null {
  ...
  if (isStdio && config.command) {
    ...
    if (isWin32) {
      ...
      if ((lowerCmd === "cmd" || lowerCmd === "cmd.exe") && args[0] === "/c" && unwrappedCommand) {
        ...
        // 先显式取值并判空，既满足类型收窄，也避免把空命令传给 Idexal Agent。

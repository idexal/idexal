import type {
  ...
  TraceId,
  IdexalStreamEvent,
  IdexalTaskNetworkDebugStatusType,
} from "./idexal-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<IdexalTaskNetworkDebugStatusType>([
...
export function idexalTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<IdexalStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): IdexalTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as IdexalTaskNetworkDebugStatusType)
    : undefined;
}

...
export const IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID = "zcode-plugins-official";
...
export const DEFAULT_PLUGIN_MARKETPLACES: DefaultPluginMarketplace[] = [
  {
    // Idexal 官方唯一市场：本地 seed 分片与 CDN 分片在 Agent storage 内合并。
    // CDN manifest 的 name 必须与该 canonical id 一致。
    id: IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID,
    source: "https://cdn-zcode.z.ai/zcode/official-plugin/marketplace.json",
    name: IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID,
    description: "Official Idexal plugins marketplace: built-in and community plugins for Idexal.",
    pluginCount: 0,
  },
];

// 商店「公开」分段只有一个 Idexal 官方市场 id，内置与 CDN 不再拆分身份。
export const PUBLIC_STORE_MARKETPLACE_IDS = [IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID] as const;

export const IDEXAL_KNOWN_TOOL_NAMES = [
  "Read",
  "Write",
  "Edit",
  "ApplyPatch",
...
export type IdexalKnownToolName = (typeof IDEXAL_KNOWN_TOOL_NAMES)[number];

export type IdexalToolFamily =
  | "file-read"
  | "file-write"
  | "shell"
  | "search"
...
const TOOL_FAMILY_BY_NAME: Record<IdexalKnownToolName, IdexalToolFamily> = {
  Read: "file-read",
  Write: "file-write",
  Edit: "file-write",
...
const TOOL_NAME_BY_LOWER = new Map<string, IdexalKnownToolName>(
  IDEXAL_KNOWN_TOOL_NAMES.map((toolName) => [toolName.toLowerCase(), toolName]),
);

export function normalizeIdexalToolName(
  value: string | null | undefined,
): IdexalKnownToolName | null {
...
export function getIdexalToolFamilyForName(
  value: string | null | undefined,
): IdexalToolFamily | null {
  const toolName = normalizeIdexalToolName(value);
  return toolName ? TOOL_FAMILY_BY_NAME[toolName] : null;
}

export function isIdexalToolFamily(
  value: string | null | undefined,
  family: IdexalToolFamily,
): boolean {
  return getIdexalToolFamilyForName(value) === family;
}

export function isIdexalFileContentWriteToolName(value: string | null | undefined): boolean {
  return normalizeIdexalToolName(value) === "Write";
}

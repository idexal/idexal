...
// Idexal runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveIdexalAssistants(
  messages: readonly IdexalPersistedMessage[],
): IdexalPersistedMessage[] {
  const merged: IdexalPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: IdexalPersistedMessage,
  next: IdexalPersistedMessage,
): IdexalPersistedMessage {
  ...
  const shiftedNextParts: IdexalPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"

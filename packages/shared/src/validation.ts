...
import { z } from "zod";
import { idexalProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { idexalProviderSchema } from "./providers.js";
import { idexalAgentProviderSchema } from "./idexal-agent-policy.js";
...
} from "./idexal-protocol/index.js";
import { idexalTaskModeSchema } from "./idexal-task-mode-schema.js";
import { PROTOCOL_V4_LIMITS } from "./idexal-protocol-v4/core.js";
import { errorAttributionSchema } from "./idexal-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./idexal-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { idexalTaskModeSchema } from "./idexal-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("idexal-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  idexalBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 IdexalTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { idexalProviderSchema };

export const idexalTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: idexalProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = idexalProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `idexalProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: idexalMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: idexalToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: idexalMcpTelemetryEventSchema,
  })
  .strict();
...
export const idexalTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const idexalPromptImageAttachmentSchema = z.object({
...
export const idexalPromptVideoAttachmentSchema = z.object({
...
export const idexalPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const idexalPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const idexalPromptAttachmentSchema = z.discriminatedUnion("kind", [
  idexalPromptImageAttachmentSchema,
  idexalPromptVideoAttachmentSchema,
  idexalPromptPdfAttachmentSchema,
  idexalPromptFileAttachmentSchema,
]);

export const idexalPersistedToolCallSchema = z.object({
...
const idexalPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const idexalPersistedMessageSchema = z.object({
  ...
  attachments: z.array(idexalPromptAttachmentSchema).optional(),
  tools: z.array(idexalPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(idexalPersistedMessagePartSchema).optional(),
...
export const idexalTaskGoalStatusSchema = z.enum([
  "active",
  "paused",
  "budget_limited",
  "complete",
...
export const idexalTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const idexalTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const idexalTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: idexalTaskGoalStatusSchema,
...
export const idexalTaskGoalChangedPatchSchema = z.object({
  action: idexalTaskTargetChangedActionSchema,
  source: idexalTaskTargetChangedSourceSchema,
  target: idexalTaskGoalSchema.nullable(),
  previousTarget: idexalTaskGoalSchema.nullable().optional(),
});

export const idexalTaskTargetStatusSchema = idexalTaskGoalStatusSchema;
export const idexalTaskTargetSchema = idexalTaskGoalSchema;
export const idexalTaskTargetChangedPatchSchema = idexalTaskGoalChangedPatchSchema;

export const idexalTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: idexalTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: idexalAgentProviderSchema.optional(),
  migrationSource: idexalTaskMigrationSourceSchema.optional(),
  ...
  status: idexalTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: idexalTaskGoalSchema.nullable().optional(),
});

export const idexalTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const idexalPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(idexalTaskIndexEntrySchema),
});

const idexalPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const idexalPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(idexalPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const idexalPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const idexalSessionFileSchema = z.object({
  meta: idexalTaskMetaSchema,
  messages: z.array(idexalPersistedMessageSchema),
  fileChanges: z.array(idexalPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(idexalPersistedTurnCheckpointSchema).optional(),
});

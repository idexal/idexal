...
// idexal-task-types-core.ts。本文件与旧 realtime 总线组同生命周期。
import type {
...
import type { IdexalPermissionResponse } from "./idexal-protocol-legacy-types.js";
import type { WorkspaceHookReviewDecision } from "./idexal-protocol-v4/workspace-hook-review.js";
...
export type TaskOwnerCommandRequest =
  ...
  | {
      ...
      taskId: string;
      runId: string;
      taskCommand: Extract<IdexalTaskRuntimeCommand, { type: "send_prompt" }>;
    }
  | {
      commandRequestId: string;
      ...
      runId: string;
      commandId: string;
      clientMode: Extract<IdexalTaskClientMode, "web-remote-replayable">;
    }
  | {
      commandRequestId: string;
...
export type TaskOwnerCommandResult =
  | { commandRequestId: string; success: true; taskCommand?: IdexalTaskRuntimeCommand }
  | {
      commandRequestId: string;
      success: false;
      error: string;

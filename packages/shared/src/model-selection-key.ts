...
export function buildNativeSupplierKey(idexalProvider: IdexalProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${idexalProvider}`;
}
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(idexalProvider);
}

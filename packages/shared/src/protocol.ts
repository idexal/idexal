...
export interface ResourceUsageProcess {
  pid: number;
  /** 进程显示名，如 idexal-main / idexal-agent-idexal-demo / node_repl */
  name: string;
...
export interface AppSettings {
   ...
   * 只影响内置浏览器出口，不影响 Idexal 自身对后端与模型 API 的请求。默认关闭，重启后生效。
   */
  ...
  /** Idexal 运行中继续输入时，是排队到下一轮，还是引导到下一次工具调用后运行 */
  ...
  /** 数据目录的根路径（替代 homedir），默认为 os.homedir()；.idexal/v2 后缀不变 */
  dataBaseDir?: string;
  ...
  /** 设置页里的临时 endpoint override；正式/测试默认 base url 由 IDEXAL_BASE_URL env 管理。 */

...
// 禁止往 IdexalAutomation 上加字段。sqlite 列名 snake_case，此处为跨域 camelCase 领域类型。
 ...
 */
export type IdexalOffPeakTaskStatus =
  | "queued"
  | "paused"
  | "running"
  | "completed"
...
export function isOffPeakTerminalStatus(
  status: IdexalOffPeakTaskStatus,
 ...
 * 稳定错误标记。idexal-cli 适配层把该业务码分类为不可重试失败并在错误消息里嵌入本标记；
 * host 终态回写据此改走"同 task_id 重新取号 → resume 续跑"而非落 failed。
 * 跨进程只能靠错误文本传递，标记必须全链路唯一且稳定，勿改；与
 * apps/idexal-cli/packages/adapters/src/model/offpeak-retry.ts 的同名常量跨包同值。
 */
...
export interface IdexalOffPeakTask {
  ...
  /** 表单 Instructions。 */
  prompt: string;
  /** 权限四档全开放，映射现有 IdexalTaskMode，默认 "default"（Ask for approval）。 */
  permissionMode: IdexalTaskMode;
  /**
...
export interface IdexalOffPeakTaskCreateParams {
  title: string;
  prompt: string;
  permissionMode: IdexalTaskMode;
  modelSelection: ModelSelection;

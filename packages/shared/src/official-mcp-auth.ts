/* Idexal 官方 Server MCP 鉴权的共享常量与类型。
   放在 shared 是因为头集合有两个消费者且分属不同包：
   - `packages/services` 侧生产身份头；
   - `apps/idexal-cli/packages/adapters` 侧（Plugin parser + MCP adapter）拦截保留头。
   两侧必须同源，否则新增身份头时会漏掉黑名单，出现静态 header 覆盖凭证的缺口。 */

/** `.mcp.json` 中 `auth.type` 的唯一合法值；区分大小写，不接受别名。 */
export const IDEXAL_OFFICIAL_MCP_AUTH_TYPE = "zcode_official" as const;

/** 第一阶段唯一合法的 provider。后续新增短期 Token 应新增 provider 值，不改变本值语义。 */
export const IDEXAL_OFFICIAL_MCP_AUTH_PROVIDER_JWT_TOKEN = "jwt_token" as const;
 ...
 * 命名空间前缀沿用 `com.idexal/`，与既有的 `com.idexal/request-context` 一致。
 */
export const OFFICIAL_MCP_AUTH_META_KEY = "com.idexal/official-mcp-auth" as const;
...
/** 服务端 `Bigmodel-Target-Type` 的取值（对齐 idexal-server 的 CodingPlanTargetType）。 */
...
//   - apps/idexal-cli/packages/adapters：请求发出前的本地校验；
//   - packages/services（host）：身份权威边界的二次校验（只依赖 @idexal/shared，
//     无法 import CLI 侧包）。
...
export const OFFICIAL_MCP_DEV_TRUSTED_ORIGINS_ENV = "IDEXAL_OFFICIAL_MCP_DEV_TRUSTED_ORIGINS";

/** Host 在 spawn 时注入的真实 workspace identity；只用于隔离/审计，不用于文件执行。 */
export const IDEXAL_WORKSPACE_IDENTITY_ENV = "IDEXAL_WORKSPACE_IDENTITY";
...
export interface OfficialMcpTrustResult {
  ...
  detail: "ok" | "invalid_input" | "origin_mismatch" | "idexal_origin_unresolved";
}
...
export interface IsOfficialMcpOriginTrustedInput {
   ...
   */
  pluginId: string;
  /** 当前 Idexal API origin，由调用方按各自口径解析后传入。 */
  idexalApiOrigin: string | undefined;
}

/**
 * 校验官方 MCP 的凭据目标：要求 HTTPS origin 与运行时 Idexal API origin 相等，
...
export function isOfficialMcpOriginTrusted(
  ...
  const expected = input.idexalApiOrigin ? normalizeHttpsOrigin(input.idexalApiOrigin) : undefined;
  if (!expected) return { detail: "idexal_origin_unresolved", trusted: false };
...
export interface CreateOfficialMcpTrustedOriginRegistryOptions {
  ...
  devTrustedOriginsRaw?: string | undefined;
  /** 当前 Idexal API origin 的解析器；两侧必须用等价口径，否则会一侧放行一侧拒绝。 */
  resolveIdexalApiOrigin: () => string | undefined | Promise<string | undefined>;
}
...
export function createOfficialMcpTrustedOriginRegistry(
  ...
  return {
    async isTrusted({ origin, pluginId }) {
      let idexalApiOrigin: string | undefined;
      try {
        idexalApiOrigin = await options.resolveIdexalApiOrigin();
      } catch {
        // 解析失败按不可信处理，绝不因为拿不到 origin 就放行。
        return { detail: "idexal_origin_unresolved", trusted: false };
      }

import type { IdexalApiRetryStatus } from "./idexal-task-types-core.js";

export function normalizeIdexalApiRetryStatus(
  value: unknown,
...
export function idexalApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): IdexalApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeIdexalApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = idexalApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isIdexalModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function idexalApiRetryFromStreamRecoveryPayload(
  value: unknown,
): IdexalApiRetryStatus | undefined {
  const record = asRecord(value);

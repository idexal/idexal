...
import { IDEXAL_AGENT_PROVIDER, IDEXAL_AGENT_PROVIDER_LABEL } from "./idexal-agent-policy.js";
import type {
  IdexalConfigOption,
...
import {
  ...
  type IdexalInteractionRequestOrigin,
  type IdexalPermissionResponse,
 ...
 */
export const idexalAutomationBotDeliveryTargetSchema = z
  .object({
...
export type IdexalAutomationBotDeliveryTarget = z.infer<
  typeof idexalAutomationBotDeliveryTargetSchema
>;
...
export interface BotTaskSummary {
  taskId: string;
  title: string;
  status: IdexalTaskRuntimeStatus | "persisted-completed" | "persisted-error" | "unknown";
  workspacePath: string;
  workspaceIdentity?: string;
  provider?: IdexalProvider;
  model?: string;
...
export interface BotTaskBroadcastPayload {
  ...
  permissionRequest?: IdexalPermissionRequest;
  elicitationRequest?: IdexalElicitationRequest;
  requestId?: string;
  error?: string;
}
...
export const botCurrentOptionsSchema = z
  .object({
    ...
    // 兼容旧 bot-config.json；CLI provider 现在统一由 Idexal Protocol 侧配置决定。
    cli: z.literal(IDEXAL_AGENT_PROVIDER).optional(),
  })
  .strict();

export const botDraftOptionsSchema = z
  .object({
    provider: z.literal(IDEXAL_AGENT_PROVIDER),
    modelSelection: modelSelectionSchema.optional(),
    mode: z.string().min(1).optional(),
  })
  .strict();
...
export const BOT_IDEXAL_PROVIDER_OPTIONS: Array<{
  id: IdexalProvider;
  label: string;
}> = [{ id: IDEXAL_AGENT_PROVIDER, label: IDEXAL_AGENT_PROVIDER_LABEL }];

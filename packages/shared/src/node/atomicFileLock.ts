import { IDEXAL_FILE_LOCK_TIMEOUT_ERROR_CODE } from "../errors.js";
...
function createFileLockTimeoutError(
  ...
  const error = new Error(
    `Timed out after ${waitedMs}ms waiting for the Idexal file lock: ${lockFile}`,
  ) as NodeJS.ErrnoException & { cause?: unknown };
  error.code = IDEXAL_FILE_LOCK_TIMEOUT_ERROR_CODE;
  error.path = filePath;
  error.syscall = "mkdir";
  error.cause = cause;
  return error;
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      ...
      if (
        ...
        throw Object.assign(new Error("Idexal file lock ownership changed during acquire"), {
          code: "EEXIST",
        });
      }
      return async () => {

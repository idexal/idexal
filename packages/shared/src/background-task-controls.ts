export type IdexalBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface IdexalBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: IdexalBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<IdexalBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = IdexalBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveIdexalBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): IdexalBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // Idexal Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseIdexalBackgroundTaskControlItems(
  value: unknown,
): IdexalBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, IdexalBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveIdexalBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: IdexalBackgroundTaskControlItem = {
      jobId,
...
export function isActiveIdexalBackgroundTaskControlItem(
  job: IdexalBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getIdexalBackgroundTaskControlItemElapsedMs(
  job: IdexalBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleIdexalBackgroundTaskControlItems(
  jobs: readonly IdexalBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<IdexalBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveIdexalBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getIdexalBackgroundTaskControlItemElapsedMs(job, now),
    }))

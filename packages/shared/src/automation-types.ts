import type { IdexalTaskMode } from "./idexal-task-types-core.js";
import type { IdexalAutomationBotDeliveryTarget } from "./bots.js";
...
/** 整条 automation 的生命周期。 */
export type IdexalAutomationLifecycleStatus = "active" | "completed" | "failed" | "paused";

/** 单条 automation 当前一轮的派发信息态（供 UI/诊断）。 */
export type IdexalAutomationDispatchStatus =
  | "idle"
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch";
...
export type IdexalAutomationLocationKind = "local" | "remote";

/** 触发来源：定时 or 立即运行。 */
export type IdexalAutomationTrigger = "schedule" | "manual";

/** 自定义重复规则；cronExpr 保留为兼容展示，调度以本字段为权威。 */
export interface IdexalAutomationScheduleRule {
 ...
 */
export type IdexalAutomationIntervalUnit = IdexalAutomationScheduleRule["unit"];

/** 单次 run 的派发结果（scheduler 权威，驱动重试）。skipped=错过触发窗口。 */
export type IdexalAutomationRunDispatchStatus =
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch"
  | "skipped";
...
export type IdexalAutomationRunOutcome = "running" | "succeeded" | "failed" | "stopped";

/** 一条定时任务定义 + 调度状态。 */
export interface IdexalAutomation {
  automationId: string;
  title: string;
  /** 5 段 cron，本地时区。 */
  cronExpr: string;
  ...
  targetTaskId?: string;
  locationKind: IdexalAutomationLocationKind;
  ...
  nextRunAt?: number;
  lastRunAt?: number;
  dispatchStatus: IdexalAutomationDispatchStatus;
...
export interface IdexalAutomationRunIdParts {
  automationId: string;
...
/**
 * runId 契约（见 IdexalAutomationRun.runId）的唯一解析入口；desktop host/scheduler 与
 * CLI telemetry 不各自按字符串猜格式。不符合两种声明格式的输入一律返回 null。
 */
export function parseAutomationRunId(runId: string): IdexalAutomationRunIdParts | null {
...
export interface IdexalAutomationRun {
...
export interface IdexalAutomationCreateParams {
  title: string;
  cronExpr: string;
   ...
   */
  intervalUnit?: IdexalAutomationIntervalUnit;
  /** 1–200 的整数间隔，必须与 intervalUnit 配对。 */
  interval?: number;
}

/** 编辑 automation 的可变字段。 */
export interface IdexalAutomationUpdateParams {
  title?: string;
  cronExpr?: string;
  prompt?: string;

 ...
 * 见 idexal-server `internal/domain/servermcp/toolerror.go` 的 `ToolError.Error()`。
 *
 * 放在 shared 是因为有三个分属不同包的消费者：
 * - `apps/idexal-cli/packages/core`：解析 MCP 结果，把 code 带进 tool result display；
 * - `packages/shared/src/idexal-protocol-v4/rows.ts`：row schema 校验该 code；

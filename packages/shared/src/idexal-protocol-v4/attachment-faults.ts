...
// 透传到 JSON-RPC `error.data.code`；客户端读回时用 readIdexalAttachmentFaultCode。

export const IDEXAL_ATTACHMENT_FAULT_CODES = {
  /** host 未实现 stat 能力。 */
...
export type IdexalAttachmentFaultCode =
  (typeof IDEXAL_ATTACHMENT_FAULT_CODES)[keyof typeof IDEXAL_ATTACHMENT_FAULT_CODES];

const KNOWN_FAULT_CODES = new Set<string>(Object.values(IDEXAL_ATTACHMENT_FAULT_CODES));

export function isIdexalAttachmentFaultCode(value: unknown): value is IdexalAttachmentFaultCode {
 ...
 */
export class IdexalAttachmentFaultError extends Error {
  readonly code: IdexalAttachmentFaultCode;

  constructor(code: IdexalAttachmentFaultCode, options?: { cause?: unknown; message?: string }) {
    // message 默认就是 fault 码本身，旧版本客户端的文本匹配仍能命中。
    super(
      options?.message ?? code,
      options?.cause === undefined ? undefined : { cause: options.cause },
    );
    this.name = "IdexalAttachmentFaultError";
    this.code = code;
  }
}
...
/**
 * 从任意错误中读出附件 fault 码：
 * - 同进程抛出的 IdexalAttachmentFaultError / 带 code 的 Error；
 * - 跨 JSON-RPC 回传的 `error.data.code`；
 * - 旧版本 CLI 只有消息文本时的兼容兜底（见下方注释）。
 */
export function readIdexalAttachmentFaultCode(
  error: unknown,
): IdexalAttachmentFaultCode | undefined {
  if (!error || typeof error !== "object") return undefined;
  const candidate = error as { code?: unknown; data?: unknown };
  if (isIdexalAttachmentFaultCode(candidate.code)) return candidate.code;
  if (candidate.data && typeof candidate.data === "object") {
    const data = candidate.data as { code?: unknown };
    if (isIdexalAttachmentFaultCode(data.code)) return data.code;
  }
  // 兼容兜底：桌面端可能连接尚未带结构化 code 的旧 idexal-cli。仅在消息「整体等于」
  // 某个 fault 码时命中，不做模糊匹配，避免把包装后的任意文本误判成确定分类。
  if (error instanceof Error && isIdexalAttachmentFaultCode(error.message)) return error.message;
  return undefined;
}

...
// 下发，host 侧 idexalTaskIndexSyncer 转成 workspace_config_options_update 广播给 UI；
...
// 与 host 侧 IdexalConfigSelectValue（idexal-task-types-core）结构对齐：
// syncer 转发 workspace_config_options_update 时零映射直通，下游 useIdexalConfig 消费面不改。
...
// 与 host 侧 IdexalConfigOption 结构对齐（id: model / mode / thought_level / 自定义）。
...
// slash 命令目录（workspace 级；与 host 侧 IdexalSlashCommand 对齐）。

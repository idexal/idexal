...
// 消费者：idexalTaskServiceAdapter/idexalSessionProjection/idexalTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromIdexalMessageParts 已迁
// idexal-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { IdexalSessionGoal } from "./idexal-protocol/index.js";
...
export function isIdexalGoalContinuationReminderText(text: string): boolean {
...
export function isIdexalGoalContinuationReminderMessage(message: IdexalMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isIdexalGoalContinuationReminderText(textFromIdexalMessageParts(message.parts)))
  );
}

export function isIdexalGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isIdexalGoalModelOnlyReminderMessage(message: IdexalMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isIdexalGoalContinuationReminderMessage(message) ||
      isIdexalGoalStateReminderText(textFromIdexalMessageParts(message.parts)))
  );
}

export function isIdexalModelOnlySyntheticUserMessage(message: IdexalMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isIdexalCompactSummaryMessage(message: IdexalMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getIdexalUserVisibleMessages(
  messages: readonly IdexalMessageWithParts[],
  _options: { target?: IdexalSessionGoal | null } = {},
): IdexalMessageWithParts[] {
  const visibleMessages: IdexalMessageWithParts[] = [];
  for (const message of messages) {
    if (isIdexalModelOnlySyntheticUserMessage(message) || isIdexalCompactSummaryMessage(message)) {
...
export function getIdexalGoalIterationByAssistantMessageId(
  messages: readonly IdexalMessageWithParts[],
  options: { maxGoalIteration?: number; target?: IdexalSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareIdexalMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isIdexalGoalContinuationReminderMessage(message)) {
...
export interface IdexalGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getIdexalGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly IdexalGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareIdexalMessagesByCreatedTime(
  left: IdexalMessageWithParts,
  right: IdexalMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: IdexalMessageWithParts, target: IdexalSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveIdexalVisibleSessionTitle(input: {
  title?: string;
  messages: readonly IdexalMessageWithParts[];
  target?: IdexalSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isIdexalGoalContinuationReminderText(normalizedTitle) &&
    !isIdexalGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isIdexalGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromIdexalMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {

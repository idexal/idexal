// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 5 个导出：旧 configOptions 投影函数（formatModelPickerValue/normalizeAvailableIdexalMode/
// getIdexalAgentModeSelectOptions/getIdexalAgentAvailableModes/
// idexalSessionSettingsToIdexalConfigOptions）。
// 消费者：services idexalConfigOptions、UI idexalSessionProjection 等旧栈。
import { formatModelPickerValue } from "./model-selection.js";
import type { IdexalSessionMode, IdexalSessionSettingsState } from "./idexal-protocol/index.js";
import type { IdexalConfigOption, IdexalTaskModeInfo } from "./idexal-task-types-core.js";
...
export function getIdexalAgentModeSelectOptions(): NonNullable<IdexalConfigOption["options"]> {
...
export function getIdexalAgentAvailableModes(): IdexalTaskModeInfo[] {
  return IDEXAL_AGENT_MODE_OPTIONS.map((mode) => ({ ...mode }));
}

export function idexalSessionSettingsToIdexalConfigOptions(

...
import { normalizeIdexalEndpointOrigin } from "./idexalEndpoint.js";
import {
...
const idexalEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeIdexalEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeIdexalEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("idexalEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = idexalEndpointOriginSchema.safeParse(raw.idexalEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, idexalEndpointOrigin: parsed.data };
  }
  const { idexalEndpointOrigin: _idexalEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  idexalInteractionBehavior: idexalInteractionBehaviorSchema.default("queue"),
  ...
  idexalEndpointOrigin: idexalEndpointOriginSchema.optional(),
});
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeIdexalEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  idexalInteractionBehavior: idexalInteractionBehaviorSchema.optional(),

...
import { z } from "zod";
import type { IdexalTaskMigrationSource, IdexalTaskMode } from "./idexal-task-types-core.js";
import { idexalAgentProviderSchema } from "./idexal-agent-policy.js";
import { idexalPermissionResponseSchema } from "./idexal-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./idexal-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./idexal-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const idexalTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly IdexalTaskMode[];
const idexalTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly IdexalTaskMigrationSource[];
const idexalTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 IdexalTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(idexalTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: idexalAgentProviderSchema.optional(),
  migrationSource: z.enum(idexalTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: idexalTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const idexalPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const idexalTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const idexalTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  idexalTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: idexalTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: idexalTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({

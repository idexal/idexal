...
import { invalidateDeferredDraftSessionForSkillChange } from "@/lib/idexalDraftSkillInvalidation.js";
...
const EMPTY_PINNED_TASKS: IdexalTaskMeta[] = [];
...
export const WorkspaceSidebarItem = memo(function WorkspaceSidebarItem({
  ...
  const workspaceIdexalState = useIdexalSessionStore((state) =>
    selectWorkspaceIdexalState(state, tab.workspacePath, tab.workspaceIdentity),
  );
  const activeTaskId = workspaceIdexalState.activeTaskId;
  const removeTaskState = useIdexalSessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useIdexalSessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const removeOptimisticTaskListItem = useIdexalSessionStore(
    (state) => state.removeOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useIdexalSessionStore((state) => state.setTaskUnreadIndicator);
  ...
  const idexalTaskService = services.idexalTaskService;
  const taskItemsRef = useRef(taskItems);
  taskItemsRef.current = taskItems;
  const workspaceIdexalStateRef = useRef(workspaceIdexalState);
  workspaceIdexalStateRef.current = workspaceIdexalState;
  ...
  const handleRemoveWorkspace = useCallback(async () => {
    ...
    if (
      hasRunningWorkspaceChat({
        workspaceState: workspaceIdexalStateRef.current,
  ...
  const handleRenameTask = useCallback(
    async (taskId: string, title: string) => {
      ...
      let meta: IdexalTaskMeta;
      try {
        meta = await idexalTaskService.renameTask({
          taskId,
          workspacePath: tab.workspacePath,
          title,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleSetTaskPinned = useCallback(
    async (taskId: string, pinned: boolean) => {
      ...
      try {
        const meta = await idexalTaskService.setTaskPinned({
          taskId,
          workspacePath: tab.workspacePath,
          pinned,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleArchiveTask = useCallback(
    async (taskId: string) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await idexalTaskService.archiveTask({
        taskId,
        workspacePath: tab.workspacePath,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
      });
  ...
  const handleSetTaskUnread = useCallback(
    async (taskId: string, unread: boolean) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await idexalTaskService.setTaskUnread({
        taskId,
        workspacePath: tab.workspacePath,
        unread,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  return (
    <li ref={itemRef} style={itemStyle} className="space-y-2">
      ...
      <RemoteSyncDialogs
        ...
        localIdexalAgentService={baseServices.idexalAgentService}
        remoteIdexalAgentService={services.idexalAgentService}
        remoteTarget={tab.remoteTarget}
        ...
        onPluginsSynced={async () => {
          await refreshWorkspacePluginCapabilitiesAfterRemoteSync({
            ...
            workspaceIdentity: tab.workspaceIdentity,
            workspacePath: tab.workspacePath,
            idexalAgentService: services.idexalAgentService,
            idexalSessionService: services.idexalSessionService,
          });
        }}
      />

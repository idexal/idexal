// composer parity：`#` 会话候选的数据源从旧 idexalSessionStore/taskQueryCache/remote*
...
import type { IdexalProvider, IdexalTaskMeta } from "@idexal/shared";
...
function getSessionLabel(task: IdexalTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: IdexalTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: IdexalTaskMeta, provider: IdexalProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, IdexalTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.idexalAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.idexalAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.idexalAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.idexalAgentService,
        },
      ];
    }

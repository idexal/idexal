...
export function useWorkspaceShellLifecycle({
  ...
  useEffect(() => {
    return () => {
      ...
      // 预热 session 会让没有 task 的 workspace 也常驻一个 Idexal Agent 进程。
      ...
      // 上一次残留的 ready/failed。单 Idexal Agent 下这里不再按 provider 循环清理。
  ...
  }, [services.idexalTaskService, workspaceIdentity, workspaceAbsPath]);
}

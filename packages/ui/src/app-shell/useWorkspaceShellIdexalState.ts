...
type WorkspaceShellIdexalState = Pick<
...
export function useWorkspaceShellIdexalState(workspaceAbsPath: string, workspaceIdentity?: string) {
  // App 之前直接订阅整个 workspaceIdexalState，streaming 每个 chunk 都会改 taskMessagesByTaskId。
  // 这会把侧边栏、Header、Git 派生逻辑一起拖进同步重渲染，性能 trace 里那串 long task 就是这样被放大的。
  // 这里把外壳真正依赖的字段收敛成浅比较选择器，避免消息流惊动无关 UI。
  const workspaceShellIdexalState = useIdexalSessionStore(
    useShallow((state): WorkspaceShellIdexalState => {
      const workspaceState = selectWorkspaceIdexalState(state, workspaceAbsPath, workspaceIdentity);
  ...
  const reloadSessionDisabled = useIdexalSessionStore((state) => {
    const workspaceState = selectWorkspaceIdexalState(state, workspaceAbsPath, workspaceIdentity);
  ...
  return { workspaceShellIdexalState, reloadSessionDisabled };
}

...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useIdexalSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useIdexalSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useIdexalSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useIdexalSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useIdexalSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useIdexalSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useIdexalSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useIdexalSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.idexalTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useIdexalSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useIdexalSessionStore
        .getState()

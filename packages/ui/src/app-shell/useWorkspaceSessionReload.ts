...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: IdexalProvider | null }) => {
      ...
      const idexalSessionStore = useIdexalSessionStore.getState();
      const latestWorkspaceState = idexalSessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: IdexalProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      idexalSessionStore.setWorkspaceInitState(
        workspaceAbsPath,
        "initializing",
        null,
        workspaceIdentity,
      ...
      if (shouldPrepareWorkspace) {
        idexalSessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        idexalSessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        idexalSessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.idexalTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithIdexalSessionService({
            ...
            workspaceIdentity,
            provider,
            idexalSessionService: services.idexalSessionService,
          });

          const latestAfterPrepare = idexalSessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = idexalSessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              idexalSessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              idexalSessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              idexalSessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        idexalSessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        idexalSessionStore.setWorkspaceInitState(
          workspaceAbsPath,
          "ready",
          null,
        ...
        if (shouldPrepareWorkspace) {
          idexalSessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          idexalSessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }

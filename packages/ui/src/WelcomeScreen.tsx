...
/**
 * WelcomeScreen —— 订阅占位 / API Key / OAuth 登录入口
 *
 * 产品规则见 docs/login-entry.md：账号 OAuth 按钮已从入口下线，
 * 但 useOAuth 状态机保留给设置页发起的 provider 连接流程复用。
 */
...
import { IdexalAboutLogo } from "@/components/ui/IdexalAboutLogo.js";
...
function LoginPanel({ active, onComplete }: LoginPanelProps) {
  ...
  const { startLogin, cancel, reset, status, error, providers, pendingProvider, refreshProviders } =
    useOAuth();
  const user = useIdexalStore((s) => s.user);
  const oauthError = useIdexalStore((s) => s.oauthError);
  const setOAuthError = useIdexalStore((s) => s.setOAuthError);
  const oauthSuccessSeq = useIdexalStore((s) => s.oauthSuccessSeq);
  const lastOAuthSuccessProvider = useIdexalStore((s) => s.lastOAuthSuccessProvider);
  const loginEntryRequest = useIdexalStore((s) => s.loginEntryRequest);
  const clearLoginEntryRequest = useIdexalStore((s) => s.clearLoginEntryRequest);
  const markLoginEntryAttemptStatus = useIdexalStore((s) => s.markLoginEntryAttemptStatus);
      ...
      <div className="space-y-6">
        ...
        {status === "idle" && !oauthError && loginMode === "providers" && (
          <div className="space-y-2">
            {/* 订阅平台的 web 门户还没就绪，这里只放禁用占位：无链接、无点击行为，
                等门户上线后再接真实入口，避免先给用户一个点了没反应的"可用"按钮。 */}
            <Button variant="default" className="h-10 w-full text-ui-base" size="lg" disabled>
              {intl.formatMessage({ id: "login.subscription.comingSoon" })}
            </Button>
            <Button
              variant="outline"
...
function LoginPanelLogo() {
  return (
    ...
    <div
      ...
      role="img"
    >
      <IdexalAboutLogo className="h-auto w-10" />
    </div>
  );
}

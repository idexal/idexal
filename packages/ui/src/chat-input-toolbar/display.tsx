...
import {
  IDEXAL_MODE_OPTION_DESCRIPTION_IDS,
  IDEXAL_MODE_OPTION_LABEL_IDS,
...
function isConfigSelectValueInOptions(option: IdexalConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: IdexalConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  option: IdexalConfigOption,
  entry: IdexalConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  intl: ReturnType<typeof useIdexalIntl>["intl"];
  locale: string;
}) {
  const retryLabel = useMemo(() => {
    ...
    // 当前 Idexal Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<IdexalConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: IdexalProvider | undefined,
  entry: Pick<IdexalConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return IDEXAL_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return IDEXAL_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}

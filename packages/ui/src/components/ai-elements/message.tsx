 ...
 * Modified by Idexal: local integration, formatting and adaptations.
...
import type { EditorInfo, FileStat, OpenInEditorOptions } from "@idexal/shared";
...
import { createIdexalFileCitationRemarkPlugin } from "@/lib/idexalFileCitationRemarkPlugin.js";
import { windowsFileLinkEscapeRemarkPlugin } from "@/lib/windowsFileLinkEscapeRemarkPlugin.js";
import { projectIdexalFileCitations } from "@/lib/idexalFileCitation.js";
import { rewriteMarkdownArtifactImageSources } from "@idexal/shared";
...
export type MessageResponseProps = {
  ...
  /** 仅 Assistant 正文开启：把完整 idexal-file-citation 投影为现有文件链接。 */
  renderIdexalFileCitations?: boolean;
};
...
export function buildMessageStreamdownRenderKey(params: {
  ...
  return [
    ...
    params.renderIdexalFileCitations ? "citations" : "plain",
...
export const messageResponsePropsAreEqual = (
  ...
  nextProps.renderIdexalFileCitations === prevProps.renderIdexalFileCitations &&
...
export const MessageResponse = memo(
  ({
    ...
    onOpenFileLink,
    onOpenExternalUrl,
    renderIdexalFileCitations = false,
    workspacePath,
    workspaceHomePath,
    workspaceIdentity,
    ...
    const projectedCitationMarkdown = useMemo(
      () =>
        renderIdexalFileCitations
          ? projectIdexalFileCitations(rawMarkdown, { streaming: renderStreaming }).visibleText
          : rawMarkdown,
      [rawMarkdown, renderStreaming, renderIdexalFileCitations],
    );
    ...
    const messageRemarkPlugins = useMemo<PluggableList>(
      () => [
        ...
        ...(renderIdexalFileCitations && workspacePath
          ? [createIdexalFileCitationRemarkPlugin(workspacePath, workspaceHomePath)]
          : []),
      ],
      [renderIdexalFileCitations, workspaceHomePath, workspacePath],
    );
    ...
    const boundaryResetKey = useMemo(
      () =>
        renderStreaming
          ...
          : `${streamdownMode}:${renderIdexalFileCitations ? "citations" : "plain"}:${hashMarkdownCacheKey(targetMarkdown)}`,
      [renderStreaming, renderIdexalFileCitations, streamdownMode, targetMarkdown],
    );

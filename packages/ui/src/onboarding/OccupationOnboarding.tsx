...
import type { OnboardingRecordEntry } from "@idexal/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useIdexalStore((state) => state.newUserOnboardingOpen);
  const setRequested = useIdexalStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useIdexalStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useIdexalStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useIdexalStore((state) => state.requestOnboardingDialog);

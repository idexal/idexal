...
import type {
  ...
  IdexalPersistedFileChange,
  IdexalProvider,
  IdexalStreamEvent,
  IdexalTimelineStatus,
  IdexalUsage,
} from "@idexal/shared";
import {
...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly IdexalPersistedFileChange[];
  usage?: IdexalUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 Idexal Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 Idexal Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly IdexalTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: IdexalTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: IdexalContextCompactionTimelineMeta;
  provider?: IdexalProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // Idexal Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: IdexalUsage | null, right: IdexalUsage): IdexalUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly IdexalPersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<IdexalStreamEvent, { type: "task_network_debug_status" }>,

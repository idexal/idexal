import type { IdexalConfigOption, IdexalTaskMeta } from "@idexal/shared";
import { getIdexalAgentModeSelectOptions } from "@idexal/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<IdexalTaskMeta, "provider" | "model">,
): string | null {
...
function createRecoveredModelOption(modelValue: string): IdexalConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): IdexalConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): IdexalConfigOption {
  return {
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<IdexalTaskMeta, "provider" | "model"> &
    Partial<Pick<IdexalTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly IdexalConfigOption[];
}): IdexalConfigOption[] {

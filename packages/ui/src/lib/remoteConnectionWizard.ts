import type { RemoteAssetInstallMode, RemoteTarget } from "@idexal/shared";
import { isValidWslUser, normalizeRemoteResourcePackageSelection } from "@idexal/shared";
...
export function withDefaultRemoteResourcePackages(target: RemoteTarget): RemoteTarget {
  ...
  return {
    ...target,
    resourcePackages: {
      // 当前分支只保留一个 Idexal Agent，SSH 向导再让用户手动挑资源包会产生无意义分叉。

...
// sessions-index 只补实时 activity/detail，最终拼出旧 IdexalGroupedTaskView 同形结果。
...
function taskKeyOf(task: IdexalTaskMeta): string {
...
function taskOrderMapKey(task: IdexalTaskMeta): string {
...
function nodeMapKey(node: IdexalGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, IdexalGroupedTaskViewStructureMember>,
): IdexalTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: IdexalTaskListItem; addedAt: number; key: string }> = [];
...
interface BuildGroupedTaskViewParams {
  ...
  taskIndexItems: IdexalTaskMeta[];
  /** sessions-index 派生的会话 activity/detail，只 enrich 命中的持久行。 */
  sessions: IdexalTaskMeta[];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, IdexalTaskListItem>();
  ...
  const membersByGroupId = new Map<string, IdexalGroupedTaskViewStructureMember[]>();
  ...
  const nodes: IdexalGroupedTaskViewNode[] = structure.groups.map((group) => {

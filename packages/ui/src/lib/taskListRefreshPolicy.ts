import type { IdexalWorkspaceTaskListChanged } from "@idexal/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<IdexalWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<IdexalWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<IdexalWorkspaceTaskListChanged, "reason">,
): boolean {

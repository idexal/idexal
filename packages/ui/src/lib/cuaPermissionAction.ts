import type { CuaAccessibilitySettingsResult } from "@idexal/shared";

function normalizeToolName(value: string | null | undefined): string {
  return value?.trim().toLowerCase().replace(/_/g, "-") ?? "";
}

export function isIdexalCuaToolName(value: string | null | undefined): boolean {
...
function didReturnFromCuaPermissionSettings(
  ...
  // main 只有在整组 staged pane 都完成并观察到 Idexal 应用级返回后才置 true。renderer focus

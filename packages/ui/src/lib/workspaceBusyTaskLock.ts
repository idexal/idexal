import type { IdexalProvider, IdexalTaskRuntimeStatus } from "@idexal/shared";

function isBusyTaskRuntimeStatus(status: IdexalTaskRuntimeStatus): boolean {
  return status === "creating" || status === "restoring" || status === "streaming";
}

function buildTaskProviderByTaskId(
  optimisticTaskMetaByTaskId: Record<string, { provider?: IdexalProvider }>,
  taskListCache?: Array<{ taskId: string; provider?: IdexalProvider }> | null,
): Record<string, IdexalProvider | undefined> {
  const providerByTaskId: Record<string, IdexalProvider | undefined> = {};
...
export function hasBusyTaskInWorkspaceProvider(
  ...
  taskRuntimeByTaskId: Record<
    string,
    { status: IdexalTaskRuntimeStatus; provider?: IdexalProvider }
  >,
  optimisticTaskMetaByTaskId: Record<string, { provider?: IdexalProvider }>,
  taskListCache?: Array<{ taskId: string; provider?: IdexalProvider }> | null,

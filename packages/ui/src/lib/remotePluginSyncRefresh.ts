...
async function refreshSlashCommandsAfterRemotePluginSync(params: {
  ...
  try {
    ...
    const currentSlashCommands = idexalSessionStore.getWorkspaceState(
      workspacePath,
      workspaceIdentityParam,
    ).slashCommands;
    // 远程插件同步入口不一定打开过命令设置页，commandsStore 可能尚未初始化。
    // 输入框 `/` 面板读取的是 idexalSessionStore.slashCommands，所以同步后必须直接
...
export async function refreshWorkspacePluginCapabilitiesAfterRemoteSync(params: {
  ...
  idexalSessionService: Pick<IIdexalSessionService, "closeSession">;
}): Promise<void> {
  ...
  if (
    ...
    await pluginStore.refresh(params.idexalAgentService);
  }

  await invalidateDeferredDraftSessionForSkillChange({
    idexalSessionService: params.idexalSessionService,
    workspacePath,

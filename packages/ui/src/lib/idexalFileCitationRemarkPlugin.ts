...
import { extractIdexalFileCitations } from "@/lib/idexalFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractIdexalFileCitations(value);
...
export function createIdexalFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function idexalFileCitationRemarkPlugin() {
    return (tree: unknown) => {

import {
  getIdexalToolFamilyForName,
  isTodoPlanToolName,
  isIdexalFileContentWriteToolName,
  normalizeIdexalToolName,
  type IdexalKnownToolName,
  type IdexalToolFamily,
} from "@idexal/shared";
...
export interface ToolCallIdentity {
  toolName: IdexalKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getIdexalToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    idexal: readNestedString(raw, ["_meta", "idexal", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawIdexal = normalizeLegacyToken(rawNames.idexal);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.idexal, source: "raw-idexal-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.idexal ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 Idexal Agent 投影会用
  ...
  if (
    // 当前 Idexal Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isIdexalFileContentWriteToolName(identity.toolName)) {
    return true;
  }

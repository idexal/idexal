import type { IdexalTaskRuntimeStatus } from "@idexal/shared";
import { getWorkspaceDisplayedTaskState } from "@/store/idexalSessionStore.js";
import type {
...
interface MarkRemoteWorkspaceRunningTasksFailedParams {
  ...
  getWorkspaceState: IdexalSessionStoreState["getWorkspaceState"];
  setTaskRuntimeState: IdexalSessionStoreState["setTaskRuntimeState"];
  reason: string;
}

function isRunningRuntimeStatus(status: IdexalTaskRuntimeStatus): boolean {
...
function collectRemoteWorkspaceRunningTaskIds(workspaceState: WorkspaceIdexalUIState): string[] {

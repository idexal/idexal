...
export async function refreshSharedSkillStoreForWorkspace(params: {
  ...
  const normalizedProvider = normalizeAgentProviderToIdexalAgent(
    params.provider ?? IDEXAL_AGENT_PROVIDER,
  );
  ...
  if (
    ...
    normalizeAgentProviderToIdexalAgent(skillStore.provider) === normalizedProvider &&

...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.idexal/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.idexal/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isIdexalSkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.idexal/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: IdexalProvider,
): T[] {
  return skills.filter(isIdexalSkill);
}

import type {
  ...
  IdexalTaskChangedFileSummary,
  IdexalTaskMeta,
} from "@idexal/shared";
import { computeLineChangeStat } from "@idexal/shared";
...
export function getTaskChangeSummary(
  task: Pick<IdexalTaskMeta, "changeSummary"> | null | undefined,
): IdexalTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: IdexalTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly IdexalPersistedFileChange[] | undefined,
): IdexalTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: IdexalTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: IdexalPersistedFileChange | null | undefined,
): IdexalTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: IdexalTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly IdexalPersistedFileChange[] | undefined,
): Map<number, IdexalTaskChangeSummary> {
  const result = new Map<number, IdexalTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }

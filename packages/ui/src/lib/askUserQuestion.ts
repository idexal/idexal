type IdexalUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: IdexalUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // Idexal Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): IdexalUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): IdexalUserQuestionAnswers | undefined {
...
function parseIdexalAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): IdexalUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedIdexalOutputAnswers = parseIdexalAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedIdexalOutputAnswers) {
    return parsedIdexalOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawIdexalOutputAnswers = parseIdexalAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawIdexalOutputAnswers) {
      return parsedRawIdexalOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawIdexalAnswers = parseIdexalAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawIdexalAnswers) {
      return parsedRawIdexalAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: IdexalUserQuestionAnswers | undefined,
  noAnswerText: string,
) {

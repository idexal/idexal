import type { FileEntry, IdexalTaskMeta, IdexalTaskRuntimeStatus } from "@idexal/shared";
import type { IFileService } from "@idexal/services";
import type { WorkspaceIdexalUIState } from "@/store/idexalSessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceIdexalUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<IdexalTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: IdexalTaskRuntimeStatus): boolean {

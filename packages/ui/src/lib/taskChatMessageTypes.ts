// re-home 产物（为删除 idexalChatMessages 铺路，模式同 idexal-task-types-core）。
// 本文件承载旧消息转换层（idexalChatMessages/idexalChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（idexalChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  IdexalAssistantMessagePart,
  IdexalAssistantMessageFeedback,
  IdexalPromptAttachment,
  IdexalTimelineMeta,
  IdexalTaskSnapshotBodyRef,
  IdexalTaskSnapshotToolFieldRef,
  IdexalTaskSnapshotToolSlice,
} from "@idexal/shared";
...
export interface TaskChatToolCall {
  ...
  /** Idexal 固定工具名；kind 仍保留给旧 Idexal Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = IdexalAssistantMessagePart;

import type { IdexalError, TraceId } from "@idexal/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@idexal/shared/idexal-protocol-v4";

export interface IdexalUiError extends IdexalError {
...
interface NormalizeIdexalUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_IDEXAL_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "Idexal session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // Idexal Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // idexal-cli 会把模型/网络错误摘要放在 data.idexal.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "idexal", "error", "message"],
    ["data", "idexal", "error", "detail"],
    ["data", "idexal", "error", "details"],
  ];
...
export function normalizeIdexalUiError(
  error: unknown,
  options: NormalizeIdexalUiErrorOptions = {},
): IdexalUiError {
  const candidates = collectMessageCandidates(error);
  // idexal-cli 已经把 provider/network 根因放进 detail 或 data.idexal.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_IDEXAL_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_IDEXAL_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "idexal", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "idexal", "error", "underlyingErrorDetail"],
  ]);

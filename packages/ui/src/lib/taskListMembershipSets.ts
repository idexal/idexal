...
> &
  Partial<Pick<IIdexalTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<IdexalTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: IdexalTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: IdexalTaskMeta[][]): IdexalTaskMeta[] {
  const itemByEntityKey = new Map<string, IdexalTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, IdexalTaskMeta>();

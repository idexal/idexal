...
import { extractConversationPreviewFileReferences } from "@idexal/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@idexal/shared";
import {
  extractIdexalFileCitationDirectives,
  resolveIdexalFileCitationPreviewKind,
} from "@/lib/idexalFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractIdexalFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 idexal-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }

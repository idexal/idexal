...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@idexal/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<IdexalTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<IdexalTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<IdexalTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<IdexalTaskMeta | null>;
  ...
  const startDraft = useIdexalSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: IdexalTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareIdexalTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {

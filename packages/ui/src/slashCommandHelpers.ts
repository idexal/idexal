...
import type { AgentSummary, Locale, SkillSummary, IdexalSlashCommand } from "@idexal/shared";
...
export function normalizeSlashCommandValue(name: string): string {
  // Idexal Agent 在远端可能直接返回 "/init" 作为命令名。
  // UI 的 value 需要去掉前导斜杠，否则插入 markdown 时会变成 "//init"，并影响 / 面板匹配。
  return name.trim().replace(/^\/+/, "");
}

export function buildSlashSuggestions(commands: IdexalSlashCommand[]): PromptInputSuggestionItem[] {
  return commands.flatMap((command) => {

...
import type { Locale, RemoteTarget, UserInfo, IdexalTaskMeta } from "@idexal/shared";
import { BUILTIN_MODEL_PROVIDER_IDS } from "@idexal/shared";
import {
...
const EMPTY_WORKSPACE_TASK_ITEMS: IdexalTaskMeta[] = [];
...
export const WorkspaceSidebar = memo(function WorkspaceSidebarComponent({
  ...
  const bumpTaskListVersion = useIdexalSessionStore((state) => state.bumpTaskListVersion);
  ...
  const activeTaskId = useIdexalSessionStore(
    (state) =>
      // Web 远程控制从全局 task 入口进入远端 workspace 时，会先按
      // workspaceIdentity 写入 activeTaskId；如果侧栏仍然只读 path-only 桶，
      // 当前任务高亮会丢失，也会把后续选择误判成未激活。
      selectWorkspaceIdexalState(state, workspacePath, workspaceIdentity).activeTaskId,
  );

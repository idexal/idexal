...
export function trajectoryToolOutputs(message: IdexalModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: IdexalModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: IdexalModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: IdexalModelTrajectoryMessage): {
  names: string;
  ids: string;
} {

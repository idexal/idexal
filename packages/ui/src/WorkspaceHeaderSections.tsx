...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: IdexalTaskMeta,
    overrides?: Partial<IdexalTaskMeta>,
  ): IdexalTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.idexalTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.idexalTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 idexalTaskMetaMerge：归档后只有旧列表状态被更新。

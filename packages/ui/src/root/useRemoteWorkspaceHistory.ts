...
import { buildSshRemoteHostKey, createUuid, stripRemoteTargetSecrets } from "@idexal/shared";
...
export function useRemoteWorkspaceHistory({
  ...
  const connectRemoteWorkspaceTarget = useCallback(
    async (
      ...
      // 如果此时立刻 addTab，会短暂走到本地 services，导致首屏读目录/预热 Idexal Agent 命中错误服务。
  ...
  const handleRemoteWorkspaceSessionClosed = useCallback(
    async (event: RemoteSessionClosedEvent) => {
      ...
      const failedTaskCount = markRemoteWorkspaceRunningTasksFailed({
        tabs: matchedTabs,
        getWorkspaceState: idexalSessionStore.getWorkspaceState,
        setTaskRuntimeState: idexalSessionStore.setTaskRuntimeState,
        reason,
      });

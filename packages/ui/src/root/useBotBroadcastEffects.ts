...
import type { IdexalConfigOption } from "@idexal/shared";
import {
  buildTaskContextUsageFromUsageUpdate,
  recordTaskContextUsageUpdate,
} from "@/lib/idexalTaskUsageFallback.js";
import { normalizeIdexalUiError } from "@/lib/idexalUiError.js";
...
export function syncBotTaskConfigOptionsToStore(params: {
  idexalSessionStore: Pick<
    ReturnType<typeof useIdexalSessionStore.getState>,
  ...
  params.idexalSessionStore.setTaskConfigOptions(
...
export function shouldMirrorBotTaskStreamToStore(params: {
  ...
  // Bugfix: 远端 Bot task 的 stream 来自 bot runtime host，不一定会被当前 ChatView 的 Idexal Agent stream 订阅收到。
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      ...
      if (stream) {
        ...
        const workspaceState = idexalSessionStore.getWorkspaceState(
        ...
        switch (event.type) {
          ...
          case "tool_call":
            idexalSessionStore.setTaskRuntimeState(
              stream.workspacePath,
              stream.taskId,
              "streaming",
              undefined,
          ...
          case "permission_request":
            idexalSessionStore.setTaskPermissionRequest(
              stream.workspacePath,
              stream.taskId,
              event,
              stream.workspaceIdentity,
          ...
          case "task_complete":
            ...
            idexalSessionStore.setTaskError(
              stream.workspacePath,
              stream.taskId,
              null,
              stream.workspaceIdentity,
            ...
            break;
          case "task_error": {
            const normalizedError = normalizeIdexalUiError(
              {
                message: event.error,
                detail: event.detail,
                code: event.code,
          ...
          case "usage_update":
            {
              ...
              idexalSessionStore.setTaskContextWindow(
                stream.workspacePath,
                stream.taskId,
                event.size,
                stream.workspaceIdentity,
              );
              idexalSessionStore.setTaskUsage(
                stream.workspacePath,
                stream.taskId,
                nextUsage,
                stream.workspaceIdentity,
          ...
          case "session_info_update":
            if (event.apiRetry !== undefined) {
              idexalSessionStore.setTaskApiRetryStatus(
                stream.workspacePath,
                stream.taskId,
                event.apiRetry ?? null,
                stream.workspaceIdentity,
      ...
      if (provider && shouldSyncVisibleTaskConfig) {
        // Bugfix: /model、/mode 可以从第三方 Bot 修改当前 task 的真实 Idexal Agent 状态。
        // 这些操作不经过 ChatInputToolbar，本地 store 以前不会同步 provider/configOptions，
        // 导致 Bot 回复已切换但 UI 下拉仍显示旧状态。
        idexalSessionStore.bindRuntimeProvider(
          refresh.workspacePath,
          provider,
          refresh.workspaceIdentity,
        );
      ...
      // 会自然出现该消息；本地拼装面（idexalChatMessages）随旧 ChatView 退役。
      if (refresh.event === "permission_request" && refresh.permissionRequest) {
        ...
        idexalSessionStore.removeTaskPermissionRequest(
          refresh.workspacePath,
          refresh.taskId,
          refresh.requestId,
          refresh.workspaceIdentity,
        ...
        // 当前 UI 窗口不一定有同一条 Idexal Agent stream 订阅，必须把新的 elicitation_request 显式写回 store。
        idexalSessionStore.setTaskElicitationRequest(
          refresh.workspacePath,
          refresh.taskId,
        ...
        // Bugfix: Bot 代用户提交 AskUserQuestion 时，当前 UI 窗口不一定能收到 Idexal Agent stream 的
        // elicitation_response。通过 bots:task 明确同步 requestId 出队，避免问答弹窗一直挂着。
        idexalSessionStore.removeTaskElicitationRequest(
          refresh.workspacePath,
          refresh.taskId,
          refresh.requestId,
          refresh.workspaceIdentity,
      ...
      if (shouldRefreshBotTaskList(refresh.event, Boolean(refresh.task))) {
        idexalSessionStore.bumpTaskListVersion(refresh.workspacePath, refresh.workspaceIdentity);
      }
    });

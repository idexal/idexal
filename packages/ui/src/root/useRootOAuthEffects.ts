...
import { markIdexalJwtInvalidRestart } from "@/root/idexalJwtInvalidRestartMarker.js";
...
export function useRootOAuthEffects({
  ...
  useEffect(() => {
    ...
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel !== IDEXAL_JWT_INVALID_BROADCAST_CHANNEL || disposed) {
        return;
      }
      void (async () => {
        ...
        markIdexalJwtInvalidRestart();
        if (typeof window !== "undefined" && !("idexal" in window)) {

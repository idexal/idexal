...
import type { GitChangeSourceId, WorkspacePurpose } from "@idexal/shared";
import { useIdexalStore } from "@/store/StoreProvider.js";
import { getVisibleTaskMetas, useIdexalSessionStore } from "@/store/idexalSessionStore.js";
...
import { useWorkspaceShellIdexalState } from "@/app-shell/useWorkspaceShellIdexalState.js";
...
export function App({
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellIdexalState, reloadSessionDisabled } = useWorkspaceShellIdexalState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellIdexalState.activeTaskId;
  ...
  const draftSessionId = useIdexalSessionStore(
  ...
  const draftFocusVersion = workspaceShellIdexalState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useIdexalStore((s) => s.notificationEnabled);
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellIdexalState.selectedProvider,
    intl,
  });
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@idexal/shared").SessionCreateSource,
    ) => {
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellIdexalState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellIdexalState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.idexalAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.idexalAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.idexalAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.idexalAgentService.installPlugin(params),
      listPlugins: (params) => services.idexalAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.idexalAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.idexalAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useIdexalSessionStore.getState();
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellIdexalState={workspaceShellIdexalState}
        theme={theme}

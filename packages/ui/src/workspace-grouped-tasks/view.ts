...
import type { IdexalGroupedTaskView, IdexalGroupedTaskViewNode } from "@idexal/services";
import type { IdexalTaskMeta } from "@idexal/shared";
import type { GroupedDraftTaskPlacement } from "@/store/idexalSessionStoreTypes.js";
import { taskKey } from "@/workspace-grouped-tasks/ids.js";

function cloneView(view: IdexalGroupedTaskView): IdexalGroupedTaskView {
  return {
...
function cloneNodes(view: IdexalGroupedTaskView): IdexalGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<IdexalGroupedTaskViewNode, { type: "group" }>,
): Extract<IdexalGroupedTaskViewNode, { type: "group" }> {
...
function filterGroupedViewByTaskKeys(
  ...
  let changed = false;
  const nodes: IdexalGroupedTaskView["nodes"] = [];
...
function getGroupedTaskGroupIds(view: IdexalGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: IdexalGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: IdexalGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: IdexalGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: IdexalGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function getNodeIdAfterGroup(
  view: IdexalGroupedTaskView,
  groupId: string,
): IdexalGroupedTaskViewNode | null {
...
function insertTaskBeforeTopLevelNode(
  view: IdexalGroupedTaskView,
  task: IdexalTaskMeta,
  beforeNode: IdexalGroupedTaskViewNode | null,
): IdexalGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null

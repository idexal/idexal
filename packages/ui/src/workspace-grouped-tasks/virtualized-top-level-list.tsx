...
import type { IdexalGroupedTaskViewNode } from "@idexal/services";
...
function getTopLevelNodeKey(node: IdexalGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: IdexalGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: IdexalGroupedTaskViewNode, index: number) => ReactNode;
}) {

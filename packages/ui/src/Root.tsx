...
import { StoreProvider, useIdexalStore } from "@/store/StoreProvider.js";
...
import { consumeIdexalJwtInvalidRestartMarker } from "@/root/idexalJwtInvalidRestartMarker.js";
...
function RootInner({
  ...
  const user = useIdexalStore((state) => state.user);
  const isRestoringOAuthSession = useIdexalStore((state) => state.isRestoringOAuthSession);
  const setUser = useIdexalStore((state) => state.setUser);
  const setIsRestoringOAuthSession = useIdexalStore((state) => state.setIsRestoringOAuthSession);
  const setOAuthError = useIdexalStore((state) => state.setOAuthError);
  const oauthPollingActive = useIdexalStore((state) => state.oauthPollingActive);
  const setOAuthPollingActive = useIdexalStore((state) => state.setOAuthPollingActive);
  const markOAuthSuccess = useIdexalStore((state) => state.markOAuthSuccess);
  const {
  ...
  const [welcomeScreenOpenReason, setWelcomeScreenOpenReason] =
    useState<WelcomeScreenOpenReason | null>(() =>
      consumeIdexalJwtInvalidRestartMarker() ? "session-expired" : null,
    );
  const [providerFamilyDomainMigrationComplete, setProviderFamilyDomainMigrationComplete] =
    useState(false);
  const loginEntryRequest = useIdexalStore((state) => state.loginEntryRequest);
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.idexalAgentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  useEffect(() => {
    ...
    void services.idexalAgentService
  ...
  const totalUnreadTaskCount = useIdexalSessionStore((state) =>
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 Idexal session。
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 Idexal SVG，但 React 接管 root 后旧壳会被整棵替换。

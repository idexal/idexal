...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@idexal/shared/idexal-protocol-v4";
...
function readPermissionRuleScopes(option: IdexalPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: IdexalPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: IdexalPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<IdexalProvider, Record<string, string>>
> = {
  glm: {
    // GLM/Idexal Agent 通过 Idexal Agent 发来的项目级记忆授权文案是英文原文。
...
function getPermissionDisplayReason(request: IdexalPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: IdexalPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: IdexalPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  // 当前 Idexal Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // Idexal Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。

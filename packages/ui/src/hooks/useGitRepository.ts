...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, IdexalPersistedFileSnapshot>,
  file: IdexalTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: IdexalPersistedFileChange | null;
  summary: IdexalTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, IdexalPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 Idexal Agent 已持久化的单轮文件快照，

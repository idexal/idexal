...
import type { IIdexalTaskService } from "@idexal/services";
import type { IdexalTaskSnapshot } from "@idexal/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IIdexalTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<IdexalTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IIdexalTaskService["getTaskSnapshotWithEtag"]>[0];

const idexalTaskServiceProxyCache = new WeakMap<IIdexalTaskService, IIdexalTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IIdexalTaskService,
  Map<string, { etag: string; snapshot: IdexalTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "idexal-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: IdexalTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: IdexalTaskSnapshot }>();
...
function createIdexalTaskServiceProxy(service: IIdexalTaskService): IIdexalTaskService {
...
/** 获取 Idexal task wrapper 服务实例 */
export function useIdexalTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IIdexalTaskService {
  // Idexal task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.idexalTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = idexalTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createIdexalTaskServiceProxy(rawService);
  idexalTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}

...
function areResolvedTaskMetasEqual(left: IdexalTaskMeta | null, right: IdexalTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: IdexalTaskMeta | null) {
  const stableTaskMetaRef = useRef<IdexalTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useIdexalSessionStore((state) =>
    selectWorkspaceIdexalState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 idexalTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task

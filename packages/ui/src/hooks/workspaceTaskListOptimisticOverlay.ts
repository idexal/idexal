...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly IdexalTaskMeta[];
  optimisticTasks: readonly IdexalTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareIdexalTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useIdexalSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useIdexalSessionStore.getState();

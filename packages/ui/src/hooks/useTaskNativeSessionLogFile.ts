...
function supportsTaskNativeSessionLogFile(_provider: IdexalProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 idexalTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: IdexalProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const idexalTaskService = useIdexalTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, idexalTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);

...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): IdexalTaskListWorkspaceScope[] {
  const scopes = new Map<string, IdexalTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useIdexalSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceIdexalState(
            state,
            tab.workspacePath,

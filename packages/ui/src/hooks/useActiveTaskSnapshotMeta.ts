...
import { idexalSessionSnapshotToTaskMeta } from "@/lib/idexalSessionProjection.js";

function resolveImmediateActiveTaskSnapshotMeta(
  previousSnapshotMeta: IdexalTaskMeta | null,
  taskId: string | null,
  taskMetaFromLists?: IdexalTaskMeta | null,
) {
...
export function useActiveTaskSnapshotMeta(
  ...
  const [snapshotMeta, setSnapshotMeta] = useState<IdexalTaskMeta | null>(null);
  ...
  useEffect(() => {
    ...
    void idexalSessionService
      // active header 只需要 session meta/标题兜底，走 Idexal Protocol 的轻量读取，
      ...
      .then((snapshot) => {
        ...
        setSnapshotMeta(idexalSessionSnapshotToTaskMeta(snapshot));
      })
      .catch(() => {
        if (cancelled) {
          return;
  ...
  }, [idexalSessionService, taskId, taskMetaFromLists, workspaceIdentity, workspacePath]);

...
import type { IPlatformService, TaskNotificationPayload } from "@idexal/shared";
import type { ConversationSnapshot, SessionSummary } from "@idexal/shared/idexal-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, idexalAgentService);
    const syncState = () => {
      setIndexState({
        signature,

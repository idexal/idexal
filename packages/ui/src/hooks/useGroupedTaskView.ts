...
import { selectWorkspaceIdexalState, useIdexalSessionStore } from "@/store/idexalSessionStore.js";
import { buildTaskEntityKey, buildTaskWorkspaceKey } from "@/lib/taskQueryCache.js";
import { mergeTaskWithOptimisticMeta } from "@/lib/idexalTaskMetaMerge.js";
import {
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: IdexalTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, IdexalTaskMeta>();
  const placementTaskByKey = new Map<string, IdexalTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, IdexalTaskMeta[]>();
  const topMissingTasks: IdexalTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: IdexalGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: IdexalGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: IdexalGroupedTaskView["nodes"][number],
  next: IdexalGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: IdexalGroupedTaskView,
  next: IdexalGroupedTaskView,
): IdexalGroupedTaskView {
...
function nodeToTopLevelRef(
  node: IdexalGroupedTaskView["nodes"][number],
): IdexalGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
const groupedViewCacheBySignature = new Map<string, IdexalGroupedTaskView>();

function readCachedGroupedView(signature: string): IdexalGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: IdexalGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<IdexalGroupedTaskView>(
  ...
  const clearPromotedGroupedDraftTask = useIdexalSessionStore(
  ...
  const displayedViewRef = useRef<IdexalGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: IdexalGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.idexalTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.idexalTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: IdexalTaskMeta[] = [];
    const settledRootTasks: IdexalTaskMeta[] = [];
    ...
    void services.idexalTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.idexalTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.idexalTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<IdexalTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.idexalTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.idexalTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: IdexalGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.idexalTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.idexalTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: IdexalTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.idexalTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: IdexalGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.idexalTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.idexalTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: IdexalGroupedTaskView = {
      ...
      try {
        ...
        await services.idexalTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.idexalTaskService, view],
  );

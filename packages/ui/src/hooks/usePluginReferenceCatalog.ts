...
interface PluginReferenceCatalogState {
  entries: IdexalPluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<IdexalPluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<IdexalPluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<IdexalPluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.idexalAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<IdexalPluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<IdexalPluginsReferenceCatalogResult>> | undefined;

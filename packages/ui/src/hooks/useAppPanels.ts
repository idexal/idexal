...
import { createUuid } from "@idexal/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@idexal/shared";
...
export function useAppPanels(options: {
  ...
  const { idexalAgentService, idexalSessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await idexalAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,

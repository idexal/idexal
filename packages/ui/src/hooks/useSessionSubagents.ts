...
import type { IdexalSessionEndedSubagent, IdexalSessionSubagentsResult } from "@idexal/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: IdexalSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly IdexalSessionEndedSubagent[],
  incoming: readonly IdexalSessionEndedSubagent[],
): IdexalSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const idexalAgentService = services.idexalAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<IdexalSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof idexalAgentService?.listSessionSubagents !== "function") {
      ...
      return idexalAgentService.listSessionSubagents({

...
import type { IdexalImportSessionsResult, IdexalImportableSessionCandidate } from "@idexal/shared";
import { logger } from "@/logger.js";
import { useIdexalTaskService } from "@/hooks/useIdexalTaskService.js";
...
export function useClaudeSessionMigration(params: {
  ...
  const idexalTaskService = useIdexalTaskService(
  ...
  const [candidates, setCandidates] = useState<IdexalImportableSessionCandidate[]>([]);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [scanError, setScanError] = useState<string | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [lastImportResult, setLastImportResult] = useState<IdexalImportSessionsResult | null>(null);
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      const nextCandidates = await idexalTaskService.scanImportableClaudeSessions({
  ...
  const importSessions = useCallback(
    async (sessionIds: string[]) => {
      ...
      try {
        ...
        const result = await idexalTaskService.importClaudeSessions({
          workspacePath: effectiveWorkspacePath,
          ...(effectiveWorkspaceIdentity ? { workspaceIdentity: effectiveWorkspaceIdentity } : {}),
          sessionIds,
        });

 ...
 * （见 idexalSessionSettingsToConfigOptions），那种「名字」在这里就当作没有——屏幕上绝不出现
 * providerId（团队套餐的它是一个 UUID）。
 */
function workflowSubagentProviderNameLookup(
  configOptions: readonly IdexalConfigOption[] | null | undefined,
...
export function useWorkflowSubagentModelProviderName(
  ...
  const configOptions = useIdexalSessionStore((state) =>
    workspacePath === undefined
      ? undefined
      : selectWorkspaceIdexalState(state, workspacePath, workspaceIdentity).configOptions,
  );

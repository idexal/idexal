...
import { isRemoteWorkspaceIdentity, IDEXAL_CUA_OFFICIAL_PLUGIN_ID } from "@idexal/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/idexalSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 idexalSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  const workspaceSessionBusy = useIdexalSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === IDEXAL_CUA_OFFICIAL_PLUGIN_ID,

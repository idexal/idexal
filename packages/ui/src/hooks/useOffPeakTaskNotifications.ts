...
import type { IdexalOffPeakTaskStatus } from "@idexal/shared";
import type { IOffPeakTaskService } from "@idexal/services";
...
function notifiableStatus(status: IdexalOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, IdexalOffPeakTaskStatus>());

...
import type {
  ...
  IdexalPluginDiagnostic,
  IdexalPluginInfo,
  IdexalPluginMarketplaceSummary,
  IdexalPluginScope,
  IdexalPluginsDescribeResult,
} from "@idexal/shared";
import type { IPluginManagementService } from "@idexal/services";
...
export interface PluginDescribeEntry {
  ...
  data?: IdexalPluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 idexal-cli 提供 (list + enable/disable)。
// UI 不再直触 IIdexalAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
// 与已 retired 的 marketplace pluginStore 无关, 故单独建一个精简 store。
export interface PluginManagementState {
  workspacePath: string | null;
  workspaceIdentity: string | null;
  configScope: IdexalPluginScope | null;
  ...
  diagnostics: IdexalPluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
...
function throwForErrorDiagnostic(diagnostics: IdexalPluginDiagnostic[]): void {

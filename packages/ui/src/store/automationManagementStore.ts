...
// 定时任务(automation)管理 store：走 idexal-agent RPC（列表 / 创建 / 编辑 / 启停 / 重跑 / 删除 + 运行历史）。
...
interface AutomationManagementState {
  ...
  refresh: (agentService: IIdexalAgentService) => Promise<void>;
  createAutomation: (
  ...
  deleteAutomation: (automationId: string, agentService: IIdexalAgentService) => Promise<void>;
  setEnabled: (
    automationId: string,
    enabled: boolean,
    agentService: IIdexalAgentService,
  ) => Promise<void>;
  restartAutomation: (automationId: string, agentService: IIdexalAgentService) => Promise<void>;

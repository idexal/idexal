...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    idexalTaskService: Pick<IIdexalTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: IdexalTaskMeta) => void;
...
function sortPinnedTasks(tasks: IdexalTaskMeta[]): IdexalTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, idexalTaskService }) {
    ...
    try {
      const items = await idexalTaskService.listPinnedTasks({
        workspacePath,

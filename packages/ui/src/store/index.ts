...
import type { IBroadcastService, BroadcastMessage } from "@idexal/services";
import type { OAuthProviderId, UserInfo } from "@idexal/shared";
import type { CodingPlanResetType } from "@idexal/shared";
...
const CODE_PREVIEW_SETTINGS_KEY = "idexal-code-preview-settings";
const PERFORMANCE_MODE_STORAGE_KEY = "idexal-performance-mode";
 ...
 */
export function createIdexalStore(
  ...
  const useStore = create<IdexalState>()((set, get) => ({
    ...
    theme: normalizeThemePreference((readSafeLocalStorage("idexal-theme") as Theme) || "zai-dark"),
    setTheme: (theme: Theme) => {
      const normalizedTheme = normalizeThemePreference(theme);
      writeSafeLocalStorage("idexal-theme", normalizedTheme);
    ...
    locale: readSafeLocalStorage("idexal-locale") || "zh-CN",
    setLocale: (locale: string) => {
      writeSafeLocalStorage("idexal-locale", locale);
      set({ locale });
    },
...
export type IdexalStore = ReturnType<typeof createIdexalStore>;

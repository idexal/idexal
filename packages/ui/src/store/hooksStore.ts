...
import type { Hook, HookConfig } from "@idexal/shared";
import type { IHooksService } from "@idexal/services";
...
function buildIdexalHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildIdexalHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "idexal") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildIdexalHookLocation(workspacePath, source.location?.scope ?? "user"),
    };

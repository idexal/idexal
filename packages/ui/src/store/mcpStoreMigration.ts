import type { McpServerConfig, NativeMcpServerRecord } from "@idexal/shared";
...
function isIdexalAgentUserServer(server: NativeMcpServerRecord): boolean {
  return (
    server.source === "idexalagentmcp" &&
    server.scope === "user" &&
    (!server.location || server.location.source === "idexal")
  );
}

export async function importLegacyCommonServersToIdexalAgent(
  ...
  const existingNames = new Set(
    nativeServers.filter(isIdexalAgentUserServer).map((server) => server.name),
  );
  let importedCount = 0;
  let skippedCount = 0;
...
export async function migrateStoredCommonMcpToIdexalAgent(
  platform: McpPlatformService | null,
  nativeServers: NativeMcpServerRecord[],
  ...
  const migration = await importLegacyCommonServersToIdexalAgent(
    ...
    legacyServers,
    nativeServers,
    "localStorage:idexal-mcp-config",
  );
  if (migration.completed) {
    // 只有确认写入 idexal agent 目录后才清理旧数据，避免 Web 端没有 desktop bridge 时丢配置。
    clearLegacyCommonMcpServers();
  }
  ...
  logger.info(
    `[mcpStore] migrated ${migration.importedCount} legacy common MCP servers to idexal agent config`,
  );

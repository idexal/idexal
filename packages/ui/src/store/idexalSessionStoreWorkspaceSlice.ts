...
} from "@/store/idexalSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/idexalSessionStoreSelectors.js";

type SetFn = (
  partial:
    | IdexalSessionStoreState
    | Partial<IdexalSessionStoreState>
    | ((
        state: IdexalSessionStoreState,
      ) => IdexalSessionStoreState | Partial<IdexalSessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: IdexalConfigOption): IdexalConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: IdexalConfigOption[]): IdexalConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly IdexalConfigOption[]): IdexalConfigOption[] {
...
function isModeConfigOption(option: IdexalConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<IdexalConfigOption["options"]>;
}): IdexalConfigOption {
...
function replaceModeConfigOption(
  currentOptions: IdexalConfigOption[] | null,
  nextModeOption: IdexalConfigOption | null,
): IdexalConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceIdexalUIState,
  updater: (options: IdexalConfigOption[] | null) => IdexalConfigOption[] | null,
): Pick<WorkspaceIdexalUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceIdexalUIState,
  taskId: string,
): { configOptions: IdexalConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: IdexalTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as IdexalTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToIdexalAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToIdexalAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToIdexalAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),

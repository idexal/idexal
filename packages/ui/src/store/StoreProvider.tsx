...
import type { IBroadcastService } from "@idexal/services";
import { createIdexalStore, type IdexalStore, type IdexalState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<IdexalStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<IdexalStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createIdexalStore(broadcastService, {
 ...
 */
export function useIdexalStore<T>(selector: (state: IdexalState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useIdexalStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useIdexalStore（store 耦合剥离配套）。
 *
 ...
 */
export function useIdexalStoreWithDefault<T>(
  selector: (state: IdexalState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);

...
import type { AttachmentRef } from "@idexal/shared/idexal-protocol-v4";
...
declare global {
  interface Window {
    __idexalComposerAttachmentUploadStoreE2E?: typeof useComposerAttachmentUploadStore;
    __idexalCurrentComposerAttachmentScopeKeyE2E?: string;
  }
}

if (shouldExposeE2EStoreBridge()) {
  // E2E 只暴露当前唯一附件 owner，供 scope 切换用例准备状态；不再把附件塞回旧 Session Store。
  window.__idexalComposerAttachmentUploadStoreE2E = useComposerAttachmentUploadStore;
}

export function exposeComposerAttachmentScopeKeyForE2E(scopeKey: string): void {
  if (window.__idexalComposerAttachmentUploadStoreE2E) {
    window.__idexalCurrentComposerAttachmentScopeKeyE2E = scopeKey;
  }
}

...
import { getDefaultWorkspaceState } from "@/store/idexalSessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly IdexalTaskMeta[]): IdexalTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: IdexalPermissionRequest["options"],
  right: IdexalPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: IdexalPermissionRequest,
  right: IdexalPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
...
): Pick<IdexalSessionStoreState, "workspaces"> {
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useIdexalChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToIdexalAgent(params.provider),

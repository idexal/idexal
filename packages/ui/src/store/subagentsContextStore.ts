...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToIdexalAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToIdexalAgent(params.provider);

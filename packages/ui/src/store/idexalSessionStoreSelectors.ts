/**
 * Idexal Session Store 选择器与内部辅助函数
 *
 * 从 idexalSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { IdexalTaskRuntimeStatus, IdexalTaskMeta } from "@idexal/shared";
...
import {
  ...
  type IdexalSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceIdexalUIState,
} from "./idexalSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceIdexalUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceIdexalUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: IdexalSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceIdexalUIState) => WorkspaceIdexalUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 Idexal Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceIdexalUIState,
  taskId: string,
): TaskRuntimeState {
...
export function getTaskUiState(workspaceState: WorkspaceIdexalUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceIdexalUIState
    | Partial<Pick<WorkspaceIdexalUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
): IdexalTaskMeta | null {
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, IdexalTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<IdexalTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceIdexalState(
  state: IdexalSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceIdexalUIState, "workspaceInit">,
): WorkspaceInitState {

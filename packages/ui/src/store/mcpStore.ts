...
import type {
  ...
  IdexalMcpListMode,
  IdexalMcpServerStatusSnapshot,
  IdexalMcpServer,
} from "@idexal/shared";
import { convertToIdexalAgentMcpServer } from "@idexal/shared";
...
import {
  importLegacyCommonServersToIdexalAgent,
  migrateStoredCommonMcpToIdexalAgent,
...
interface McpStoreState {
  ...
  servers: IdexalMcpServer[];
  statusSnapshots: Record<string, IdexalMcpServerStatusSnapshot>;
  ...
  addIdexalAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateIdexalAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  deleteIdexalAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  beginServerStatusListRefresh: (mode?: IdexalMcpListMode) => number;
  ...
  mergeServerStatusSnapshots: (
    statuses: Record<string, IdexalMcpServerStatusSnapshot>,
    requestEpoch: number,
    mode?: "connect" | "status",
  ) => void;
  getServer: (id: string) => IdexalMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForIdexal: (provider: string) => IdexalAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<IdexalMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 Idexal Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.idexal/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 Idexal Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToIdexalAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addIdexalAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateIdexalAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteIdexalAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "idexalagentmcp" : source;
    ...
    addIdexalAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("idexalagentmcp", name, config, projectPath),
    updateIdexalAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("idexalagentmcp", name, config, projectPath),
    deleteIdexalAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("idexalagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "idexalagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForIdexal: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, IdexalMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "idexalagentmcp") continue;
      ...
      const result: IdexalAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const idexalAgentServer = convertToIdexalAgentMcpServer(server.name, server.config);
        if (idexalAgentServer) result.push(idexalAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "idexalagentmcp";
    ...
    migrateLegacyCommonMcp: async () => {
      ...
      const migration = await importLegacyCommonServersToIdexalAgent(
        mcpPlatformService,
        result.servers ?? {},

...
import {
  ...
  type IdexalSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: IdexalSavedWorkflowEntry[];
  invalid: IdexalSavedWorkflowInvalidEntry[];
  runs: IdexalSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: IdexalAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: IdexalAgentSavedWorkflowTarget,
  agentService: IIdexalAgentService,
): Promise<{ list: IdexalWorkflowsListResult; runs: IdexalSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as IdexalSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: IdexalAgentSavedWorkflowTarget | null,

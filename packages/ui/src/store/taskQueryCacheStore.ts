...
import type { IdexalTaskListItem } from "@idexal/services";
import type { IdexalTaskMeta } from "@idexal/shared";
import { matchesTaskListMembershipKind } from "@idexal/shared/idexal-protocol-v4";
import { mergeTaskMetaCandidates } from "@/lib/idexalTaskMetaMerge.js";
import { compareIdexalTaskListItems } from "@/lib/taskListOrdering.js";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: IdexalTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: IdexalTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: IdexalTaskMeta;
    nextTask: IdexalTaskMeta;
    previousState: TaskListMembershipState;
    nextState: TaskListMembershipState;
  }) => void;
  setTaskUnreadOverlay: (
    task: Pick<IdexalTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">,
...
type CachedTaskListItem = IdexalTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: IdexalTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, IdexalTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareIdexalTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
...
export function upsertTaskQueryCacheTaskMeta(task: IdexalTaskMeta): void {
  useTaskQueryCacheStore.getState().upsertTaskMeta(task);
}

export function updateTaskQueryCacheTaskMetaPreservingMembership(task: IdexalTaskMeta): void {

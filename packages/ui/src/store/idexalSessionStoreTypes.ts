/* oxlint-disable eslint(max-lines) -- Idexal Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * Idexal Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 idexalSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type IdexalWorkspaceInitStatus,
  type IdexalTaskRuntimeStatus,
  type IdexalProvider,
  type IdexalTaskMeta,
  type IdexalContextCacheUsage,
  ...
  type IdexalSessionActiveTurnKind,
  type IdexalContextUsageBreakdownItem,
  type InputId,
  type SessionCreateSource,
} from "@idexal/shared";
import type { IdexalUiError } from "@/lib/idexalUiError.js";
import type {
...
export interface WorkspaceInitState {
  status: IdexalWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: IdexalTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 Idexal Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: IdexalBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: IdexalPermissionRequest | null;
  pendingPermissionRequests: IdexalPermissionRequest[];
  elicitationRequest: IdexalElicitationRequest | null;
  pendingElicitationRequests: IdexalElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: IdexalUiError | null;
}
...
export interface WorkspaceIdexalUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * Idexal Agent 的 workspace 初始化状态。
   *
   * 单 Idexal Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  taskConfigOptionsByTaskId: Record<string, IdexalConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, IdexalTaskMeta>;
  ...
  /** 当前 workspace 选中的 Idexal Agent provider */
  ...
  /** Idexal Agent configOptions（模型、模式、思考级别等） */
  configOptions: IdexalConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: IdexalTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface IdexalSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceIdexalUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceIdexalUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: IdexalPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: IdexalElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
...
const FALLBACK_PROVIDER: IdexalProvider = IDEXAL_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceIdexalUIState {
  // 单 Idexal Agent 迁移后默认 provider 必须收敛到 glm。

...
import type { IdexalTaskListSortBy, IIdexalTaskService } from "@idexal/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, IdexalTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    idexalTaskService: Pick<IIdexalTaskService, "listTaskList">;
    sortBy: IdexalTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: IdexalTaskMeta, sortBy?: IdexalTaskListSortBy) => void;
...
function sortTimelineTasks(
  ...
  sortBy: IdexalTaskListSortBy = "updated",
): IdexalTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, idexalTaskService, sortBy, limit }) {
    ...
    try {
      const result = await idexalTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    idexalTaskService: session.services.idexalTaskService,
    sortBy,
    limit,
  });
}

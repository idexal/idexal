  ...
  const revalidateAndPrependPlugin = useCallback(
    async (current: DraftSuggestedPluginFlow, requestVersion: number) => {
      await invalidateDeferredDraftSessionForSkillChange({
        idexalSessionService: resolution.services.idexalSessionService,
        workspacePath,
        workspaceIdentity,
    ...
    [
      ...
      resolution.services.idexalSessionService,
      targetParams,
      workspaceIdentity,
      workspacePath,
    ],
      ...
      if (
        ...
        (kind === "install" &&
          (!flow.result?.pluginName ||
            flow.result.marketplace !== IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID ||

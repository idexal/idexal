import { BufferWriter, serialize } from "@idexal/rpc";
...
interface AttachmentUploadAgent {
  ...
  attachmentCommitV4(params: IdexalAgentAttachmentTerminalParams): Promise<V4AttachmentPutResult>;
...
function measureAttachmentChannelRequestBytes(method: string, params: unknown): number {
  ...
  serialize(writer, [100, 2_147_483_647, ServiceChannels.IdexalAgent, method]);
...
export async function uploadAttachmentTransaction(
  ...
  const beginParams: IdexalAgentAttachmentBeginParams = {
    ...common,
  ...
  try {
    ...
    for (let chunkIndex = begin.nextChunkIndex; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      const chunkParams: IdexalAgentAttachmentChunkParams = {
        ...common,
        chunkIndex,
    ...
    const terminal = common satisfies IdexalAgentAttachmentTerminalParams;

...
function ReadyV4ConversationProvider({
  ...
  const { idexalAgentService } = services;
  const platform = usePlatform();
  const bundle = useMemo(() => {
    const transport = createAgentConversationTransport(idexalAgentService, {
      workspacePath,
      workspaceIdentity,
      // 主 workspace resolver 已识别远端 endpoint，但这里曾丢弃
      // remoteSessionId，导致远端绝对路径被交给本机 idexal-media。仅本地 endpoint 注入转换器。

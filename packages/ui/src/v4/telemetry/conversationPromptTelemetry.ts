...
function resolveLegacyConversationModelValue(params: {
  ...
  if (!configProvider || !modelName || configProvider === IDEXAL_AGENT_PROVIDER) {
...
/** V4 config.provider 是实际模型 provider id；agentProvider 表示 Idexal 运行时。 */
export function buildV4ConversationPromptTelemetryExtraDetail(params: {
  agentProvider?: IdexalProvider;
  configProvider?: string | null;
  modelName?: string | null;
  askMode?: string | null;
  ...
  const agentProvider = params.agentProvider ?? IDEXAL_AGENT_PROVIDER;
  ...
  return {
    ...
    // agent 表示 Idexal 运行时，不能用模型 provider id 替代。
    agent: agentProvider,
  };
}

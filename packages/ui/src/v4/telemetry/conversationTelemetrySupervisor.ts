...
export interface ConversationPromptTelemetrySeed {
  localTtft?: import("@idexal/shared").LocalTtftContext;
...
export function resolveSendAckSettlement(ack: {
  ...
  // idexal 没有乐观渲染，要等投影回流出 userInput row 才算发送成功。
...
function toLegacyNetworkEvent(
...
): Extract<IdexalStreamEvent, { type: "task_network_debug_status" }> {
  return {
  ...
  } as Extract<IdexalStreamEvent, { type: "task_network_debug_status" }>;
}
...
function toToolLifecycleEvent(input: {
  ...
  if (!input.hasStarted && fact.phase !== "completed" && fact.phase !== "failed") {
    ...
    } as Extract<IdexalStreamEvent, { type: "tool_call" }>;
  }
  return {
    ...
    raw: {},
    ...(skillMetadata ? { skillMetadata } : {}),
  } as Extract<IdexalStreamEvent, { type: "tool_call_update" }>;
}

function toUsage(fact: Extract<ConversationTelemetryFact, { kind: "usage.delta" }>): IdexalUsage {
  return {
...
function mergeUsage(left: IdexalUsage | null, right: IdexalUsage): IdexalUsage {
...
export class ConversationTelemetrySupervisor {
  ...
  private processPromptFact(
    ...
    switch (fact.kind) {
      ...
      case "stream.chunk": {
        ...
        } as Extract<IdexalStreamEvent, { type: "agent_thought_chunk" | "agent_message_chunk" }>;
  ...
  private handleTerminal(
    ...
    const terminalEvent = // 修复原因：旧 adapter 把所有 TurnComplete（包括 cancelled）先投影成
      ...
      ) as Extract<IdexalStreamEvent, { type: "task_complete" | "task_error" }>;
    this.reportFinalizedSteps(
      lifecycle,
      recordAgentStepTelemetryEvent({
        taskId: lifecycle.taskKey,
  ...
  private handleCompaction(
    ...
    const timeline: IdexalContextCompactionTimelineMeta = {
      version: 1,
      kind: "synthetic",

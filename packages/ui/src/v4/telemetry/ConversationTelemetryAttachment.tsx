...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@idexal/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@idexal/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.idexalAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.idexalAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.idexalAgentService as object | null | undefined;

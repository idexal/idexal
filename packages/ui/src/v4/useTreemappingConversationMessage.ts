...
// 复用 treemappingActivity 的既有解析规则，不复活 idexalChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@idexal/shared/idexal-protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(idexalAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [idexalAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {

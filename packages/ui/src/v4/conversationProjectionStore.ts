...
const RUNTIME_RECYCLED_ERROR = "Idexal agent runtime 已被回收，重连未成功";
 ...
 *
 * 三种文案都来自同一次回收：transport 关闭时 IdexalProtocolClient.rejectAll 打断在途请求
...
function isRuntimeRecycleError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("Idexal agent transport closed") ||
    message.includes("Idexal Protocol client disposed") ||
    message.includes("Idexal Protocol client is disposed") ||
    message.includes("Idexal Agent runtime is not running")
  );
}
...
export class ConversationProjectionStore {
  ...
  /**
   * 有界退避重连。重订阅走 start-if-needed（idexalAgentService.subscribeConversationV4），

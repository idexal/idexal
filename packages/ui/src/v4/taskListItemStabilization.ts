...
//
// sessions-index / Controller tasks-index 每个内容帧都会全量重建 IdexalTaskMeta[]，
...
export function buildTaskListItemIdentityKey(meta: IdexalTaskMeta): string {
...
export function areTaskListItemsEquivalent(left: IdexalTaskMeta, right: IdexalTaskMeta): boolean {
  return areStabilizedValuesEquivalent(left, right);
}

/** 引用稳定化：等价条目复用旧对象；顺序与内容全等时复用整个旧数组。 */
export function stabilizeTaskListItems<T extends IdexalTaskMeta>(previous: T[], next: T[]): T[] {

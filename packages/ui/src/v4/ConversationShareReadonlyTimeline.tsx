 ...
 * 公开页 bundle 体积与无 Desktop 宿主（window.idexal / PlatformProvider / tab store）的运行环境；
...
import { getCompactToolCallStatusMessageId, type Locale } from "@idexal/shared";
import type {
  ArtifactRow,
  AssistantTextRow,
  ConversationRow,
...
import { IdexalIntlProvider, useIdexalIntl } from "@/i18n/IntlProvider.js";
...
function resolveImportedArtifactPath(
  ...
  // 取舍：这里只校验形状（.idexal-share/<dir>/shared-artifacts/<file> 四段），刻意不把
  ...
  // .idexal-share/*/shared-artifacts/ 内，无越权读放大。
...
export function ConversationShareReadonlyTimeline({
  ...
  const labels: ReadonlyLabels =
    locale === "zh-CN"
      ? {
          ...
          unsupportedRows: "部分内容需要更新 Idexal 查看",
        }
      : {
          history: "Reasoning",
          ...
          unsupportedRows: "Some content requires a newer version of Idexal",
        };
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <IdexalIntlProvider initialLocale={locale}>

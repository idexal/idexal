...
// 纯函数：ToolCallBlock 及其 renderers（execute/read/edit/...）吃的是旧 Idexal Agent 的
// TaskChatToolCall 形态；v4 row 自包含，字段一一映射即可，不需要看别的行。
import { buildIdexalStreamingToolInputPreview } from "@idexal/shared";
import type { ToolCallRow } from "@idexal/shared/idexal-protocol-v4";
...
function resolveToolInputPreview(row: ToolCallRow): ResolvedToolInputPreview {
  ...
  const preview = buildIdexalStreamingToolInputPreview(row.inputText);
  return {
...
export function toolCallRowToLegacyNode(row: ToolCallRow): TaskChatToolCallTreeNode {
  ...
  return {
    toolCall: {
      ...
      // kind 兼容旧聚合分类：v4 下没有旧 Idexal Agent 快照形态，直接用固定工具名。
      kind: row.toolName,

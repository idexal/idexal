// 把若干 workspace scope 的 sessions-index 会话聚合成 IdexalTaskMeta[]（响应式），
...
// remote shard（web/手机远控/SSH workspace）经 @idexal/rpc proxy 走同一条 sessions-index 链路。
...
/**
 * 订阅给定 workspace scope 的 sessions-index，聚合出 IdexalTaskMeta[]（running 置顶，其余按 updatedAt 降序）。
...
export function useWorkspaceSessionsIndexItems(
  ...
  const baseAgentService = baseServices.idexalAgentService;
  ...
  useEffect(() => {
    ...
    for (const scope of scopesRef.current) {
      // 防御：测试/降级环境可能没有 idexalAgentService（sessions-index 传输面），此时不订阅该 scope。
      const agentService = scope.agentService ?? baseAgentService;
      if (!agentService) {
        continue;
      }
  ...
  // 从各 store 聚合会话 → IdexalTaskMeta + hydration 状态（tick 驱动重算，引用稳定）。
  const previousItemsRef = useRef<IdexalTaskMeta[]>([]);
  const previousHydratingRef = useRef<string[]>([]);
  return useMemo(() => {
    const metas: IdexalTaskMeta[] = [];
    ...
    metas.sort((a, b) => compareIdexalTaskListItems(a, b, "updated"));

...
} from "@/store/idexalSessionStore.js";
import type { ElicitationFormDraft } from "@/store/idexalSessionStoreTypes.js";
...
function resolveV4ElicitationRequest(
  projected: IdexalElicitationRequest | null,
  botProgress: IdexalElicitationRequest | null,
): IdexalElicitationRequest | null {
...
function buildV4ElicitationProgressKey(request: IdexalElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useIdexalStoreWithDefault((state) => state.notificationEnabled, true);
  const botElicitationProgress = useIdexalSessionStore(
    (state) =>
      getTaskUiState(getWorkspaceState(state, workspacePath, workspaceIdentity), sessionId)
        .elicitationRequest,
  );
  const localElicitationDraft = useIdexalSessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useIdexalSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: IdexalPermissionOption, feedback?: string) => {

...
import { IDEXAL_AGENT_PROVIDER, resolveExecutionState } from "@idexal/shared";
...
import type { SessionConfigState } from "@idexal/shared/idexal-protocol-v4";
import type { IModelSelectionService } from "@idexal/services";
...
import { submissionModeSchema } from "@idexal/shared/idexal-protocol-v4";
import { prepareWorkspaceWithIdexalSessionService } from "@/hooks/useWorkspacePrepare.js";
import { useIdexalSessionService } from "@/hooks/useIdexalSessionService.js";
...
function shouldHydrateWorkspaceCatalog(params: {
  configOptions: readonly IdexalConfigOption[];
  sessionId: string | null;
  slashCommands: readonly IdexalSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const idexalSessionService = useIdexalSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithIdexalSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useIdexalSessionStore.getState();

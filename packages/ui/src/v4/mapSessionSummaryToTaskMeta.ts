// sessions-index 的 SessionSummary → 侧栏实时 detail IdexalTaskMeta 映射。
...
import type { TraceId, IdexalProvider, IdexalTaskMeta } from "@idexal/shared";
import type { SessionSummary } from "@idexal/shared/idexal-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): IdexalTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: IdexalTaskMeta;
}

/**
 * SessionSummary → 实时 detail IdexalTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as IdexalProvider } : {}),
      ...(status ? { status } : {}),

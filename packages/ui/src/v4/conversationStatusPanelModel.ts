import type { GitRepositorySummary } from "@idexal/shared";
import type {
...
import { workflowRunStepCounts } from "@idexal/shared/idexal-protocol-v4";
...
function buildRunningWorkflowRuns(
  ...
  for (const run of runs ?? []) {
    ...
    // 计数与聊天紧凑卡同源（唯一实现在 @idexal/shared 的 workflowRunStepCounts：表内 + 表外）。

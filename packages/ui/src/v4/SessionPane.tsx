...
import type { SessionCreateSource } from "@idexal/shared";
...
import { localizeConversationShareUrl } from "@idexal/shared";
import type {
...
import { prepareWorkspaceWithIdexalSessionService } from "@/hooks/useWorkspacePrepare.js";
import {
...
import { parseModelPickerValue } from "@/lib/idexalSessionProjection.js";
...
import type { GroupedDraftTaskState } from "@/store/idexalSessionStoreTypes.js";
import {
...
export interface SessionPaneProps {
  ...
  activeTaskChangeSummary?: IdexalTaskChangeSummary | null;
...
export function SessionPane({
  ...
  // send_result 的落定信号：用户消息真正画进对话历史。idexal 没有乐观渲染，
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<IdexalUiError | null>(null);
  ...
  const workspaceConfigOptions = useIdexalSessionStore(
  ...
  useEffect(() => {
    ...
    useIdexalSessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useIdexalSessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const theme = useIdexalStoreWithDefault((state) => state.theme, "system");
  const codePreviewSettings = useIdexalStoreWithDefault(
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      const groupedDraftTask =
        type === "createSession"
          ? useIdexalSessionStore.getState().getWorkspaceState(workspacePath, workspaceIdentity)
              .groupedDraftTask
          : null;
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await idexalSessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useIdexalSessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "IDEXAL_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      const displayProvider = provider ?? IDEXAL_AGENT_PROVIDER;
      ...
      const store = useIdexalSessionStore.getState();
      ...
      try {
        await idexalTaskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
        ...
        const prepareResult = await prepareWorkspaceWithIdexalSessionService({
          workspacePath,
          workspaceIdentity,
          provider: displayProvider,
          idexalSessionService,
          ...
          >
            <ConversationTimeline
              ...
              headerSlot={
                ...
                // rows 为空，但只读块必须留下来显示「需要更新 Idexal」，不能整块消失。
                importedShare &&

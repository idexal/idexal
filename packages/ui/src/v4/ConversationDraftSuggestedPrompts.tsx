...
export function ConversationDraftSuggestedPrompts({
  ...
  const { locale, intl } = useIdexalIntl();
            ...
            <Popover key={item.id} open={isPopoverAnchor} modal={false}>
              ...
              >
                <Button
                  ...
                  className="idexal-draft-prompt-waterfall h-8 min-w-0 justify-start gap-1.5 overflow-hidden rounded-lg px-3 text-left text-ui-caption font-normal leading-4.5 text-foreground"
                  style={
                    {
                      "--idexal-draft-prompt-waterfall-delay": `${index * 65}ms`,
                    } as CSSProperties
                  }
                >
                  <span

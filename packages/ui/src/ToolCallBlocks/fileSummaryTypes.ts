...
import type { WorkflowRunState } from "@idexal/shared/idexal-protocol-v4";
...
export function readRawToolCallInput(raw: unknown): unknown {
  ...
  // Idexal protocol 的 permission/request payload 按 schema 把工具参数放在 input，
...
export interface WorkflowRunCardSummary {
   ...
   *
   * 两个数都出自 `workflowRunStepCounts`（@idexal/shared）：`nodes` 表内 + `usage.nodesUnlisted`

...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@idexal/shared";
import type { IdexalPlanStep } from "@idexal/shared";
...
const todoTextClasses: Record<IdexalPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): IdexalPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: IdexalPlanStep["status"] }) {
  if (status === "completed") {

import type { useIdexalIntl } from "@/i18n/IntlProvider.js";
import type { ToolCallBlockRenderContext } from "@/ToolCallBlocks/shared.js";
import { isSubagentColor } from "@/lib/subagentColors.js";

type AgentIntl = ReturnType<typeof useIdexalIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "idexal", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "idexal", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "idexal", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "idexal", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const idexal = meta && isPlainRecord(meta.idexal) ? meta.idexal : null;
  const idexalBackgroundAgent =
    idexal && isPlainRecord(idexal.backgroundAgent) ? idexal.backgroundAgent : null;
  const taskNotification =
    idexal && isPlainRecord(idexal.taskNotification) ? idexal.taskNotification : null;
  const backgroundAgent = idexalBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "idexal", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "idexal", "taskNotification", "summary"]);

...
import type { IdexalTaskPendingInteraction } from "@idexal/shared";
import { cn } from "@/components/lib/utils.js";

interface TaskInteractionBadgeProps {
  interaction?: IdexalTaskPendingInteraction;
...
function isAskUserQuestionInteraction(
  interaction: IdexalTaskPendingInteraction | undefined,
): boolean {
...
export function TaskInteractionBadge({
  ...
  const style =
    progress !== undefined
      ? ({
          "--idexal-interaction-progress": String(progress),
          "--idexal-interaction-remaining-ms": `${remainingMs}ms`,
        } as CSSProperties)
      : undefined;
  ...
  const badgeContent = (
    <>
      {progress !== undefined ? (
        <span
          aria-hidden="true"
          className="idexal-task-interaction-countdown-fill absolute inset-0 origin-left bg-interaction-ask-fill transition-opacity group-hover/interaction-badge:opacity-0 group-focus-visible/interaction-badge:opacity-0 motion-reduce:transition-none"
        />
      ) : null}
      {canSnooze ? (

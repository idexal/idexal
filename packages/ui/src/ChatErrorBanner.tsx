 ...
 *
 * 显示 Idexal Agent 链路中的错误，带 traceId 方便排查。
 */
...
const LOCALIZED_ERROR_CODES = new Set([
  ...
  "IDEXAL_RUNTIME_MODEL_UNAVAILABLE",
  "IDEXAL_BIGMODEL_TEAM_PLAN_MEMBER_REQUIRED",
]);
...
function isModelConfigMissingError(error: Pick<IdexalUiError, "code" | "message">): boolean {
...
export function resolveChatErrorBannerDisplayMessage(
  ...
  if (isSuspiciousEmptyModelResultMessage(error.message)) {
    return intl.formatMessage({ id: "idexal.error.modelSuspiciousEmpty" });
  }

  return error.code && LOCALIZED_ERROR_CODES.has(error.code)
    ? intl.formatMessage({ id: `idexal.error.${error.code}` })
    : error.message;
}

export function shouldSuppressChatErrorBanner(
  error: Pick<IdexalUiError, "code" | "message">,
): boolean {
  // 只有历史恢复残留的模型不可用提示才隐藏；当前发送/草稿报错需要展示，
  // 否则 registry 移除模型后用户会看到“请求没返回”而没有任何可操作反馈。
  return Boolean(
    error.code === "IDEXAL_RUNTIME_MODEL_UNAVAILABLE" &&

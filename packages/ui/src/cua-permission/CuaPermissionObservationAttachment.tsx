...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@idexal/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@idexal/shared/idexal-protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type IdexalAgentCuaPermissionObservation,
...
function cuaPermissionObservationKey(observation: IdexalAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.idexalAgentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.idexalAgentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);

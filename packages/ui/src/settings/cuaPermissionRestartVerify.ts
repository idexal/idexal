// 重启 Helper 后的 accessibility 验证:吸收 tccd 传播 lag + 决定是否升级到"重启 Idexal"兜底。
//
...
// 直到超时仍 stale(或一直 unavailable/抛错)才返回 true,触发 UI 升级到"重启 Idexal"兜底。
import { isCuaPermissionStatusAvailable, type CuaPermissionStatusResult } from "@idexal/services";
 ...
 *          `true`  —— 直到超时仍 stale(或持续 unavailable/抛错),升级到"重启 Idexal"兜底。
 *
 * 语义说明:
 *  - `granted` → 重启吃到授权了,解决。

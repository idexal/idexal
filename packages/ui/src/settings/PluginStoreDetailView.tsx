...
import type { IdexalPluginComponentKind, IdexalPluginsDescribeResult } from "@idexal/shared";
...
const SECTION_ORDER: IdexalPluginComponentKind[] = ["mcp", "skill", "command", "agent", "hook"];

const SECTION_TITLE_IDS: Record<IdexalPluginComponentKind, string> = {
...
const SECTION_ICONS: Record<IdexalPluginComponentKind, typeof Server> = {
  mcp: Server,
  skill: WandSparkles,
  command: Terminal,
  agent: Bot,
...
export function PluginStoreDetailView({
  ...
  return (
    <div className="space-y-8" data-testid="plugin-store-detail" data-plugin-id={item.id}>
      ...
      {/* 组件分区：已安装走权威枚举（IdexalPluginInfo.components），候选走 plugins/describe 按需拉取。 */}
      {componentsLoading ? (
        <div
...
function InfoSection({
  ...
  item: StorePluginItem;
  describeMetadata: IdexalPluginsDescribeResult["metadata"];
}) {
  const { intl } = useIdexalIntl();

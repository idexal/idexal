import type {
  ...
  IdexalPluginInfo,
  IdexalPluginComponentKind,
  IdexalPluginsDescribeResult,
} from "@idexal/shared";
import { isPluginCommand, isUserCommand, IDEXAL_COMMAND_AGENT_SOURCE } from "@idexal/shared";
...
const COMPONENT_KIND_ORDER: IdexalPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: IdexalPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<IdexalPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: IdexalMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: IdexalMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === IDEXAL_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(
  ...
  return servers.filter((server) => {
    if (server.source !== "idexalagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: IdexalPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: IdexalPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: IdexalMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,

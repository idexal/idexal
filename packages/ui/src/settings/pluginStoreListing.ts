...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<IdexalAvailablePluginSummary, "name" | "listing">[],
  name: string,
): IdexalPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: IdexalPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: IdexalPluginStoreListing;
  summary?: IdexalAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: IdexalPluginInfo;
  installedMeta?: IdexalInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<IdexalInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly IdexalPluginMarketplaceSummary[],
  locale: string,
): IdexalPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: IdexalPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: IdexalAvailablePluginSummary[];
  installedPlugins: IdexalInstalledPluginSummary[];
  plugins: IdexalPluginInfo[];
  restorableBuiltins: IdexalAvailablePluginSummary[];
}): StorePluginItem[] {

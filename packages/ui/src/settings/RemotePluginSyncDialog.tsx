...
import type {
  ...
  RemoteTarget,
  IdexalPluginOptionValue,
  IdexalPluginInfo,
  IdexalPluginMarketplaceSummary,
  IdexalPluginUserConfigOption,
  IdexalPluginsOverviewResult,
} from "@idexal/shared";
import type { IPluginSyncService, IIdexalAgentService } from "@idexal/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, IdexalPluginOptionValue>;
  userConfig: Record<string, IdexalPluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localIdexalAgentService?: RemotePluginSyncAgentService | null;
  remoteIdexalAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: IdexalPluginsOverviewResult,
  plugins: readonly IdexalPluginInfo[],
): RemotePluginSyncCandidate[] {
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: IdexalPluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: IdexalPluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: IdexalPluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: IdexalPluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localIdexalAgentService || !params.remoteIdexalAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localIdexalAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localIdexalAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteIdexalAgentService.getPluginsOverview({
...
function isPathPluginOption(option: IdexalPluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: IdexalPluginOptionValue,
  localOption: IdexalPluginUserConfigOption | undefined,
  remoteOption: IdexalPluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: IdexalPluginInfo,
): { options: Record<string, IdexalPluginOptionValue>; skippedCount: number } {
  const options: Record<string, IdexalPluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, IdexalPluginOptionValue>,
  right: Record<string, IdexalPluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteIdexalAgentService = params.remoteIdexalAgentService;
  if (!remoteIdexalAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteIdexalAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteIdexalAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteIdexalAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteIdexalAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteIdexalAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteIdexalAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteIdexalAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteIdexalAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteIdexalAgentService.cancelPluginOperation({ operationId });
          },

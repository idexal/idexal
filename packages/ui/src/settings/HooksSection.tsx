...
import { invalidateDeferredDraftSessionForRuntimeChange } from "@/lib/idexalDraftSkillInvalidation.js";
...
function isEditableHook(hook: Hook): boolean {
  ...
  return hook.editable ?? (!hook.location || hook.location.source === "idexal");
}

// workspace-hook-trust：editable=false 且 source=idexal 的行是「上游/祖先 idexal.json
// 里的只读工作区 Hook」。它们不是外部格式兼容导入源，塞进 Legacy 会让 Import 按钮
// 必然失败（importHook 拒绝 source=idexal），也违反「只读但可逐条 Trust」的约定。
// 这类行应留在 Installed 分组，由信任状态门控 Switch，走行内 Trust 流程。
function isReadOnlyIdexalHook(hook: Hook): boolean {
  return hook.editable === false && (hook.location?.source ?? "idexal") === "idexal";
}

function isInCompatibilitySection(hook: Hook): boolean {
  return !isEditableHook(hook) && !isReadOnlyIdexalHook(hook);
}
...
function buildPluginHookRows(
  plugins: readonly Pick<IdexalPluginInfo, "enabled" | "hookDetails" | "id" | "name">[],
  installedPlugins: readonly Pick<IdexalInstalledPluginSummary, "id" | "scope">[],
...
export function HooksSection({ workspacePath, workspaceIdentity }: HooksSectionProps) {
  ...
  const idexalSessionService = useIdexalSessionService(
  ...
  const editableHooks = useMemo(
    () =>
      hooksState.hooks.filter(
        (hook) =>
          (isEditableHook(hook) || isReadOnlyIdexalHook(hook)) &&
  ...
  const invalidateDraft = useCallback(
    ...
    [targetWorkspaceIdentity, targetWorkspacePath, idexalSessionService],
  );

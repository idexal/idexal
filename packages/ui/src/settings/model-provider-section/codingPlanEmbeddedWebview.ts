...
import type { CodingPlanWebviewLocale } from "@idexal/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "idexal:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeIdexalEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeIdexalEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeIdexalEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__idexalLang__ 供 idexalBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          zcodejwttoken: credentials.idexalJwtToken?.trim() || null,
  ...
  return `(() => {
  ${storageUpdates}
  const idexalTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", idexalTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", idexalTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", idexalTheme === "zai-dark");
  localStorage.setItem("idexal-theme", idexalTheme);
  localStorage.setItem("idexal:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 idexalBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__idexalLang__ = ${JSON.stringify(resolvedLocale)};
  const idexalReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__idexalReportContext__ = idexalReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(idexalReportContext));
  window.dispatchEvent(new CustomEvent("idexal-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: idexalReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__idexalReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "idexal-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__idexalLang__ 并派发 idexal-coding-plan-lang-change 事件，
 * 官网侧（idexalBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__idexalLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("idexal-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  ...
  // BigModel OAuth callback 同样落盘（见 resolveBigModelStartPlanIdexalJwt）。
  return provider === "zai"

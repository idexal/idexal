...
import { IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID } from "@idexal/shared";
...
export function PluginStorePage({
  ...
  // 目录自动刷新（Catalog Auto-Refresh）：只针对 Idexal 官方市场。每次进入商店页都刷新 CDN 目录，
  // 否则新上架插件要等用户手动点刷新才可见；以 10 分钟窗口节流，并在发起时占位防抖（失败/在飞不重复），
  // 判据见 officialMarketplaceAutoRefresh。状态放模块级而非组件 ref，因为每次进入都是重新挂载。
  useEffect(() => {
    const official = marketplaces.find((item) => item.id === IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID);
    if (
      official &&
      claimMarketplaceAutoRefresh(IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID, official.lastUpdated)
    ) {
      void updateMarketplace(IDEXAL_OFFICIAL_PLUGIN_MARKETPLACE_ID, pluginManagementService);
    }
  ...
  }, [normalizedWorkspaceIdentity, skillsService, workspacePath, idexalSessionService]);

...
import {
  ...
  type IdexalAutomationScheduleRule,
} from "@idexal/shared";
import {
...
interface AutomationEditViewProps {
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: IdexalAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: IdexalAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: IdexalAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: IDEXAL_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<IdexalAutomationScheduleRule | undefined>(
    () =>

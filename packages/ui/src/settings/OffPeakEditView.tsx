...
interface OffPeakEditViewProps {
  editing: IdexalOffPeakTask | null;
  ...
  onDeleteHistory?: (task: IdexalOffPeakTask) => void;
...
export function OffPeakEditView({
  ...
  const thoughtLevelOption = useMemo<IdexalConfigOption | null>(
    () =>
    ...
    >
      ...
      {tab === "history" ? (
        <OffPeakHistoryTab
          ...
          {...(onOpenSession
            ? {
                onOpenSession: (task: IdexalOffPeakTask) =>
                  task.sessionId
                    ? onOpenSession({

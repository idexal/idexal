import type { IdexalCommand, IdexalSlashCommand } from "@idexal/shared";
...
function commandToSlashCommand(command: IdexalCommand): IdexalSlashCommand | null {
  if (!command.enabled) {
    return null;
  }
...
export function mergeSlashCommandsAfterCommandRefresh(
  currentSlashCommands: readonly IdexalSlashCommand[],
  refreshedCommands: readonly IdexalCommand[],
): IdexalSlashCommand[] {
  ...
  const customCommands: IdexalSlashCommand[] = [];

...
function SavedWorkflowLastRunBadge({
  ...
  run: IdexalSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: IdexalSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: IdexalSavedWorkflowEntry) => void;
  onRun: (entry: IdexalSavedWorkflowEntry) => void;
  onRevise: (entry: IdexalSavedWorkflowEntry) => void;
  onCopyPath: (entry: IdexalSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: IdexalSavedWorkflowEntry) => void;
  onDelete: (entry: IdexalSavedWorkflowEntry) => void;
}

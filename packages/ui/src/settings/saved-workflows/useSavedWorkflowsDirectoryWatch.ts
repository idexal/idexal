...
import type { IFileWatcherService } from "@idexal/services";
...
function savedWorkflowsDirectoryPath(workspacePath: string): string {
  ...
  return `${trimmed}${separator}.idexal${separator}workflows`;
}
 ...
 *
 * 项目组传 `workspacePath`（拼出 `<ws>/.idexal/workflows`）；全局组传 `directory`（协议 list 回的
 * 绝对目录，即 `~/.idexal/workflows`），二者择一——`directory` 优先。
 */
export function useSavedWorkflowsDirectoryWatch({
  ...
  useEffect(() => {
    ...
    void fileWatcherService
      ...
      .catch((error: unknown) => {
        logger.debug("[SavedWorkflows] 监听 .idexal/workflows 失败（目录可能尚不存在）", {
          path: directoryPath,

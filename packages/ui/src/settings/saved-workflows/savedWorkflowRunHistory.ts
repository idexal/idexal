...
import type { IdexalSavedWorkflowRun, IdexalSavedWorkflowRunStatus } from "@idexal/shared";
...
): Map<string, IdexalSavedWorkflowRun> {
  const byName = new Map<string, IdexalSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: IdexalSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: IdexalSavedWorkflowRun, now: number): number {

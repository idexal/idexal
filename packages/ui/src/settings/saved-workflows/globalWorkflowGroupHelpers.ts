...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type IdexalSavedWorkflowRun } from "@idexal/shared";
import type { IdexalAgentSavedWorkflowTarget } from "@idexal/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: IdexalAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: IdexalSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {

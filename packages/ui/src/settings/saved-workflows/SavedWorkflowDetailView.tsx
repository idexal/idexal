...
import {
  ...
  type IdexalSavedWorkflowEntry,
  type IdexalSavedWorkflowMeta,
  type IdexalSavedWorkflowRun,
  type IdexalWorkflowsGetResult,
} from "@idexal/shared";
import type { IIdexalAgentService, IdexalAgentSavedWorkflowTarget } from "@idexal/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<IdexalWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: IdexalSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: IdexalAgentSavedWorkflowTarget;
  agentService: IIdexalAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: IdexalSavedWorkflowEntry | undefined;
  runs: readonly IdexalSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: IdexalSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: IdexalSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: IdexalSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: IdexalSavedWorkflowMeta = {
      description,

...
// max-lines 400；载体是 `useServices().idexalAgentService`，RPC 一律带 `{ scope: "global" }`。
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { IdexalSavedWorkflowEntry, IdexalSavedWorkflowRun } from "@idexal/shared";
...
export function useSavedWorkflowGlobalGroup({
  ...
  const { idexalAgentService: agentService, fileWatcherService } = useServices();
  ...
  const [launchEntry, setLaunchEntry] = useState<IdexalSavedWorkflowEntry | null>(null);
  const [moveEntry, setMoveEntry] = useState<IdexalSavedWorkflowEntry | null>(null);
  ...
  // 目录监听：list 回的绝对目录（`~/.idexal/workflows`）本机可直接 watch。
  ...
  const handleRun = useCallback(
    (entry: IdexalSavedWorkflowEntry) => {
      launcher.clearError();
  ...
  const handleDelete = useCallback(
    async (entry: IdexalSavedWorkflowEntry) => {
  ...
  const handleCardDelete = useCallback(
    (entry: IdexalSavedWorkflowEntry) => void handleDelete(entry),
    [handleDelete],
  );
  const handleCardMove = useCallback((entry: IdexalSavedWorkflowEntry) => setMoveEntry(entry), []);
  const handleOpen = useCallback(
    (entry: IdexalSavedWorkflowEntry) => onOpenDetail(entry.name),
    [onOpenDetail],
  );
  // 全局档的目标项目按 `run.cwd` 反查已打开项目；没打开就两个入口都关掉
  // （实例必须开在发起它的项目）。门与实参构造与项目档共用（产物不需要 toolCallId）。
  const resolveRunTarget = useCallback(
    (run: IdexalSavedWorkflowRun) => findProjectByCwd(localProjects, run.cwd) ?? null,
    [localProjects],
  );

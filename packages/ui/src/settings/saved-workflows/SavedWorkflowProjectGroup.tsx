...
export function SavedWorkflowProjectGroup({
  ...
  const agentService = services.idexalAgentService;
  ...
  const launch = useCallback(
    async (entry: IdexalSavedWorkflowEntry, args: Record<string, unknown>) => {
  ...
  const handleCardPromote = useCallback(
    (entry: IdexalSavedWorkflowEntry) => void handlePromote(entry),
    [handlePromote],
  );

...
import type { IdexalSavedWorkflowRun } from "@idexal/shared";
import type {
...
export function useSavedWorkflowRunOpeners(options: {
  ...
  resolveTarget: (run: IdexalSavedWorkflowRun) => SavedWorkflowRunOpenTarget | null;
  ...
  handleOpenRun: (run: IdexalSavedWorkflowRun, workflowName: string) => void;
  handleOpenArtifact: (run: IdexalSavedWorkflowRun, artifactId: string) => void;
} {
  const { onOpenWorkflowArtifact, onOpenWorkflowRun, resolveTarget } = options;

  const handleOpenRun = useCallback(
    (run: IdexalSavedWorkflowRun, workflowName: string) => {
  ...
  const handleOpenArtifact = useCallback(
    (run: IdexalSavedWorkflowRun, artifactId: string) => {
      if (!run.parentSessionId) return;
      const target = resolveTarget(run);
      if (!target) return;
      // 行上的 chip 载荷（`IdexalSavedWorkflowRun.artifacts`）带最新版的 `contentType`：

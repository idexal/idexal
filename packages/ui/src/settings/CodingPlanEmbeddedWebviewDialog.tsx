...
import { RENDERER_IDEXAL_ENDPOINT_URLS } from "@/lib/rendererIdexalEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 官网页购买成功后的回调。
   *
   * 官网页通过 preload 注入的 window.idexalBridge.notifyPurchaseComplete({ provider })
   * 发送 idexal:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const theme = useIdexalStoreWithDefault((state) => state.theme, "zai-dark");
  const userId = useIdexalStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const computedWebviewUrl = useMemo(() => {
    ...
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_IDEXAL_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_IDEXAL_E2E_STORE_BRIDGE === "1",
        ...
        const credentials: CodingPlanEmbeddedCredentials =
          provider === "zai"
            ? {
                ...
                idexalJwtToken: values[1],
              }
            : {
                bigmodelAccessToken: values[0],
                // BigModel OAuth callback 同样会落盘 idexal JWT；官网用它在
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // ipc-message: 官网页通过 preload 的 window.idexalBridge.notifyPurchaseComplete
      // 发回购买完成信号（idexal:coding-plan-purchase-complete）。
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:idexal-coding-plan"
            src={webviewUrl}
            className={cn(

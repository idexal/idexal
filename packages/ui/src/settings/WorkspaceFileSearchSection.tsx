...
function ActiveWorkspaceFileSearchEditor({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("[WorkspaceFileSearchSection] 读取 .idexalignore 失败", {
  ...
  const applyTransform = useCallback(
    async (transform: "sync-gitignore" | "reset-defaults") => {
      ...
      try {
        ...
        logger.warn("[WorkspaceFileSearchSection] 应用 .idexalignore 分区操作失败", {
          transform,
          error: error instanceof Error ? error.message : String(error),
        });
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("[WorkspaceFileSearchSection] 保存 .idexalignore 失败", {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  // 保存语义是"把编辑框内容落盘"：template 态（.idexalignore 尚未创建）即使未编辑也允许保存，
  ...
  // 「打开文件位置」：.idexalignore 位于 workspace 根，打开根目录即所在位置

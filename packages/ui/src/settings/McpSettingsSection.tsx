 ...
 * Supports the unified Idexal Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToIdexalAgentMcpServer,
...
import { isIdexalAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@idexal/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "idexalagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 idexalAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isIdexalAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "idexalagentmcp" || !server.enabled) {
      continue;
    }
    ...
    if (!idexalAgentServer) {
      continue;
    }
    seenNames.add(server.name);
    result.push(idexalAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: IdexalAgentMcpServer[]): {
  mode: "status";
  mcpServers: IdexalAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: IdexalMcpServer[]): string {
  return servers
    .filter((server) => server.source === "idexalagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: IdexalPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, IdexalAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<IdexalMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForIdexal("idexal");
  ...
  async function handleSave(form: FormState, prev?: IdexalMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: IdexalMcpServer) {
  ...
  function handleEdit(server: IdexalMcpServer) {
  ...
  function handleOpenAuthorization(server: IdexalMcpServer) {

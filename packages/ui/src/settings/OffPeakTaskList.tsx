...
interface OffPeakTaskListProps {
  ...
  onOpen: (task: IdexalOffPeakTask) => void;
  onPause: (task: IdexalOffPeakTask) => void;
  onContinue: (task: IdexalOffPeakTask) => void;
  onCancel: (task: IdexalOffPeakTask) => void;
  onDelete: (task: IdexalOffPeakTask) => void;
  onOpenSession: (task: IdexalOffPeakTask) => void;
}

// 按状态分组会让任务在运行和终态切换时跳位，破坏用户对已有卡片位置的预期；
// 列表只按不可变的创建时间倒序，状态变化不再影响顺序。
function sortOffPeakTasksByCreatedAt(tasks: readonly IdexalOffPeakTask[]): IdexalOffPeakTask[] {

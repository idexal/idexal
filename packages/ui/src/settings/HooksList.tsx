...
import type { Hook, PluginHookDetail, PluginScope } from "@idexal/shared";
...
function ConfiguredHookRow({
  ...
  onTrust?: (hook: Hook) => Promise<void>;
  onToggle: (hook: Hook, enabled: boolean) => Promise<void>;
  /** 上游/祖先 idexal.json 的只读工作区 Hook：不可编辑、不可 toggle，但仍可逐条 Trust。 */
  readOnly?: boolean;
      ...
      actions={
        <>
          ...
          {/* workspace-hook-trust：未通过信任审核的工作区 Hook 禁止直接启用，
              ...
              只读行（上游 idexal.json）的 Switch 始终禁用：信任后展示真实配置状态，

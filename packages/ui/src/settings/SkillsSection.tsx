...
import { IDEXAL_AGENT_PROVIDER } from "@idexal/shared";
...
export function SkillsSection({
  ...
  const setEnabled = useCallback(
    async (skillId: string, enabled: boolean) => {
      ...
      // 移除三方来源后，技能状态统一写入 Idexal Agent 上下文，避免旧 provider 前缀带来分桶漂移。
      const targetSkill = skills.find((skill) => skill.id === skillId);
      const effectiveProvider: IdexalProvider = IDEXAL_AGENT_PROVIDER;
      try {
  ...
  const scopedProviderSkills = useMemo(() => {
    const allProviderSkills = filterSkillsForProvider(skills, IDEXAL_AGENT_PROVIDER);

...
import type { CuaOsSupport, CuaPermissionKind, RemoteTarget } from "@idexal/shared";
import {
  DesktopCommandIds,
  isRemoteWorkspaceIdentity,
  IDEXAL_CUA_OFFICIAL_PLUGIN_ID,
} from "@idexal/shared";
import { isCuaPermissionStatusAvailable, type CuaPermissionRestartOptions } from "@idexal/services";
...
export function ComputerUseSection({
  ...
  const cuaPlugin = plugins.find((plugin) => plugin.id === IDEXAL_CUA_OFFICIAL_PLUGIN_ID);
  const cuaEnabled = cuaPlugin?.enabled ?? false;
  const cuaToggling = togglingPluginId === IDEXAL_CUA_OFFICIAL_PLUGIN_ID;
  ...
  // 重启 Helper 后验证仍持续 stale → 显示"重启 Idexal"兜底按钮。accessibility 变 granted 时自愈清除。
  ...
  // 兜底:重启 Helper 后仍持续 stale 时,用户可一键重启 Idexal(复用 OAuth 登出同款 RelaunchApp)。
  // 新 Idexal 进程会干净地重新拉起 Helper,绕过当前进程里可能卡住的重启机制(孤儿/socket/状态污染)。
  ...
  const onTogglePlugin = useCallback(
    async (next: boolean) => {
      ...
      const completed = await runAfterSuccessfulPluginEnabledChange({
        submit: () =>
          setPluginEnabled(IDEXAL_CUA_OFFICIAL_PLUGIN_ID, next, pluginManagementService),
        isCurrent: () =>
          mountedRef.current &&
  ...
  // 自愈:accessibility 在后续任一次查询里变成 granted 时,清除"重启 Idexal"兜底(说明问题已解决)。
  useEffect(() => {
  ...
  // 保留重新授权入口与“重启 Idexal”兜底，避免把不可由单次 Helper 重启修复的状态误导成已解决。

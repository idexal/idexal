...
export function SettingsPage({
  ...
  const { intl, localePreference, setLocalePreference } = useIdexalIntl();
  ...
  >([]);
  const interfaceMode = useIdexalStore((state) => state.interfaceMode);
  const setInterfaceMode = useIdexalStore((state) => state.setInterfaceMode);
  const theme = useIdexalStore((state) => state.theme);
  const setTheme = useIdexalStore((state) => state.setTheme);
  const codePreviewSettings = useIdexalStore((state) => state.codePreviewSettings);
  const setCodePreviewSettings = useIdexalStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useIdexalStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useIdexalStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useIdexalStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useIdexalStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useIdexalStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useIdexalStore((state) => state.setNotificationSoundEnabled);
  ...
  const setNewUserOnboardingOpen = useIdexalStore((state) => state.setNewUserOnboardingOpen);
  ...
  const [idexalInteractionBehavior, setIdexalInteractionBehavior] =
    useState<IdexalInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setIdexalInteractionBehavior(settings.idexalInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setIdexalInteractionBehavior(sharedSettings.idexalInteractionBehavior ?? "queue");
  ...
  const handleIdexalInteractionBehaviorChange = useCallback(
    async (behavior: IdexalInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ idexalInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setIdexalInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
  const handleFooterLocaleChange = useCallback(
    (value: string) => {
      ...
      if (value === "ar" || value === "en-US" || value === "fr" || value === "zh-CN") {
        runUserAction({
          input: { featureId: "settings.locale", action: "change_locale", trigger: "select" },
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          <GeneralSectionContent
                            ...
                            idexalInteractionBehavior={idexalInteractionBehavior}
                            ...
                            onIdexalInteractionBehaviorChange={
                              handleIdexalInteractionBehaviorChange
                            }

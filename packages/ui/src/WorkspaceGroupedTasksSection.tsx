...
import type { IdexalGroupedTaskView, IdexalTaskGroupColor } from "@idexal/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type IdexalTaskMeta } from "@idexal/shared";
...
function getGroupedTaskViewSignature(view: IdexalGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const groupedDraftTask = useIdexalSessionStore(
    (state) =>
      selectWorkspaceIdexalState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useIdexalSessionStore(
    (state) =>
      selectWorkspaceIdexalState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useIdexalSessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<IdexalGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<IdexalGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: IdexalTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: IdexalTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.idexalTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: IdexalTaskMeta) => {
      ...
      void workspaceServices.services.idexalTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: IdexalTaskGroupColor) => {
  ...
  const setViewWithGroupedTaskAnimation = useCallback(
    (nextView: IdexalGroupedTaskView) => {

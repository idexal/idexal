...
import { idexalTaskMetaSchema } from "../../shared/src/validation.js";
...
test("current task metadata is accepted without upgrading third-party Agent identities", () => {
  assert.equal(idexalTaskMetaSchema.parse(meta).provider, "glm");
  for (const provider of ["claude", "codex", "gemini", "opencode"]) {
    assert.equal(idexalTaskMetaSchema.safeParse({ ...meta, provider }).success, false, provider);
  }
});
...
test("Idexal subagent identity wins over retired Codex nicknames", () => {
  const tool = {
    id: "tool-example",
    kind: "Agent",
    title: "Agent",

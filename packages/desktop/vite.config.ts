...
import { resolveIdexalEndpointOrigin, pickProductEndpointEnv } from "@idexal/shared/idexalEndpoint";
...
function resolveIdexalEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "idexal:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 IDEXAL_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const idexalEnv = resolveIdexalEnv(env.IDEXAL_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const idexalProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    IDEXAL_ENV: idexalEnv,
  });
  const e2eCoverageEnabled =
    env.IDEXAL_E2E_COVERAGE === "1" || process.env.IDEXAL_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_IDEXAL_E2E_STORE_BRIDGE === "1" || process.env.VITE_IDEXAL_E2E_STORE_BRIDGE === "1";
  const idexalEndpointOrigin = resolveIdexalEndpointOrigin({
    env: idexalEnv,
    envBaseOrigin: env.IDEXAL_BASE_URL ?? env.IDEXAL_ENDPOINT_ORIGIN,
  });
  ...
  return {
    ...
    define: {
      ...
      __IDEXAL_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      "import.meta.env.VITE_IDEXAL_BASE_URL": JSON.stringify(idexalEndpointOrigin),
      // 兼容旧 renderer 读取名；新代码统一读 VITE_IDEXAL_BASE_URL。
      "import.meta.env.VITE_IDEXAL_ENDPOINT_ORIGIN": JSON.stringify(idexalEndpointOrigin),
      ...
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 IDEXAL_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_IDEXAL_E2E_STORE_BRIDGE": JSON.stringify(

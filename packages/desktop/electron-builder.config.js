...
const desktopProductIdentity = resolveDesktopProductIdentity({
  ...process.env,
  IDEXAL_ENV: builtinProviderConfig.environment,
});
...
const shouldEnableMacSigning =
  process.env.IDEXAL_ENABLE_MAC_SIGN === "1" && Boolean(macSigningIdentity);
...
const desktopDistDir = process.env.IDEXAL_DESKTOP_DIST_DIR || "dist";
...
const REQUIRED_ASAR_RUNTIME_MODULES = [
  ...
  "pngjs",
  // @idexal/services 的代理连通性探测会动态 require("undici") 取 ProxyAgent。
...
const WINDOWS_INSTALL_MANIFEST_NAME = ".idexal-install-manifest";
...
function resolveElectronDownloadMirror(env = process.env) {
  const existingMirror =
    env.IDEXAL_ELECTRON_RUNTIME_MIRROR ||
...
if (
  ...
  process.env.IDEXAL_ENABLE_MAC_SIGN === "1" &&
  !macSigningIdentity
) {
  throw new Error(
    "Idexal Preview macOS packaging requires APPLE_SIGNING_IDENTITY or CSC_NAME when IDEXAL_ENABLE_MAC_SIGN=1",
  );
}
...
function resolveAppAsarPath(context) {
  if (context.electronPlatformName === "darwin") {
    const appName = `${context.packager?.appInfo?.productFilename ?? "Idexal"}.app`;
...
async function injectHoistedRuntimeModulesIntoAsar(context) {
  ...
  const stagingDir = mkdtempSync(resolve(tmpdir(), "idexal-app-asar-"));
  try {
...
export default {
  ...
  extraMetadata: {
    ...
    idexalProductFlavor: desktopProductIdentity.flavor,
    homepage: "https://idexal.com",
    author: {
      name: "Idexal",
      email: "contact@idexal.com",
    },
  },
  ...
  extraResources: [
      ...
      ? [
          {
            ...
            // 只是不吸附，所以这里不做存在性断言。
            from: "resources/macos-window-bounds/idexal-window-bounds",
            to: "macos-window-bounds/idexal-window-bounds",
          },
        ]
      : []),
    {
    ...
    {
      // Provider Registry 的 Idexal Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      from: builtinProviderConfig.sourcePath,
      to: "config/provider/idexal-builtin.json",
    },
    {
      ...
      : []),
    {
      ...
      // 桌面端内置的是 agent 的 JS bundle（glm/idexal.cjs，由 prepare:agent-bundle 生成），
      // Host 进程用 app 自带的 Electron Node runtime（ELECTRON_RUN_AS_NODE）执行 `idexal.cjs app-server --stdio`，
  ...
  protocols: [
    {
      ...
      // 展示名跟随安装包身份；scheme 仍保持 idexal，因此两个应用中最后注册者会成为默认 handler。
  ...
  mac: {
    ...
    // idexal 之前只有本地未签名打包配置，CI 即使注入了证书变量，
  ...
  linux: {
    ...
    // desktop 包名是 scoped package（@idexal/desktop），electron-builder 默认会把
    // Linux executable/Icon 推成 @idexaldesktop。部分桌面环境无法按这个 icon name 命中
    // hicolor 图标，最终回退成系统齿轮。这里固定成稳定的小写名称，让 Icon=idexal
    // 与 /usr/share/icons/hicolor/*/apps/idexal.png 保持一致。
    executableName: desktopProductIdentity.linuxExecutableName,
    category: "Development",
    maintainer: "Idexal <contact@idexal.com>",
  },
  deb: {

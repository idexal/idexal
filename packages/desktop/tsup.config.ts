import { pickProductEndpointEnv } from "@idexal/shared/idexalEndpoint";
...
function loadEnvFiles(): Record<string, string> {
  ...
  // 真实环境变量优先级最高
  if (process.env.IDEXAL_ENV) vars.IDEXAL_ENV = process.env.IDEXAL_ENV;
  if (process.env.IDEXAL_BASE_URL) vars.IDEXAL_BASE_URL = process.env.IDEXAL_BASE_URL;
  if (process.env.VITE_IDEXAL_BASE_URL)
    vars.VITE_IDEXAL_BASE_URL = process.env.VITE_IDEXAL_BASE_URL;
...
const { environment: idexalEnv } = await loadBuiltinProviderConfig();
// 安装包身份与后端环境分轴：IDEXAL_PREVIEW_IDENTITY=1 让生产后端的构建仍以 Idexal Preview 身份打包运行。
const idexalProductFlavor = resolveDesktopProductFlavor({ ...process.env, IDEXAL_ENV: idexalEnv });
console.log(`[tsup] IDEXAL_ENV=${idexalEnv} IDEXAL_PRODUCT_FLAVOR=${idexalProductFlavor}`);

export function resolveDesktopTsupBundleSecurityOptions(
  runtimeEnv: Record<string, string | undefined> = process.env,
) {
  const isProduction = runtimeEnv.NODE_ENV === "production";
  const isE2ECoverageBuild = runtimeEnv.IDEXAL_E2E_COVERAGE === "1";
  return {
...
function createSharedDefines() {
  return {
    __IDEXAL_VERSION__: JSON.stringify(buildMetadata.appVersion),
    __IDEXAL_COMMIT__: JSON.stringify(buildMetadata.buildCommitId),
    __IDEXAL_BUILD_TIME__: JSON.stringify(buildMetadata.buildTime),
    __IDEXAL_ENV__: JSON.stringify(idexalEnv),
    __IDEXAL_ENDPOINT_ENV__: JSON.stringify(pickProductEndpointEnv(env)),
    __IDEXAL_PRODUCT_FLAVOR__: JSON.stringify(idexalProductFlavor),
    // Computer Use Helper build identity — helperInstaller 读它决定下载哪个 Helper bundle。
    // 缺失时 installer 抛 "Packaged Idexal is missing its embedded Computer Use Helper build identity"。
    // CI 构建时通过 IDEXAL_CUA_HELPER_BUILD_ID env 注入；dev 为空串走兜底（dev helper 不走下载）。
    __IDEXAL_CUA_HELPER_BUILD_ID__: JSON.stringify(
      process.env.IDEXAL_CUA_HELPER_BUILD_ID?.trim() ?? "",
    ),
    // 客户端只有一个 CDN 配置，与发布端 OSS 目标列表分离。
    __IDEXAL_CDN_BASE_URL__: JSON.stringify(env.IDEXAL_CDN_BASE_URL?.trim() || ""),
  };
}
...
export default defineConfig([
  {
    ...
    entry: {
      ...
      "main/idexalDataSizeWorker": "src/main/idexalDataSizeWorker.ts",
    ...
    noExternal: [
      ...
      "@idexal/provider",
      "@idexal/provider-node",
      // services 已内联进 main，但其 producer import 曾被保留为裸包引用；
      // electron-builder 又会排除 node_modules/@idexal，导致安装包启动即 ERR_MODULE_NOT_FOUND。
  ...
  },
  {
    ...
    platform: "node",
    target: "node22",
    // 与 host 同构：常驻 cron scheduler 进程复用 @idexal/services（tasks-index + cron），

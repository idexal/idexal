...
type PatchedIpcRenderer = IpcRenderer & { __idexalArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __idexalArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__idexalArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__idexalArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__idexalArmsIpcPatched) {
    return;
  }
  ...
  patched.__idexalArmsIpcPatched = true;
}

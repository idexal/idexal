...
contextBridge.exposeInMainWorld("__IDEXAL_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatIdexalRendererProcessName,
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__idexalFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatIdexalRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 idexal-* title，便于按窗口角色筛选。
...
contextBridge.exposeInMainWorld("idexal", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@idexal/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  reportLocalTtftBatch: (batch: import("@idexal/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.idexal/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@idexal/shared").ChromeBrowserDataImportOptions) =>
  ...
  getIdexalStdioTapDevState: (): Promise<IdexalStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetIdexalStdioTapDevState),

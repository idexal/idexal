...
// - 在官网页主世界挂 window.idexalBridge，暴露三个能力：
...
// - getLang 读 main world 的 window.__idexalLang__（由 App executeJavaScript 注入）；
//   onLangChange 在 main world 监听 idexal-coding-plan-lang-change CustomEvent
...
const PUBLIC_BRIDGE_KEY = "idexalBridge";
const NATIVE_BRIDGE_KEY = "__idexalCodingPlanWebviewNativeBridge__";
const LANG_VAR = "__idexalLang__";
const LANG_CHANGE_EVENT = "idexal-coding-plan-lang-change";
const REPORT_CONTEXT_VAR = "__idexalReportContext__";
  ...
  try {
    ...
    if (
      !isTrustedCodingPlanWebviewOrigin(url.origin, {
        e2eStoreBridgeEnabled: process.env.VITE_IDEXAL_E2E_STORE_BRIDGE === "1",
      })
    ) {
      return false;
    }
    ...
    // 该页支付成功后仍需 idexalBridge.notifyPurchaseComplete 通知 App 刷新模型设置。
    ...
    notifyPurchaseComplete(payload: NotifyPurchaseCompletePayload) {
      try {
        ipcRenderer.sendToHost(CodingPlanWebviewChannels.PurchaseComplete, {
          ...
          timestamp: Date.now(),
        } satisfies import("@idexal/shared").CodingPlanPurchaseCompletePayload);
      } catch {
  ...
  contextBridge.executeInMainWorld({
    func: (
      ...
      const bridge = {
        ...
        // App locale 变化时用 executeJavaScript 派发 idexal-coding-plan-lang-change 事件。

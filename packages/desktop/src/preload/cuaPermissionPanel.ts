...
const CUA_PERMISSION_PANEL_STATE_CHANNEL = "idexal:cua-permission-panel-state";

interface CuaPermissionPanelState {
  permission: CuaPermissionKind;
  /** main 进程维护的 Idexal 当前界面语言；浮窗不得另读系统语言或 localStorage。 */
  locale: Locale;
  /** 真实 Idexal 图标（data URL）；读不到时为 null，页面保留占位图形。 */

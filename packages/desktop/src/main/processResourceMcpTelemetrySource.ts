...
import {
  ...
  IDEXAL_MCP_RESOURCE_SAMPLE_INTERVAL_MS,
...
interface StoredSample {
  sample: IdexalMcpResourceSample;
...
function purgeExpired(now: number): void {
  for (const [key, entry] of latest) {
    if (now - entry.receivedAt > IDEXAL_MCP_RESOURCE_SAMPLE_INTERVAL_MS * 2) latest.delete(key);
  }
}
...
export function ingestMcpResourceSamples(
  ...
  const parsed = idexalMcpResourceSamplesSchema.safeParse(raw);
  ...
  sample(context) {
    ...
    for (const [key, group] of groupSamples()) {
      recordExternalAppResourceSample({
        ...
        intervalMs: IDEXAL_MCP_RESOURCE_SAMPLE_INTERVAL_MS,

...
/* eslint-disable max-lines -- host process 统一处理 main↔host 生命周期、日志、Idexal Agent，拆分前先保持跨进程消息收口。 */
...
export interface HostInitMessage {
  ...
  /** Main 解析后的 Idexal Built-in Provider Config 路径；Host/Services 不感知 Electron 安装布局。 */
...
export function spawnHostProcess(
  ...
  const execArgv = [
    ...(RUNTIME_IDEXAL_DEBUG ? [`--inspect-brk=${RUNTIME_IDEXAL_DEBUG}`] : []),
    "--no-warnings",
  ];
  const child = electronUtilityProcess.fork(hostModulePath, [], {
    serviceName: formatIdexalHostProcessName(label),
    execArgv,
    env: {
      ...buildHostProcessEnv(dependencies.hostProcessLocalEnv),
      ...buildHostE2ECoverageEnv(),
      IDEXAL_PROCESS_LABEL: label,
      // macOS-only: the Computer Use Helper launcher runs inside this forked host utilityProcess, whose
      // code-signing identity is a nested Electron helper (NOT dev.idexal.app). Publish THIS (main
      // Electron) process's pid — which IS dev.idexal.app — so helperLauncher passes it as
      ...
      ...(process.platform === "darwin" ? { IDEXAL_CUA_LAUNCHER_PID: String(process.pid) } : {}),
      ...(dependencies.desktopContextPromptEnabled
        ? {
            [IDEXAL_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]: dependencies.desktopContextPromptEnabled()
              ? "1"
              : "0",
          }
        : {}),
    ...
    if (result.data.type === HostResponseTypes.BotRemoteWorkspaceRuntimePortRequest) {
      ...
      if (!handler) {
        child.postMessage({
          ...
          requestId: request.requestId,
          ok: false,
          // Bugfix: 远端 Bot 不能在缺少 runtime bridge 时回落到本地 Idexal Agent，
...
export function disposeHostProcess(
  ...
  // 如果 main 仍按 150/300ms 强杀 host，host 会先退出，idexal-cli/app-server 子进程就可能被 init 接管成孤儿。

...
function resolveDefaultReportIntervalMs(): number {
  ...
  if (process.env.IDEXAL_ENV === "test" && process.env.IDEXAL_E2E_RUN_ID?.trim()) {
    return 60_000;
  }
  return 300_000;
}
...
export function ingestToolExecResource(
  ...
  const parsed = idexalToolExecResourceSchema.safeParse(raw);
...
function auditDisabledAgentMetricProbe(logger: ResourceLogger | undefined): void {
  if (
    ...
    process.env.IDEXAL_ENV !== "test" ||
    !process.env.IDEXAL_E2E_RUNTIME_LOG_DIR?.trim()
  ) {
    return;
  }

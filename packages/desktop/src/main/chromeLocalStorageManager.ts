...
import type { ChromeBrowserDataImportError } from "@idexal/shared";
...
export async function readChromeCookiesWithHelper(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "idexal-chrome-cookie-helper-"));
...
export async function importChromeLocalStorage(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "idexal-chrome-local-storage-"));
  const userDataPath = join(tempRoot, "User Data");
  const targetProfilePath = join(userDataPath, "Default");
  let originCount = 0;
  try {

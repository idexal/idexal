...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@idexal/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "idexal-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "idexal.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "idexal.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@idexal/local-ttft", "1");
  const duration = meter.createHistogram("idexal.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("idexal.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("idexal.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "idexal.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("idexal.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("idexal.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("idexal.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("idexal.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("idexal.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("idexal.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("idexal.local_ttft.records");
  const drops = meter.createCounter("idexal.local_ttft.dropped");

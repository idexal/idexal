...
import { formatTimestamp } from "@idexal/shared";
import { cleanupExpiredLogFiles, LOG_RETENTION_DAYS } from "./logRetention.js";
import { getAppConfigDir, maybeThrowInjectedFsFault } from "@idexal/services/node";

function getLogDir() {
  const e2eLogDir =
    process.env.IDEXAL_ENV === "test" ? process.env.IDEXAL_E2E_RUNTIME_LOG_DIR?.trim() : undefined;
  if (e2eLogDir) {
    return e2eLogDir;
  }
...
/**
 * main 进程日志，默认写入 ~/.idexal/v2/logs/YYYY-MM-DD.log；E2E 测试使用 worker 专属目录。

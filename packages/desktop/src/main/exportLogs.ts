...
function getIdexalDataDir() {
  return getAppConfigDir();
}

function getIdexalCliDir() {
  return join(homedir(), ".idexal", "cli");
}

function getIdexalCliLogDir() {
  return join(getIdexalCliDir(), "log");
}
...
function getCuaHelperRunDir() {
  return join(homedir(), ".idexal", "computer-use", "run");
}
...
function isExcludedRelativePath(relativePath: string): boolean {
  ...
  // 过去显式收集 ~/.idexal/cli/debug 会把这类上下文带进手动导出和反馈完整日志，这里按目录段统一跳过。
  ...
  // ~/.idexal/v2/dev 保存 stdio-traffic 等高频协议流，真实机器上会累计到 GB 级。
...
function shouldApplyLogExportRetention(archivePath: string): boolean {
  ...
  if (
    ...
    normalizedArchivePath.startsWith(".idexal/cli/log/")
  ) {
    return true;
  }
...
async function createLogArchiveArtifacts(
  ...
  const idexalCliLogDir = getIdexalCliLogDir();
  // GLM / idexal-cli 的运行日志写在 ~/.idexal/cli/log，不在应用主数据目录 ~/.idexal/v2 下。
  // 如果导出日志只扫描 v2，定位 agent CLI 启动、协议或崩溃问题时会缺少最关键的原生侧日志。
  await collectLogArchiveFilesFromDirectory(
    idexalCliLogDir,
    posix.join(".idexal", "cli", "log"),
    visitedDirs,
    files,
  );

  const idexalCliDir = getIdexalCliDir();
  // 排查 agent CLI 问题还需要它的运行配置与模型 IO 轨迹。
  // config.json 是当前生效配置；rollout 是 model-io 调用轨迹，
  // 二者都不在 ~/.idexal/cli/log 下，需要额外收集才能完整还原现场。
  await collectLogArchiveFile(
    join(idexalCliDir, "config.json"),
    posix.join(".idexal", "cli", "config.json"),
    files,
  );
  await collectLogArchiveFilesFromDirectory(
    join(idexalCliDir, "rollout"),
    posix.join(".idexal", "cli", "rollout"),
    visitedDirs,
    files,
  );
  ...
  // ~/.idexal/computer-use/run/<socket>.exit.log，既不在 app data 也不在 ~/.idexal/cli 下。
  // 同目录下有 .tokens broker 凭据，因此按文件名白名单只收 *.exit.log，不递归该目录。
  await collectLogArchiveFilesByName(
    getCuaHelperRunDir(),
    posix.join(".idexal", "computer-use", "run"),
...
export async function createFeedbackLogArchiveFromExportLogs(
  ...
  return createFeedbackDiagnosticArchive({
    sources: [
      ...
      { directory: getIdexalCliLogDir(), archivePrefix: ".idexal/cli/log" },
      {
        directory: getCuaHelperRunDir(),
        archivePrefix: ".idexal/computer-use/run",
        exitLogsOnly: true,
      },
    ],
...
export async function exportLogs(
  ...
  try {
    ...
    const getSourceDir = dependencies.getIdexalDataDir ?? getIdexalDataDir;
    ...
    const exportBaseName = `idexal-logs-${timestamp}`;

...
function getUserCliConfigPath(): string {
  return buildDirectoryConfigPath(IDEXAL_MCP_DESCRIPTOR, "user");
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  // `.idexal` 是强优先级来源；只要读到 MCP server，同 scope 的 `.agents` 就不再参与。
...
export async function loadCliMcpFromUserDirectory(
  ...
  // 去掉其他 provider 后，Idexal Agent 只按目录约定读取；先 workspace，再 user。

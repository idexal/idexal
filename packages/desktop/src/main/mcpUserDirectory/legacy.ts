...
function buildLegacyCommonMcpStorageCandidates(request?: MigrateLegacyCommonMcpRequest): string[] {
  ...
  candidates.push(join(appData, "ai.z.idexal", "store.json"));

  candidates.push(
    join(localAppData, "ai.z.work", "EBWebView", "Default", "Local Storage", "leveldb"),
    join(appData, "Idexal", "Local Storage", "leveldb"),
    join(appData, "Idexal", "Partitions", "idexal-embedded-browser", "Local Storage", "leveldb"),
    join(appData, "Idexal Dev", "Local Storage", "leveldb"),
    join(
      appData,
      "Idexal Dev",
      "Partitions",
      "idexal-embedded-browser",
      "Local Storage",
      "leveldb",
    ),
  );

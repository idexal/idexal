...
const ABOUT_APPLICATION_NAME = "Idexal Desktop App";
...
> = {
  "zh-CN": {
    aboutTitle: "关于 Idexal",
    versionLabel: "版本",
    okButtonLabel: "确定",
    optimizedForAppleSilicon: "已针对 Apple Silicon 优化。",
    copyright: (year) => `版权所有 © ${year} Idexal。`,
  },
  "en-US": {
    aboutTitle: "About Idexal",
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? IDEXAL_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? IDEXAL_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? IDEXAL_BUILD_TIME),
    environment: normalizeValue(options.environment ?? IDEXAL_ENV),

...
import {
  ...
  updateIdexalStdioTapDevMenuState,
...
import { resolveIdexalBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveIdexalEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopIdexalDataSizeTelemetry,
  stopDesktopIdexalDataSizeTelemetry,
} from "./desktopIdexalDataSizeTelemetry.js";
...
import { mapIdexalEnvToArmsRumEnv } from "@idexal/shared";
import {
...
// 会和开发态已打开的 Idexal Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.IDEXAL_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".idexal", "v2", "setting.json");
...
async function resolveCurrentIdexalEndpointOrigin() {
  return resolveIdexalEndpointOrigin({
    env: IDEXAL_ENV,
    envBaseOrigin: resolveIdexalEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).idexalEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 Idexal 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: IDEXAL_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: IDEXAL_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    IDEXAL_TELEMETRY_ENABLED && IDEXAL_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，idexal-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.IDEXAL_E2E_RUN_ID?.trim()) {
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onIdexalEndpointChanged: handleIdexalEndpointChanged,
    idexalEndpointEnvBaseOrigin: resolveIdexalEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveIdexalEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (IDEXAL_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentIdexalEndpointOrigin();
  if (origin === DEFAULT_IDEXAL_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleIdexalEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveIdexalEndpointSelection(), mainSettingService.get()]).then(
    ([idexalEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        idexalEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          idexalBuiltinProviderConfigFilePath: resolveIdexalBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(IDEXAL_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 Idexal 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: IDEXAL_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapIdexalEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (IDEXAL_TELEMETRY_ENABLED && IDEXAL_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: IDEXAL_VERSION,
  ...
  registerDesktopIdexalDataSizeTelemetry({
    ...
    isIdexalBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getIdexalDataRootDir(),
    stateFile: join(app.getPath("userData"), "idexal-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 IDEXAL_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    IDEXAL_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime

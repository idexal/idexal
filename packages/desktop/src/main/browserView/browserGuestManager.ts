...
export class BrowserGuestManager {
  ...
  attachGuest(
    ...
    if (guest.getType() !== "webview") {
      ...
      // CDP/Runtime 输入会直接操作 Idexal composer。IAB 只允许真实 <webview> guest fail closed。
      this.log?.(
  ...
  private async installRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    await guest.executeJavaScript(`(() => {
      const id = "__idexal_browser_recording_cursor";
      ...
      window["__idexalBrowserRecordingCursorCleanup"]?.();
      ...
      window["__idexalBrowserRecordingCursorCleanup"] = () => {
        ...
        cursor.remove();
        delete window["__idexalBrowserRecordingCursorCleanup"];
      };
    })()`);
  }
  ...
  private async removeRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    ...
    await guest
      .executeJavaScript('window["__idexalBrowserRecordingCursorCleanup"]?.()')
      .catch(() => undefined);
  }
  ...
  private finalizeTabs(
    ...
    // 产品语义：IAB tab 在当前 Idexal 进程内默认持久。keep 是状态标记集合，不是清理白名单；

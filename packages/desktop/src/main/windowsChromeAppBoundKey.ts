...
import { IDEXAL_COMMIT, IDEXAL_VERSION } from "@idexal/shared";
import {
...
const HELPER_PROTOCOL = "IDEXAL_BROWSER_IMPORT_V1";
const HELPER_VERSION_PROTOCOL = "IDEXAL_BROWSER_IMPORT_HELPER";
const HELPER_VERSION = "2";
const HELPER_TIMEOUT_MS = 90_000;
const HELPER_MAX_OUTPUT_BYTES = 128 * 1024;
const HELPER_FILENAME = "idexal-browser-import-helper.exe";
...
async function verifyAuthenticodePair(
  ...
  const script = [
    "$stream=[IO.File]::Open($idexalArg0,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);",
    "try{",
    "$helper=Microsoft.PowerShell.Security\\Get-AuthenticodeSignature -LiteralPath $idexalArg0;",
    "$app=Microsoft.PowerShell.Security\\Get-AuthenticodeSignature -LiteralPath $idexalArg1;",
...
async function runLockedPackagedHelper(options: {
  ...
  const invocation =
    options.mode === "broker"
      ? "$request=[Console]::In.ReadToEnd();$response=$request | & $idexalArg0 --broker --parent-pid $idexalArg2;"
      : "$response=& $idexalArg0 --version;";
  const script = [
    "$stream=[IO.File]::Open($idexalArg0,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read);",
    "try{",
    "$sha=[Security.Cryptography.SHA256]::Create();",
    ...
    "finally{$sha.Dispose()};",
    `if($digest -ne $idexalArg1){${hashMismatchResponse}};`,
    invocation,
...
async function verifyHelper(helperPath: string, options: ReadAppBoundKeyOptions): Promise<string> {
  ...
  try {
    ...
    const expectedAppVersion = options.expectedAppVersion ?? IDEXAL_VERSION;
    const expectedBuildCommit = options.expectedBuildCommit ?? IDEXAL_COMMIT;
    if (
      version.exitCode !== 0 ||
      fields.length !== 5 ||

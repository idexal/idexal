...
import { formatIdexalRendererProcessName } from "@idexal/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatIdexalRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatIdexalRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatIdexalRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}

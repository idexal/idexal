...
import type { DesktopTitleBarTheme, Locale } from "@idexal/shared";
import {
  DEFAULT_LOCALE,
...
// Coding Plan 官网页专用 preload：挂 window.idexalBridge 供官网回传购买完成信号。
...
function attachEmbeddedBrowserWindowOpenHandler(options: {
  ...
  options.guestWebContents.on("will-navigate", (event, url) => {
    ...
    // 离开可信购买页时必须阻断 guest 导航并交给系统浏览器，避免第三方页面继承 idexalBridge。
    event.preventDefault();
    void shell.openExternal(url).catch((error: unknown) => {
...
export function createBrowserWindow(options: {
  ...
  win.webContents.on("will-attach-webview", (event, webPreferences, params) => {
    ...
    //
    // Coding Plan 官网页例外：它需要 window.idexalBridge 回传购买完成信号，
    ...
    // 如果放任 Electron 默认处理，又会创建脱离 Idexal 的 BrowserWindow。这里由宿主重新打开

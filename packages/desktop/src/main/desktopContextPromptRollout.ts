...
export { IDEXAL_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV };
...
export function createElectronDesktopContextPromptConfigFetcher(options: {
  ...
  return async (signal) => {
    ...
    const url = new URL(`${buildIdexalEndpointUrls(endpointOrigin).origin}/api/v1/client/configs`);
    ...
    return await new Promise<unknown>((resolve, reject) => {
      ...
      const sourceHeaders = buildIdexalSourceHeadersFromContext({
        appVersion: options.appVersion,
        arch: process.arch,
        deviceMid: options.deviceMid,
        endpointOrigin,
        platform: process.platform,
        releaseChannel: IDEXAL_ENV,

...
import {
  ...
  resolveIdexalEndpointOrigin,
} from "@idexal/shared";
import { readIdexalStdioTapDevState, setIdexalStdioTapDevEnabled } from "@idexal/services/node";
...
export const HELP_TOGGLE_IDEXAL_STDIO_TAP_MENU_ID = "help.toggle-idexal-stdio-tap";
const IDEXAL_ENDPOINT_PROMPT_WIDTH = 460;
const IDEXAL_ENDPOINT_PROMPT_HEIGHT = 210;
const CODING_PLAN_WEBVIEW_PARTITION = "persist:idexal-coding-plan";
...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.idexal/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.idexal/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.idexal/v2:", error);
  }
...
async function promptCustomIdexalEndpoint(
  targetWindow: BrowserWindow | null | undefined,
  currentValue: string,
): Promise<string | undefined> {
  return showIdexalEndpointPromptWindow({
    currentValue,
...
function buildIdexalEndpointPromptHtml(currentValue: string): string {
  const value = escapeHtmlAttribute(currentValue);
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>Idexal Endpoint</title>
    <style>
  ...
  <body>
    <form id="form">
      <label for="endpoint">Idexal endpoint origin</label>
    ...
    <script>
      ...
      const submit = (value) => { document.title = "idexal-endpoint-submit:" + encodeURIComponent(value); };
      document.getElementById("form").addEventListener("submit", (event) => {
        event.preventDefault();
        submit(input.value);
      });
      document.getElementById("cancel").addEventListener("click", () => {
        document.title = "idexal-endpoint-cancel";
      });
      input.focus();
      input.select();
    </script>
...
function showIdexalEndpointPromptWindow(options: {
  currentValue: string;
  ...
  return new Promise((resolve) => {
    ...
    const promptWindow = new BrowserWindow({
      width: IDEXAL_ENDPOINT_PROMPT_WIDTH,
      height: IDEXAL_ENDPOINT_PROMPT_HEIGHT,
      parent: options.parentWindow,
      modal: Boolean(options.parentWindow),
      resizable: false,
      minimizable: false,
      maximizable: false,
      title: "Idexal Endpoint",
      webPreferences: {
    ...
    promptWindow.on("page-title-updated", (event, title) => {
      if (title === "idexal-endpoint-cancel") {
        event.preventDefault();
        finish(undefined);
        return;
      }
      if (!title.startsWith("idexal-endpoint-submit:")) {
        return;
      }
      event.preventDefault();
      finish(decodeURIComponent(title.slice("idexal-endpoint-submit:".length)));
    });
    ...
    void promptWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        buildIdexalEndpointPromptHtml(options.currentValue),
      )}`,
    );
  });
}

async function setIdexalEndpointOverride(options: {
  value: string | undefined;
  settingService: { update(patch: { idexalEndpointOrigin?: string | undefined }): Promise<void> };
  onIdexalEndpointChanged: () => Promise<void> | void;
  logger: { warn: (...args: unknown[]) => void };
}) {
  if (IDEXAL_ENV === "production") {
    return;
  }
  const normalized = options.value ? normalizeIdexalEndpointOrigin(options.value) : undefined;
  await options.settingService.update({ idexalEndpointOrigin: normalized });
  await options.onIdexalEndpointChanged();
}
...
function toggleIdexalStdioTapDevProxy(options: {
  logger: { info: (...args: unknown[]) => void };
  updateIdexalStdioTapDevMenuState: () => void;
}) {
  const current = readIdexalStdioTapDevState();
  const next = setIdexalStdioTapDevEnabled(!current.enabled);
  options.updateIdexalStdioTapDevMenuState();
...
function resolveChangelogUrl(
  locale: Locale,
  endpointOrigin = DEFAULT_IDEXAL_ENDPOINT_ORIGIN,
): string {
  // 帮助菜单里的外链以前只有固定英文地址，切到中文界面后仍会落到英文 changelog。
  // 这里统一收口到主进程按当前应用语言分流，避免菜单模板里手写分支后续再出现多处不一致。
  const origin = buildIdexalEndpointUrls(endpointOrigin).origin;
...
async function resolveCurrentIdexalEndpointOrigin(settingService: {
  get(): Promise<{ idexalEndpointOrigin?: string }>;
  ...
  return resolveIdexalEndpointOrigin({
    ...
    overrideOrigin: settings.idexalEndpointOrigin,
  });
}
...
export async function executeDesktopCommand(options: {
  ...
  settingService: {
    get(): Promise<Pick<AppSettings, "idexalEndpointOrigin" | "desktopZoomLevel">>;
    update(
      patch: Partial<Pick<AppSettings, "idexalEndpointOrigin" | "desktopZoomLevel">>,
    ): Promise<void>;
  };
  idexalEndpointEnvBaseOrigin?: string | null;
  credentialsDir: string;
  ...
  switch (options.command) {
    ...
    case DesktopCommandIds.OpenChangelog:
      await openChangelog(
        ...
        await resolveCurrentIdexalEndpointOrigin({
          ...options.settingService,
          envBaseOrigin: options.idexalEndpointEnvBaseOrigin,
        }),
      );
      return;
    case DesktopCommandIds.CheckForUpdates:
      // 按产品身份而不是后端环境放行：生产后端的 Preview 同样没有更新器。
      if (IDEXAL_PRODUCT_FLAVOR === "production") {
    ...
    case DesktopCommandIds.OpenResourceManager:
      openResourceManager();
      return;
    case DesktopCommandIds.ToggleIdexalStdioTapDevProxy:
      toggleIdexalStdioTapDevProxy({
        logger: options.logger,
        updateIdexalStdioTapDevMenuState: options.updateIdexalStdioTapDevMenuState,
      });
      return;
    case DesktopCommandIds.SetIdexalEndpointProduction:
      await setIdexalEndpointOverride({
        value: DEFAULT_IDEXAL_ENDPOINT_ORIGIN,
        settingService: options.settingService,
        onIdexalEndpointChanged: options.onIdexalEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetIdexalEndpointTest:
      await setIdexalEndpointOverride({
        value: options.idexalEndpointEnvBaseOrigin ?? resolveRuntimeIdexalEndpointOrigin(),
        settingService: options.settingService,
        onIdexalEndpointChanged: options.onIdexalEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetIdexalEndpointCustom: {
      const current =
        (await options.settingService.get()).idexalEndpointOrigin ?? DEFAULT_IDEXAL_ENDPOINT_ORIGIN;
      const value = await promptCustomIdexalEndpoint(targetWindow, current);
      if (!value) {
        return;
      }
      try {
    ...
    case DesktopCommandIds.ResetIdexalEndpoint:
      await setIdexalEndpointOverride({
        value: undefined,
        settingService: options.settingService,
        onIdexalEndpointChanged: options.onIdexalEndpointChanged,

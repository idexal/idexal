...
import { getAppConfigDir } from "@idexal/services/node";
import {
...
const LOCAL_ONLY_CRASH_SUBMIT_URL = "https://idexal.invalid/local-crash-only";
...
export function registerCrashEventMonitor(
  ...
  app.on("render-process-gone", (_event, webContents, details) => {
    ...
    // 这里在事件后补两次延迟归档，把原始 dump 复制到 ~/.idexal/v2/crash/archive，

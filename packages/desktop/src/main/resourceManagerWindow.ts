...
import {
  formatIdexalAgentProcessName,
  formatIdexalGpuProcessName,
  formatIdexalHostProcessName,
  formatIdexalMainProcessName,
  formatIdexalRendererProcessName,
  formatIdexalUtilityProcessName,
...
function collectElectronProcesses(): ResourceUsageProcess[] {
  ...
  for (const m of metrics) {
    if (m.type === "GPU") {
      ...
      push(baseProcess(m.pid, formatIdexalGpuProcessName(), "gpu", cpuPercent, memoryBytes));
    }
  }
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    push(
      baseProcess(
        rendererPid,
        formatIdexalRendererProcessName(win.getTitle() || `Window ${win.id}`),
        "renderer",
        cpuPercent,
        memoryBytes,
      ),
  ...
  for (const [label, child] of hostProcesses) {
    ...
    push(
      baseProcess(child.pid, formatIdexalHostProcessName(label), "host", cpuPercent, memoryBytes),
    );
  }
  ...
  for (const m of metrics) {
    ...
    push(
      baseProcess(
        m.pid,
        m.type === "Utility"
          ? formatIdexalUtilityProcessName(m.name || String(m.pid))
          : formatIdexalUtilityProcessName(m.type, "process"),
        "utility",
        cpuPercent,
        memoryBytes,
      ),
...
function collectUnsampledAgentProcesses(sampledPids: Set<number>): ResourceUsageProcess[] {
  ...
  for (const processes of hostAgentProcesses.values()) {
    for (const agent of [...processes.values()].sort(
      ...
      rows.push({
        pid: agent.pid,
        name: formatIdexalAgentProcessName(agent.provider, agent.workspacePath),
        category: "base",
        groupKey: "cli",
        groupLabel: "cli",
        cpuPercent: 0,

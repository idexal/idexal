...
import { PlatformChannels, type RendererActionTraceConfigV1 } from "@idexal/shared";
...
function resolveRuntimeConfig(
  ...
  if (isTruthy(env.IDEXAL_LOCAL_TTFT_ENABLED)) config = { ...config, localTtftEnabled: true };
  if (!isTruthy(env.IDEXAL_RENDERER_ACTION_TRACE_ENABLED)) return config;
  return {
    ...config,
    enabled: true,
    sampleRatio: 1,

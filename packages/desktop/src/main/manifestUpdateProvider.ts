...
function buildElectronManifestUrl(options: {
  ...
  const url = options.manifestUrl?.trim()
    ...
    : new URL(ELECTRON_MANIFEST_API_PATH, normalizeIdexalEndpointOrigin(options.endpointOrigin));
...
export class ManifestUpdateProvider extends Provider<UpdateInfo> {
  ...
  private resolveBaseUrl = new URL(DEFAULT_IDEXAL_ENDPOINT_ORIGIN);
  ...
  constructor(
    ...
    this.resolveBaseUrl = new URL(
      normalizeIdexalEndpointOrigin(options.endpointOrigin ?? DEFAULT_IDEXAL_ENDPOINT_ORIGIN),
    );
  }
  ...
  override async getLatestVersion(): Promise<UpdateInfo> {
    ...
    return {
      ...
      idexalReleaseChannel: releaseChannel,
    } as UpdateInfo;
  }
  ...
  private async resolveEndpointOrigin(): Promise<string> {
    const resolved =
      ...
      DEFAULT_IDEXAL_ENDPOINT_ORIGIN;
    return normalizeIdexalEndpointOrigin(resolved);
  }

...
import type { CuaHelperInstallerOptions } from "@idexal/services/node";
import {
  resolveHelperPermissionSubjectIdentity,
  type HelperPermissionSubjectIdentity,
} from "@idexal/services/cua-permission-broker";
import type {
...
async function verifyHelperPermissionIdentityUnchanged(
  ...
  if (!cuaHelperBundleFingerprintUnchanged(identity.appPath, fingerprint)) {
    throw new Error(`Idexal Computer Use changed while its ${phase} signature was being verified`);
  }
  ...
  if (!sameHelperPermissionIdentity(identity, currentIdentity)) {
    throw new Error(`Idexal Computer Use permission identity changed during ${phase} verification`);
  }
  if (!cuaHelperBundleFingerprintUnchanged(identity.appPath, fingerprint)) {
    throw new Error(
      `Idexal Computer Use changed while its ${phase} permission identity was being resolved`,
    );
  }
  return fingerprint;
}
...
export async function openCuaPermissionOnboarding(
  ...
  if (platform !== "darwin") {
    return {
      success: false,
      error: "Idexal Computer Use permissions are only available on macOS.",
    };
  }
  ...
  try {
    ...
    // IDEXAL_CUA_HELPER_ALLOW_UNSIGNED_LOCAL 承接：通过 dev 校验时 ensureInstalled 会正常返回本地 app，
    // 根本不会进到这个 catch；只有真正校验失败才会到这里。
    return {
      success: false,
      error: `Idexal Computer Use is unavailable (install/verification failed): ${messageOf(error)}`,
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      returnedFromSettings: false,
      error: `Idexal Computer Use permission identity verification failed: ${messageOf(error)}`,
    };
  }
...
export async function prepareCuaHelperPermissionDrag(
  ...
  try {
    ...
    if (!cuaHelperBundleFingerprintUnchanged(helperAppPath, verifiedFingerprint)) {
      throw new Error("Idexal Computer Use changed while its drag signature was being verified");
    }
    const identity = await (
      options.resolveHelperIdentity ?? resolveHelperPermissionSubjectIdentity
    )(helperAppPath);
    if (!cuaHelperBundleFingerprintUnchanged(helperAppPath, verifiedFingerprint)) {
      throw new Error("Idexal Computer Use changed while its drag identity was being resolved");
    }
    return {
      success: true,
      helperAppPath,

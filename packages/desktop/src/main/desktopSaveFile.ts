...
import type { SaveFileRequest, SaveFileResult } from "@idexal/shared";
import { PlatformChannels } from "@idexal/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "idexal-save-file-"));

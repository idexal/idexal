 ...
 *    「系统设置」之上，用户要在它和设置页之间拖拽。只设 focusable 时点击仍会激活 Idexal.app，
...
interface CreateCuaPermissionDragPanelOptions {
  ...
  /** Idexal 当前界面语言。每次 show 都重新读取，禁止浮窗自行猜测系统语言。 */
  getLocale: () => Locale;
  logger: {
...
export const CUA_PERMISSION_PANEL_STATE_CHANNEL = "idexal:cua-permission-panel-state";
    ...
    show(permission: CuaPermissionKind): void {
      ...
      if (!window || window.isDestroyed()) {
        ...
        window.onceReadyToShow(() => {
          ...
          // 英文 Idexal 也会显示硬编码中文。先发当前 locale 再显示，避免中文 fallback 闪屏。

...
import { IDEXAL_AGENT_LIFECYCLE_LOG_MARKER } from "@idexal/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 idexal-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapIdexalEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
    .init({
      ...
      endpoint: IDEXAL_ARMS_RUM_ENDPOINT,
      env: armsRumEnv,
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(IDEXAL_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${IDEXAL_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  IDEXAL_TELEMETRY_ENABLED && IDEXAL_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();

...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@idexal/shared";

import type { IdexalDataSizeScanResult } from "./idexalDataSizeScanner.js";
import {
  readIdexalDataSizeTelemetryState,
  writeIdexalDataSizeTelemetryState,
  type IdexalDataSizeTelemetryState,
} from "./idexalDataSizeTelemetryState.js";
import { scanIdexalDataDirectoryInWorker } from "./idexalDataSizeWorkerClient.js";

export type { IdexalDataSizeTelemetryState } from "./idexalDataSizeTelemetryState.js";

const IDEXAL_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const IDEXAL_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const IDEXAL_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const IDEXAL_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const IDEXAL_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const IDEXAL_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const IDEXAL_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const IDEXAL_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const IDEXAL_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const IDEXAL_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const IDEXAL_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const IDEXAL_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface IdexalDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: IdexalDataSizeTelemetryTiming = {
  abortedRetryMs: IDEXAL_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: IDEXAL_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: IDEXAL_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: IDEXAL_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: IDEXAL_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: IDEXAL_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: IDEXAL_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: IDEXAL_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: IDEXAL_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: IDEXAL_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface IdexalDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface IdexalDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isIdexalBusy: () => boolean;
  logger: IdexalDataSizeTelemetryLogger;
  readState: () => Promise<IdexalDataSizeTelemetryState | null>;
  report: (result: IdexalDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<IdexalDataSizeScanResult>;
  timing?: Partial<IdexalDataSizeTelemetryTiming>;
  writeState: (state: IdexalDataSizeTelemetryState) => Promise<void>;
}
...
function buildIdexalDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: IdexalDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_idexal_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      schema_version: 1,
      idexal_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / IDEXAL_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createIdexalDataSizeTelemetryScheduler(
  dependencies: IdexalDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: IdexalDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isIdexalBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[idexal-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<IdexalDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: IdexalDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[idexal-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("Idexal data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("Idexal data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[idexal-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[idexal-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: IdexalDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[idexal-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createIdexalDataSizeTelemetryScheduler> | null = null;

export function registerDesktopIdexalDataSizeTelemetry(options: {
  context: Parameters<typeof buildIdexalDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isIdexalBusy: () => boolean;
  logger: IdexalDataSizeTelemetryLogger;
  ...
  stopDesktopIdexalDataSizeTelemetry();
  desktopScheduler = createIdexalDataSizeTelemetryScheduler({
    ...
    isIdexalBusy: options.isIdexalBusy,
    logger: options.logger,
    readState: () => readIdexalDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildIdexalDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanIdexalDataDirectoryInWorker(
        {
          maxDurationMs: IDEXAL_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: IDEXAL_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeIdexalDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[idexal-data-size] scheduler start failed", error);
  });
}

export function stopDesktopIdexalDataSizeTelemetry(): void {

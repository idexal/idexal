...
export type IdexalDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type IdexalDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: IdexalDataSizePartialReason }
);

export interface IdexalDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface IdexalDataSizeScanOptions extends IdexalDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("Idexal data size scan aborted", "AbortError");
}
 ...
 */
export async function scanIdexalDataDirectory(
  options: IdexalDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<IdexalDataSizePartialReason, "io_error"> | null = null;
...
export function isIdexalDataSizeScanResult(value: unknown): value is IdexalDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<IdexalDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,

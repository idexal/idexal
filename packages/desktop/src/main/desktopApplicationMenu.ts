...
import { readIdexalStdioTapDevState } from "@idexal/services/node";
...
import {
  HELP_TOGGLE_IDEXAL_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_IDEXAL_ENDPOINT_PRODUCTION_MENU_ID = "help.idexal-endpoint.production";
...
export function updateIdexalStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_IDEXAL_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readIdexalStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  idexalEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readIdexalStdioTapDevState();
      ...
      ? [
          {
            ...
            submenu: [
              ...
              ...(IDEXAL_PRODUCT_FLAVOR === "production"
                ? [
                    {
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_IDEXAL_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleIdexalStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(
                    DesktopCommandIds.ToggleIdexalStdioTapDevProxy,
                  ),
              },
              { type: "separator" as const },
            ]
        ...
        ...(IDEXAL_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpIdexalEndpoint),
                submenu: [
                  {
                    id: HELP_IDEXAL_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpIdexalEndpointProduction),
                    type: "radio" as const,
                    checked: (options.idexalEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetIdexalEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpIdexalEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetIdexalEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpIdexalEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetIdexalEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        idexalEndpointSelection: options.idexalEndpointSelection,

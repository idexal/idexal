/**
 * idexal-cli 角色（`cli_chat` / `cli_aux`）的资源样本来源（注册表第五行）。
 *
...
function resolveSampleIntervalMs(sample: AgentLaneResourceSample): number {
  return Math.max(IDEXAL_CLI_RESOURCE_SAMPLE_INTERVAL_MS, sample.intervalMs);
}
...
export const cliProcessResourceSampleSource: ProcessResourceSampleSource = {
  ...
  sample(context) {
    ...
    if (
      ...
      context.now - lastDeliveredAt < IDEXAL_CLI_RESOURCE_SAMPLE_INTERVAL_MS
    ) {
      return;
    }

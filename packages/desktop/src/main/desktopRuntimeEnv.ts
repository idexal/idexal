...
import type { ConnectOptions } from "@idexal/server/remote";
import { listSSHConfigAliasesFromLocalConfig } from "@idexal/services/node";
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@idexal/idexal-cua/broker/helperConstants";
import {
  IDEXAL_APP_VERSION_ENV,
  IDEXAL_AGENT_RUNTIME,
  IDEXAL_DYNAMIC_WORKFLOW_MODE_ENV,
  IDEXAL_ENV,
  IDEXAL_PRODUCT_FLAVOR,
  IDEXAL_RUNTIME_ENV_KEY,
  IDEXAL_VERSION,
  buildIdexalToolEnvPassthroughEnv,
...
export const desktopRuntimeEnv: IdexalRuntimeEnv = isLocalDevelopmentRuntime
  ? "development"
  : "production";
// 身份看编译期 flavor 而不是 IDEXAL_ENV：IDEXAL_PREVIEW_IDENTITY=1 的生产后端构建同样是 Preview，
// 需要独立的应用名、Electron 数据目录和 Helper 安装子目录才能与正式版并排运行。
const isPreviewPackagedRuntime = !isLocalDevelopmentRuntime && IDEXAL_PRODUCT_FLAVOR === "preview";
...
// e2e 默认工作区依赖 home 路径，因此提供显式覆盖，避免测试写到开发者真实 ~/IdexalProject。
export const runtimeHomePath = readRuntimeEnvOverride("IDEXAL_DESKTOP_HOME_DIR");
...
export const shouldUseElectronDefaultUserDataPath = isTruthyRuntimeEnvOverride(
  "IDEXAL_DESKTOP_USE_ELECTRON_DEFAULT_USER_DATA",
);
export const runtimeUserDataPath =
  readRuntimeEnvOverride("IDEXAL_DESKTOP_USER_DATA_DIR") ??
...
export const runtimeSessionDataPath =
  readRuntimeEnvOverride("IDEXAL_DESKTOP_SESSION_DATA_DIR") ??
...
export async function isDockerDaemonAvailable(): Promise<boolean> {
  const { isDockerAvailable } = await import("@idexal/server/remote");
  return isDockerAvailable();
}

export async function listAvailableWSLDistros() {
  const { listWSLDistros } = await import("@idexal/server/remote");
  return listWSLDistros();
}

export async function listAvailableDockerContainers() {
  const { listDockerContainers } = await import("@idexal/server/remote");
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    ...
    return { IDEXAL_TELEMETRY_RUNTIME_DISTRIBUTION: "packaged" };
  }
  ...
  return applySelectedIdexalEnvLinks(merged);
}
...
function resolveAvailableDevelopmentMockCdnDir(): string | undefined {
  ...
  const releaseDir = join(mockCdnDir, "releases", IDEXAL_VERSION);
...
function shouldUseRemoteCdnInDevelopment(localEnv: LocalRuntimeEnv = {}): boolean {
  return isTruthyEnvFlag(resolveEnvValue("IDEXAL_DEV_REMOTE_ASSET_USE_CDN", localEnv));
}
...
function resolveRemoteCdnBaseUrls(
  ...
  const raw = resolveEnvValue("IDEXAL_REMOTE_ASSET_CDN_BASE_URL", localEnv);
  return resolveOrderedRemoteCdnBaseUrls({
    ...options,
    env: IDEXAL_ENV,
    overrideBaseUrl: raw,
    version: IDEXAL_VERSION,
  });
}
...
export function resolveIdexalEndpointEnvBaseOrigin(
  localEnv: LocalRuntimeEnv = {},
): string | undefined {
  const buildEnv = readProductEndpointEnv();
  // main 进程临时验证更新服务时不会重新写 .env，命令行传入的 endpoint 必须优先于本地文件。
  return (
    process.env["IDEXAL_BASE_URL"]?.trim() ||
    process.env["IDEXAL_ENDPOINT_ORIGIN"]?.trim() ||
    localEnv.IDEXAL_BASE_URL?.trim() ||
    localEnv.IDEXAL_ENDPOINT_ORIGIN?.trim() ||
    buildEnv.IDEXAL_BASE_URL?.trim() ||
    buildEnv.IDEXAL_ENDPOINT_ORIGIN?.trim() ||
    undefined
  );
}
...
function applySelectedIdexalEnvLinks(env: Record<string, string>): Record<string, string> {
  const endpointEnv = {
    ...readProductEndpointEnv(),
    ...env,
    IDEXAL_ENV,
  ...
  return {
    ...
    ...env,
    IDEXAL_BASE_URL: env.IDEXAL_BASE_URL ?? resolveRuntimeIdexalEndpointOrigin(endpointEnv),
...
function resolveHostProcessNodeEnv(): IdexalRuntimeEnv {
  return desktopRuntimeEnv;
}

function resolveRemoteAssetCacheDir(localEnv: LocalRuntimeEnv = {}): string {
  const overrideCacheDir = resolveEnvValue("IDEXAL_REMOTE_ASSET_CACHE_DIR", localEnv);
  if (overrideCacheDir) {
...
function resolveBundledIdexalAgentBinaryPath(): string | undefined {
  const runtime = IDEXAL_AGENT_RUNTIME;
...
export function resolveBundledGlmBinaryPath(): string | undefined {
  return resolveBundledIdexalAgentBinaryPath();
}
...
function resolveHostProcessBinaryEnv(
  ...
  // Idexal Agent 与 app 协议适配强绑定版本，生产包必须优先使用随包携带的固定 runtime。
...
function resolveDynamicWorkflowModeHostEnv(options: {
  ...
  if (!options.isPackaged) {
    ...
    return mode ? { [IDEXAL_DYNAMIC_WORKFLOW_MODE_ENV]: mode } : {};
  }
  if (options.isPreview) {
    return { [IDEXAL_DYNAMIC_WORKFLOW_MODE_ENV]: "alwaysOn" };
  }
  return {};
}
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
    ...
    process.platform !== "darwin"
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ["1", "true", "on"].includes(
              rawInheritedEnv.IDEXAL_CUA_HELPER_ALLOW_UNSIGNED_LOCAL?.trim().toLowerCase() ?? "",
            )
          ? rawInheritedEnv.IDEXAL_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.IDEXAL_HOME?.trim() || join(homedir(), ".idexal"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
  ...
  const agentTelemetryEnv = readIdexalAgentTelemetryEnv(rawInheritedEnv);
  ...
  const inheritedEnv = applySelectedIdexalEnvLinks({
    ...sanitizeIdexalRuntimeEnv(rawInheritedEnv),
    ...buildIdexalToolEnvPassthroughEnv(rawInheritedEnv),
  });
  ...
  if (packagedDesktop) {
    delete inheritedEnv.IDEXAL_CUA_HELPER_ALLOW_UNSIGNED_LOCAL;
  }
  const dynamicWorkflowModeHostEnv = resolveDynamicWorkflowModeHostEnv({
    inheritedValue: rawInheritedEnv[IDEXAL_DYNAMIC_WORKFLOW_MODE_ENV],
  ...
  delete inheritedEnv[IDEXAL_DYNAMIC_WORKFLOW_MODE_ENV];
  ...
  return {
    ...
    ...agentTelemetryEnv,
    // Idexal 运行时不再使用 NODE_ENV；它会被用户 shell、包管理器和测试框架复用。
    // 这里显式下发 IDEXAL_RUNTIME_ENV，并在继承环境里清掉 NODE_ENV，避免 host/agent/Bash 被污染。
    [IDEXAL_RUNTIME_ENV_KEY]: resolveHostProcessNodeEnv(),
    // 显式注入编译期产品身份，保证主进程与 host 的身份语义一致；地址独立解析。
    // inheritedEnv 从 .env 通用变量补齐 Idexal/ZAI 链接，未覆盖时统一使用线上默认值。
    IDEXAL_ENV,
    // Preview 与生产版共享任务、配置和凭据，但不同版本的 Helper 不能互相覆盖或触发降级保护。
    // 只隔离 computer-use 下的运行组件，不改写 IDEXAL_HOME / IDEXAL_DATA_BASE_DIR 业务数据根。
    ...(isPreviewPackagedRuntime ? { IDEXAL_CUA_HELPER_INSTALL_VARIANT: "preview" } : {}),
    ...
    [IDEXAL_APP_VERSION_ENV]: IDEXAL_VERSION,
    ...(dataBaseDir !== homedir() ? { IDEXAL_DATA_BASE_DIR: dataBaseDir } : {}),
    ...(windowsAppInstallDir ? { [IDEXAL_WINDOWS_APP_INSTALL_DIR_ENV]: windowsAppInstallDir } : {}),
    ...(bundledCuaHelperAppPath
      ? { [IDEXAL_CUA_BUNDLED_HELPER_APP_PATH_ENV]: bundledCuaHelperAppPath }
      : {}),
    ...(resolvedGlmBinaryPath ? { GLM_BINARY_PATH: resolvedGlmBinaryPath } : {}),
    ...(resolvedLarkCliBinaryPath ? { IDEXAL_LARK_CLI_BINARY: resolvedLarkCliBinaryPath } : {}),
  };
}

...
import { PlatformChannels, type CuaPermissionKind, type Locale } from "@idexal/shared";
import {
...
// on macOS"），连随包 Idexal 图标都读不到时用它兜底 —— 否则 startDrag 抛异常，用户完全拖不动。
...
/**
 * 拖拽光标与浮窗 tile 共用的 Idexal 图标（模块级缓存，避免每次拖拽读磁盘）。
 *
 * 不能用 `nativeImage.createFromNamedImage("NSApplicationIcon")`：那取的是**当前宿主 app** 的
 * 图标，dev 下宿主是 Electron.app，于是拖拽时显示 Electron 默认图标。
 * 改为显式读随包的 Idexal 图标（electron-builder 已把 build/icon.png 打进 resources/icon.png）。
 */
let cachedIdexalIcon: Electron.NativeImage | null = null;

function resolveIdexalIcon(): Electron.NativeImage {
  if (cachedIdexalIcon && !cachedIdexalIcon.isEmpty()) return cachedIdexalIcon;
  const iconPath = app.isPackaged
    ? join(process.resourcesPath, "icon.png")
    : join(import.meta.dirname, "..", "..", "build", "icon.png");
  const image = nativeImage.createFromPath(iconPath);
  cachedIdexalIcon = image.isEmpty()
    ? nativeImage.createFromDataURL(CUA_HELPER_DRAG_ICON_DATA_URL)
    : image;
  return cachedIdexalIcon;
}
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const maybeFinishReturn = () => {
      if (
        ...
        latestIdexalReturnSequence > observedSystemSettingsAfterSequence
      )
        finish();
    };
    ...
    const inspectFrontmost = (forceAtEventBoundary = false) => {
      ...
      const inspectionStartedWhileAway = latestBlurSequence > latestIdexalReturnSequence;
      activeInspections += 1;
      void readFrontmostBundleId().then(
        (bundleId) => {
          ...
          if (
            ...
            // 如果本次查询期间又收到 blur，该 blur 也必须早于可接受的 Idexal
            // return edge。这会排除“先在 Idexal 内部切窗，后打开 Settings”的旧 focus。
            // LaunchServices 的结果可早于 Electron blur 投递。若 pre-open 探针先读到
            // Settings，而探测启动前恰有一次 Idexal focus/activate，立即把旧 focus 当成「返回」会误判。
    ...
    const onBlur = () => {
      ...
      // pre-open 的 lsappinfo 查询可能已经采到 Idexal，却卡在第二个 info 子进程。
    ...
    const onFocus = () => {
      ...
      latestIdexalReturnSequence = ++applicationEventSequence;
      inspectFrontmost();
      maybeFinishReturn();
    };
    const onQuit = () => finish(new Error("Idexal quit during CUA permission onboarding"));
    const onAbort = () =>
      finish(signal.reason ?? new Error("CUA permission onboarding origin window closed"));
    observationTimer = setTimeout(
      () =>
        finish(
          new Error(`System Settings did not return to Idexal within ${Math.max(1, timeoutMs)}ms`),
        ),
    ...
    // 只有确实观察到 System Settings，后续 Idexal focus 才能推进，内部窗口切换不会误判。
    inspectFrontmost();
...
function createDragPanelForSession(
  ...
  return createCuaPermissionDragPanel({
    ...
    getLocale,
    // tile 用真实 Idexal 图标，与系统设置权限列表里那一行的图标对得上，用户才能把
...
function resolveWindowBoundsBinaryPath(): string {
  const relative = join("macos-window-bounds", "idexal-window-bounds");
...
export function registerCuaPermissionIpcHandlers(options: {
  ...
  ipcMain.on(PlatformChannels.StartCuaHelperPermissionDrag, (event) => {
    ...
    const icon = resolveIdexalIcon().resize({

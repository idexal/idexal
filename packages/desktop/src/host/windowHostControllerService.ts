...
interface ResolvedWindowHostControllerSource {
  ...
  taskService?: IIdexalTaskService;
...
function taskKey(task: Pick<IdexalTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: IdexalTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@idexal/shared/idexal-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: IdexalTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: IdexalTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IIdexalTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IIdexalTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IIdexalTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IIdexalAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: IdexalTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@idexal/shared").WindowHostAttachmentScope;

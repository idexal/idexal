...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteIdexalTaskService: <T extends object>(service: T) => T;
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    tokenProvider: async () =>
      (await localCredentialService.load(IDEXAL_JWT_TOKEN_KEY))?.trim() || null,
  });
  const conversationShareService = new ConversationShareService({
    idexalAgentService: params.connectionServices.idexalAgentService,
  ...
  const reportingRemoteIdexalTaskService = params.createReportingRemoteIdexalTaskService(
    params.connectionServices.idexalTaskService,
  );
  ...
  const remoteIdexalTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteIdexalTaskService,
  );
  const remoteIdexalSessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.idexalSessionService,
  );
  ...
  params.connectionServices.idexalAgentService.onDynamicSessionRuntimePreferencesRequest()(
    (request) => {
      const startedAt = Date.now();
      const requestContext = {
        event: "idexal_protocol.runtime_preferences.host_request_received",
      ...
      void (async () => {
        ...
        await params.connectionServices.idexalAgentService.respondSessionRuntimePreferences({
          requestId: request.requestId,
          resolution,
        });
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、Idexal Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(IIdexalTaskService, remoteIdexalTaskService)
    .register(IIdexalAgentService, params.connectionServices.idexalAgentService)
    .register(IIdexalSessionService, remoteIdexalSessionService)
    .register(IConversationShareService, conversationShareService)
    .register(
      IBotsService,
      createBotsService({
        credentialService: localCredentialService,
        idexalTaskService: remoteIdexalTaskService,

import { IDEXAL_AGENT_PROVIDER_NOT_READY_CODE } from "@idexal/shared";
...
export function resolveRpcLogLevel(message: string, ...args: unknown[]): RpcLogLevel {
  ...
  if (message.startsWith("[rpc:call] idexal-agent.backgroundBashOutputV4 OK (")) return "debug";

  // workspace 首次绑定时，provider registry 会在 runtime identity 查询之后同步；
  // 这个明确带错误码的失败是启动握手状态，不是服务降级，避免每个 workspace 都留下一条 warn。
  if (
    message.includes("idexal-session.getWorkspaceRuntimeIdentity FAIL") &&
    args.some(
      (arg) =>
        typeof arg === "object" &&
        arg !== null &&
        "code" in arg &&
        (arg as { code?: unknown }).code === IDEXAL_AGENT_PROVIDER_NOT_READY_CODE,
    )
  ) {
    return "info";
  }

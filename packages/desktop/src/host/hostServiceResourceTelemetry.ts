...
import { IIdexalAgentService, type ServiceCollection } from "@idexal/services";
 ...
 * 本机 CLI 是 local，远端 idexal-server 上的 CLI 是 remote，样本自报的硬件维度由 main 覆盖全局默认值。
...
export function registerHostServiceResourceTelemetry(
  ...
  const agentService = options.services.getOptional(IIdexalAgentService);
  if (!agentService) {
    return NO_TELEMETRY;
  }

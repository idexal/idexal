...
import {
  ...
  idexalStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@idexal/shared";
import { resolveDefaultIdexalAgentCommand } from "@idexal/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultIdexalAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = idexalStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);

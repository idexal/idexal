...
import {
  ...
  createIdexalAgentConnectionScope,
  type IdexalAgentV4ClientMode,
  collectServiceMemoryDiagnostics,
} from "@idexal/services";
import {
  createLocalServices,
...
import {
  ...
  type IdexalAutomationRunOutcome,
  type ModelSelection,
} from "@idexal/shared";
import {
...
import type {
  ...
  RemoteConnection,
} from "@idexal/server/remote";
import type { RemoteTarget } from "@idexal/shared";
...
import { createRemoteConnectionProgressContext } from "@idexal/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["IDEXAL_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 idexal-* title，方便系统进程列表过滤。
process.title = formatIdexalHostProcessName(process.env["IDEXAL_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("idexal-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 idexal-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  ...
  try {
    const snapshot = await params.idexalTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  workspacePath: string;
  workspaceIdentity?: string;
  outcome: IdexalAutomationRunOutcome;
  error?: string;
}): Promise<void> {
  ...
  void params.idexalTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.idexalTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        idexalTaskService: params.idexalTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const idexalTaskService = activeServices?.getOptional(IIdexalTaskService);
  if (!idexalTaskService) {
    throw new Error("Idexal task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        idexalTaskService.listDeletedTaskIds(workspaceScope),
        idexalTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      await idexalTaskService.setConfigOption({
        taskId,
        traceId,
        configId: "mode",
      ...
      const task = await idexalTaskService.createTask({
        ...
        mode: request.permissionMode as IdexalTaskMode,
    ...
    await idexalTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const idexalTaskService = targetServices.getOptional(IIdexalTaskService);
  if (!idexalTaskService) {
    throw new Error("Idexal task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await idexalTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isIdexalTaskMeta(value: unknown): value is IdexalTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteIdexalTaskService<T extends object>(
  service: T,
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: IdexalPromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: IdexalTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // idexalSessionStore 的整条消费链词表都是 IdexalStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→IdexalStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 idexal-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: IdexalTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: IdexalTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote Idexal task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote Idexal task ready event is not subscribable");
    }
  ...
  // remote workspace 的 Idexal Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: IdexalTaskMeta; started: boolean } | undefined;
...
function warmUpIdexalAgent(
  ...
  const idexalSessionService = services.getOptional(IIdexalSessionService);
  if (!idexalSessionService) {
    return;
  }
  void idexalSessionService
    .initializeWorkspace({
      workspacePath,
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `Idexal agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `Idexal agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `Idexal agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`Idexal agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IIdexalAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteIdexalTaskService: (service) =>
      createReportingRemoteIdexalTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 idexal-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IIdexalTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IIdexalTaskService),
        agentService: services.getOptional(IIdexalAgentService),
    ...
    const taskService = activeServices?.getOptional(IIdexalTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IIdexalAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IIdexalTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IIdexalTaskService["archiveTask"]>[0]
            | Parameters<IIdexalTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IIdexalTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IIdexalTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IIdexalTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IIdexalTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: IdexalAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IIdexalAgentService);
  const connectionScope = agentService
    ? createIdexalAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IIdexalTaskService);
  if (taskService) {
    overrides.set(
      IIdexalTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IIdexalAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as IdexalTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!idexalTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "Idexal task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    ...
    if (!idexalTaskService) {
      logger.warn(
        "session message delivery result received before Idexal task service initialized",
      );
      return;
    }
    void idexalTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              idexalAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              idexalBuiltinProviderConfigFilePath: msg.idexalBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 idexalAgentService 转到这个 executor，
        ...
        const idexalTaskService = services.getOptional(IIdexalTaskService);
        if (idexalTaskService) {
          const reportingIdexalTaskService = createReportingRemoteIdexalTaskService(
            idexalTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IIdexalTaskService, reportingIdexalTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@idexal/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 IDEXAL_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: IDEXAL_VERSION,
    // 远端 idexal-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。

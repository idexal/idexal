import type { IRemoteBackend, RemoteUploadOptions } from "@idexal/server/remote";
import { quotePosixPathArg } from "@idexal/server/remote/posixShell.js";
import type { TraceId, IdexalPromptAttachment } from "@idexal/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.idexal/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".idexal/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: IdexalPromptAttachment[] = [];
...
function getAttachmentLocalPath(attachment: IdexalPromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: IdexalPromptAttachment[] }).attachments
            : undefined,
        });

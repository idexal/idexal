...
function resolveScheduledAt(automation: IdexalAutomation, now: number): number {
...
async function handleClaimed(automation: IdexalAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: IdexalAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: IdexalOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: IdexalAutomationTrigger =

...
import "@idexal/ui/styles.css";
import { connectViaMessagePort, createMessagePortServiceConnection } from "@idexal/client";
import {
  InternalChannels,
...
(
  window as Window & {
    __IDEXAL_RENDERER_START__?: number;
    __IDEXAL_LAUNCH_MARKS__?: LaunchMarks | null;
  }
).__IDEXAL_RENDERER_START__ = rendererStartedAt;
(window as Window & { __IDEXAL_LAUNCH_MARKS__?: LaunchMarks | null }).__IDEXAL_LAUNCH_MARKS__ =
  launchMarks;
...
function registerE2EStoreBridgesIfEnabled() {
  ...
  if (env.VITE_IDEXAL_E2E_STORE_BRIDGE !== "1") {
    return;
  }

  void import("@idexal/ui/e2e-store-bridge").then(({ registerE2EStoreBridges }) => {
...
{
  const saved = localStorage.getItem("idexal-theme") || "zai-dark";
  const resolved =
    saved === "system"
...
const isLocalDevelopmentRuntime =
  (globalThis as typeof globalThis & { __IDEXAL_LOCAL_DEVELOPMENT_RUNTIME__?: boolean })
    .__IDEXAL_LOCAL_DEVELOPMENT_RUNTIME__ === true;
...
function StartupReadyNotifier() {
  useEffect(() => {
    ...
    (window as Window & { __IDEXAL_REACT_COMMIT_AT__?: number }).__IDEXAL_REACT_COMMIT_AT__ =
      Date.now();
    // HTML 启动壳的弹出动画结束时，React 首屏可能还没 commit，直接移除壳会露出空白。
    // 这里在 React commit 后通知 index.html，再由启动壳统一判断动画和 React ready 两个条件后退场。
    window.dispatchEvent(new Event("idexal-react-startup-ready"));
  }, []);
...
function initializeBusinessRoot(port: MessagePort): void {
  ...
  syncAppTelemetryContext({
    bridge: {
      syncTelemetryContext: (context) => window.idexal.syncTelemetryContext(context),
    },

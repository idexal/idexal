...
export function buildRemoteWorkspaceSessionServices(
  ...
  return {
    ...
    // MCP / 插件状态检查走 idexalAgentService 的控制面 app-server。
    // SSH 远端如果沿用本机 base service，会把远端 MCP 配置拿到本机 app-server 里检查，
    // 其 PATH / cwd 都不是远端环境，导致同步后仍显示 `spawn npx ENOENT`。
    idexalAgentService: remoteServices.idexalAgentService,
    idexalTaskService: remoteServices.idexalTaskService,
    idexalSessionService: remoteServices.idexalSessionService,
    ...
    // 远端 skill 同步的 import 必须写入 SSH 主机的 ~/.idexal/skills。
    // 如果继续沿用 base service，UI 会显示同步成功但实际写到本机 ~/.idexal/skills。
    skillSyncService: remoteServices.skillSyncService,
    // 远端 MCP 同步的 import 必须写入 SSH 主机的 ~/.idexal/cli/config.json。
    // 这里与 skillSyncService 一样走 remote service，避免把远端配置写回本机用户目录。
    mcpSyncService: remoteServices.mcpSyncService,
    // 远端 plugin 同步会写入 SSH 主机的 ~/.idexal/plugins 和 plugins.dirs；

...
const shouldRequireWindowsBrowserImportHelper =
  target.os === "win32" && process.env.IDEXAL_ENABLE_WINDOWS_BROWSER_IMPORT === "1";
// CUA 权限浮窗靠 idexal-window-bounds 读系统设置窗口 bounds 才能吸附。该 Swift 产物被
    ...
    ? [
        {
          ...
          isReady: () =>
            existsSync(
              join(bundledToolsRoot, "browser-import", "idexal-browser-import-helper.exe"),
            ),
        },
      ]
    : []),
    ...
    ? [
        {
          ...
          isReady: () =>
            existsSync(
              join(desktopRoot, "resources", "macos-window-bounds", "idexal-window-bounds"),
            ),
        },
      ]
    : []),

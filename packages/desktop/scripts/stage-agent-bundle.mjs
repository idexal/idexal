// Agent bundle 的暂存动作：把 apps/idexal-cli/packages/cli/dist/idexal.cjs 放进
...
// desktopRuntimeEnv.ts 的 resolveBundledIdexalAgentBinaryPath() 解析，候选**只有**
...
export const AGENT_BUNDLE_SOURCE_RELATIVE = "apps/idexal-cli/packages/cli/dist/idexal.cjs";
...
export function resolveAgentBundlePaths({ repoRoot, platformKey }) {
  ...
  return {
    ...
    glmDir,
    stagedBundlePath: resolve(glmDir, "idexal.cjs"),
 ...
 * （idexal-agent / idexal-acp 等）和旧 meta 会被一并打进安装包（CI 干净检出不会有，本地会）。
 */

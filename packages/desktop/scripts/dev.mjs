...
if (process.platform === "darwin" && existsSync(electronBinary)) {
  ...
  // idexal:// 交给一个没有项目入口的 Electron 默认壳。给本地启动副本补齐产品
  ...
  const devBundle = await prepareDevElectronAppBundle({
    electronAppPath,
    runtimeRoot: resolve(root, "../../.idexal-runtime/desktop-dev"),
    electronVersion: electronPackage.version,
    arch: process.arch,
  });
  electronCommand = devBundle.executablePath;
  console.log(`[dev] Prepared macOS Idexal Dev bundle: ${devBundle.appPath}`);
}

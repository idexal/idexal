...
const WINDOWS_POWERSHELL_SECURITY_BOOTSTRAP = [
  ...
  "try{",
  "$idexalSecurityModule=[IO.Path]::Combine($PSHOME,'Modules','Microsoft.PowerShell.Security','Microsoft.PowerShell.Security.psd1');",
  "Import-Module -Name $idexalSecurityModule -Force -ErrorAction Stop;",
  "}catch{",
...
export function createEncodedPowerShellArgs(script, values = []) {
  ...
  const valueBindings = values
    .map(
      (value, index) =>
        `$idexalArg${index}=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('${encodeUtf8Value(value)}'));`,
    )
    .join("");

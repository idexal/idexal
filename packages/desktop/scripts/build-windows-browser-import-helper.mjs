...
const targetArch = (process.env.IDEXAL_TARGET_ARCH ?? arch()).toLowerCase();
const targetKey = `win32-${targetArch}`;
const outputDir = resolve(desktopRoot, `bundled-tools/${targetKey}/browser-import`);
const outputPath = resolve(outputDir, "idexal-browser-import-helper.exe");
...
if (targetArch === "arm64" && !process.env.IDEXAL_CSC_PATH) {
  throw new Error(
    "Windows arm64 browser import helper requires IDEXAL_CSC_PATH pointing to an arm64-capable Roslyn csc.exe",
  );
}
if (!cscPath) {
  throw new Error("Windows C# compiler not found; set IDEXAL_CSC_PATH to a trusted csc.exe");
}

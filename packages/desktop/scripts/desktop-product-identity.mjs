/**
 * 构建期开关：为真时安装包使用 Preview 身份，而后端环境仍由 `IDEXAL_ENV` 单独决定。
 * 典型用法是 `IDEXAL_ENV=production IDEXAL_PREVIEW_IDENTITY=1`，得到一个连接生产后端、
 * 可与正式版并排安装的 `Idexal Preview`。
 */
export const IDEXAL_PREVIEW_IDENTITY_ENV = "IDEXAL_PREVIEW_IDENTITY";

const PRODUCTION_IDENTITY = Object.freeze({
  flavor: "production",
  appId: "dev.idexal.app",
  productName: "Idexal",
  linuxExecutableName: "idexal",
  linuxPackageName: "idexal",
  cuaHelperInstallVariant: null,
});

const PREVIEW_IDENTITY = Object.freeze({
  flavor: "preview",
  appId: "dev.idexal.app.preview",
  productName: "Idexal Preview",
  linuxExecutableName: "idexal-preview",
  linuxPackageName: "idexal-preview",
...
function normalizeDesktopIdexalEnv(env) {
  return env.IDEXAL_ENV?.trim().toLowerCase() === "production" ? "production" : "test";
}

/**
 * 开关只有一种开启拼写 `1`（`0` / 空 = 关闭），与 CI workflow 规则和 release 门的
 * `$IDEXAL_PREVIEW_IDENTITY == "1"` 精确比较保持同一套语义。其它拼写在构建期直接失败，
 * 避免 `true` 之类在 YAML 路由层漏匹配、却在脚本层被当成开启，把 Preview 包打进生产验收目录。
 */
export function isPreviewIdentityRequested(env = process.env) {
  const value = env[IDEXAL_PREVIEW_IDENTITY_ENV]?.trim() ?? "";
  if (value === "1") {
    return true;
  }
  if (value === "" || value === "0") {
  ...
  throw new Error(
    `invalid ${IDEXAL_PREVIEW_IDENTITY_ENV}=${env[IDEXAL_PREVIEW_IDENTITY_ENV]}; expected 1 or 0`,
  );
}

/**
 * 产品身份（flavor）与后端环境（`IDEXAL_ENV`）是两个轴：
 * - `IDEXAL_ENV=test` 一律是 Preview，测试后端不能顶着正式 `Idexal` 身份覆盖用户的正式安装；
 * - `IDEXAL_ENV=production` 默认是正式身份，显式 `IDEXAL_PREVIEW_IDENTITY=1` 时改用 Preview 身份。
 * 未知 `IDEXAL_ENV` 继续按 test 处理，和共享层 normalizeIdexalEnv 的 fail-safe 默认值一致。
 */
export function resolveDesktopProductFlavor(env = process.env) {
  if (isPreviewIdentityRequested(env)) {
    return "preview";
  }
  return normalizeDesktopIdexalEnv(env) === "production" ? "production" : "preview";
}
 ...
 * 生产后端的 Preview 包靠 productName（`Idexal Preview-<version>-...`）与正式包区分。
 */
export function resolveDesktopArtifactSuffix(env = process.env) {
  return normalizeDesktopIdexalEnv(env) === "test" ? "_TEST" : "";
}

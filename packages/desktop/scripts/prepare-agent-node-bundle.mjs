...
// 桌面打包态的 agent 运行时资产：把 agent 的 JS bundle（idexal.cjs）放进 bundled-agents/<platform>/glm，
...
// - Electron 41 内置 Node 24.x，与 idexal-cli 的目标运行时一致；
// - 单平台体积从 ~180MB 降到 ~16MB，且同一份 JS 跨平台通用；
// - app-server 命令路径不会加载 @idexal/tui，所以这里天然不打包 TUI。
//
...
const cliBundlePath = resolve(repoRoot, "apps/idexal-cli/packages/cli/dist/idexal.cjs");
const adaptersRoot = resolve(repoRoot, "apps/idexal-cli/packages/adapters");
const pnpmRunEnv = {
  ...process.env,
  // pnpm 11 会在 apps/idexal-cli 子 workspace 执行 run 前触发 install；
  // 子 workspace 不能解析根 workspace 的 @idexal/shared，Docker/web app 打包会因此卡在插件 runtime 构建。
  PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
};
const BROWSER_USE_PLUGIN_PACKAGE_NAME = "@idexal/browser-use-plugin";

// 平台目录命名：darwin/win32/linux + x64/arm64，
// 支持 IDEXAL_TARGET_OS / IDEXAL_TARGET_ARCH 覆盖（交叉打包时由 CI 注入）。
...
const platform = normalizePlatform(process.env.IDEXAL_TARGET_OS || "") || process.platform;
const arch = normalizeArch(process.env.IDEXAL_TARGET_ARCH || "") || process.arch;
const platformKey = `${platform}-${arch}`;

const glmDir = resolve(desktopRoot, "bundled-agents", platformKey, "glm");
// idexal.cjs / .node-bundle-meta.json 的落点由 stage-agent-bundle.mjs 自己解析（同源）。
// node_repl 宿主抽成独立包
// @idexal/node-repl-host 之后，browser-use 不再产出 dist/mcp/server.js，CUA 资产
// （docs/computer-use.md、scripts/computer-use-client.mjs）也已归 @idexal/zcode-cua-plugin。
...
// 在 idexal.cjs 旁找 packages/bundled-skills 并原地读取。漏 stage 它，桌面包的 /workflow 会展开成
...
function buildCliBundle() {
  console.log("[prepare:agent-bundle] building idexal-cli app-server bundle ...");
  // 复用仓库根脚本（turbo build:desktop-agent --filter=@idexal/cli），命中缓存时几乎瞬时。
...
function buildOfficialPluginRuntimes() {
  for (const plugin of officialPluginPackages) {
    ...
    runCommand(
      "pnpm",
      ["--dir", resolve(repoRoot, "apps/idexal-cli"), "--filter", plugin.packageName, "build"],
      {
        cwd: repoRoot,
        env: pnpmRunEnv,
      },
...
function stageOfficialPlugins() {
  for (const plugin of officialPluginPackages) {
    ...
    const manifestPath = resolve(sourceRoot, ".idexal-plugin", "plugin.json");
...
// Electron 生产包只带 resources/glm/idexal.cjs 时，app-server 进程的
// __dirname 附近没有官方插件目录，启动时 seed 找不到 source，用户侧不会自动得到内置插件。
// 这里把官方插件按 bootstrap 的 rootCandidates 期望放到 glm/packages/*-plugin，
// 让 Electron Node 运行 idexal.cjs 时复用同一套 filesystem seed 逻辑。
// browser-use runtime 的声明生成依赖 @idexal/core/dist。CI 干净检出没有该产物，

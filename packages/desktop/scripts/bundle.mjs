...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    IDEXAL_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.IDEXAL_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  IDEXAL_TARGET_OS              与 --os 等价
  IDEXAL_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.IDEXAL_TARGET_OS ?? null,
    arch: process.env.IDEXAL_TARGET_ARCH ?? null,
    skipPrepare: process.env.IDEXAL_SKIP_PREPARE === "1",
    skipBuild: process.env.IDEXAL_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };

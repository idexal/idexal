import { ensureDeviceMid as ensureSharedDeviceMid } from "@idexal/services/node";
 ...
 * 写设备身份文件，而 buildIdexalSourceHeaders() 只读不生成。3.12.0 起 Start Plan 权益由
 ...
 * 远端 stdio entry 承担同一职责，复用同一个文件、字段与锁，与同机 idexal-cli 共享同一个设备身份。
 *
 * 失败不阻断启动：设备标识缺失只会让 Idexal endpoint 请求少一个 header（与修复前行为一致），
...
export async function ensureRemoteServerDeviceMid(
  ...
  try {
    ...
    options.log(
      "deviceMid 初始化失败，Idexal endpoint 请求将不带 X-Device-Mid:",

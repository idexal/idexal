...
import {
  ...
  IDEXAL_RPC_HOST_CAPABILITY_HEADER,
  IDEXAL_VERSION,
  type BotProvider,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("idexal-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("IDEXAL_SERVER_ID") || hostname() || "idexal-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("IDEXAL_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("IDEXAL_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("IDEXAL_SERVER_NAME") }
      : {}),
    version: IDEXAL_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("IDEXAL_SERVER_TOKEN")),
...
const idexalLiteTokenCookieName = "idexal_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${idexalLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(idexalLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(IDEXAL_RPC_HOST_CAPABILITY_HEADER);
  ...
  const handleBotCallback = async (c: Context) => {
    ...
    const webhookSecret = c.req.header("x-idexal-bot-secret");

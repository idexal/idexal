...
import { IDEXAL_AGENT_PROVIDER, resolveIdexalRuntimeEnv } from "@idexal/shared";
import { createHash, randomUUID } from "node:crypto";
import { existsSync } from "node:fs";
import { cp, lstat, mkdir, mkdtemp, readdir, readFile, readlink, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, join, resolve } from "node:path";
import type { IRemoteBackend } from "@idexal/server/remote/backend.js";
import {
  buildRemoteExecutableReplaceCommand,
  buildRemoteMoveCommand,
  type DeployLoggers,
  waitForClose,
} from "@idexal/server/remote/deployShared.js";
import {
  buildWriteLiteralFileCommand,
  quotePosixPathArg,
} from "@idexal/server/remote/posixShell.js";
import { createTarGzArchive } from "@idexal/server/remote/localTarGz.js";
import {
  buildRemoteAgentBundleWrapper,
  isRemoteAgentBundleWrapperCurrent,
  REMOTE_AGENT_BUNDLE_NAME,
} from "@idexal/server/remote/idexalAgentBundleWrapper.js";
import {
  deployRemoteAgentWrapper,
  isWslBackend,
} from "@idexal/server/remote/idexalAgentWrapperDeploy.js";
import {
...
} from "@idexal/server/remote/idexalAgentOfficialPluginAssets.js";
import { repairLegacyRemoteOfficialPluginDirectoryPermissions } from "@idexal/server/remote/idexalAgentOfficialPluginPermissionRepair.js";

const DEV_AGENT_BUNDLE_RELATIVE_PATH = "apps/idexal-cli/packages/cli/dist/idexal.cjs";
const DEV_AGENT_BUNDLE_ENV = "IDEXAL_REMOTE_DEV_AGENT_BUNDLE";
const REMOTE_DEV_AGENT_BUNDLE_NAME = REMOTE_AGENT_BUNDLE_NAME;
const REMOTE_DEV_AGENT_VERSION_FILE_NAME = ".dev-version";

interface DeployDevelopmentIdexalAgentRuntimeParams {
...
function shouldUseDevelopmentAgentBundle(): boolean {
  if (resolveIdexalRuntimeEnv(process.env) !== "development") {
    return false;
  }
...
async function hashDevelopmentOfficialPluginPackage(
  ...
  const manifestPath = join(packageRoot, ".idexal-plugin", "plugin.json");
  if (!existsSync(manifestPath)) {
    throw new Error(`[idexal-agent-deploy] missing official plugin manifest: ${manifestPath}`);
  }
...
function assertDevelopmentOfficialPluginRequiredAssets(
  ...
  for (const requiredRelativePath of REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS) {
    ...
    if (!existsSync(requiredPath)) {
      throw new Error(
        `[idexal-agent-deploy] missing official plugin required asset: ${requiredPath}`,
      );
    }
  }
}
...
async function shouldSkipDevelopmentIdexalAgentDeploy(params: {
  backend: IRemoteBackend;
  remoteVersionFile: string;
  ...
  params.loggers.log(
    `[idexal-agent-deploy] ${IDEXAL_AGENT_PROVIDER}: 开发态 idexal.cjs 未变化，跳过`,
  );
  return true;
}
...
async function stageDevelopmentOfficialPluginPackages(params: {
  ...
  for (const packageName of REMOTE_AGENT_OFFICIAL_PLUGIN_PACKAGE_NAMES) {
    const sourceRoot = join(params.repoRoot, "apps", "idexal-cli", "packages", packageName);
...
async function uploadDevelopmentOfficialPluginPackages(params: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "idexal-remote-agent-packages-"));
    ...
    const cleanupRemoteStaging = async (): Promise<void> => {
      try {
        ...
        params.loggers.logWarn(
          `[idexal-agent-deploy] ${IDEXAL_AGENT_PROVIDER}: 清理 owner staging 失败 (${ownerSuffix}): ${String(error)}`,
        );
      }
    };
    params.loggers.log(`[idexal-agent-deploy] ${IDEXAL_AGENT_PROVIDER}: 开发态上传官方插件资源`);
    try {
...
export async function deployDevelopmentIdexalAgentRuntime(
  backend: IRemoteBackend,
  params: DeployDevelopmentIdexalAgentRuntimeParams,
  loggers: DeployLoggers,
): Promise<boolean> {
  ...
  if (
    await shouldSkipDevelopmentIdexalAgentDeploy({
      backend,
  ...
  // 开发态 SSH 远端过去只会部署本地 idexal.cjs，不会携带 packages/*-plugin。
  // builtin plugin seed 依赖 agent 包旁边的官方插件源资源，所以 dev 部署需要同步 bundle 与插件资源。
  // 本地修改 apps/idexal-cli 后，远端测试仍运行滞后的发布包。这里改为上传 dev 启动时刚构建的
  // dist/idexal.cjs，并用远端已部署的 node 包一层 wrapper 启动，保证 agent 仍运行在目标机器内。
  loggers.log(
    `[idexal-agent-deploy] ${IDEXAL_AGENT_PROVIDER}: 开发态上传本地 idexal.cjs ${devVersion.slice(0, 12)}`,
  );
  ...
  loggers.log(
    `[idexal-agent-deploy] ${IDEXAL_AGENT_PROVIDER}: 开发态部署完成 ${devVersion.slice(0, 12)}`,
  );
  return true;
}

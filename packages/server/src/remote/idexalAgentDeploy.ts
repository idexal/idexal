...
import type { IRemoteBackend, RemoteEnvironment } from "@idexal/server/remote/backend.js";
import {
  REMOTE_BASE,
  type DeployLoggers,
...
import type { RemoteAssetInstaller } from "@idexal/server/remote/remoteAssetInstaller.js";
import { buildWriteLiteralFileCommand } from "@idexal/server/remote/posixShell.js";
import { deployDevelopmentIdexalAgentRuntime } from "@idexal/server/remote/idexalAgentDevDeploy.js";
import {
...
} from "@idexal/server/remote/remoteAssetLiveIdentity.js";

const REMOTE_AGENT_RUNTIME_BASE = `${REMOTE_BASE}/agents`;

export interface DeployIdexalAgentRuntimeOptions extends RemoteAssetDeployOptions {
  platformArch: string;
...
function isSelectedIdexalAgentComponent(
  componentId: string,
...
async function shouldSkipIdexalAgentDeploy(params: {
  ...
  // wrapper 在、但 idexal.cjs 缺失（被清理 / 旧原生二进制部署残留）时也要重新部署。
  ...
  params.loggers.log(
    `[idexal-agent-deploy] ${IDEXAL_AGENT_PROVIDER}: 制品 SHA ${params.expectedArtifactSha256} 已部署，跳过`,
  );
  return true;
}
...
/**
 * 部署 Idexal Agent runtime 到远程机器。
 *
 * 生产态只用 manifest SHA 判断制品是否变化；语义版本不参与跳过决策。
 */
export async function deployIdexalAgentRuntime(
  backend: IRemoteBackend,
  env: RemoteEnvironment,
  options: DeployIdexalAgentRuntimeOptions,
  loggers: DeployLoggers,
): Promise<void> {
  const provider = IDEXAL_AGENT_PROVIDER;
  const runtime = IDEXAL_AGENT_RUNTIME;
  const componentId = provider;
  if (!isSelectedIdexalAgentComponent(componentId, options.selectedResourcePackageIds)) {
    loggers.log(`[idexal-agent-deploy] ${provider}: 未选择资源包 ${componentId}，跳过检查和部署`);
    return;
  }

  // binaryName 指 wrapper 可执行文件名（如 idexal-agent / idexal-agent.exe）——
  // 一个调用远端 node 执行 idexal.cjs 的壳脚本。
  const binaryName = runtime.resolveEntrySegments(env.platform).at(-1);
  if (!binaryName) {
    loggers.logWarn(`[idexal-agent-deploy] ${provider}: 无法解析 agent 入口名称，跳过部署`);
    return;
  }
  ...
  if (
    await deployDevelopmentIdexalAgentRuntime(
      backend,
      {
  ...
  try {
    ...
    loggers.logWarn(
      `[idexal-agent-deploy] ${provider}: 读取 manifest SHA 失败，将重新部署: ${String(error)}`,
    );
  }

  if (
    await shouldSkipIdexalAgentDeploy({
      backend,
      remoteBinaryPath,
      remoteBundlePath,
      runtimeResourceDir: runtime.bundledResourceDir,
  ...
  loggers.log(`[idexal-agent-deploy] ${provider}: 开始部署 v${runtime.version}...`);
  ...
  if (permissionRepairSucceeded) {
    ...
    await installOfficialPluginPackages();
    // 2) packages 替换成功后再安装编译产物 idexal.cjs（跨平台同一份，glm 组件里就是它）。
    await installBundle();
  }
  // 3) 写入 wrapper（即 resolver 期望的 idexal-agent），用远端已部署的 node 执行 idexal.cjs。
  ...
  loggers.log(`[idexal-agent-deploy] ${provider}: 部署完成 v${runtime.version}`);
}

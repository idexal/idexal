...
import type { RemoteAssetInstallMode } from "@idexal/shared";
import type { IRemoteBackend, StdioStream } from "@idexal/server/remote/backend.js";
import {
  REMOTE_BASE,
...
import { quotePosixPathArg, quotePosixShellArg } from "@idexal/server/remote/posixShell.js";
import {
  buildComponentArtifactUrlCandidates,
  buildReleaseAssetUrlCandidates,
  buildReleaseBaseCandidates,
  resolveRemoteCdnBaseUrls,
} from "@idexal/server/remote/remoteAssetCdn.js";
import {
...
import type {
  ...
  RemoteDownloadTool,
  RemoteSha256Tool,
} from "@idexal/server/remote/remoteAssetPreflight.js";
import {
  resolveRemoteAssetFetch,
  type RemoteAssetNetworkPort,
} from "@idexal/server/remote/remoteAssetNetwork.js";
...
function buildStaleRemoteStagingCleanupCommand(parentDir: string, patterns: string[]): string {
  ...
  return [
    ...
    // 新连接只回收超过 24 小时的 Idexal owner staging，避免误删当前 owner 或正常时长内的活跃部署。
...
export class LocalUploadAssetInstaller implements RemoteAssetInstaller {
  ...
  async installDirectory(params: {
    ...
    const localTarPath = join(
      tmpdir(),
      `idexal-remote-${params.componentId}-${Date.now()}-${randomUUID()}.tar.gz`,
    );

import type { HelloMessage, HelloAckMessage } from "@idexal/shared";
import { IDEXAL_VERSION, formatZodError, helloMessageSchema } from "@idexal/shared";
 ...
 * 1. Read lines from stdout until we find a idexal-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a idexal-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no idexal-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "idexal-hello"
            ) {
              rejectWithDiagnostics(`Invalid idexal-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {

...
import { waitForClose } from "@idexal/server/remote/deployShared.js";
...
export async function detectRemoteAssetTools(
  ...
  if (!download) {
    throw new Error(
      "远端服务器缺少 curl 或 wget，无法直接下载 Idexal 远程资源。请安装 curl/wget，或切回“本地下载后上传”。",
    );
  }
  if (!tar) {
    throw new Error(
      "远端服务器缺少 tar，无法解压 Idexal 远程资源。请安装 tar，或切回“本地下载后上传”。",
    );
  }
  if (!sha256) {
    throw new Error(
      "远端服务器缺少 sha256sum、shasum 或 openssl，无法校验 Idexal 远程资源。请安装其中一个校验工具，或切回“本地下载后上传”。",
    );
  }

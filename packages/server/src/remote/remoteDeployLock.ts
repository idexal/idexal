...
import { REMOTE_BASE } from "@idexal/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `idexal-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `idexal-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",

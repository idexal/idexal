...
import { deployIdexalAgentRuntime } from "./idexalAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@idexal/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { quotePosixPathArg } from "@idexal/server/remote/posixShell.js";
import { checkServerBundleRequiredMarkers } from "@idexal/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@idexal/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@idexal/server/remote/idexalAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@idexal/server/remote/remoteAssetCache.js";
import {
...
} from "@idexal/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@idexal/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@idexal/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@idexal/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@idexal/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the idexal server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/idexal-server.cjs 可启动，不代表随包工具仍存在。
      ...
      await deployIdexalAgentRuntime(
        backend,
        env,
        {
          ...assetDeployOptions,
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/idexal-server.cjs",
      remotePath: `${REMOTE_BASE}/idexal-server.cjs`,
    ...
    // 部署 Idexal Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployIdexalAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/idexal-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", IDEXAL_VERSION);
    if (version !== IDEXAL_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${IDEXAL_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", IDEXAL_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/idexal-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/idexal.cjs`);

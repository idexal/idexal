import { SocketProtocol, ChannelClient } from "@idexal/rpc";
import type { IServiceAccessor } from "@idexal/services";
import { RemoteServiceAccess } from "@idexal/client";
import {
...
/**
 * Connect to a remote idexal server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 idexal-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'IDEXAL_SERVER_RUNTIME_ROOT="$HOME/.idexal/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${IDEXAL_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${IDEXAL_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${IDEXAL_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${IDEXAL_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.idexal/server/node ~/.idexal/server/idexal-server.cjs`;
}

...
import { fileExists } from "@idexal/server/remote/deployShared.js";
import { extractTarGzArchive } from "@idexal/server/remote/localTarGz.js";
import {
...
async function ensureRemoteComponentDirFromCdnInternal(
  ...
  if (!forceRefresh && initialMissingPaths) {
    ...
    // idexal.cjs 的 glm cache 后，再补传 packages 会一直复用残缺 cache。
...
export async function parseRemoteAssetManifestFromResponse(
  ...
  for (const [index, rawComponent] of rawComponents.entries()) {
    ...
    if (!mountRule) {
      ...
      // 当前客户端只认识 Idexal Agent 与基础运行时，未知组件应跳过，不能阻断当前组件下载。
      continue;
    }
...
async function isValidReleaseDir(releaseDir: string, platformArch: string): Promise<boolean> {
  ...
  if (!(await fileExists(releaseDir, "server", "idexal-server.cjs"))) {
    return false;
  }
...
export function resolveFallbackRemoteAssetCacheDir(): string {
  return join(tmpdir(), "idexal-remote-assets-cache");
}

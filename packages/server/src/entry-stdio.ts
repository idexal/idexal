import { disposeServiceResourcesAndWait, getAppConfigDir } from "@idexal/services/node";
import {
  IDEXAL_VERSION,
...
const log = (...args: unknown[]) =>
  console.error(formatLogPrefix("idexal-server:stdio", process.pid), ...args);
...
if (process.argv.includes("--version")) {
  process.stdout.write(IDEXAL_VERSION + "\n");
  process.exit(0);
}
...
async function main() {
  ...
  // 远端主机没有 Desktop main，没人写 telemetry-state.json，services 发往 Idexal endpoint

  ...
  async #runRefreshLoop(): Promise<AccountProviderConfigSnapshot> {
    ...
    while (this.#pendingReasons.size > 0) {
      ...
      try {
        ...
        const configuredProviders = config.idexalBuiltinProviders;
        const { providers, states } = await this.#resolve({
          configRevision: config.idexalBuiltinRevision,
          configuredProviders,
        ...
        if (
          ...
          currentConfig.idexalBuiltinRevision !== config.idexalBuiltinRevision
        ) {
          this.#pendingReasons.add("superseded-resolution");
          continue;
        }
        const basedOnIdexalBuiltinRevision = config.idexalBuiltinRevision;

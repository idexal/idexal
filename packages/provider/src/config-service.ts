...
import type { ModelSelection } from "@idexal/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly idexalBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#idexalBuiltinSource = dependencies.idexalBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#idexalBuiltinSource.onDidChange((reason) => this.#emit(`idexalBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [idexalBuiltin, personal] = await Promise.all([
      this.#idexalBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([idexalBuiltin.revision, personal.revision]),
      idexalBuiltinRevision: idexalBuiltin.revision,
      personalRevision: personal.revision,
      idexalBuiltinProviders: idexalBuiltin.providers,
      idexalBuiltinProviderTemplates:
        idexalBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      idexalBuiltinModelRules: idexalBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const idexalBuiltin = await this.#idexalBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = idexalBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = idexalBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 Idexal Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(idexalBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = idexalBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? idexalBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...idexalBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(idexalBuiltin, current.providers);
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        idexalBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(idexalBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(idexalBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(idexalBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(idexalBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#idexalBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _idexalBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  idexalBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    idexalBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(idexalBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? idexalBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return idexalBuiltin.providers.overlay(personal);
}

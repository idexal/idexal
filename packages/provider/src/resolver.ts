...
import type { completeModelConfigDataSchema } from "@idexal/shared/model-config";
import type {
  ...
  resolve(input: ProviderConfigResolverInput): ProviderConfigResolution {
    const accountProviders = new ProviderConfigMap(
      input.accountProviders
        .entries()
        .filter(([providerId]) => input.idexalBuiltinProviders.has(providerId))
        .map(([providerId, config]) => [providerId, config.withoutGroup()] as const),
    );
    const concreteBuiltinProviders = input.idexalBuiltinProviders.overlay(accountProviders);
    const providerTemplates = input.idexalBuiltinProviderTemplates;
    ...
    for (const providerId of resolveProviderOrder(input, effectiveProviders)) {
      ...
      const models = orderedModelIds.map((modelId): ResolvedProviderModel => {
        ...
        const effectiveBuiltinConfig = input.idexalBuiltinModelRules.resolve({
          providerId,
          templateId,
          modelId,
          apiType: config.api?.type,
...
function resolveProviderOrder(
  ...
  const builtinIds = input.idexalBuiltinProviders
    .keys()

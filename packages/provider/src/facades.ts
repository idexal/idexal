...
export class ProviderSettingsFacade {
  ...
  getView(): ProviderSettingsView {
    ...
    return createProviderSettingsView({
      ...
      idexalBuiltinProviders: snapshot.config.idexalBuiltinProviders,
      idexalBuiltinProviderTemplates: snapshot.config.idexalBuiltinProviderTemplates,
  ...
  resolveModelConfig(input: ResolveModelConfigInput): ModelConfigResolution {
    ...
    if (!("personalConfig" in input)) {
      const modelRules = ModelConfigRules.composeEffective(
        snapshot.config.idexalBuiltinModelRules,
    ...
    const inheritedConfig = snapshot.config.idexalBuiltinModelRules.resolve({
      providerId: input.providerId,
      templateId: provider.templateId,
      modelId: input.modelId,
      apiType: provider.config.api?.type,
...
function createProviderSettingsView(input: {
  revision: number;
  idexalBuiltinProviders: ProviderRegistryServiceSnapshot["config"]["idexalBuiltinProviders"];
  idexalBuiltinProviderTemplates: ProviderRegistryServiceSnapshot["config"]["idexalBuiltinProviderTemplates"];
  ...
  return Object.freeze({
    ...
    providerTemplates: Object.freeze(
      (input.idexalBuiltinProviderTemplates ?? ProviderTemplateMap.empty())
        .entries()

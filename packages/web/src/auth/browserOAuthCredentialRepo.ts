...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const idexalJwtToken = tokenSet.idexalJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (idexalJwtToken) {
      this.localStorage.setItem(IDEXAL_JWT_TOKEN_KEY, idexalJwtToken);
    } else {
      this.localStorage.removeItem(IDEXAL_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const idexalJwtToken = this.localStorage.getItem(IDEXAL_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(idexalJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(idexalJwtToken, this.now()).kind === "expired") {
  ...
  loadIdexalJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(IDEXAL_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(IDEXAL_JWT_TOKEN_KEY) ||

...
function createWebZaiOAuthConfig(env: WebImportMetaEnv = {}): WebZaiOAuthConfig {
  ...
  const idexalEndpointUrls = buildIdexalEndpointUrls(
  ...
  return {
    ...
    // BigModel 用 appId 而不是 client_id，且默认值就是桌面端在用的 "idexal"。
    bigmodelAppId: env.VITE_BIGMODEL_OAUTH_APP_ID?.trim() || "idexal",
    redirectUri: idexalEndpointUrls.webShareCallbackUrl,
    shareRedirectUri: idexalEndpointUrls.webShareCallbackUrl,
    ...(devOrigin ? { devOrigin } : {}),

...
import { connectViaWebSocket } from "@idexal/client";
...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@idexal/shared";
...
async function renderConversationSharePage(): Promise<void> {
  ...
  document.title = routeLocale === "zh-CN" ? "Idexal 会话分享" : "Idexal Conversation Share";
  ...
  const endpointOrigin =
    import.meta.env.VITE_IDEXAL_BASE_URL?.trim().replace(/\/+$/u, "") || window.location.origin;
  const mockMode =
  ...
  const getMockToken = () =>
    mockMode && window.sessionStorage.getItem("idexal:share:mock-auth") === "owner"
      ? "mock-owner-token"
      : null;
  const onLogout = () => {
    if (mockMode) {
      window.sessionStorage.removeItem("idexal:share:mock-auth");
  ...
  root.render(
    <ConversationShareLandingLoader
      shareCode={shareCode}
      client={client}
      getAccessToken={() => getMockToken() ?? webAuthService.getIdexalJwtToken()}
      onLogin={(provider) => {
        if (mockMode) {
          window.sessionStorage.setItem("idexal:share:mock-auth", "owner");
          window.location.reload();
          return;
        }

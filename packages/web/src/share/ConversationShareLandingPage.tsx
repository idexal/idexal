...
import { BIGMODEL_PROVIDER_ID, ZAI_PROVIDER_ID } from "@idexal/shared";
import type { ConversationSharePreview } from "@idexal/shared";
import { ConversationShareReadonlyTimeline } from "@idexal/ui/conversation-share-readonly";
import { renderOAuthProviderIcon } from "@idexal/ui/oauth-provider-icon";
import { applyTheme, resolveTheme, type Theme } from "@idexal/ui/useTheme";
...
interface Copy {
  ...
  retry: string;
  continueInIdexal: string;
  switchToDarkTheme: string;
  switchToLightTheme: string;
  continueHelp: string;
  downloadIdexal: string;
  retryOpen: string;
...
const COPY: Record<ConversationShareLandingLocale, Copy> = {
  "zh-CN": {
    ...
    outdatedTitle: "需要更新 Idexal",
    outdatedDescription: "这个分享由更新版本的 Idexal 创建，请升级后再查看。",
    unavailableTitle: "分享不可访问",
    unavailableDescription: "当前账号没有权限，或者分享内容已不存在。",
    retry: "重试",
    continueInIdexal: "去 Idexal 继续",
    switchToDarkTheme: "切换到深色主题",
    switchToLightTheme: "切换到浅色主题",
    continueHelp: "如果没有自动打开 Idexal，请先下载客户端，或再次尝试打开。",
    downloadIdexal: "下载 Idexal",
    artifactCountOne: "{count} 个结果物",
    artifactCountOther: "{count} 个结果物",
    retryOpen: "再次打开",
  },
  "en-US": {
    brand: "Idexal Conversation Share",
    ...
    outdatedTitle: "Update Idexal to continue",
    outdatedDescription:
      "This share was created by a newer version of Idexal. Please update to view it.",
    unavailableTitle: "Share unavailable",
    unavailableDescription:
      "This account is not allowed to view the share, or it no longer exists.",
    retry: "Try again",
    continueInIdexal: "Continue in Idexal",
    switchToDarkTheme: "Switch to dark theme",
    switchToLightTheme: "Switch to light theme",
    continueHelp: "If Idexal did not open, download the app or try opening it again.",
    downloadIdexal: "Download Idexal",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {importLink && headerView.continueVisible ? (
                      <a
                        ...
                        href={importLink}
                        aria-label={copy.continueInIdexal}
                        title={copy.continueInIdexal}
                        onClick={handleContinue}
                      >
                        <span className={headerView.continueCompact ? "sr-only" : "truncate"}>
                          {copy.continueInIdexal}
                        </span>
                        <ArrowUpRightIcon
                          className={
                            headerView.continueCompact
                  ...
                  >
                    ...
                    {importLink ? (
                      <span className={SHARE_CONTINUE_MEASURE_CLASS}>
                        <span>{copy.continueInIdexal}</span>
                        <ArrowUpRightIcon
        ...
        {importLink && showContinueHelp ? (
          ...
          <ShareContentInset>
            ...
            >
              ...
              <a className="text-brand underline underline-offset-2" href={IDEXAL_DOWNLOAD_URL}>
                {copy.downloadIdexal}
              </a>
            </div>
          </ShareContentInset>
          ...
          <ShareContentInset>
            ...
            >
              <a
                ...
                onClick={handleContinue}
              >
                <span className="truncate">{copy.continueInIdexal}</span>
...
export function ConversationShareLandingLoader({
  ...
  const handleThemeChange = useCallback((nextTheme: Theme) => {
    localStorage.setItem("idexal-theme", nextTheme);

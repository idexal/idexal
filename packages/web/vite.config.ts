...
// Vite 配置在 Node 加载期执行，不能导入 @idexal/shared 根入口。
...
} from "@idexal/shared/idexalEndpoint";
...
export default defineConfig(({ mode }) => {
  ...
  const idexalEndpointOrigin = resolveRuntimeIdexalEndpointOrigin(endpointEnv);
  ...
  return {
    ...
    define: {
      ...
      __IDEXAL_VERSION__: JSON.stringify(version),
      __IDEXAL_COMMIT__: JSON.stringify(env.IDEXAL_COMMIT || "unknown"),
      __IDEXAL_ENV__: JSON.stringify(idexalEnv),
      "import.meta.env.VITE_IDEXAL_BASE_URL": JSON.stringify(idexalEndpointOrigin),
      // 兼容旧 Web runtime 读取名；新代码统一读 VITE_IDEXAL_BASE_URL。

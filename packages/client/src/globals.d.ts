...
/**
 * window.idexal 类型定义 —— 仅包含需要 main 进程参与的平台操作
 *
...
declare global {
  interface Window {
    idexal: {
      ...
      /** 通过系统原生另存为对话框保存文件 */
      saveFile?(
        payload: import("@idexal/shared").SaveFileRequest,
      ): Promise<import("@idexal/shared").SaveFileResult>;
      /** 将当前页面的 print 媒体版面导出为 PDF（Chromium 打印引擎，矢量文本） */
      printPageToPdf?(): Promise<import("@idexal/shared").PrintPageToPdfResult>;
      ...
      importChromeBrowserData?(
        options?: import("@idexal/shared").ChromeBrowserDataImportOptions,

...
import { createServiceLogger } from "@idexal/services/node";
import {
...
function exposeWebSocket(
  ...
  services.exposeOnChannelServer(
    server,
    scope ? new Map([[IIdexalAgentService.channelName, scope.service]]) : new Map(),
  );
  socket.onClose(() => {
    void scope?.dispose();
    rawServer.dispose();
...
export async function createCoreHttpServer(
  ...
  const info: ServerRemoteInfo = {
    serverId: options.serverId ?? hostname() ?? "idexal-server",
  ...
  app.use("/ws/host", async (context, next) => {
    const capability = context.req.header(IDEXAL_RPC_HOST_CAPABILITY_HEADER);

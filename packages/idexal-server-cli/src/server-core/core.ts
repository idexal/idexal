...
import { IIdexalAgentService } from "@idexal/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitIdexalBuiltinProviderConfigFilePath =
    process.env[IDEXAL_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const idexalBuiltinProviderConfigFilePath = explicitIdexalBuiltinProviderConfigFilePath
    ? explicitIdexalBuiltinProviderConfigFilePath
    : typeof __IDEXAL_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeIdexalBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __IDEXAL_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!idexalBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 Idexal Built-in Provider Config，且未设置 ${IDEXAL_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IIdexalAgentService));

...
const SERVER_INSTALL_OWNERSHIP_PRODUCT = "idexal-server";
...
async function readOwnership(layout: ServerLayout): Promise<ServerInstallOwnership> {
  ...
  if (!markerStat?.isFile()) {
    throw new Error(`Idexal Server ownership marker is missing or invalid: ${layout.installFile}`);
  }
  try {
    ...
    throw new Error(`Idexal Server ownership marker is invalid: ${layout.installFile}`, {
      cause: error,
    });
  }
}
...
export async function validateServerInstallOwnership(
  ...
  const [ownership, canonicalServerRoot] = await Promise.all([
    ...
    realpath(layout.serverRoot).catch((error: unknown) => {
      throw new Error(`Idexal Server ownership root cannot be resolved: ${layout.serverRoot}`, {
        cause: error,
      });
    }),
  ]);
  if (ownership.canonicalServerRoot !== canonicalServerRoot) {
    throw new Error(
      `Idexal Server ownership root mismatch: expected ${ownership.canonicalServerRoot}, received ${canonicalServerRoot}`,
    );
  }
  return ownership;
}

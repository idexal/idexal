...
function getDefaultServerDataRoot(): string {
  const configured = process.env.IDEXAL_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".idexal", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\idexal-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".idexal") {

...
function stablePosixLauncher(bootstrap?: StableLauncherBootstrap): string {
  const fallback = bootstrap
    ...
    : 'echo "No current Idexal Server release" >&2; exit 1';
  return `#!/bin/sh
...
if [ -n "$RELEASE_DIR" ]; then
  case "$RELEASE_DIR" in
    "$ROOT/releases"/*) ;;
    *) echo "Invalid current Idexal Server release" >&2; exit 1 ;;
  esac
...
function stableWindowsLauncher(bootstrap?: StableLauncherBootstrap): string {
  const fallback = bootstrap
    ...
    : "echo No current Idexal Server release 1>&2\r\nexit /b 1\r\n";
  return `@echo off\r
set "ROOT=%~dp0.."\r
set "IDEXAL_SERVER_ROOT=%ROOT%"\r
powershell -NoProfile -NonInteractive -Command "$root=[IO.Path]::GetFullPath($env:IDEXAL_SERVER_ROOT); $current=Join-Path $root 'current.json'; if (Test-Path -LiteralPath $current) { $j=Get-Content -Raw -LiteralPath $current ^| ConvertFrom-Json; if ($j.releaseDir) { $release=[IO.Path]::GetFullPath([string]$j.releaseDir); $releases=[IO.Path]::GetFullPath((Join-Path $root 'releases')); if (-not $release.StartsWith($releases + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) { exit 1 } } }"\r
if errorlevel 1 (\r
  echo Invalid current Idexal Server release 1>&2\r
  exit /b 1\r
)\r
for /f "delims=" %%I in ('powershell -NoProfile -Command "$path=Join-Path $env:IDEXAL_SERVER_ROOT 'current.json'; if (Test-Path -LiteralPath $path) { $j=Get-Content -Raw -LiteralPath $path ^| ConvertFrom-Json; $j.releaseDir }"') do set "RELEASE_DIR=%%I"\r
...
export async function writeStableLauncher(
  ...
  const launcherPath = join(layout.stableBinDir, platform === "win32" ? "idexal.cmd" : "idexal");

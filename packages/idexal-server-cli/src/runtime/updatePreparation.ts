...
export async function prepareOnlineUpdate(layout: ServerLayout): Promise<UpdatePreparation> {
  const catalogUrl = process.env.IDEXAL_SERVER_RELEASE_MANIFEST_URL?.trim();
  if (!catalogUrl) {
    const pending = await new ReleaseManager(layout).readPending();
    if (!pending) {
      throw new Error(
        "Idexal Server release source is not configured and no offline pending release is available",
      );
    }
  ...
  const temporaryDir = await mkdtemp(join(tmpdir(), "idexal-server-update-"));
...
async function prepareComponentUpdate(input: {
  ...
  const temporaryDir = await mkdtemp(join(tmpdir(), "idexal-server-components-"));

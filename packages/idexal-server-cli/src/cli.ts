...
import { IDEXAL_VERSION } from "@idexal/shared";
import {
...
export function serviceRegistrationEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.IDEXAL_SERVER_SKIP_SERVICE_REGISTRATION !== "1";
}
...
async function runServe(
  ...
  if (shouldRegisterService(daemonRequested, supervisorProcess)) {
    ...
    if (existing && (existing.state === "ready" || existing.state === "starting")) {
      if ((existing.serviceRegistered && !legacyIdentityMigration) || startupMode === "fallback") {
        if (json) stdout(io, existing);
          ...
          stdout(
            io,
            `Idexal Server ${existing.state} at ${existing.host ?? ""}:${existing.port ?? ""}`,
          );
        return 0;
      }
    ...
    if (!serviceStarted) {
      const child = fork(
        ...
        [...SERVER_SERVICE_ARGS, "--server-root", layout.serverRoot],
        {
          ...
          env: {
            ...process.env,
            IDEXAL_DATA_BASE_DIR: layout.dataBaseDir,
            IDEXAL_SERVER_ROOT: layout.serverRoot,
          },
        },
      );
      child.unref();
    ...
    const started = await waitForPersistedStatus(
      ...
      () => {
        if (childEarlyExit) {
          throw new Error(
            `Idexal Server daemon exited before ready (code=${childEarlyExit.code ?? "null"} signal=${childEarlyExit.signal ?? "none"}); check ${layout.statusFile} for details`,
          );
        }
      },
      serviceStarted,
    ...
    if (json) stdout(io, started);
      ...
      stdout(io, `Idexal Server ${started.state} at ${started.host ?? ""}:${started.port ?? ""}`);
    process.stdin.pause();
      ...
      launch: (generation, release) => {
        ...
        return fork(corePath, [String(generation)], {
          ...
          env: {
            ...inheritedEnv,
            ...releaseWiring,
            ...(runtimeRoot ? { IDEXAL_SERVER_RUNTIME_ROOT: runtimeRoot } : {}),
          },
  ...
  else stdout(io, `Idexal Server ${status.state} at ${status.host ?? ""}:${status.port ?? ""}`);
...
async function runControl(
  ...
  try {
    ...
    if (command === "status") {
      ...
      result = persisted.status ?? createStoppedServerStatus(IDEXAL_VERSION);
...
async function runUninstall(
  ...
  const first = await (io.confirm?.("Type DELETE to uninstall Idexal Server: ") ??
    Promise.resolve(""));
...
async function finishUninstall(
  ...
  try {
    ...
    // Idexal allowlist，并把保留项写入结果供用户审计。先落盘 marker，再释放 lock，
    // 让并发 Supervisor 在删除 run 目录的最后窗口也会 fail-closed。
    await writeFile(
      layout.uninstalledFile,
      `${JSON.stringify({ uninstalled: true, uninstalledAt: Date.now(), version: IDEXAL_VERSION, preservedPaths }, null, 2)}\n`,
...
async function delegateLegacyCli(argv: readonly string[], io: CliIO): Promise<number> {
  const candidate =
    process.env.IDEXAL_LEGACY_CLI_ENTRY?.trim() ||
    join(dirname(fileURLToPath(import.meta.url)), "idexal.cjs");
  try {
    await access(candidate);
  } catch {
    stdout(io, argv.length ? `Unknown command: ${argv[0]}` : "Idexal TUI");

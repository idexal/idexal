...
interface StageOptions {
  ...
  distDir: string;
  /** 既有 CLI/Agent bundle（idexal.cjs，自包含 CJS） */
  ...
  /** 未被 pnpm 链接到 node_modules 的 workspace 包（例如 @idexal/tui）。 */
...
# 由 idexal-server staging 生成：定位发行根后用随包 Node 启动 Server CLI。
...
/** 组装 `idexal-server-<os>-<arch>/` 发行目录；只做本地组装，不上传、不发布。 */
export async function stageRelease(options: StageOptions): Promise<StagedRelease> {
  ...
  const releaseName = `idexal-server-${options.target}`;
  ...
  await cp(options.agentBundlePath, join(runtimeDir, "idexal.cjs"), { dereference: true });
  ...
  const launcherPath = join(
    releaseDir,
    "bin",
    options.target.startsWith("win32-") ? "idexal.cmd" : "idexal",
  );
  ...
  const componentSpecs = [
    ...
    { id: "agent-runtime", paths: ["runtime/idexal.cjs", "runtime/licenses/agent"] },
    ...(plugins.length > 0
      ? [
          {
            id: "official-plugins",

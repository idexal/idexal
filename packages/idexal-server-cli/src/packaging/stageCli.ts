...
async function ensureNodeBinary(repoRoot: string, target: ServerTarget): Promise<string> {
  ...
  const cacheDir = join(
    repoRoot,
    "node_modules/.cache/idexal-server-cli",
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "idexal-server-node-"));
  try {
...
async function ensureAgentBundle(repoRoot: string, skipBuild: boolean): Promise<string> {
  const bundlePath = join(repoRoot, "apps/idexal-cli/packages/cli/dist/idexal.cjs");
...
export async function resolveNativeToolsDir(
  ...
  const cacheDir = join(repoRoot, "node_modules/.cache/idexal-server-cli", "tools", target);
...
async function resolveWorkspacePackageDirs(repoRoot: string): Promise<Map<string, string>> {
  ...
  for (const root of [join(repoRoot, "apps/idexal-cli/packages"), join(repoRoot, "packages")]) {
...
async function main(): Promise<void> {
  ...
  if (!(await pathExists(join(distDir, "server-cli.js")))) {
    throw new Error(
      `Missing tsup output in ${distDir}; run pnpm --filter @idexal/server-cli build first`,
    );
  }
  ...
  const [nativeToolsDir, officialPluginsDir, workspacePackageDirs] = await Promise.all([
    ...
    Promise.resolve(join(repoRoot, "apps/idexal-cli/packages")),

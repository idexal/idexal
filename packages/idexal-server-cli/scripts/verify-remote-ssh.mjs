 ...
 *
 * 前置：docker 可用；已运行 `pnpm --filter @idexal/server-cli stage --target <target>`。
...
const HOST_CAPABILITY_HEADER = "x-idexal-rpc-host-capability";
...
const releaseArchive = join(packageRoot, "dist-release", `idexal-server-${target}.tar.gz`);
const containerName = `idexal-server-verify-${process.pid}`;
const imageTag = "idexal-server-verify-sshd:ubuntu22";
...
async function main() {
  ...
  await run("ls", [releaseArchive], { quiet: true }).catch(() => {
    throw new Error(
      `Release archive missing: ${releaseArchive}; run pnpm --filter @idexal/server-cli stage --target ${target}`,
    );
  });

  // 一次性 ssh 密钥，容器仅信任本次运行生成的公钥。
  const workDir = await mkdtemp(join(tmpdir(), "idexal-server-verify-"));
  ...
  await ssh(
    `mkdir -p /root/idexal-server && tar -xzf /root/idexal-server-${target}.tar.gz -C /root/idexal-server --strip-components=1`,
  );

  log("start daemon on remote");
  const { stdout: daemonOutput } = await ssh(
    "/root/idexal-server/bin/idexal serve --daemon --json",
  );
  ...
  await ssh(
    "cd /root/idexal-server/runtime && ./node -e \"const pty=require('node-pty');const p=pty.spawn('/bin/echo',['pty-ok'],{cols:80,rows:24});let o='';p.onData(d=>o+=d);p.onExit(()=>{process.exit(o.includes('pty-ok')?0:1)})\"",
  );
  log("node-pty ok");

  log("verify remote lifecycle status/stop");
  const { stdout: statusOutput } = await ssh("/root/idexal-server/bin/idexal status --json");
  assert(
    JSON.parse(statusOutput.trim().split("\n").pop()).state === "ready",
    "remote status ready",
  );
  await ssh("/root/idexal-server/bin/idexal stop --json");
  const { stdout: stoppedOutput } = await ssh("/root/idexal-server/bin/idexal status --json");

import { z } from "zod";
import { decodeIdexalBuiltinRelease, type IdexalBuiltinRelease } from "./idexal-builtin-release.js";
...
export interface IdexalBuiltinDownloadOptions {
...
export async function downloadIdexalBuiltinRelease(
  options: IdexalBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeIdexalBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`Idexal Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }

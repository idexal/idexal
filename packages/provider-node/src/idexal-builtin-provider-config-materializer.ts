...
export interface MaterializeIdexalBuiltinProviderConfigOptions {
 ...
 */
export async function materializeIdexalBuiltinProviderConfig(
  options: MaterializeIdexalBuiltinProviderConfigOptions,
): Promise<string> {
  const content = `${serializeIdexalBuiltinRelease(
    decodeIdexalBuiltinRelease(JSON.parse(options.content)),
  )}\n`;
  const filePath = join(

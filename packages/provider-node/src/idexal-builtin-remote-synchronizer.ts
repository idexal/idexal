...
import type { NodeIdexalBuiltinProviderConfigSource } from "./idexal-builtin-provider-config-source.js";
import type { IdexalBuiltinRelease } from "./idexal-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type IdexalBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface IdexalBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: IdexalBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface IdexalBuiltinRefreshEvent {
  readonly result: IdexalBuiltinRefreshResult;
...
export class IdexalBuiltinRemoteSynchronizer {
  readonly #options: IdexalBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<IdexalBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: IdexalBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<IdexalBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<IdexalBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("Idexal Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: IdexalBuiltinRefreshEvent): void {

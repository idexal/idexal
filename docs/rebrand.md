...
- **手机 Web 宽度下输入框的 Send 按钮不可达（实测，未修复，待与产品对齐修法）**：390×844（iPhone 12/13/14 宽度）冷启动 Web 目标并跳过鉴权进入主壳后，侧栏保持展开占 195px，而 `.chat-composer-input-surface` 实测 `x=216, width=271`（右边界 487），已经超出 390 的视口；其内层卡片带 `overflow-hidden`，且 `documentElement.scrollWidth === 390`（页面不产生横向滚动），于是工具栏右侧的 `Send`（`x=446..474`）、模型选择器（`idexal/auto/best-coding`）、`On` 与分支 chip `main` 全部被裁到视口外且**无法通过滚动触达**。链路是 `group/toolbar flex items-end gap-3`（nowrap）+ 右侧组 `shrink-0`，加上外层内容列没有把宽度约束住，所以 composer 被撑到比可用列宽更宽。属上游布局在小屏下的适配缺失，不是品牌改动引入的。两个候选修法需要产品先定：① 让内容列以 `min-w-0` 约束住 composer 宽度并允许工具栏在窄屏换行/收纳次要控件；② 在窄断点收起侧栏（`WorkspaceSidebarCollapsedRail` 已有组件定义，但主壳当前没有渲染它，需新增渲染与状态逻辑），把 195px 还给内容区。二者取舍不同（前者保住侧栏信息密度，后者保住工具栏一行布局），因此不擅自叠加兜底分支。
  - 修法待定前的机制核查（v3.15.12 补充，均为实测/全仓检索结论，避免后人重复排查）：
    - `V4ComposerToolbar` 的 `isMobileViewport` 是**死 prop**：只在类型（`V4ComposerToolbar.tsx:334`）和解构默认值（`:372`）出现，组件体内从未读取，所以工具条不存在任何“手机布局分支”可接。
    - 三个 `isMobileViewport: false` 字面量（`ConversationComposer.tsx:827/832`、`SessionPane.tsx:2083`）只喂 `ComposerAutoFocusOptions` 的聚焦决策，与布局无关，不能误当成“移动端已实现只是没开”。
    - `WorkspaceSidebarCollapsedRail` 只有定义与 `WorkspaceSidebar.tsx:156` 的再导出，主壳渲染路径里没有任何使用点，因此“窄屏自动收起为已有 rail”并非接现成组件，需要新写渲染与状态逻辑。
    - 结论：让 `Send` 在 390px 可达是一次真正的响应式设计工作（要定：哪些控件收纳、侧栏是否转为抽屉、Send 落位），且改的是桌面共用的 composer 组件。按仓库规则不在未对齐时叠加兜底分支，故本版本仍只记录不实现。
  - 量测边界（同一缺陷在不同宽度表现不同，供定阈值）：390px 干净加载 → 侧栏 195、composer 271、`Send` 右边界 474、不可达、无横向滚动；768px → 侧栏 264、composer 右边界 736、`Send` 可达；480px → `aside` 实测宽度 0 且 `Send` 可达。480 与 390 的反差说明侧栏宽度不是纯 CSS 断点驱动，其收起触发条件尚未定位，定修法前需先查清这条链路。

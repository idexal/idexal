...
缺键工具、类型级防漏、重复键检测：原先全部不存在。

已落地为 `scripts/check-i18n.mjs`，挂在 `pnpm verify:pre-push` 上（`coverage` 子命令单看覆盖率）。
拦五项：重复键、孤儿键、译文空值、`{placeholder}` 与英文不一致、`ar`/`fr` 键集不对称，
外加"阿语行混入 CJK"。占位符那条必须是拦截而不是提示——漏掉 `{time}` 会渲染出字面量，
属于用户可见的坏界面，而类型检查完全看不见它。

**没有进门禁的一项**：值里残留未译英文词的扫描（本次正是靠它抓出
`Demandez anything à Idexal` 与桌面阿语里混入的中文）。它只作为**写作时的辅助扫描**保留：
法语里 `session`/`Menu`/`Total`/`Quota`/`Mention` 都是合法同源词，约 380 键里有 38 条误报，
做成硬门禁只会逼人把它关掉。分界是——机器能判对错的进 CI，需要人判断的留在写作流程里。
每项门禁都各自注入过真实缺陷验证有判别力（细节见 `docs/i18n-rtl.md`）。

**这条已经真实咬过一次**：最初的草稿版本用正则数键名，字符集写成 `[A-Za-z0-9_.]`，
...
正式版因此在 `vm` 沙箱里把语言表当模块求值再读真实键集，
并且**自检解析器是否吃到了那些别扭的键**——吃不到就判定失败，而不是安静地报"通过"。

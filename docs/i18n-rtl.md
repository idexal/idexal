...
后来在我自己新写的桌面译文里又复现了一次（阿语句子里混入中文），说明**边写边扫**才是有效做法。

这些不变量已经固化成一条命令，并挂在 `pnpm verify:pre-push` 上：

```
node scripts/check-i18n.mjs            # 门禁：失败即 exit 1
node scripts/check-i18n.mjs coverage   # 只看各语言覆盖率
```

它检查的是：无重复键、无孤儿键（译文里不得出现 `en-US` 没有的键）、译文无空值、
`{placeholder}` 集合逐条与英文一致、`ar`/`fr` 覆盖键集对称、以及阿语行不得混入 CJK 字符。

两个实现约束值得记住：

- **不许用正则数键名**。本仓库有含连字符与非 ASCII 的键
  （`sidebar.settings.locale.en-US`、`feedback.severity.P1-高.label`），
  字符集写成 `[A-Za-z0-9_.]` 会**静默跳过**它们，于是"0 孤儿键 / 0 不对称"
  变成从未看过那 17 个键的绿灯——3.16.1–3.16.3 发布的覆盖率正是这样算错的。
  现在用 `vm` 沙箱把语言表当模块求值，再读真实键集。
- **每条新检查都要证明它有判别力**。这四项各自注入一次真实缺陷验证过
  （删键 → 报不对称；删 `{used}{total}` → 报占位符；塞 `言語` → 报混入 CJK；造孤儿键 → 报未定义键），
  否则"通过"和"根本没在看"无法区分。

...
| ------- | ---- | ------------------- | ------------------------------------------- |
| `en-US` | 5859 | 100%（事实来源）    | 全量                                        |
| `ar`    | 317  | ≈5.4%               | 登录/引导/API key + 常驻外壳 + 设置页前两区 |
| `fr`    | 317  | ≈5.4%               | 同上，键集与 `ar` 逐一相同                  |
| `zh-CN` | 5858 | 遗留                | 仅兼容旧配置，不再新增文案                  |
...
`SettingsPage` 只在桌面渲染进程挂载（`packages/web` 不引用它），而再起一个隔离 dev 实例要先跑
`@idexal/desktop` 的 `pre-dev`，它会 `rmSync('./out', {recursive:true})`，与用户正在运行的开发实例的
`tsup --watch` 抢同一份 `out/`。因此**阿语设置页在 RTL 下的排版几何属于未测面**：3.16.2 只有静态门禁
与字符串级校验。要放开验证，前提是要么关闭开发实例，要么把桌面构建产物目录参数化，
让 QA 实例写到别处。
...
Tailwind 的物理工具类（`ml-/mr-/pl-/pr-/left-/right-/translate-x-/text-left/rounded-l/border-l/space-x-`）
确实不认 `dir`，`packages/ui/src` 内计数 **≥400 处、分布在 197 个文件**。这条事实成立，
但它的含义只是"这些不会自动镜像"，**不等于"这些是错的"**：外壳的实测结果表明大量物理类要么出现在
左右对称的布局里，要么其容器本身已被 flex/grid 的 `dir` 翻转。

因此这里的准确状态是**未逐组件复核**，而不是"已知存在镜像缺陷"。
把它们改为逻辑属性（`ms-/me-/ps-/pe-/start-/end-/text-start/text-end`）仍是独立的工程，

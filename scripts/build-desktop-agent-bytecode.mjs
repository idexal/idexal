...
const defaultEntryPath = join(repoRoot, "apps/idexal-cli/packages/cli/dist/idexal.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.IDEXAL_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "idexal.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `idexal.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `idexal.bytecode-runtime-${runtimeHash}.cjs`;

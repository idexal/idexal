...
assert.ok(archive, "Usage: node scripts/idexal-distribution-smoke.mjs <archive.tar.gz>");
const directory = await realpath(await mkdtemp(join(tmpdir(), "idexal-release-smoke-")));
const root = join(directory, "idexal");
const runner = join(root, "bin/idexal.mjs");
const workspace = join(directory, "workspace");
const env = {
  ...process.env,
  IDEXAL_DATA_BASE_DIR: join(directory, "data"),
  NODE_PATH: "",
  NODE_OPTIONS: "",
  TERM: "xterm-256color",
};
...
try {
  ...
  await writeFile(
    runtimeCheck,
    'import { runTui } from "@idexal/tui"; if (typeof runTui !== "function") throw new Error("Missing TUI export"); console.log("tui-runtime-ok");',
  );
  ...
  await until(
    () => /Idexal/.test(screen) && /(?:登录|\/login|输入提示词|Type a prompt)/i.test(screen),

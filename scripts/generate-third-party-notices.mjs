...
export async function generateThirdPartyNotices(root = repositoryRoot) {
  ...
  for (const record of copied) {
    ...
    for (const file of record.modifiedFiles ?? []) {
      ...
      if (!(await readFile(join(root, file), "utf8")).includes("Modified by Idexal:"))
  ...
  const sections = [
    ...
    ...patches.map(
      (item) =>
        `- ${item.package}: modified by Idexal; the changes are recorded in ${item.file} in the source repository.`,
    ),

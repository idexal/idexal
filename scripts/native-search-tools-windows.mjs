...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "idexal-native-search-build-"));
  ...
  console.log("==> Idexal native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DIDEXAL_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DIDEXAL_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DIDEXAL_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DIDEXAL_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DIDEXAL_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DIDEXAL_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DIDEXAL_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DIDEXAL_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(

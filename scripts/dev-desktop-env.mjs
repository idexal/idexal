  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          IDEXAL_ENV: requestedEnv,
          IDEXAL_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@idexal/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@idexal/desktop", "dev:runtime"]);
} catch (error) {

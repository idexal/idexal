...
const IDEXAL_AGENT_RUNTIME = {
  glm: {
    version: readIdexalAgentRuntimeVersion(),
  },
};
...
const isBootstrapWithRemote = process.env.IDEXAL_BOOTSTRAP_WITH_REMOTE === "1";

/**
 * Node dist 下载源。默认走国内镜像，`IDEXAL_NODE_DIST_MIRROR` 可覆盖（与
...
export function nodeDistBase(env = process.env) {
  const mirror = env.IDEXAL_NODE_DIST_MIRROR?.trim();
  return (mirror || DEFAULT_NODE_DIST_BASE).replace(/\/+$/u, "");
}
const BROWSER_USE_PLUGIN_PACKAGE_NAME = "@idexal/browser-use-plugin";
// node_repl 宿主抽成独立包 @idexal/node-repl-host 之后，browser-use
// 不再产出 dist/mcp/server.js，CUA 资产也已归 @idexal/zcode-cua-plugin。这是**第三份**平行清单
...
const remoteOfficialPluginPackages = [
  ...
  // 清单、packages/server/src/remote/idexalAgentOfficialPluginAssets.ts 的远端合同保持一致。
  {
    // 远端 shared-host 必须部署 node_repl runtime，否则只剩 skill 而没有 mcp__node_repl__js ——
    // 该 runtime 现由 @idexal/node-repl-host 提供（见下一个条目），browser-use 只带自己的
    // client script 与 skill/docs。
    packageName: "@idexal/browser-use-plugin",
    relativePath: "apps/idexal-cli/packages/browser-use-plugin",
  ...
  },
  {
    ...
    packageName: "@idexal/node-repl-host",
    relativePath: "apps/idexal-cli/packages/node-repl-host",
    requiresRuntime: true,
...
// 随 CLI 内置的技能包（不是插件）：远端 agent 的 bootstrap 沿官方插件同款候选目录在 idexal.cjs 旁
...
const remoteBundledSkillPack = {
  relativePath: "apps/idexal-cli/packages/bundled-skills",
  requiredPaths: [
...
const remoteOfficialPluginRequiredPaths = [
  "packages/browser-use-plugin/.idexal-plugin/plugin.json",
  "packages/node-repl-host/.idexal-plugin/plugin.json",
];

function readIdexalAgentRuntimeVersion() {
  const runtimeSourcePath = join(rootDir, "packages/shared/src/idexal-agent-runtime.ts");
  const runtimeSource = readFileSync(runtimeSourcePath, "utf8");
  const match = runtimeSource.match(/version:\s*["']([^"']+)["']/);
  if (!match?.[1]) {
    throw new Error("Unable to parse Idexal Agent runtime version");
  }
  return match[1];
}
...
async function extractArchiveMember(url, destinationDir, archiveMember) {
  const tempDir = mkdtempSync(join(tmpdir(), "idexal-node-dist-"));
...
async function prepareNodeBinaries() {
  for (const platformKey of remotePlatforms) {
    ...
    try {
      ...
      console.error(
        `  [error] 请检查 CI runner 的外网访问、tar/xz 依赖，或用 IDEXAL_NODE_DIST_MIRROR 覆盖下载源（当前 ${nodeDistBase()}）`,
      );
      throw error;
    }
  }
...
function runBootstrapServerRemoteBuild() {
  ...
  // 这里不能复用已有 idexal-server.cjs：开发时 package version 常不变，旧 bundle 会把缺少新 RPC 的
...
function copyServerBundle() {
  ...
  copyFileSync(
    join(rootDir, "packages/server/dist/remote/idexal-server.cjs"),
    join(serverDir, "idexal-server.cjs"),
  );
  console.log("  [ok] mock-cdn server/idexal-server.cjs");
}
...
function buildRemoteOfficialPluginRuntimes() {
  for (const plugin of remoteOfficialPluginPackages) {
    ...
    runCommand(
      pnpmCommand,
      ["--dir", join(rootDir, "apps/idexal-cli"), "--filter", plugin.packageName, "build"],
      {
        cwd: rootDir,
        env: process.env,
      },
...
function stageRemoteOfficialPlugins(glmDir) {
  for (const plugin of remoteOfficialPluginPackages) {
    ...
    const manifestPath = join(sourceRoot, ".idexal-plugin", "plugin.json");
...
// 远端 agent 现在跑编译出来的 idexal.cjs（而不是各平台独立的原生二进制）：
// 远端部署时已经有一份独立 node（跑 idexal-server.cjs），agent 复用它执行 idexal.cjs 即可，
// 不必再为每个平台准备一份内嵌 node 的 SEA 二进制。idexal.cjs 跨平台同一份，逐平台只是放进各自的
// glm/<platform> 组件目录，保持现有 manifest 组件结构不变。
async function stageRemoteAgentBundles() {
  console.log("==> Building idexal-cli bundle for remote agents");
  // 复用桌面同款构建脚本（turbo build:desktop-agent --filter=@idexal/cli），命中缓存时几乎瞬时。
  runCommand(process.execPath, [join(rootDir, "scripts/build-desktop-agent-cli.mjs")], {
    cwd: rootDir,
    env: process.env,
  });
  // browser-use runtime 的 tsc 依赖 @idexal/core/dist。远端资产也必须先构建
  // agent CLI 依赖，避免 CI 干净检出时被开发机缓存掩盖的 TS2307。
  buildRemoteOfficialPluginRuntimes();
  const cliBundlePath = join(rootDir, "apps/idexal-cli/packages/cli/dist/idexal.cjs");
  ...
  for (const platformKey of remotePlatforms) {
    ...
    // 干净重建：glm 组件现在只含 idexal.cjs，清掉历史遗留的原生二进制 / 旧 meta，
    // 避免被打进组件 tar 把远端资源撑大。
    rmSync(glmDir, { recursive: true, force: true });
    mkdirSync(glmDir, { recursive: true });
    copyFileSync(cliBundlePath, join(glmDir, "idexal.cjs"));
    stageRemoteOfficialPlugins(glmDir);
    await stageRemoteBundledSkillPack(glmDir);
    console.log(`  [ok] mock-cdn glm/${platformKey}/idexal.cjs`);
  }
}
...
// glm 承载 idexal-cli app-server 协议 schema。即使 runtime 版本未变化，
// idexal.cjs 也可能随 app 代码变更；跨 release 复用旧 glm 会让远端 agent 拒绝新协议字段。
...
function buildReusableComponentRequiredPaths(componentId, platformKey) {
  switch (componentId) {
    ...
    case "glm":
      // GLM 现在是编译产物 idexal.cjs（跨平台同一份），远端用已部署的 node 执行它。
      // 复用时还要确认官方插件 seed 资源完整，否则旧 release 会继续产出 0 builtin plugin 的远端资源包。
      return ["idexal.cjs", ...remoteOfficialPluginRequiredPaths];
    case "bfs":
      return ["bfs"];
    case "ripgrep":
...
export function buildRemoteComponentDefinitions(platformKey) {
  const baseComponents = [
    ...
    },
    {
      ...
      // 这里复用 IDEXAL_AGENT_RUNTIME.glm.version，保持 manifest 版本与运行时描述一致。
      semanticPrefix: IDEXAL_AGENT_RUNTIME.glm.version,

...
APP_PATH="${1:-${IDEXAL_MACOS_RELEASE_APP_PATH:-/Applications/Idexal.app}}"
# 安装包身份与后端环境分轴：IDEXAL_PREVIEW_IDENTITY=1 让生产后端的构建仍是 Idexal Preview。
# 只认 "1"，与 CI workflow / release 门的精确比较同一套语义（其它拼写一律视为未开启）。
is_preview_identity_requested() {
  [[ "${IDEXAL_PREVIEW_IDENTITY:-}" = "1" ]]
}
APP_BUNDLE_NAME="$(basename "$APP_PATH")"
APP_DISPLAY_NAME="${APP_BUNDLE_NAME%.app}"
APP_EXECUTABLE_NAME="${IDEXAL_APP_EXECUTABLE_NAME:-$APP_DISPLAY_NAME}"

if [ "${APP_PATH:-}" = "--help" ] || [ "${APP_PATH:-}" = "-h" ]; then
  cat <<'USAGE'
Usage:
  bash scripts/doctor-macos-release-app.sh /Applications/Idexal.app
  IDEXAL_MACOS_RELEASE_APP_PATH=/Applications/Idexal.app pnpm run doctor:macos-release
...
run_quiet_validation() {
  ...
  shift
  local output_file
  output_file="$(mktemp "${TMPDIR:-/tmp}/idexal-macos-release-doctor.XXXXXX")"

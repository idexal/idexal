...
export async function packageNativeSearchTools({
  platform = process.env.IDEXAL_TARGET_OS || process.platform,
  arch = process.env.IDEXAL_TARGET_ARCH || process.arch,
...
async function main() {
  await packageNativeSearchTools({
    platform: readOption("platform") ?? process.env.IDEXAL_TARGET_OS ?? process.platform,
    arch: readOption("arch") ?? process.env.IDEXAL_TARGET_ARCH ?? process.arch,

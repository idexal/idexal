...
export function resolveProductionRemoteAssetCacheDir(
  ...
  if (platform === "darwin") {
    return pathApi.join(homeDir, "Library", "Application Support", "Idexal", "remote-assets-cache");
  }

  if (platform === "win32") {
    const appDataDir = env.APPDATA?.trim() || pathApi.join(homeDir, "AppData", "Roaming");
    return pathApi.join(appDataDir, "Idexal", "remote-assets-cache");
  }

  const configDir = env.XDG_CONFIG_HOME?.trim() || pathApi.join(homeDir, ".config");
  return pathApi.join(configDir, "Idexal", "remote-assets-cache");
}
...
export function buildDesktopRemoteProdEnv(
  ...
  const cacheDir =
    baseEnv.IDEXAL_REMOTE_ASSET_CACHE_DIR?.trim() ||
    resolveProductionRemoteAssetCacheDir(baseEnv, platform, homeDir);

  return {
    ...baseEnv,
    // remote CDN 基址现在跟随 IDEXAL_ENV 分流；该脚本用于复现生产态下载链路，
    // 因此需要同时强制 production 和 CDN 开关，避免默认 test 环境落到测试资源 CDN。
    IDEXAL_ENV: "production",
    IDEXAL_DEV_REMOTE_ASSET_USE_CDN: "1",
    IDEXAL_REMOTE_ASSET_CACHE_DIR: cacheDir,
  };
}
...
export function runDesktopRemoteProdDev() {
  ...
  const child = spawn(resolvePnpmCommand(), ["--filter", "@idexal/desktop", "dev"], {
    cwd: repoRoot,
    stdio: "inherit",

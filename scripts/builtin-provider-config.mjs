...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.IDEXAL_ENV !== undefined) value = parsed.IDEXAL_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid IDEXAL_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.IDEXAL_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/idexal-builtin.json",
  );
  try {
    ...
    const { decodeIdexalBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/idexal-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeIdexalBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "idexal-builtin.json"), config.content, "utf8");
  return config;
}

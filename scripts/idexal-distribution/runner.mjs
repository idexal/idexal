...
const agentEntry = join(root, "agent", "idexal.cjs");

function usage() {
  return `Usage:
  idexal --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  idexal --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      IDEXAL_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      IDEXAL_AGENT_SERVER_COMMAND: process.execPath,
      IDEXAL_SERVER_HOST: options.host,
      IDEXAL_SERVER_WORKSPACE: options.workspace,
      IDEXAL_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      IDEXAL_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("Idexal Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: idexal --web [options] (idexal --web --help for details)\n");
    }

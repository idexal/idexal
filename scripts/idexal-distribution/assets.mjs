...
import {
  collectSeaTuiAssets,
  seaTuiAssetPrefix,
} from "../../apps/idexal-cli/packages/cli/scripts/sea-tui-assets.mjs";
import { supportedTargets } from "../../apps/idexal-cli/packages/cli/scripts/sea-targets.mjs";
  ...
  try {
    for (const target of supportedTargets) {
      const { assets, manifest } = await collectSeaTuiAssets({
        root: resolve(root, "apps/idexal-cli"),
        stagingDirectory,
        target,
      });
...
async function copyRuntimePackageTree({ packageName, packageRoot, requireFrom, seen }) {
  ...
  for (const dependencyName of Object.keys(dependencies)) {
    try {
      ...
      console.warn(`[idexal] optional package ${dependencyName} is unavailable; skipping`);
    }
  }
}
...
export async function copyRuntimeNodeModules(packageRoot) {
  ...
  await copyRuntimePackageTree({
    ...
    requireFrom: createRequire(resolve(root, "apps/idexal-cli/packages/cli/package.json")),
    seen: new Set(),
  });
}
...
export async function patchNodePtyPrebuilds(packageRoot) {
  ...
  for (const packageName of lydellNodePtyPackages) {
    ...
    try {
      ...
      console.warn(`[idexal] ${packageName} is unavailable; skipping node-pty prebuild patch`);
      continue;
    }

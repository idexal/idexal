...
function printUsageAndExit() {
  process.stderr.write(
    [
      "Usage: idexal-stdio-tap.mjs --workspace-key <key> [--log-dir <dir>] -- <command> [...args]",
      "",
    ].join("\n"),
  );
  process.exit(2);
...
function parseArgs(argv) {
  ...
  let workspaceKey = "";
  let logDir = process.env.IDEXAL_STDIO_TAP_LOG_DIR?.trim() || "";
  ...
  return {
    ...
    logDir: logDir || join(homedir(), ".idexal", "v2", "dev", "stdio-traffic"),
    workspaceKey,
  };
}
...
child.on("error", (error) => {
  ...
  process.stderr.write(`[idexal-stdio-tap] spawn failed: ${error.message}\n`);
});

...
function resolveBootstrapWithRemoteEnv(baseEnv = process.env) {
  ...
  return {
    ...
    IDEXAL_BOOTSTRAP_WITH_REMOTE: "1",
...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @idexal/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@idexal/rpc", "@idexal/web", "@idexal/formal-proof"]) {
...
runGit(["submodule", "update", "--init", "--recursive", "apps/idexal-cli"]);
